            ...
            def onehit(p, kp):
                ...
                if p == Wp:
                    if len(kp & (Sset - {Wp})) > 1 or len(kp & (O1set - {Wp})) > 1:
                        return False
                if FULL_H1 and p == Up:
                    if len(kp & (O1set - {Up})) > 1 or len(kp & (O2set - {Up})) > 1:
                        return False
                return True

            # backtracking class assignment, V and W first
            order = [Vp, Wp, Up] + [p for p in pts if p not in (Vp, Wp, Up)]
            assigned = {}
            paircount = Counter()
...
def main():
    import os
    global FULL_H1
    FULL_H1 = os.environ.get("FULL_H1", "0") == "1"
    print(f"FULL_H1={FULL_H1}")
    rng_seeds = range(8)
    results = []
    for name, (pts, d2) in {
    ...
    with open("t1b_results.json", "w") as f:
