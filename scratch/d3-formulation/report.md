<!-- Provenance: report body produced by the D3 candidate-lemma formulation
     ...
     2026-07-06 — ALL CHECKS PASS:
       ...
       - Class-fullness check (added after the agent's definitive third
         draft): W16-sq's 16 classes are ALL full exact-4 radius classes in
         EXACT integer arithmetic (claim verified — this is the fact that
         kills L1/L3 at n=16 without numerics); W15/W16-tri are not full
         (exact4_variant=false, census-level falsifiers only), as claimed.
       - This transcription is the agent's DEFINITIVE third message (n=17
         closed out; W16-tri stage-2-ALIVE cross-check; W16-sq full-H1 +
         fully-exact confirmation). It supersedes the two earlier drafts.
...
**The stop condition is met for the decided shape.** The uniform ideal-theoretic lemma — in every formulation whose conclusion is "the double-apex constraint-pattern family is uniformly ℂ-dead under equality-plus-separator saturation" — is **falsified by explicit constructions** at n = 15, 16, 20, found this session and verified independently (exact integer arithmetic for the lattice witnesses; 50–60-digit precision with proven class structure for the ring witnesses). The falsifiers survive every equality-shaped strengthening extractable from the leaf's proven combinatorial shadow: pair-distinctness, triple-noncollinearity, full one-hit at all three Moser vertices, and full exactness of every 4-class. One equality-shaped remnant survives untested (Candidate B, the critical-row fragment); its test is specified and cheap; its prognosis is poor. The falsification also bounds the standing fallback: for n ≥ 15 the pattern family contains realizable members, so no pattern-level bank covers the tail. What the battery does establish: for n ∈ {12, 13, 14} sampled admissible patterns die uniformly (4500+/4500+ across seven cap profiles, two stages; n = 17 spot probe 30/40 dead, 0 alive, 10 solver-unresolved), and the kills localize to induced dead windows of ≤ 8 points (stage-2 kills to ≤ 5 where mined) — so a bounded-n census extension for the head remains supported, while the n ≥ 15 tail provably requires convexity-coupled content beyond the pattern level. This is the fallback-trigger report, with the line drawn precisely. Nothing here contradicts the dichotomy or D2 results, which concern mined cores at n = 12 and stand (§6).

## 1. The Lean obligation, read precisely

The conclusion asks for x ≠ w′ ∈ A, both off the surplus cap, with dist x oppApex1 = dist w′ oppApex1 and dist x oppApex2 = dist w′ oppApex2. Two distinct points sharing both radii are the two intersections of circles about the apexes — mirror images across line(oppApex1, oppApex2); equivalently perpBisector(x, w′) = line(oppApex1, oppApex2). Downstream, `U2NonSurplusSqueeze.oppCap2_escape_gen` (PROVEN, in-file) refutes such a pair: off-surplus points lie strictly on one side of that chord; mirror pairs straddle it. So, as the docstring states, proving the theorem means deriving the leaf contradiction; the Card12Up residual = "the two-large-cap leaf hypotheses are unsatisfiable for every |A| ≥ 12". Hypotheses: A strictly convex (`ConvexIndep`), K4 everywhere, circumscribed non-obtuse Moser triangle {U,V,W} ⊆ A with cap triple, surplus cap > 4 plus second cap ≥ 5 (`exists_secondLargeCap_of_noM44` under `hNoM44`), a `CriticalShellSystem` (every q ∈ A on an exact-4 full shell of some center γ(q) ∈ A), the five `U1Depth5.CriticalSourceRows`, U3 packets, and `U1LargeCapRouteBTailLiveData` on six named pairwise-distinct points p,q,t1,t2,t3,u.
...
| T8 — n=13 mining | does k\* grow at 13? | stage-1-dead: k\* ∈ {4,5}; stage-2: one cube k\*=4, one cube k\* > 5 (run ended mid-mine) | EMPIRICALLY VERIFIED (partial) |
| T1/T1b — all-K4 subset search (lattice windows) + admissible-labeling backtracker (census H1 and full H1) | does a realizable admissible pattern exist? | **Yes at n = 15, 16** (§4); all-K4 subsets also exist at 12/13/14 (12 even exact-4) but no admissible labeling found there (bounded search) | §4 |
...
| CEGAR — witness-collinearity saturation (msolve char-0) | does Γ-saturation kill the n=15 lattice witness? | Yes: round 0 (its own 33 collinear triples + 105 pair separators) already DEAD | EMPIRICALLY VERIFIED |

## 4. The witnesses (central finding)

**W15 (triangular lattice, n = 15, caps (6,6,6)).** Subset {(0,3),(1,2),(1,3),(1,4),(2,0),(2,1),(2,2),(2,3),(2,4),(3,1),(3,2),(3,5),(4,1),(4,2),(5,2)} ((i,j) ↦ (i + j/2, j√3/2); d² = di² + di·dj + dj², exact integers). Carries a census-admissible cube (C1, C2, C4, H1 at V,W, two caps ≥ 5); labeling/classes in `t1_results.json`. Verified twice: exact integer combinatorics (independent re-implementation) and the ported msolve pipeline (stage 1 ALIVE, stage 2 ALIVE). The lattice points realize the cube over ℝ with 15 distinct points ⇒ **the Δ-saturated census-level uniform statement is false at n = 15**. Label: EMPIRICALLY VERIFIED (exact integer arithmetic, two independent checks; hand-checkable). It fails H1 at U; its realization has 33 collinear triples; CEGAR shows every distinct ℂ-realization of its cube hits one of those 33 collinearities — the Γ-repair kills this particular witness.

**W16-tri (triangular lattice, n = 16, caps (7,6,6)).** Same construction one size up; exact-integer combinatorics verified; msolve stage 2 ALIVE (stage 1 timed out — immaterial; the real realization decides). Fails H1 at U. A second census-level falsifier.

**W16-sq (square lattice, n = 16, caps (7,6,6)).** Subset {(0,3),(1,1),(1,5),(2,2),(2,3),(2,4),(3,0),(3,2),(3,4),(3,6),(4,2),(4,3),(4,4),(5,1),(5,5),(6,3)}. Exact-integer verification: C1/C2/C4, **full H1 (U too)**, and **all 16 chosen classes are full exact-4 radius classes**. Both msolve cross-checks timed out; the configuration itself realizes the cube with 16 distinct points, which settles it. Not general position (20 collinear triples; its U,V,W happen to be collinear in the realization). Falsifies L1 and L3 (below) at n = 16 in exact arithmetic.

**W16a, W16b, W20 (two concentric regular m-gons, twist π/m — general position).**

- m = 8, b = cos(3π/8) + √(cos²(3π/8)+1) ≈ 1.4534 (n = 16): the outer-point class {A±2, B-pair} and the inner-point class reduce to the **same** quadratic b² − 2cos(3π/8)·b − 1 = 0 — all-K4 PROVEN (identical-quadratic derivation + dihedral transitivity; members distinct since b ≠ 1).
...
At 60-digit precision (cluster threshold 10⁻⁴⁵, near-miss guard 10⁻⁶): every point has an **exactly-4 full** radius class; min |collinearity determinant| ≥ 0.049 (no 3 collinear — general position); min pairwise d² ≥ 0.146. For each, a **full-H1 admissible labeling with every chosen class equal to its full exact-4 radius class** was found and independently re-verified at 50 digits: `t9_wit_m8_1.4534.json` (profile (7,6,6)), `t9_wit_m8_2.2853.json` ((7,7,5)), `t9_wit_m10_0.6180.json` ((8,8,7)). Labels: configuration existence and all-K4 PROVEN (routine algebra); general position, exactness margins, witness-cube admissibility EMPIRICALLY VERIFIED at 50–60 digits with structural exactness for the class coincidences. FLAG FOR IMPLEMENTER: exact certification of the three ring witnesses (coordinates in ℚ(cos(π/m), sin(π/m), b); finitely many determinant/margin nonvanishings, symbolic, routine).

**Consequence — collapse of the separator ladder.** L0 = Δ-saturation with census H1; L1 = + full H1; L2 = + Γ (noncollinearity); L3 = + E (full exactness at every center). L0 false at n = 15, 16 (W15, W16-tri); L1 and L3 false at n = 16 in exact arithmetic (W16-sq); L1, L2, L3 **simultaneously false at n = 16 and 20** (ring witnesses: general-position, full-H1, fully exact, distinct-realizable). Any induced subsystem of a realizable pattern is realizable (restrict the realization — PROVEN, trivial), so these patterns contain **no dead window at any size**: every covering/bounded-extraction proof shape dies with the statement, and so does every "syzygy/ideal-membership uniform in parameters" formulation — a realizable admissible pattern refutes the conclusion regardless of proof technology (Wu/Chou, characteristic sets, resultants, syzygies). The falsifiers are real point configurations, not encoding artifacts.

**Scope note.** At n = 12, 13, 14 no witness was found: lattice windows yield all-K4 subsets but no admissible labeling within budget; the two-ring family has no shared-root b at m = 6, 7; 4500+ sampled admissible patterns all die. Witness-existence at 12–14 is OPEN; at n ≥ 15 it is settled (three independent constructions; plausibly most n ≥ 15 via uneven rings and lattice variants — untested).
...
(iii) **Falsification executed:** consistent throughout — 4500+/4500+ sampled patterns dead at 12/13/14; stage-1 kills localize at k\* ≤ 8 (239 committed cubes), k\* ∈ {4,5} on fresh n=13 mining; stage-2 kills at k\* ∈ {4,5} (T7), one n=13 stage-2 cube open past k=5. A 12–14 witness would kill it; bounded searches found none.
...
**Artifacts** (session scratchpad `/private/tmp/claude-501/-Users-adam-projects-math-projects-erdos-97-96-formalization/fe6e52a9-1de1-4034-b676-5c7e83aecbac/scratchpad/d3/`, reproducible from committed repo data + stated seeds): `t1_results.json`, `t1.log`, `t1b.log` (lattice witnesses, census-H1 and full-H1 searches); `t9_wit_m8_1.4534.json`, `t9_wit_m8_2.2853.json`, `t9_wit_m10_0.6180.json` (ring witnesses + labelings); `cegar0.log`; `t6_{754,655,664,854,665,965}.log` + JSONs (965 final: stage1 {DEAD 12, ALIVE 19, TIMEOUT 9}, stage2 {DEAD 18, ALIVE 0, TIMEOUT 1}, 0 distinct-alive); `t7_654.log`/`t7_654.json`; `t8_754.log`; `analyze_cores.py` output (T5/T2/T3 tables; witness-1/2 verification in task output `b9qrhiqc5`). Repo inputs, read-only: `/Users/adam/projects/math-projects/erdos-97-96-formalization/scratch/u12-census-port/**`, `lean/Erdos9796Proof/P97/U1LargeCapRouteBTail.lean`, `lean/Erdos9796Proof/P97/SurplusM44Packet.lean`, `lean/Erdos9796Proof/P97/U1CarrierInjection.lean`.
