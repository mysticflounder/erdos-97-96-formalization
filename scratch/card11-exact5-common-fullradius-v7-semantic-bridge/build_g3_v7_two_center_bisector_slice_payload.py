#!/usr/bin/env python3
"""Build the retained G3 v7 two-center bisector-parity ordinal payload."""
...
FAMILY = "v7-global-two-center-bisector-parity"
ROUTE = "common"
SOURCE_START = 1_180_699
SOURCE_END = 1_370_778
EXPECTED_COUNT = 52_693
EXPECTED_FIRST = 31
EXPECTED_LAST = 190_070
EXPECTED_SHA256 = (
    "20a0c91c72ec694631b1d396ff0cf41e25257311c554b10171eba1441dea073f"
)
            ...
            if match := ROUTE_RE.match(line):
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            "v7 two-center position outside source block: "
                            f"{current_position}"
                        )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} v7 two-center ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("v7 two-center ordinals are not increasing")
