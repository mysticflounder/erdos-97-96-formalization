#!/usr/bin/env python3
"""Build the retained G3 source-center/source-choice ordinal payload.

The output records positive LEB128 deltas between one-based ordinals in the
6,642-clause common family.  The surrounding Lean ledger reconstructs each
typed forward or reverse source-center occurrence.
"""
...
FAMILY = "source-center-iff-some-source-row-choice"
ROUTE = "common"
SOURCE_START = 429_702
SOURCE_END = 436_343
EXPECTED_COUNT = 317
EXPECTED_FIRST = 19
EXPECTED_LAST = 4_618
EXPECTED_SHA256 = (
    "8712f151e8de57bc3a4cf1b15bff435a38cca94a6f2430c637c31e8982710f36"
)
            ...
            if match := ROUTE_RE.match(line):
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            f"source-center position outside source block: "
                            f"{current_position}"
                        )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} source-center ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("source-center ordinals are not increasing")
