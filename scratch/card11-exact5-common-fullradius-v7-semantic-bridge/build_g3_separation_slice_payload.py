#!/usr/bin/env python3
"""Build the retained G3 separation-family ordinal payload.

The output is a strict raw ASCII85 envelope of positive LEB128 deltas between
the retained one-based ordinals in the exact separation-family emission order.
The selector-only G3 clause and inherited base slice are not included.
"""
...
FAMILY = "inherited-separation"
ROUTE = "common"
SOURCE_START = 207_970
SOURCE_END = 387_856
EXPECTED_COUNT = 45_581
EXPECTED_FIRST = 1
EXPECTED_LAST = 179_883
EXPECTED_SHA256 = (
    "093a555cd19437e1fe64c1647424fb6fc72cf16267d632f5fcb6c4263719c1a4"
)
            ...
            if match := ROUTE_RE.match(line):
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            f"separation position outside source block: "
                            f"{current_position}"
                        )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} separation ordinals, got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("separation ordinals are not strictly increasing")
    ...
    print(f"ordinals={len(ordinals)}")
    print(f"range={ordinals[0]}..{ordinals[-1]}")
