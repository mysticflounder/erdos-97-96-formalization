#!/usr/bin/env python3
"""Build the retained G3 local-equality-transitivity ordinal payload.

The output records positive LEB128 deltas between one-based ordinals in the
3,960-clause common family.  The surrounding Lean ledger reconstructs the
typed equality triangle and branch for every retained ordinal.
"""
...
FAMILY = "local-equality-transitivity"
ROUTE = "common"
SOURCE_START = 414_048
SOURCE_END = 418_007
EXPECTED_COUNT = 341
EXPECTED_FIRST = 18
EXPECTED_LAST = 3_960
EXPECTED_SHA256 = (
    "96c931a65198c813fd6c30bc1953e3965b654ef30f381d8118b1f81a6a4585b6"
)
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} local transitivity ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("local transitivity ordinals are not increasing")
