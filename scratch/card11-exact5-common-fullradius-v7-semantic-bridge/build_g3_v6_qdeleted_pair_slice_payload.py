#!/usr/bin/env python3
"""Build the retained G3 V6 q-deleted-pair-definition ordinal payload."""
...
FAMILY = "v6-qdeleted-pair-witness-iff"
ROUTE = "common"
SOURCE_START = 936_379
SOURCE_END = 1_023_498
EXPECTED_COUNT = 13_214
EXPECTED_FIRST = 22
EXPECTED_LAST = 86_328
EXPECTED_SHA256 = (
    "aef1b4637c1f81d2511c36fbe30f02ed99c81b795edd6b9b00be6d645d84e600"
)
            ...
            if match := ROUTE_RE.match(line):
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            "V6 q-deleted-pair position outside source block: "
                            f"{current_position}"
                        )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} V6 q-deleted-pair ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("V6 q-deleted-pair ordinals are not increasing")
