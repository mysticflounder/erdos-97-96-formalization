#!/usr/bin/env python3
"""Build the retained first-apex global-equality-seed ordinal payload."""
...
FAMILY = "first-apex-exact-five-global-equality-seeds"
SOURCE_START = 585_249
SOURCE_END = 585_258
ROUTES = {"s2_o9": 1, "s3_o0": 2, "s3_o9": 3}
EXPECTED_COUNT = 22
EXPECTED_FIRST = 1
EXPECTED_LAST = 29
EXPECTED_SHA256 = (
    "7595edb226393f1edc56676199c1c98b43e72209bd343a5f308fdef03271fde9"
)
            ...
            if match := ROUTE_RE.match(line):
                ...
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            f"seed position outside source block: "
                            f"{current_position}"
                        )
                    ordinals.append(
                        3 * (current_position - SOURCE_START) + ROUTES[route]
                    )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} first-apex global-equality ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("first-apex global-equality ordinals are not increasing")
