#!/usr/bin/env python3
"""Build the retained G3 source-center/no-Q-free ordinal payload.

The output records positive LEB128 deltas between one-based ordinals in the
11,592-clause common family.  The surrounding Lean ledger reconstructs the
typed source-center row and four-set for every retained ordinal.
"""
...
FAMILY = "source-choice-no-qfree-after-that-source-deletion"
ROUTE = "common"
SOURCE_START = 483_223
SOURCE_END = 494_814
EXPECTED_COUNT = 426
EXPECTED_FIRST = 82
EXPECTED_LAST = 11_451
EXPECTED_SHA256 = (
    "21c261eea5364472e081f1f5a30e2d89d2d17cf57031913fa370cd4f8c38ab44"
)
            ...
            if match := ROUTE_RE.match(line):
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            "no-Q-free position outside source block: "
                            f"{current_position}"
                        )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} no-Q-free ordinals, got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("no-Q-free ordinals are not strictly increasing")
