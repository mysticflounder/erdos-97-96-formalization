#!/usr/bin/env python3
"""Build the retained common G3 inherited-source-tail ordinal payload.

The three route-specific guarded apex-shell clauses are deliberately omitted:
the Lean ledger renders those directly.  The output covers the remaining
common clauses, beginning with the blocker-shell clause, as positive LEB128
deltas between one-based ordinals in the 26,191-clause inherited tail.
"""
...
FAMILY = "inherited-source-tail"
ROUTE = "common"
SOURCE_START = 387_857
SOURCE_END = 414_047
EXPECTED_COUNT = 5_244
EXPECTED_FIRST = 2
EXPECTED_LAST = 26_188
EXPECTED_SHA256 = (
    "c2305210a7bde0f0a66180eaec41d94bdba0c239afdde2fd2a5ada1c28a6aa80"
)
            ...
            if match := ROUTE_RE.match(line):
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            f"source-tail position outside source block: "
                            f"{current_position}"
                        )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} source-tail ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("source-tail ordinals are not strictly increasing")
