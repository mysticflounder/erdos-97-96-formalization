#!/usr/bin/env python3
"""Build the retained G3 selected-row/local-equality ordinal payload.

The output records positive LEB128 deltas between one-based ordinals in the
11,694-clause common family.  The surrounding Lean ledger reconstructs the
typed selected row and support pair for every retained ordinal.
"""
...
FAMILY = "selected-row-internal-radius-equalities"
ROUTE = "common"
SOURCE_START = 418_008
SOURCE_END = 429_701
EXPECTED_COUNT = 8_619
EXPECTED_FIRST = 2
EXPECTED_LAST = 11_694
EXPECTED_SHA256 = (
    "95d3e943b67456bf9e7530e97fc3eeac455ae91fd9542494e7b806bb5bbdce65"
)
            ...
            if match := ROUTE_RE.match(line):
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            f"selected-local-equality position outside source "
                            f"block: {current_position}"
                        )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} selected-local-equality ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("selected-local-equality ordinals are not increasing")
