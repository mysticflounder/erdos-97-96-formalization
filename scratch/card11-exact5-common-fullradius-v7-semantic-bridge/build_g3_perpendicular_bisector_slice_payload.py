#!/usr/bin/env python3
"""Build the retained perpendicular-bisector exclusion ordinal payload."""
...
FAMILY = "perpendicular-bisector-core-exclusions"
ROUTE = "common"
SOURCE_START = 589_879
SOURCE_END = 594_498
EXPECTED_COUNT = 1_293
EXPECTED_FIRST = 12
EXPECTED_LAST = 4_525
EXPECTED_SHA256 = (
    "032e4e539ca7e2da19f7bf82f2dbd61ece1137dbfa3fdfb7c797c5ae79062c55"
)
            ...
            if match := ROUTE_RE.match(line):
                ...
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            "perpendicular-bisector position outside source "
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} perpendicular-bisector ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("perpendicular-bisector ordinals are not increasing")
