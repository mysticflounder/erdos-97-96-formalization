#!/usr/bin/env python3
"""Build the retained G3 duplicate-center exclusion ordinal payload."""
...
FAMILY = "duplicate-center-core-exclusions"
ROUTE = "common"
SOURCE_START = 585_259
SOURCE_END = 589_878
EXPECTED_COUNT = 714
EXPECTED_FIRST = 12
EXPECTED_LAST = 4_617
EXPECTED_SHA256 = (
    "db8be91545e751da99a466cde0a12271b19c7812865b76fae3a0fb0d1a45e82c"
)
            ...
            if match := ROUTE_RE.match(line):
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            "duplicate-center position outside source block: "
                            f"{current_position}"
                        )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} duplicate-center ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("duplicate-center ordinals are not increasing")
