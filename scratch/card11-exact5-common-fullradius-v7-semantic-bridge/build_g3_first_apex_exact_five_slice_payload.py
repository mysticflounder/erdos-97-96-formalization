#!/usr/bin/env python3
"""Build the retained G3 guarded first-apex exact-five ordinal payload."""
...
FAMILY = "first-apex-named-exact-five-entire-class"
SOURCE_START = 494_815
SOURCE_END = 494_849
ROUTE_ORDINAL = {"s2_o9": 0, "s3_o0": 1, "s3_o9": 2}
EXPECTED_COUNT = 89
EXPECTED_FIRST = 1
EXPECTED_LAST = 105
EXPECTED_SHA256 = (
    "5650a78f5a970891f6e427fd56ae3da38a5b934384dc112ab4260bcc53474854"
)
    ...
    with path.open(encoding="utf-8") as source:
        for line in source:
            ...
            if match := ROUTE_RE.match(line):
                route = match.group(1)
                if current_family == FAMILY and current_position is not None:
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            "first-apex exact-five position outside source block: "
                            f"{current_position}"
                        )
                    if route not in ROUTE_ORDINAL:
                        raise SystemExit(f"unexpected guarded route {route}")
                    ordinal = (
                        3 * (current_position - SOURCE_START)
                        + ROUTE_ORDINAL[route]
                        + 1
                    )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} first-apex exact-five ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("first-apex exact-five ordinals are not increasing")
