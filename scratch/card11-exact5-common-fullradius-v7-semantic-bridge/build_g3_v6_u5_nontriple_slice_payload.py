#!/usr/bin/env python3
"""Build the retained G3 V6 U5 nontriple-incompatibility ordinal payload."""
...
FAMILY = "v6-u5-nontriple-equilateral-incompatibilities"
ROUTE = "common"
SOURCE_START = 1_023_499
SOURCE_END = 1_141_398
EXPECTED_COUNT = 275
EXPECTED_FIRST = 27_784
EXPECTED_LAST = 106_741
EXPECTED_SHA256 = (
    "2f63bb69e441604413ad292e1981660e40ff4ac505e3dc68e426fe6c0844cdb3"
)
            ...
            if match := ROUTE_RE.match(line):
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            "V6 U5 nontriple position outside source block: "
                            f"{current_position}"
                        )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} V6 U5 nontriple ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("V6 U5 nontriple ordinals are not increasing")
