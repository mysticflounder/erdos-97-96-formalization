#!/usr/bin/env python3
"""Build the retained G3 selected-row global-equality ordinal payload."""
...
FAMILY = "selected-row-global-equality-seeds"
ROUTE = "common"
SOURCE_START = 573_555
SOURCE_END = 585_248
EXPECTED_COUNT = 8_199
EXPECTED_FIRST = 1
EXPECTED_LAST = 11_694
EXPECTED_SHA256 = (
    "97367f010fe01a55c44105aef8754daea0bf4cae7b15110052b6435d957e7d32"
)
            ...
            if match := ROUTE_RE.match(line):
                if (
                    ...
                    if not SOURCE_START <= current_position <= SOURCE_END:
                        raise SystemExit(
                            "selected-global-equality position outside source block: "
                            f"{current_position}"
                        )
...
def main() -> None:
    ...
    if len(ordinals) != EXPECTED_COUNT:
        raise SystemExit(
            f"expected {EXPECTED_COUNT} selected-global-equality ordinals, "
            f"got {len(ordinals)}"
        )
    ...
    if any(left >= right for left, right in zip(ordinals, ordinals[1:])):
        raise SystemExit("selected-global-equality ordinals are not increasing")
