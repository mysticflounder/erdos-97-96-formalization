#!/usr/bin/env python3
"""Compile and admit the authenticated wave-3 weighted theorem bank."""
...
CERTIFICATE_DIR = HERE / "wave3-theorem-search"
ROOT_CNF = HERE / "postwave-wave2-base.cnf"
RECEIPTS = tuple(HERE / f"wave3-cut-{index}.json" for index in range(1, 9))
FRAGMENT = HERE / "wave3-weighted-orbits.dimacs"
AGGREGATE = HERE / "postwave-wave3-base.cnf"
MANIFEST = HERE / "wave3-weighted-orbits.manifest.json"
ADMISSION = HERE / "wave3-piqd-admission.json"
MANIFEST_SCHEMA = "p97-exact17-piqd-wave3-weighted-orbits/v1"
...
def require_session(value: dict[str, Any], clauses: int) -> None:
    expected = {
        "id": SESSION_ID,
        "lane": "sat",
        "state": "live",
...
def main() -> int:
    ...
    if sha256(ROOT_CNF) != ROOT_SHA256:
        raise ValueError("wave-3 root SHA-256 mismatch")

    helper = load_module("wave3_bank_helper", BASE_COMPILER)
    formula_chain = load_module("wave3_formula_chain", FORMULA_CHAIN)
    orbit = load_module("wave3_weighted_orbit_compiler", ORBIT_COMPILER)
    uploader = load_module("wave3_piqd_uploader", UPLOADER)
    source_paths = (Path(__file__).resolve(), BASE_COMPILER, FORMULA_CHAIN, ORBIT_COMPILER, UPLOADER)
    frozen_sources = {
    ...
    if len(certificates) != 8:
        raise ValueError("expected exactly eight wave-3 certificates")
    ...
    if raw_count != 272 or len(proposed) != 272:
        raise ValueError("wave-3 certificate orbits are not eight disjoint 34-clause sets")
    ...
    for batch in chain.appended_batches:
        for clause in batch:
            prior_checked += 1
            match = helper.subsumed_candidate(frozenset(clause), candidate_sets, postings)
            if match is not None:
                subsumed.add(match)
    novel = tuple(
    ...
    if not novel:
        raise ValueError("wave-3 theorem bank contributes no novel exact-17 clauses")
