#!/usr/bin/env python3
"""Run a bounded source-checked exact-17 PIQD wave-5 successor batch."""
...
def cut_receipts(index: int) -> list[Path]:
    return [HERE / f"wave5-cut-{prior}.json" for prior in range(1, index)]


def expected_clauses(index: int) -> int:
    if index == 1:
        return ROOT_CLAUSES
    payload = json.loads((HERE / f"wave5-cut-{index - 1}.json").read_text())
...
def main() -> int:
    ...
    for index in range(1, WAVE_SIZE + 1):
        model_log = HERE / f"wave5-model-{index}.log"
        solve_receipt = HERE / f"wave5-solve-{index}.json"
        analysis = HERE / f"wave5-analysis-{index}.json"
        cut_receipt = HERE / f"wave5-cut-{index}.json"
        outputs = (model_log, solve_receipt, analysis, cut_receipt)
        if any(path.exists() for path in outputs):
            raise FileExistsError(f"wave5 index {index} already has output")
        ...
        if checked.get("status") != "PASS" or not checked.get("source_verified"):
            raise RuntimeError(f"wave5 index {index} failed source verification")
