"""Authenticate one v25 static-canary SAT survivor and mine all 48 source orders."""
...
MINING_SCHEMA = "p97_rigid221_exact12_static_canary_all_order_mining.v5"
...
def mine(workdir: Path) -> dict[str, Any]:
    ...
    validator_result = validate_v25_workdir(
        workdir,
        repo_root,
        piqd_root=workdir / "piqd-journal",
    )
    if validator_result.get("status") != "SAT_WITNESS_REPLAYED":
        raise MiningError("all-order mining requires a v25-validated SAT survivor")
    ...
    return {
        ...
        "source": {
            "v25_validator": validator_result,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs="?",
        default=Path("scratch/arm-static-cell6-v25-live-898fbd78-20260816"),
    )
