"""Authenticate one v23 static-canary SAT survivor and mine all 48 source orders."""
...
def mine(workdir: Path) -> dict[str, Any]:
    ...
    validator_result = validate_v23_workdir(
        workdir,
        repo_root,
        piqd_root=workdir / "piqd-journal",
    )
    if validator_result.get("status") != "SAT_WITNESS_REPLAYED":
        raise MiningError("all-order mining requires a v23-validated SAT survivor")
    ...
    return {
        ...
        "source": {
            "v23_validator": validator_result,
