"""Authenticate one static-canary SAT survivor and mine all 48 source orders."""
...
from census.card_head.exact12_apex_first_opposite_shared_pair_surplus_common_five_membership_family_bank import (
    BANK_SCHEMA as APEX_FIRST_OPPOSITE_SHARED_PAIR_SURPLUS_COMMON_FIVE_BANK_SCHEMA,
)
from census.card_head.exact12_apex_first_opposite_shared_pair_surplus_common_five_membership_family_bank import (
    EXPECTED_BANK_SHA256 as EXPECTED_APEX_FIRST_OPPOSITE_SHARED_PAIR_SURPLUS_COMMON_FIVE_BANK_SHA256,
)
from census.card_head.exact12_apex_first_opposite_shared_pair_surplus_common_five_membership_family_bank import (
    FAMILY_ID as APEX_FIRST_OPPOSITE_SHARED_PAIR_SURPLUS_COMMON_FIVE_FAMILY_ID,
)
from census.card_head.exact12_apex_first_opposite_shared_pair_surplus_common_five_membership_family_bank import (
    _sha256_json as _apex_first_opposite_shared_pair_surplus_common_five_bank_sha256,
)
from census.card_head.exact12_apex_first_opposite_shared_pair_surplus_common_five_membership_family_bank import (
...
def mine(workdir: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or summary.get("lean_terminal_ingress_ready") is not False
    ...
    apex_first_opposite_shared_pair_surplus_common_five_job = job.get(
        "apex_first_opposite_shared_pair_surplus_common_five_membership_family_bank"
    )
    apex_first_opposite_shared_pair_surplus_common_five_snapshot = dict(
        apex_first_opposite_shared_pair_surplus_common_five_bank
    )
    apex_first_opposite_shared_pair_surplus_common_five_digest = (
        apex_first_opposite_shared_pair_surplus_common_five_snapshot.pop(
            "bank_sha256", None
        )
    )
    if (
        apex_first_opposite_shared_pair_surplus_common_five_job
        != {
            "schema": APEX_FIRST_OPPOSITE_SHARED_PAIR_SURPLUS_COMMON_FIVE_BANK_SCHEMA,
            "family_id": APEX_FIRST_OPPOSITE_SHARED_PAIR_SURPLUS_COMMON_FIVE_FAMILY_ID,
            "sha256": EXPECTED_APEX_FIRST_OPPOSITE_SHARED_PAIR_SURPLUS_COMMON_FIVE_BANK_SHA256,
            "lean_terminal_ingress_ready": False,
        }
        or apex_first_opposite_shared_pair_surplus_common_five_bank.get("schema")
        != APEX_FIRST_OPPOSITE_SHARED_PAIR_SURPLUS_COMMON_FIVE_BANK_SCHEMA
        or apex_first_opposite_shared_pair_surplus_common_five_bank.get("family_id")
        != APEX_FIRST_OPPOSITE_SHARED_PAIR_SURPLUS_COMMON_FIVE_FAMILY_ID
        or apex_first_opposite_shared_pair_surplus_common_five_digest
        != EXPECTED_APEX_FIRST_OPPOSITE_SHARED_PAIR_SURPLUS_COMMON_FIVE_BANK_SHA256
        or apex_first_opposite_shared_pair_surplus_common_five_digest
        != _apex_first_opposite_shared_pair_surplus_common_five_bank_sha256(
            apex_first_opposite_shared_pair_surplus_common_five_snapshot
        )
    ):
        raise MiningError(
            "apex/first-opposite/shared-pair/surplus common-five bank provenance failed"
        )
    try:
        attest_apex_first_opposite_shared_pair_surplus_common_five_membership_family_bank_live_sources(
        ...
        raise MiningError(
            "apex/first-opposite/shared-pair/surplus common-five bank source "
