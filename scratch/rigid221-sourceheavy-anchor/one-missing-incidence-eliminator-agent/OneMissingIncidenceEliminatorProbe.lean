...
/-!
# One missing-incidence eliminator probe

This scratch theorem records the strongest direct source-clean consequence
found for the concrete missing-incidence arm `c₁ ∉ BO`.  It is deliberately
not stated as a contradiction: the resulting common-deletion packet has the
deleted point `c₁` itself as its first center, so existing terminal consumers
do not close the branch from this fact alone.
-/
...
#print axioms xvDeletion_c1_not_mem_BO_yields_self_commonDeletion
