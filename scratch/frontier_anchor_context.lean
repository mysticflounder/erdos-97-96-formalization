...
theorem false_of_twoCapSources_mutualCrossMembership_distinctBlockers
  ...
  rcases commonCollisionEndpointOmission_or_complementaryMembership
      P Pρ source source' hsource hsource' with hcommon | hcomplementary
  · rcases exists_mutualBlockerCapIndices_same_only_first
        P Pρ T source source' hsourcesNe hsource hsource'
        hmutual hblockersNe' with
      ⟨i, j, hi, hj, _hsameOnlyFirst⟩
    by_cases hfirst :
        i = S.oppIndex1 ∨ j = S.oppIndex1
    · sorry
    · have hiNe : i ≠ S.oppIndex1 :=
        fun hiFirst => hfirst (Or.inl hiFirst)
      have hjNe : j ≠ S.oppIndex1 :=
        fun hjFirst => hfirst (Or.inr hjFirst)
      exact false_of_twoCapSources_of_blockers_off_firstCap
        source source' hsourcesNe hsource.2.1 hsource'.2.1
        hmutual hblockersNe'
        (S.capInteriorByIndex_not_mem_capByIndex_of_ne hi hiNe)
        (S.capInteriorByIndex_not_mem_capByIndex_of_ne hj hjNe)
  · exact false_of_twoCapSources_complementaryMembership
      P Pρ T source source' hsourcesNe hsource hsource'
      hmutual hblockersNe' hcomplementary.1
