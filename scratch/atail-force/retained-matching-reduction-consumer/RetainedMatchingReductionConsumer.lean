...
/-!
# Source-faithful consumer of the retained blocker-matching reduction
...
namespace Problem97
namespace ATailRetainedMatchingReductionConsumerScratch
...
/-- Parent-facing source-faithful residual after consuming the geometric
reduction. -/
structure RetainedMatchingReductionConsumerResidual
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    (R : FrontierCommonDeletionParentResidual F) : Type where
  omission : RetainedInteriorDirectedOmission R
  oriented : OrientedRetainedCommonDeletion omission
  reverseCoupling : RetainedReverseCouplingOutcome oriented
  oppositeLargeCap :
    5 ≤ (S.capByIndex S.oppIndex1).card ∨
      5 ≤ (S.capByIndex S.oppIndex2).card

/-- The production matching reduction reaches exactly one localized reverse
hit or one paired common-deletion residual. -/
theorem nonempty_retainedMatchingReductionConsumerResidual
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : FrontierCommonDeletionParentResidual F}
    (G : RetainedMatchingGeometricReduction R) :
    Nonempty (RetainedMatchingReductionConsumerResidual R) := by
  rcases nonempty_orientedRetainedCommonDeletion
      G.omission G.commonDeletion with ⟨oriented⟩
  rcases nonempty_retainedReverseCouplingOutcome oriented with ⟨reverseCoupling⟩
  exact ⟨{
    omission := G.omission
    oriented := oriented
    reverseCoupling := reverseCoupling
    oppositeLargeCap := G.oppositeLargeCap }⟩

#print axioms commonFirstApexPair_center_mem_firstCapInterior
#print axioms nonempty_orientedRetainedCommonDeletion
#print axioms nonempty_retainedReverseCouplingOutcome
#print axioms actualRow_center_eq_reverseBlocker_of_reverseHit
#print axioms exists_fresh_firstCap_commonDeletion_of_reverseHit
#print axioms nonempty_retainedMatchingLargeCapConsumerOutcome
#print axioms nonempty_retainedMatchingReductionConsumerResidual

end ATailRetainedMatchingReductionConsumerScratch
end Problem97
