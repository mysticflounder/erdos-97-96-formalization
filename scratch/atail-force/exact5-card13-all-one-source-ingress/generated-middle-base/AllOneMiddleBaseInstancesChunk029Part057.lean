...
/-- Candidate middle source assertion `h1913` (frontier_outside). -/
theorem middleSource_h1913 :
    ((((sourceQ T) == 0#4) || ((sourceQ T) == 1#4) || ((sourceQ T) == 2#4) || ((sourceQ T) == 3#4) || ((sourceQ T) == 9#4) || ((sourceQ T) == 10#4) || ((sourceQ T) == 11#4) || ((sourceQ T) == 12#4))) = true := by
  simpa only [allOneOutsideMiddleBlockBit] using
    allOne_frontierQ_outside_surplus_block T
