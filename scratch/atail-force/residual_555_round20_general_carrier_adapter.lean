...
import seven_point_split_bridge_order_core

/-!
# General-carrier adapter for the round-20 `(5,5,5)` residual

The pinned label map is `(O,A,C,D,E,U,W) = (0,1,2,3,4,6,8)`.  The adapter
uses only the five selected rows centered at `O,A,D,C,E`, the nine equality
closures they supply, and the cyclic subsequence `O,A,D,E,C`.
-/
...
namespace Problem97
namespace Census554
namespace Round20GeneralCarrierAdapter
...
/-- Five selected supports supplying the exact round-20 equality closure.
...
/-- Assemble the generic split-bridge equality core from the five carrier
...
def FiveRowSupportData.toCore {carrier : Finset ℝ²}
    ...
    SevenPointSplitBridgeOrderCore.Core (rowPattern F) := by
  ...
  exact {
    ...
    EA_EW := EdgeClosure.row data.e data.a data.w hae hwe }
...
theorem false_of_five_rows_and_cyclic_subsequence
  ...
  exact SevenPointSplitBridgeOrderCore.false_of_core_of_neg (realizes F) core
...
end Round20GeneralCarrierAdapter
end Census554
end Problem97
