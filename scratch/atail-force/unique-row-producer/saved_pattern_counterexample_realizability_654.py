#!/usr/bin/env python3
"""Hash-bound dual-engine QF_NRA check for a saved (6,5,4) pattern."""
...
EXPECTED_CANONICAL_EQUALITY_SHA256 = {
...
def main() -> None:
    ...
    parser.add_argument(
        "--pattern",
        choices=sorted(EXPECTED_CANONICAL_EQUALITY_SHA256),
        required=True,
    )
    parser.add_argument("--output", type=Path)
    args, remaining = parser.parse_known_args()

    saved = load_module("p97_saved_pattern_registry_exact", SAVED_PROBE)
    exact = load_module("p97_saved_pattern_exact_query", BASE_EXACT)
    ...
    if canonical_digest != EXPECTED_CANONICAL_EQUALITY_SHA256[args.pattern]:
