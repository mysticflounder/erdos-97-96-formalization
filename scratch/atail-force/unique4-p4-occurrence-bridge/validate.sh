...
TASK_DIR="../scratch/atail-force/unique4-p4-occurrence-bridge"
GENERATED_DIR="$TASK_DIR/generated"
BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/p4-mutual-bridge.XXXXXX")"
trap 'rm -rf "$BUILD_DIR"' EXIT

UV_CACHE_DIR="/private/tmp/p4_mutual_uv_cache" uv run python \
  "$ROOT_DIR/scratch/atail-force/unique4-p4-occurrence-bridge/generate.py" --verify-only
UV_CACHE_DIR="/private/tmp/p4_mutual_uv_cache" uv run python \
