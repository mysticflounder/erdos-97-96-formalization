...
def render_all(entries: list[dict[str, Any]], provenance: dict[str, str]) -> str:
    ...
    def _fold_chain(helper: str, suffix: str) -> str:
        names = [f"bridgeChunk{number:02d}{suffix}" for number in range(1, len(sizes) + 1)]
        acc = names[0]
        for nxt in names[1:]:
            acc = f"({helper} _ _ {acc} {nxt})" if helper == "happ" else f"({helper} _ _ _ _ {acc} {nxt})"
        return acc

    length_chain = _fold_chain("hlen", "_length")
    wf_chain = _fold_chain("happ", "_wf")
...
theorem bridgeEntries_wf : bridgeEntries.all MirrorEntry.wf = true := by
  have happ : ∀ l₁ l₂ : List MirrorEntry,
      ...
      (l₁ ++ l₂).all MirrorEntry.wf = true := by
    intro l₁ l₂ h₁ h₂
