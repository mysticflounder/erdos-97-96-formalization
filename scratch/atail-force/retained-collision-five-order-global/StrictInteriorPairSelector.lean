...
map.  Equal blockers give a cap-localized collision.  Distinct blockers give
the production directed-omission packet without needing injectivity on the
whole retained class.
-/

open scoped EuclideanGeometry

namespace Problem97
namespace ATailStrictInteriorPairSelectorScratch
...
/-- If the localized collision cap is the large opposite cap, a third strict
cap point avoids the exact collision shell and renews a common deletion at the
...
/-- The large-cap split turns the localized collision into a fresh common
deletion packet unless the *other* opposite cap is the large cap. -/
inductive LocalizedCollisionLargeCapOutcome
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : FrontierCommonDeletionParentResidual F}
    (P : RetainedInteriorBlockerCollision R) : Type
...
theorem RetainedInteriorBlockerCollision.nonempty_largeCapOutcome
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : FrontierCommonDeletionParentResidual F}
    (P : RetainedInteriorBlockerCollision R) :
    Nonempty (LocalizedCollisionLargeCapOutcome P) := by
  rcases oppositeLargeCap_disjunction_of_parent R with hfirst | hsecond
  · rcases P.exists_fresh_commonDeletion hfirst with
      ⟨fresh, hfreshI, hfresh₁, hfresh₂, hfreshShell, ⟨packet⟩⟩
    exact ⟨LocalizedCollisionLargeCapOutcome.firstCapFreshCommonDeletion
      fresh hfreshI hfresh₁ hfresh₂ hfreshShell packet⟩
  · exact ⟨LocalizedCollisionLargeCapOutcome.secondCapResidual hsecond⟩
...
#print axioms nonempty_retainedInteriorPairOutcome
#print axioms RetainedInteriorBlockerCollision.supports_eq
#print axioms RetainedInteriorBlockerCollision.blocker_mem_capInterior
#print axioms RetainedInteriorBlockerCollision.shell_inter_cap_eq_sources
#print axioms RetainedInteriorBlockerCollision.exists_fresh_commonDeletion
#print axioms RetainedInteriorBlockerCollision.nonempty_largeCapOutcome
#print axioms nonempty_strictInteriorParentOutcome

end ATailStrictInteriorPairSelectorScratch
end Problem97
