...
TASK_DIR="../scratch/atail-force/unique4-p4-row-radius-occurrence-bridge"
GENERATED_DIR="$TASK_DIR/generated"
BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/p4-row-radius-bridge.XXXXXX")"
trap 'rm -rf "$BUILD_DIR"' EXIT

UV_CACHE_DIR="/private/tmp/p4-row-radius-uv-cache" uv run --no-project python \
  "$ROOT_DIR/scratch/atail-force/unique4-p4-row-radius-occurrence-bridge/generate.py" --verify-only
UV_CACHE_DIR="/private/tmp/p4-row-radius-uv-cache" uv run --no-project python \
  "$ROOT_DIR/scratch/atail-force/unique4-exact-two-core-map/materialize_p4_compact_seed0.py" --verify-only
(cd "$ROOT_DIR/scratch/atail-force/unique4-p4-row-radius-occurrence-bridge" && \
  UV_CACHE_DIR="/private/tmp/p4-row-radius-uv-cache" uv run --no-project python -m unittest -v test_generate.py)
...
for module in Core Families BridgeChunk01 BridgeChunk02 BridgeChunk03 BridgeChunk04 BridgeAll Main Orientation AxiomAudit; do
