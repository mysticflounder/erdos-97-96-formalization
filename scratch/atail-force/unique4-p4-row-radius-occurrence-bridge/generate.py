#!/usr/bin/env python3
"""Generate the exact P4 selected-row/radius occurrence bridge.

The authenticated occurrence ledger is filtered by its family field rather
than by a clause interval: the 441 selected records occupy eleven disjoint
output/source runs.  This generator is the sole writer of ``generated/*.lean``
and verify-only mode re-renders every expected file before byte comparison.
"""
...
FAMILY = "selected_row_subset_radius_class"
CHUNK_SIZES = (128, 128, 128, 57)
EXPECTED = {
...
SELECTED_EXPECTED = {
    "ledger": "6069793c0aab9d0762baa4e34dd1961c170428bbd66432bc52b71ea567803660",
    "compact_cnf": "eaa7177c528e38c2855a3566e42143cc67621ffd951e0febdfbce17c2bdf55ba",
    "source_core": "f1d3af58f148a6279b4bc812b6856e3675f226e2e084a4d923150330a27bbe1a",
}
MATCH_STATUS_COUNTS = {
    "semantic_unique_canonical_literal_reordering": 361,
    "semantic_unique_exact_literal_order": 80,
}
...
def p4_row(center: int, point: int) -> int:
    if center == point:
        raise ValueError("row atom cannot be centered at its point")
...
def shape_for(core_clause: dict) -> tuple[int, int, int]:
    ...
    for atom in core_clause["named_literals"]:
        ...
        if family == "selected_row_membership":
            if atom["literal"] >= 0:
                raise ValueError(f"non-negative row literal at core {core_clause['core_clause_index']}")
            rows.append((atom["parameters"]["center"], atom["parameters"]["point"]))
        elif family == "complete_radius_equality":
            if atom["literal"] <= 0:
                raise ValueError(f"non-positive radius literal at core {core_clause['core_clause_index']}")
            equalities.append(atom["parameters"])
        else:
            raise ValueError(f"unexpected family {family!r} at core {core_clause['core_clause_index']}")
    if len(rows) != 2 or len(equalities) != 1:
        raise ValueError(f"unexpected atom count at core {core_clause['core_clause_index']}")
    centers = {center for center, _ in rows}
    if len(centers) != 1:
        raise ValueError(f"row-center drift at core {core_clause['core_clause_index']}")
    center = next(iter(centers))
    left, right = sorted(point for _, point in rows)
    equality = equalities[0]
    if left == right or center in {left, right}:
        raise ValueError(f"degenerate row/radius shape at core {core_clause['core_clause_index']}")
    if equality["center"] != center or {equality["left"], equality["right"]} != {left, right}:
        raise ValueError(f"radius parameters drift at core {core_clause['core_clause_index']}")
    return center, left, right


def expected_literals(shape: tuple[int, int, int]) -> list[int]:
    center, left, right = shape
    return [-p4_row(center, left), -p4_row(center, right), p4_radius(center, left, right)]
...
def load_entries() -> list[dict]:
    ...
    if len(selected) != sum(CHUNK_SIZES):
        raise ValueError(f"unexpected selected-family count: {len(selected)}")
    entries = [record.copy() for _, record in selected]
    ...
    if output_indices != sorted(output_indices) or len(set(output_indices)) != len(output_indices):
        raise ValueError("output clause indices are not strictly unique/increasing")
    if len(set(source_indices)) != len(source_indices):
        raise ValueError("source-core clause indices are not unique")
    if Counter(entry["match_status"] for entry in entries) != MATCH_STATUS_COUNTS:
        raise ValueError("selected ledger match-status census drifted")
    ...
    for entry in entries:
        ...
        if not (1 <= output_index <= len(compact_lines) and 1 <= source_index <= len(source_lines)):
            raise ValueError("selected clause index is outside its authenticated source")
        ...
        if core is None:
            raise ValueError(f"source core clause missing: {source_index}")
...
def render_chunk(number: int, entries: list[dict]) -> str:
    ...
    for entry_name, entry in zip(entry_names, entries, strict=True):
        center, left, right = entry["shape"]
        lines.append(
            f"/- output {entry['output_clause_index']}; source core "
            f"{entry['source_core_clause_index']} -/"
        )
        lines.append(
            f"def {entry_name} : BridgeEntry := "
            f"⟨{lean_list(entry['dense_literals'])}, .selectedRowRadius {center} {left} {right}⟩"
        )
...
def render_all() -> str:
    ...
    chunk_lengths = ", ".join(f"bridgeChunk{number:02d}_length" for number in range(1, len(CHUNK_SIZES) + 1))
    chunk_wfs = ", ".join(f"bridgeChunk{number:02d}_wf" for number in range(1, len(CHUNK_SIZES) + 1))
...
theorem bridgeEntries_length : bridgeEntries.length = {sum(CHUNK_SIZES)} := by
  simp [bridgeEntries, {chunk_lengths}]

theorem bridgeEntries_wf : bridgeEntries.all entryWF = true := by
  simp [bridgeEntries, {chunk_wfs}]
