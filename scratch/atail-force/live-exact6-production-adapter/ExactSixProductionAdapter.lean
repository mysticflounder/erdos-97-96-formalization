...
/-!
# Production adapter for the exact-six period-three obstruction

This scratch file first reproduces the generic finite ordered-set endpoint from
`live-largecap-kalmanson-occurrence/ExactSixPeriodThreeOrder.lean`, then applies
it to the production `FullParentExactFiveAllReverseData` packet.  Keeping the
endpoint here makes this file independently kernel-checkable even though
scratch files are not importable Lake modules.
-/
...
namespace LiveExactSixProductionAdapter
...
open LiveLargeCapKalmansonOccurrence
...
theorem false_of_fullParentExactFiveAllReverseData_of_secondCap_card_eq_six
  ...
  have hnext0 : A.transition.successor (K.source i0) = K.source i1 := by
    calc
      A.transition.successor (K.source i0) = K.transition.successor (K.source i0) := by
  ...
  have hnext1 : A.transition.successor (K.source i1) = K.source i2 := by
    calc
      A.transition.successor (K.source i1) = K.transition.successor (K.source i1) := by
        rw [A.cycle_transition]
  ...
  have hnext2 : A.transition.successor (K.source i2) = K.source i0 := by
    calc
      A.transition.successor (K.source i2) = K.transition.successor (K.source i2) := by
        rw [A.cycle_transition]
  ...
  exact false_of_periodThree_straddles_card_le_four U q0 q1 q2 b0 b1 b2
    (by omega) (hqmem i0) (hqmem i1) (hqmem i2)
    (hbmem i0) (hbmem i1) (hbmem i2)
    (hsourceIndex_ne hi01) (hsourceIndex_ne hi12) (hsourceIndex_ne hi20)
    h0 h1 h2

/-- Immediate exact-five call site: on a six-point second opposite cap, only
the independent mutual-omission sibling still needs a consumer. -/
...
#print axioms false_of_fullParentExactFiveAllReverseData_of_secondCap_card_eq_six
#print axioms false_of_fullParentExactFive_of_secondCap_card_eq_six_and_mutualConsumer

end

end LiveExactSixProductionAdapter
end Problem97
