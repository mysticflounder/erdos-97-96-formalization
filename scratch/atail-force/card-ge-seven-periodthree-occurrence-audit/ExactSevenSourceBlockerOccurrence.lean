...
/-!
# Exact-seven source/blocker occurrence on the period-three all-reverse arm

On a seven-point closed physical cap, the strict cap interior has cardinality
five.  The period-three packet contributes three distinct physical sources and
three pairwise-distinct actual blockers.  Each blocker differs from the two
sources of the transition edge it straddles.  Therefore one blocker must be
the third, nonincident source.  In transition language, some source's chosen
critical blocker is exactly its successor.

The final theorem also exposes the consequences in an arbitrary faithful
whole-carrier row table: the row at that successor is the exact critical row,
contains the source, and deleting the source destroys every K4 radius at the
successor.  These are consequences of the retained common critical system,
...
namespace Problem97
namespace ATailExactSevenSourceBlockerOccurrenceScratch
...
theorem exists_nonincident_blocker_source_collision_of_card_le_five
...
/-- Whole-carrier and deletion-critical form of the exact-seven occurrence.
For the returned source, every faithful all-center K4 table is forced to use
the source's exact critical row at the successor center, and deleting the
source destroys all K4 radii at that center. -/
...
#print axioms exists_nonincident_blocker_source_collision_of_card_le_five
...
end ATailExactSevenSourceBlockerOccurrenceScratch
end Problem97
