...
/-!
# Seven-point tail split-bridge order core

The equal-distance spine

`OA = OC = AC = OD = OE`, `DA = DE`

together with bridges `AU = AO`, `DU = DA`, `CW = CD`, and `EW = EC`
is incompatible with the four orientations `OAC`, `OAD`, `OAE`, and `OEC`.
After normalizing `O,A` to `(0,0),(1,0)`, the `U` bridge gives an upper
bound on the first coordinate of `D`.  The normalized spine then forces
`CD > 1/2` and `CE < 1/4`, while the `W` bridge and the triangle inequality
force `CD < 2 CE < 1/2`.
...
namespace Problem97
namespace Census554
namespace SevenPointSplitBridgeTailOrderCore
...
/-- The normalized five-point spine and the first split bridge force
`CD > 1/2` and `CE < 1/4`. -/
private theorem normalized_tail_split_bridge_bounds
    {c h x y w z : ℝ}
    (hCunit : c ^ 2 + h ^ 2 = 1)
    (hACunit : (c - 1) ^ 2 + h ^ 2 = 1)
    (hDunit : x ^ 2 + y ^ 2 = 1)
  ...
  have hhUpperSq : h ^ 2 < (7 / 8 : ℝ) ^ 2 := by
    rw [hhsq]
    norm_num
  have hhUpper : h < (7 / 8 : ℝ) :=
    (sq_lt_sq₀ hCpos.le (by norm_num)).1 hhUpperSq
  have hySqUpper : y ^ 2 < (1 / 2 : ℝ) ^ 2 := by
    nlinarith only [hDunit, hxSqGt, hhsq]
  have hyUpper : y < (1 / 2 : ℝ) :=
    (sq_lt_sq₀ hDpos.le (by norm_num)).1 hySqUpper
  have htwohyLtH : 2 * h * y < h := by
    have hmul := mul_lt_mul_of_pos_left hyUpper hCpos
    nlinarith only [hmul]
  have htwohyUpper : 2 * h * y < (7 / 8 : ℝ) :=
    lt_trans htwohyLtH hhUpper
  have hCDSq : (c - x) ^ 2 + (h - y) ^ 2 = 2 - x - 2 * h * y := by
    rw [hc]
    nlinarith only [hDunit, hhsq]
  have hCD : (1 / 2 : ℝ) ^ 2 < (c - x) ^ 2 + (h - y) ^ 2 := by
    rw [hCDSq]
    nlinarith only [hxUpper, htwohyUpper]
  exact ⟨hCD, hCE⟩

/-- The tail split-bridge distance equalities are incompatible with the four
...
theorem metric_orientation_incompatibility
  ...
  have hbase : dist (T A) (T O) = 1 := by
    nlinarith only [hbaseSq, (dist_nonneg : 0 ≤ dist (T A) (T O))]
  ...
  obtain ⟨hCDSqBound, hCESqBound⟩ :=
    normalized_tail_split_bridge_bounds hCunit hACunit hDunit hEunit
      hDADEscalar hCpos hDpos hEpos hOECscalar hxUpper
  ...
  have hCESq : dist (T C) (T E) ^ 2 =
      (c - w) ^ 2 + (h - z) ^ 2 := by
    rw [SurplusCOMPGBank.dist_sq_coord]
  have hCD : (1 / 2 : ℝ) < dist (T C) (T D) := by
  ...
  have hEWT : dist (T E) (T W) = dist (T E) (T C) :=
    (map_dist_eq hEC_EW).symm
  have htriW := dist_triangle (T C) (T E) (T W)
  rw [hCWT, hEWT, dist_comm (T E) (T C)] at htriW
  nlinarith only [hCD, hCE, htriW]

/-- Equality-closure form of the seven-point tail split-bridge obstruction. -/
...
/-- A realized tail split-bridge core cannot have the four positive orientations. -/
theorem false_of_core
  ...
  · simpa [edgeDist] using EdgeClosure.sound hreal core.EC_EW
  · exact hOAC
  · exact hOAD
  · exact hOAE
  · exact hOEC
