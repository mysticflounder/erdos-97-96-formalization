...
/-! Mirror reduced-union semantic folding helper -/
...
theorem mirrorBands_sat_of_each (source : BVExpr.Assignment) :
    ∀ items : List BVLogicalExpr,
      (∀ item ∈ items, item.eval source = true) →
        (Problem97.ATailExactFiveDistinctReducedUnionVerifiedMirror.bands items).eval source = true := by
  intro items h
  induction items with
  | nil => rfl
  | cons item items ih =>
      simp only [Problem97.ATailExactFiveDistinctReducedUnionVerifiedMirror.bands, BVLogicalExpr.eval_gate, Gate.eval,
        Bool.and_eq_true]
