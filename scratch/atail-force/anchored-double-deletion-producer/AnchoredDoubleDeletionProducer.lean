...
/-!
# Scratch: anchored double-deletion producer normal form

This module sharpens the source-faithful common-pair classifier.  Outside the
exact-card-four first-apex escape, a source whose actual blocker is the first
apex cannot make deletion of the first frontier point fail.  On a directed
cross-membership arm, simultaneous failure of both frontier deletions at an
actual blocker is therefore equivalent to equality with the already-known
blocker selected for the first frontier source.
-/

open scoped EuclideanGeometry

namespace Problem97
namespace ATailAnchoredDoubleDeletionProducerScratch
...
critical supports.  The proof uses the no-`q`-free field at the first source,
so it does not identify independently chosen anonymous rows. -/
...
blocker's exact critical shell.  This is the source-indexed fiber constraint
that is absent from an anonymous blocker graph. -/
...
theorem six_le_sources_outside_qBlockerFiber
    ...
    (hcard : 9 < D.A.card) :
    6 ≤
      (Finset.univ \ qBlockerFiber P).card := by
...
In particular, the requested genuinely third blocker is not an intermediate
witness on this surface: producing it is already the parent contradiction. -/
...
either preserves one frontier deletion or is the known `P.q`-blocker.  The
exceptional blocker has a source fiber of cardinality at most four by
...
/-- The source-faithful negation of the proposed producer: off the card-four
escape, no source has a genuinely third actual blocker at which both frontier
deletions fail. -/
...
#print axioms firstApexClass_card_eq_four_of_qDeletion_blocked_at_actualFirstApex
#print axioms qDeletion_survives_of_actualBlocker_eq_firstApex_of_not_cardFour
#print axioms criticalSupports_eq_of_actualBlocker_eq
#print axioms source_mem_qBlockerShell_of_actualBlocker_eq
#print axioms qBlockerFiber_card_le_four
#print axioms six_le_sources_outside_qBlockerFiber
#print axioms both_frontierDeletions_blocked_iff_actualBlocker_eq_qBlocker
#print axioms frontierDeletion_survival_or_actualBlocker_eq_qBlocker
#print axioms not_exists_distinct_actualBlocker_with_both_frontierDeletions_blocked

end ATailAnchoredDoubleDeletionProducerScratch
end Problem97
