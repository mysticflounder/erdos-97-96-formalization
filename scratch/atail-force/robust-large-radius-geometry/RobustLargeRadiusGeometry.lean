...
/-!
# Scratch: global-minimality transition for a large physical-apex radius

The local robust classifier leaves an ambient radius class of cardinality at
least five at the physical second apex when its opposite cap has cardinality
at least six.  This file tests the global input still present immediately
upstream: cardinality minimality of the entire counterexample.

Choose two distinct members of the radius class in the strict opposite-cap
interior and apply global minimality to their two-point deletion.  A
cardinality-minimal blocking subdeletion has size one or two.  The resulting
transition is exact:

* one deletion survives at both the fresh blocker and the physical apex,
  yielding the existing common-deletion packet; or
* the physical apex itself is the fresh blocker, forcing its given radius to
  be the unique K4 radius and to have cardinality exactly five; or
* one fresh critical four-shell contains both chosen cap-interior points and
  both corresponding single deletions are blocked at that fresh center.

This is a global source-indexed reduction.  It does not assert that either of
the last two large-cap residuals is contradictory.
-/

open scoped EuclideanGeometry

namespace Problem97
namespace ATailRobustLargeRadiusGeometryScratch
...
private theorem overrideAt_selectedAt_support
    ...
    (hsource : source ∈ A) :
    ((H.overrideAt C hblocked).selectedAt source hsource).toCriticalFourShell.support =
...
/-- The physical apex itself can be the fresh two-point-deletion blocker only
when the given radius is the unique K4 radius and its ambient class has
cardinality exactly five.  The opposite cap is still large in this scratch
arm, so this is deliberately distinct from the closed cap-five terminal. -/
structure LargeCapUniqueFiveSecondApexRadius
    (D : CounterexampleData) (S : SurplusCapPacket D.A) : Type where
  radius : ℝ
  radius_pos : 0 < radius
  class_card_eq_five :
...
private theorem singleton_core_transition
  ...
  have hpartnerA : partner ∈ D.A := (mem_selectedClass.mp hpartnerRadius).1
...
#print axioms RobustLargeRadiusParentSurface
#print axioms SharedCriticalPairAtLargeSecondApex
#print axioms LargeCapUniqueFiveSecondApexRadius
#print axioms nonempty_minimalDeletionOutcome_of_largeSecondApexRadius

end ATailRobustLargeRadiusGeometryScratch
end Problem97
