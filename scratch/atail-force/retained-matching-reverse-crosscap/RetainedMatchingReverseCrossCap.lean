...
/-!
# Cross-cap completion of the retained reverse-hit arm

The production matching consumer leaves one apparently special arm: the
deleted source's reverse critical shell meets the first opposite cap in
exactly the two retained sources, while the *second* opposite cap has at
least five points.

That arm does not require a new Kalmanson consumer.  The reverse shell has
cardinality four, so exactly two shell points lie outside the first cap.  A
five-point second cap has at least three strict-interior points, all outside
the first cap.  Hence one of those strict-interior points is absent from the
reverse shell.  Exact critical-shell deletion semantics and the parent's
first-apex deletion robustness then produce a fresh common-deletion packet.
-/

open scoped EuclideanGeometry

namespace Problem97
namespace ATailRetainedMatchingReverseCrossCap
...
/-- The second-cap-large reverse-hit arm always contains a strict second-cap
source outside the reverse shell.  Deleting that source preserves K4 both at
the first apex and at the reverse blocker. -/
...
/-- All three production large-cap outcomes have the same terminal normal
form: a source-valid common deletion at the first apex and an actual critical
blocker.  In particular, the second-cap reverse-hit branch is not a separate
Kalmanson residual. -/
...
/-- The complete retained matching reduction therefore reaches a common
deletion packet without any remaining cap-side split. -/
...
end ATailRetainedMatchingReverseCrossCap
end Problem97
