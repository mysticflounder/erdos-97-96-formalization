...
This scratch module records the generic MUS-style lift of the Stage-I
two-source deletion argument.  Let `U ⊆ A` be a finite set of sources at
pairwise distinct distances from a prescribed center.  If deleting all of
`U` destroys every `K4` at that center, but restoring any one source restores
a `K4`, then every source lies on its own exact ambient four-point selected
class.  The selected classes for distinct sources are pairwise disjoint.
...
No axiom, `sorry`, `admit`, or `native_decide` is introduced.  This file is
CHECKED-SCRATCH by a direct Lean check recorded in the Stage-I validation log.
-/
...
#print axioms MinimalDeletionCore.shellAt_radius_eq
#print axioms criticalSelectedFourClass_of_minimal_deletion_member
#print axioms exists_minimalDeletionCore
#print axioms MinimalDeletionCore.capByIndex_card_ge_six_of_two_sources
#print axioms MinimalDeletionCore.capByIndex_card_ge_six
