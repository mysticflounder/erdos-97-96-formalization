#!/usr/bin/env python3
"""Positive-model and mutation tests for the v4 closure obstructions."""
...
def main() -> int:
    ...
    # even a single changed perpendicular-bisector clause.
    ...
    structure_audit = verify_artifact.verify_v4_clause_blocks(
        structure_clauses, structure_manifest, structure_global_vars
    )
    perpendicular_block = next(
        block for block in structure_manifest["clause_blocks"]
        if block["name"] == "perpendicular-bisector-core-exclusions"
    )
    changed_clauses = [list(clause) for clause in structure_clauses]
    changed_index = int(perpendicular_block["first_clause_1based"]) - 1
    changed_clauses[changed_index][0] *= -1
    try:
        verify_artifact.verify_v4_clause_blocks(
        ...
        raise AssertionError(
            "changed perpendicular-bisector clause was not rejected"
        )
    ...
    summary = {
        ...
        "mutations": {
            ...
            "changed_perpendicular_clause_structurally_rejected":
    ...
    print(
        ...
        "and perpendicular-bisector mutations rejected with independently "
        "checked DRAT proofs"
    )
    return 0
