...
SCHEMA = "card11_exact5_common_fullradius.v4"
...
def verify_v4_clause_blocks(
    ...
    if perpendicular_last != expected_formula_end:
        raise VerificationError(
            "perpendicular-bisector block is not the final production block"
        )
    return {
...
def main() -> int:
    ...
    global_vars = parse_global_variables(manifest, cnf_vars)
    structural = verify_v4_clause_blocks(clauses, manifest, global_vars)
    ...
    verification["v4_structural_audit"] = structural
