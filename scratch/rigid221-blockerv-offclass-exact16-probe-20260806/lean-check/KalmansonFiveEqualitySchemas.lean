...
/-!
# Five-equality Kalmanson schemas

Two seven-point contradictions extracted from the exact-sixteen `BlockerV`
four-row census.  They are cardinality-independent consequences of strict
Kalmanson inequalities.  Both increasing and decreasing boundary-index forms
are supplied so finite certificate consumers can remain orientation-neutral.
-/
...
five-equality four-row signature. -/
theorem false_of_seven_ccw_five_shell_equalities_A_of_increasing
    {A : Finset ℝ²} (hA : ConvexIndep A)
    {n : ℕ} {phi : Fin n → ℝ²}
    (hphi_inj : Function.Injective phi)
    (hphi_image : Finset.univ.image phi = A)
    ...
    (hbag : dist (phi b) (phi a) = dist (phi b) (phi g))
  ...
  simp only [dist_comm] at hbae hbag hcbf hdbe heaf
  linarith

/-- Decreasing-index companion of
`false_of_seven_ccw_five_shell_equalities_A_of_increasing`. -/
theorem false_of_seven_ccw_five_shell_equalities_A_of_decreasing
    {A : Finset ℝ²} (hA : ConvexIndep A)
    {n : ℕ} {phi : Fin n → ℝ²}
    (hphi_inj : Function.Injective phi)
    (hphi_image : Finset.univ.image phi = A)
...
theorem false_of_seven_ccw_five_shell_equalities_B_of_increasing
    {A : Finset ℝ²} (hA : ConvexIndep A)
    {n : ℕ} {phi : Fin n → ℝ²}
    (hphi_inj : Function.Injective phi)
    (hphi_image : Finset.univ.image phi = A)
    ...
    (hde : d < e) (hef : e < f) (hfg : f < g)
    (hbae : dist (phi b) (phi a) = dist (phi b) (phi e))
    (hbaf : dist (phi b) (phi a) = dist (phi b) (phi f))
  ...
  have hK2_cdfg := dist_add_dist_lt_diagonal_sum_of_ccw
    hA hphi_inj hphi_image hccw hcd (hde.trans hef) hfg
  simp only [dist_comm] at hbae hbaf hcbg hdbe heag
  linarith

/-- Decreasing-index companion of
`false_of_seven_ccw_five_shell_equalities_B_of_increasing`. -/
theorem false_of_seven_ccw_five_shell_equalities_B_of_decreasing
    {A : Finset ℝ²} (hA : ConvexIndep A)
    {n : ℕ} {phi : Fin n → ℝ²}
    (hphi_inj : Function.Injective phi)
    (hphi_image : Finset.univ.image phi = A)
