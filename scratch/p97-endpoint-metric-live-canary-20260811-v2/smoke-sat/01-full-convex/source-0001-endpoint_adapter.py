...
def _stage_atoms(
    system: Mapping[str, Any], stage: str
    ...
    included = {
        "exact-metric-relaxation": {"gauge", "equalities", "exactness", "distinctness"},
        "full-convex": set(atoms),
        "convex-only-relaxation": {"gauge", "equalities", "distinctness", "convexity"},
    }[stage]
    selected = {
...
def build_stage_smt2(
    ...
    atoms, counts = _stage_atoms(system, stage)
    ...
    for category in ("gauge", "equalities", "exactness", "distinctness", "convexity"):
...
def prepare_stage(
    ...
    *,
    ...
    if type(source_paths) not in {tuple, list} or any(
        not isinstance(path, Path) for path in source_paths
    ):
        raise EndpointMetricPiqdError("source paths must be exact Path values")
    if type(_fixture_only) is not bool:
        raise EndpointMetricPiqdError("fixture-only seam must be exact Boolean")
    if not source_paths and not (
        _fixture_only
        and system["system_id"] in {"smoke-sat", "smoke-unsat"}
        and system["sources"] == []
    ):
        raise EndpointMetricPiqdError(
    ...
    commands, counts = build_stage_smt2(system, stage)
...
def verify_sat_model(
    ...
    _atoms, expected = _stage_atoms(system, stage)
...
def _open_published_capture(output_directory: Path) -> _PublishedCapture:
    if not isinstance(output_directory, Path):
        raise EndpointMetricPiqdError("published output must be a Path")
...
def run_staged_system(
    ...
    *,
    ...
    if not isinstance(output_directory, Path):
        raise EndpointMetricPiqdError("output directory must be an exact Path")
...
def validate_published_output(
    ...
    if not isinstance(selected_root, Path):
        raise EndpointMetricPiqdError("repo_root must be a Path")
