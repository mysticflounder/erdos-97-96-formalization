...
def _closure_circle_index(closure: _EdgeClosure, n: int) -> _ClosureCircleIndex:
    """Index closure roots and same-center equality classes by bit mask."""

    labels = tuple(range(n))
    roots = [
        [
            (left, left) if left == right else closure.find(_edge(left, right))
            for right in labels
        ]
        for left in labels
    ]
...
def _three_triad_collision_core_from_closure(
    ...
    *,
    ...
    for a, b, c, d, e in permutations(range(n), 5):
        if not (roots[b][c] == roots[b][d] == roots[d][a]):
            continue
        if not (roots[c][a] == roots[c][d] == roots[c][e]):
            continue
    ...
    def u_mask(a: int, b: int) -> int:
        ...
        if key not in u_masks:
            u_masks[key] = sum(
                1 << u
                for u in labels
                if roots[u][a] == roots[u][b] and roots[b][u] == roots[b][a]
            )
        return u_masks[key]
    ...
    def v_mask(c: int, b: int, o: int) -> int:
        ...
        if key not in v_masks:
            v_masks[key] = sum(
                1 << v
                for v in labels
                if roots[c][v] == roots[c][o] and roots[v][b] == roots[v][o]
            )
        return v_masks[key]
            ...
            for b in labels_in(a_circle):
                ...
                for c in labels_in(c_mask):
                    ...
                    for u in labels_in(u_mask):
                        v_mask = circle_masks[u][o] & circle_masks[c][a]
                        for v in labels_in(v_mask):
                            va_root = roots[v][a]
                            if roots[v][b] == va_root and roots[v][c] == va_root:
                                return {
                                    "o": o,
                                    "a": a,
                                    "b": b,
...
def _seven_point_circle_network_collision_core_from_closure(
    ...
    *,
    ...
    labels = tuple(range(n))
    _, circle_masks = index if index is not None else _closure_circle_index(closure, n)
                ...
                for c in labels_in(bo_circle):
                    ...
                    for u in labels_in(oa_circle & ca_circle):
                        ...
                        for v in labels_in(av_circle & ub_circle):
                            w_mask = (
                                bo_circle & ca_circle & ub_circle & circle_masks[v][b]
                            )
...
def _exact_off_circle_core(
    ...
    return _exact_off_circle_core_from_closure(rows, _row_equality_closure(n, rows), n)
...
def _perpendicular_bisector_core(
    ...
    return _perpendicular_bisector_core_from_closure(_row_equality_closure(n, rows), n)
    ...
    *,
    ...
    for left, right in combinations(range(n), 2):
        witnesses = [
            point
            for point in range(n)
            if point not in (left, right) and roots[point][left] == roots[point][right]
        ]
        if len(witnesses) >= 3:
    ...
    *,
        ...
        checks.extend(
            [
                (
                    ...
                    lambda: _five_point_bisector_circle_core_from_closure(
                        closure, n, order, index=shared_closure_index()
                    ),
                ),
                (
                    "equality-convex-rhombus-equilateral",
                    lambda: _convex_rhombus_equilateral_core_from_closure(
                        closure, n, order, index=shared_closure_index()
                    ),
                ),
                (
    ...
    if include_extended:
        checks[2:2] = [
            (
                "equality-equal-k4",
                lambda: _equal_k4_core_from_closure(
                    closure, n, index=shared_closure_index()
                ),
            ),
            (
                "equality-equilateral-bisector-collision",
                lambda: _equilateral_bisector_collision_core_from_closure(
                    closure, n, index=shared_closure_index()
                ),
            ),
            (
                "equality-three-triad-collision",
                lambda: _three_triad_collision_core_from_closure(
                    closure, n, index=shared_closure_index()
                ),
            ),
            (
                "equality-surplus-source-collision",
                lambda: _surplus_source_collision_core_from_closure(
                    closure, n, index=shared_closure_index()
                ),
            ),
            (
                "equality-six-row-anchor-collision",
                lambda: _six_row_anchor_collision_core_from_closure(
                    closure, n, index=shared_closure_index()
                ),
            ),
            (
                "equality-seven-point-orbit-collision",
                lambda: _seven_point_orbit_collision_core_from_closure(
                    closure, n, index=shared_closure_index()
                ),
            ),
            (
                "equality-seven-point-circle-network-collision",
                lambda: _seven_point_circle_network_collision_core_from_closure(
                    closure, n, index=shared_closure_index()
                ),
            ),
        ]
...
def _exact_metric_unsat_core(
    ...
    if verdict == z3.unsat:
        ...
        return {
            ...
            "row_indices": [
                index
                for index, (tag, _equalities, _exactness) in enumerate(row_constraints)
                if tag in core
            ],
...
def extract_systems(
    ...
    for path in paths:
        ...
        def visit(
            value: Any, json_path: tuple[Any, ...], context: dict[str, Any]
        ) -> None:
            ...
            if isinstance(value, dict):
                ...
                if value.get("assignment") is not None:
                    ...
                    if str(value.get("status", "")).upper() != "SAT":
                        ...
                        if frame.n != n:
                            raise ValueError(
                                f"profile {profile} builds n={frame.n}, not {n}"
                            )
                        order = tuple(hull_order(frame))
                        if sorted(order) != list(range(n)):
                            raise ValueError(
                                f"invalid hull order for profile {profile}"
                            )
...
def _probe_system(
    ...
    *,
    ...
    for suffix, cyclic_order in ordered_orientations:
        five_point_core = _five_point_bisector_circle_core(rows, n, cyclic_order)
    ...
    for row in rows:
        ...
        equality_residuals.extend(
            abs(d2_float(row.center, point) - radius2) for point in row.support[1:]
        )
        if row.exact:
            support = frozenset(row.support)
            exact_gaps.extend(
    ...
    distinct_margins = [
        d2_float(left, right) for left in range(n) for right in range(left + 1, n)
    ]
    hull_margins = [
...
def _summary(results: Iterable[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    decisive = Counter(result.get("decisive_stage") or "none" for result in results)
