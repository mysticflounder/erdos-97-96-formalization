...
Author: orchestrator. Status: PROVEN below (pen-and-paper, short);
AWAITING ADVERSARIAL AUDIT. Resolves the `{{NEEDS_PROOF}}` item
"blocker-annotation existence lemma" from
...
is what the lane actually needs.
...
- The per-configuration claim "every convex K4 configuration, with
  ...
  concentration) and is not needed. It remains CONJECTURED at fixed n
...
its incidence abstraction (`census/p97_search/node.py`).
...
S(y) is an unchanged witness class. A′ ≠ ∅ since |A| ≥ 9. ∎
...
MINIMUM cardinality n, and pick any witness system S. By the floor,
n ≥ 9. If some x ∈ A had indeg(x) = 0 under S, Lemma D would give a
K4 configuration of size n − 1 ≤ N: if n = 9 this contradicts the
floor, and if n > 9 it contradicts minimality of n. So min in-degree
≥ 1, and by E1 the node of (A, S) is B1-annotatable. It is realizable
by construction, has n points, k = 4, and (for n > 9) carries some
closed cap profile in `profiles_for(n, 4)` by frame completeness — so
it lies in a covered cell, contradicting that cell's claim. ∎
...
- The witness-system choice is quantified correctly: Theorem C picks
  ONE arbitrary witness system for the minimal A and needs the census
  to cover the resulting single node. Census cells enumerate all
  canonical B1-annotatable nodes, so any choice is covered; no
  "for all systems" existence claim is used anywhere.
...
- k-generality: Lemma D and Theorem C use k = 4 only through the
  banked floor and the cap-profile frame. For the k = 3 control arm,
  the same proofs hold with the appropriate floor (`card_ge_five_of_K4`
  is k-general per the design doc §6; frameless cells only), giving
  the same composition with 5 in place of 9.
...
1. R-FIBER4's citation caveat stays TRUE as written (per-cell claims
   ...
   lemma is superseded: the lane-level target claim ("no
   counterexample with n ≤ N") follows from covered cells via
   Theorem C with NO unproven annotation-existence hypothesis.
2. Publishing rule: any published unconditional non-existence claim
   ...
   range plus Theorem C. Kernel gate applies to the two banked Lean
   theorems cited (floor + frame), both already kernel-checked to core
   axioms (design doc §5, §8).
...
- Per-configuration min-in-degree ≥ 1 at fixed n > 9 remains
  CONJECTURED and unused.
- This note is AWAITING ADVERSARIAL AUDIT; until then Theorem C may
  not be cited by any published claim.
