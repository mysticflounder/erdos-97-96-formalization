#!/usr/bin/env python3
"""Independently replay the generic two-Kalmanson path-motif CNF layer."""
...
def load_appender():
    ...
    spec = importlib.util.spec_from_file_location("two_k_path_appender", path)
...
def reflected_atoms() -> tuple[tuple[int, int], ...]:
    return tuple((6 - center, 6 - point) for center, point in EXPECTED_ATOMS)
...
def replay_templates(manifest: dict) -> int:
    templates = manifest.get("two_kalmanson_path_motif_checked_templates")
    ...
    for name, atoms in (
        ...
        checked = app.producer_bank.certify_two_kalmanson_cancellation(
            rows_for_atoms(atoms), 7, tuple(range(7)), proposed["core"]
        )
        if checked != proposed:
            raise ValueError(f"{name} template does not replay canonically")
        if checked.get("lean_consumer") != app.LEAN_CONSUMER:
...
def main() -> int:
    ...
    order = app.support.support.export.source.normalized_order(order_index, positions)
    if tuple(manifest.get("order", ())) != order:
        raise ValueError("derived manifest has inconsistent cyclic order")
    variables_map = app.support.support.export.selected_variables()
    clauses = sorted(reconstruct(order, variables_map))
    if manifest.get("two_kalmanson_path_motif_clauses") != len(clauses):
        raise ValueError("motif clause count mismatch")
    if manifest.get("two_kalmanson_path_motif_clause_sha256") != app.clauses_digest(clauses):
    ...
    report = {
        "schema": "p97-rigid221-exact17-two-kalmanson-path-motif-cnf-validation-v1",
        "status": "PASS",
