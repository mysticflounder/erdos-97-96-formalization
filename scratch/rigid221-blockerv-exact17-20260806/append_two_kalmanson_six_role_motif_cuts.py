#!/usr/bin/env python3
"""Compile a checked six-role generic two-Kalmanson motif into exact-17 CNF."""
...
def load_support():
    ...
    path = HERE / "append_two_kalmanson_path_motif_cuts.py"
    spec = importlib.util.spec_from_file_location("two_k_six_role_support", path)
...
SOURCE_FILES = (
    ...
    HERE / "append_two_kalmanson_path_motif_cuts.py",
    HERE / "append_ordered_row_schema_cuts.py",
    support.GENERIC_LEAN_SOURCE,
    support.PRODUCER_SOURCE,
)
MOTIF_ATOMS = (
    (1, 2), (1, 4),
    (4, 0), (4, 2),
...
def reflected_atoms() -> tuple[tuple[int, int], ...]:
    return tuple((5 - center, 5 - point) for center, point in MOTIF_ATOMS)
...
def main() -> int:
    ...
    order = support.support.support.export.source.normalized_order(
    ...
    variable_map = support.support.support.export.selected_variables()
    clauses = sorted(motif_clauses(order, variable_map))
    if source_hashes() != frozen_sources:
        raise RuntimeError("six-role motif sources changed during generation")
    ...
    manifest.update({
        "schema": "p97-rigid221-exact17-two-kalmanson-six-role-motif-cnf-v1",
        "cnf": str(output.relative_to(REPO)),
        "cnf_sha256": cnf_sha,
        "clauses": base_clause_count + len(clauses),
        "two_kalmanson_six_role_motif_base": {
        ...
        "two_kalmanson_six_role_motif_atoms": [list(pair) for pair in MOTIF_ATOMS],
        "two_kalmanson_six_role_motif_clauses": len(clauses),
        "two_kalmanson_six_role_motif_clause_sha256": clauses_digest(clauses),
        "two_kalmanson_six_role_motif_checked_templates": templates,
        "two_kalmanson_six_role_motif_lean_consumer": support.LEAN_CONSUMER,
        "two_kalmanson_six_role_motif_generation_source_hashes": frozen_sources,
    })
    blocks = dict(base_manifest.get("clause_blocks", {}))
    blocks["learned_nogoods"] = int(blocks.get("learned_nogoods", 0)) + len(clauses)
    blocks["two_kalmanson_six_role_motif"] = len(clauses)
    manifest["clause_blocks"] = blocks
    kinds = dict(base_manifest.get("learned_record_kinds", {}))
    kinds["two-kalmanson-six-role-motif"] = len(clauses)
