...
for seed in {0..23}; do
  nice -n 10 uv run python \
    ...
    --initial-patterns-glob \
      'scratch/rigid221-blockerv-exact17-20260806/three-shell-bank6-shard-*.json' \
    --seed "${seed}" \
    > "scratch/rigid221-blockerv-exact17-20260806/three-shell-rotation1-shard-${seed}.json" &
done
