#!/usr/bin/env python3
"""Round-5 v12: v8 with a sound Boolean power-pattern prepass.

Before the first mixed Boolean/real check, v12 solves the subset of v8's
...
SCHEMA_PATH = HERE / "schema_v12.json"
...
def verify_frozen_provenance(
    ...
    if expected_script_sha256 is not None and current["script_sha256"] != expected_script_sha256:
        raise RuntimeError("worker v12 source hash differs from parent invocation")
    return current
...
class FreeConstantSortData:
    """The identity and sort data that v11's ``get_vars`` exposed for a leaf."""
...
class FreeConstantSortClassifier:
    """Collect free-constant sort data once per AST node in one build/context.

    The memo deliberately belongs to an instance created inside
    ``build_boolean_relaxation``.  Z3 AST ids are context-local, so sharing this
    cache across builds or contexts would be unsound.  The ordered tuples also
    preserve ``z3.z3util.get_vars``'s first-occurrence de-duplication by printed
    constant name, including its behavior for same-named constants of different
    sorts.
    """

    def __init__(self) -> None:
        self._context: z3.Context | None = None
        self._memo: dict[int, tuple[FreeConstantSortData, ...]] = {}

    def classify(self, expression: z3.ExprRef) -> tuple[FreeConstantSortData, ...]:
        if not z3.is_expr(expression):
            raise TypeError("free-constant classification requires a Z3 expression")
        if self._context is None:
            self._context = expression.ctx
        elif expression.ctx is not self._context:
            raise ValueError("a free-constant classifier cannot span Z3 contexts")
        root_id = expression.get_id()
        if root_id in self._memo:
            return self._memo[root_id]

        stack: list[tuple[z3.ExprRef, bool]] = [(expression, False)]
        while stack:
            ...
            if z3.is_const(node):
                if is_expr_val(node):
                    ...
                    sort = node.sort()
                    self._memo[node_id] = (
                        FreeConstantSortData(str(node), sort.kind(), str(sort)),
                    )
                continue
            ...
            if not expanded:
                ...
                stack.extend(
                    ...
                    if child.get_id() not in self._memo
                )
                continue

            by_symbol: dict[str, FreeConstantSortData] = {}
            for child in children:
                for item in self._memo[child.get_id()]:
                    by_symbol.setdefault(item.symbol, item)
            self._memo[node_id] = tuple(by_symbol.values())
        return self._memo[root_id]


def free_constant_sort_names(constraint: z3.BoolRef) -> tuple[str, ...]:
    """Return the sorts of every free constant, with v11-compatible semantics."""
    data = FreeConstantSortClassifier().classify(constraint)
    return tuple(sorted({item.sort_name for item in data}))
...
def is_boolean_only_constraint(constraint: z3.BoolRef) -> bool:
    ...
    data = FreeConstantSortClassifier().classify(constraint)
    return all(item.sort_kind == z3.Z3_BOOL_SORT for item in data)
...
def build_boolean_relaxation(
    ...
    classifier = FreeConstantSortClassifier()
    for family in sorted(encoding.groups):
        for constraint in encoding.groups[family]:
            if not z3.is_bool(constraint):
                raise TypeError("an Encoding constraint must itself have Bool sort")
            sort_data = classifier.classify(constraint)
            if all(item.sort_kind == z3.Z3_BOOL_SORT for item in sort_data):
                solver.add(constraint)
                included[family] += 1
            else:
                excluded[family] += 1
                for sort in sorted({item.sort_name for item in sort_data}):
                    if sort != "Bool":
...
def run_boolean_prepass(
    ...
    while True:
        ...
        solver.add(cut)
        full_gate.add_pre_first_cut(cut, f"track_v12_pre_power_{len(learned):04d}")
...
def solve_case(
    ...
    try:
        ...
        # literals explicitly so the serialized v12 formula is replayable.
        for family in sorted(encoding.groups):
        ...
        if prepass["sound_full_case_unsat"] is True:
            ...
            while True:
                ...
                gate.add_later_cut(cut, f"track_v12_full_power_{full_phase_count:04d}")
                learned.append((cut, admitted))
    ...
    write_json(case_dir / "power_cuts_v12.json", cut_artifact)
    ...
    result.update({
        ...
        "power_cut_file": "power_cuts_v12.json",
        "power_cut_file_sha256": file_sha256(case_dir / "power_cuts_v12.json"),
...
def parser() -> argparse.ArgumentParser:
    ...
    common.add_argument("--artifacts", type=Path, default=HERE / "artifacts-v12")
