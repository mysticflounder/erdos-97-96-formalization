"""Round-5 v34: tracker-admission repair for the frozen v33 slice.

This successor changes no mathematical constraint.  It materializes the
v32 S-profile family as an empty group when the profile is ``DDD`` so that
the exact tracker allowlist and the frozen formula agree.  The resulting
tracked empty conjunction is a provenance tautology and is excluded from the
semantic Boolean universe.
"""
...
SCHEMA_PATH = HERE / "schema_v34.json"
TEST_PATH = HERE / "test_round5_cegar_v34.py"
...
FOCUSED_TEST_COMMAND = (
    ...
    "-p no:cacheprovider "
    "test_round5_cegar_v34.py test_round5_cegar_v34_telemetry.py"
)
WORKER_MODULE_ID = "round5_cegar_v34"
DEFAULT_ARTIFACT_DIR = HERE / "artifacts-v34-tracker-admission-repair"
ARTIFACT_NAMES = {
    "assignment_journal": "assignment-journal-v34",
    "boolean_master": "boolean_master_v34.smt2.gz",
    "boolean_prepass": "boolean_prepass_v34.json",
    "cegar_trace": "cegar_trace_v34.json",
    "full_frozen": "full_frozen_v34.smt2.gz",
    "witness": "witness_v34.json",
}
...
class Encoding(v33.Encoding):
    """The exact v33 encoding with a total family-to-tracker registry."""
...
def source_contract() -> dict[str, object]:
    """The mathematical source contract is exactly v33's contract."""
    return _BASE_V33_SOURCE_CONTRACT()
...
def _focused_test_attestation() -> dict[str, object]:
    ...
    if not isinstance(attestation, dict):
        raise TypeError("schema_v34 lacks a structured focused-test attestation")
...
def _dependency_hashes() -> dict[str, str]:
    paths = {
        "round5_cegar_v33.py": HERE / "round5_cegar_v33.py",
        "schema_v33.json": HERE / "schema_v33.json",
        "test_round5_cegar_v33.py": HERE / "test_round5_cegar_v33.py",
    }
    ...
    inherited.update(
        {
            ...
            "v34_dependency_sha256": _dependency_hashes(),
            ...
            "spawn_contract": {
                ...
                "encoding": "round5_cegar_v34.Encoding",
...
def verify_frozen_provenance(
    ...
    if frozen != current:
        ...
        raise RuntimeError(f"frozen provenance differs from v34 worker: {differing}")
    if expected_script_sha256 is not None and (
        current["script_sha256"] != expected_script_sha256
    ):
        raise RuntimeError("worker v34 source hash differs from parent invocation")
    return current


@contextlib.contextmanager
def _bound_v34_runtime() -> Iterator[None]:
    """Layer v34 identity over the captured, nonrecursive v33 runtime."""
...
def _promote_artifacts(case_dir: Path, result: dict[str, object]) -> None:
    ...
    mapping = {
        "cegar_trace_v33.json": "cegar_trace_v34.json",
        "boolean_prepass_v33.json": "boolean_prepass_v34.json",
        "boolean_master_v33.smt2.gz": "boolean_master_v34.smt2.gz",
        "full_frozen_v33.smt2.gz": "full_frozen_v34.smt2.gz",
        "witness_v33.json": "witness_v34.json",
        "assignment-journal-v33": "assignment-journal-v34",
    }
    ...
    if isinstance(journal, dict) and journal.get("directory") == (
        ...
        journal["directory"] = "assignment-journal-v34"
    reason = result.get("reason_incomplete")
    if reason == "v33_assignment_budget_exhausted":
        result["reason_incomplete"] = "v34_assignment_budget_exhausted"
    elif reason == "v33_wall_clock_budget_exhausted":
        result["reason_incomplete"] = "v34_wall_clock_budget_exhausted"
...
def _bound_v33_solve_dispatch() -> Iterator[None]:
    bindings: tuple[tuple[str, object], ...] = (
        ("Encoding", Encoding),
        ("_bound_v33_runtime", _bound_v34_runtime),
...
def solve_case(
    ...
    result["theory_classification"] = (
        "exact v33 mathematical formula plus v34 empty-family tracker admission; "
        "complete Boolean assignment with dual raw replay"
    )
    result["solver_lane"] = "v33-composed replay-authenticated runtime over v34.Encoding"
...
def _worker_attestation(frozen: dict[str, object]) -> dict[str, object]:
    current = provenance()
    if current != frozen:
        raise RuntimeError("v34 worker source differs from frozen parent provenance")
    return {
        "module": WORKER_MODULE_ID,
        "schema_version": SCHEMA_VERSION,
        "script_sha256": current["script_sha256"],
        "provenance_sha256": canonical_sha256(current),
        "encoding_class": "round5_cegar_v34.Encoding",
        "engine": "round5_cegar_v19.solve_case",
        "telemetry_source_sha256": telemetry.telemetry_source_sha256(),
...
def _validate_child_result(
    ...
    if result.get("semantic_bool_count") != expected_count:
        errors.append(
            "semantic Boolean count does not match v34 case profile: "
    ...
    if not isinstance(adapter, dict):
        ...
        if adapter.get("encoding") != "round5_cegar_v34.Encoding":
...
def run_matrix(
    ...
    if any(case.arm != "fresh" for case in cases):
        raise ValueError("v34 matrix admits only ordinary fresh cases")
...
def hard_canary_cases(profiles: str = "DDD") -> list[Case]:
    ...
    if profiles != "DDD":
        raise ValueError("v34 hard-canary admits only the DDD profile")
    cases = [
    ...
    if len({case.case_id for case in cases}) != len(HARD_CANARY_ROLE_TRIPLES):
        raise AssertionError("v34 hard-canary role triples are not unique")
    return cases
