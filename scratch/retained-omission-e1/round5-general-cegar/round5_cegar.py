...
# Richness later proves that an apex cannot actually be used.  Schema v6 keeps
# every carrier label in the blocker domain and imposes the full critical-row
# packet identification uniformly, rather than assuming that exclusion.
    ...
    return {
        ...
        "role_map": {
            ...
            "rich_apex_exclusion": (
                ...
                "FullyDeletionRobustAt.centerAt_ne; this is recorded here, but "
                "schema v6 still permits every carrier center; the full-row "
                "and no-qfree constraints reject an apex center without "
                "assuming the bridge as a domain restriction"
            ),
...
def _worker(
    payload: tuple[dict[str, object], int, str, int, str]
) -> dict[str, object]:
    case_data, timeout_ms, out_dir, seed, expected_script_sha256 = payload
    return solve_case(
        Case(**case_data),
        timeout_ms,
        Path(out_dir),
...
def command_case(args: argparse.Namespace) -> int:
    ...
    write_json(
        ...
        {**provenance(), "argv": sys.argv, "case_count": 1},
    )
    result = solve_case(case, args.timeout_ms, run_dir, args.seed)
    print(
        json.dumps(
            {
...
def command_retry(args: argparse.Namespace) -> int:
    ...
    for field, expected in provenance().items():
        if prior.get(field) != expected:
            raise SystemExit(
                f"cannot retry summary with mismatched {field}: "
                f"{prior.get(field)!r} != {expected!r}"
            )
