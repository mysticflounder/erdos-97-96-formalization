...
class IncidenceRoundTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.assignment = json.loads(
            (ROOT / "generated/post_core_cut_v2_assignment.json").read_text()
        )
...
class MetricRoundTests(unittest.TestCase):
    ...
    def test_first_joint_unsat_is_rich_radius_pattern(self) -> None:
        self.assertEqual(
            self.results["first_joint_unsat_stage"], "rich_radius_pattern"
        )
        rich = next(
            query
        ...
        self.assertEqual(rich["clause_count"], 921)
        self.assertEqual(rich["z3"]["status"], "UNSAT")
        self.assertEqual(rich["cvc5"]["status"], "UNSAT")
