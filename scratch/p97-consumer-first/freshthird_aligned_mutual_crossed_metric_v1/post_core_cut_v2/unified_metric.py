...
SOURCE = HERE.parent.parent / "freshthird_aligned_mutual_crossed_v1"
ASSIGNMENT_PATH = SOURCE / "generated" / "global_k4_assignment.json"
...
def validate_incidence_proposal(a: dict[str, Any]) -> list[str]:
    ...
    for source, support in expected.items():
        check(rows[source] == support, f"direct crossed exact named row at {source}")
    check(
...
class UnifiedEncoding:
    ...
    def _build(self) -> None:
        ...
        # The aligned branch provides singleton first-apex radius slices for
        # both distinct C sources inside the first cap.
        first_apex = apices[0]
        first_interior = caps[0] - {apices[1], apices[2]}
        ...
        self.add(
            ...
            d2(x, y, first_apex, r("c_first")) != d2(x, y, first_apex, r("c_second")),
...
def malformed_controls(encoding: UnifiedEncoding) -> dict[str, list[Clause]]:
    ...
    singleton_base = encoding.groups["named_rows_and_singletons"]
    return {
        ...
        "malformed_break_singleton": singleton_base
        + [
            Clause(
                "insert_other_C_source_into_singleton",
                d2(encoding.x, encoding.y, r("apex0"), r("c_first"))
                == d2(encoding.x, encoding.y, r("apex0"), r("c_second")),
                "malformed control",
            )
        ],
    }
