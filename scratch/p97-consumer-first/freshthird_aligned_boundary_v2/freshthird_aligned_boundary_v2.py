#!/usr/bin/env python3
"""Exact-17 finite diagnostic for the aligned FreshThird boundary packet (v2).

This imports the existing exact17/global incidence canary and adds only the
finite incidence/order projection of
``FreshThirdAlignedMutualBlockerBoundaryPacket`` plus the source-proved
two-radius adjacent-cap grid.  It is deliberately not a Euclidean realization
and has no n-to-general lift.
"""
...
# For first apex index oppIndex1, the left/right adjacent closed caps project
# to cap 1 (oppIndex2) and cap 2 (surplus); cap 0 is the own strict cap.
ADJACENT_CAPS = (1, 2)
...
class BoundaryCanary:
    def __init__(self, forced_orientation: str | None = None) -> None:
        ...
        # Source-proved retained first-apex classes.  These are finite support
        # booleans, not a Euclidean circle realization.
        self.radius_points = [[z3.Bool(f"aligned_radius{r}_point{p}") for p in range(N)] for r in range(2)]
        self.grid_hits = [
            [[z3.Bool(f"aligned_grid_radius{r}_side{side}_point{p}") for p in range(N)] for side in range(2)]
            for r in range(2)
        ]
        self._build()
    ...
    def _build(self) -> None:
        ...
        # Exact 2x2 adjacent-cap grid ingress.  Each retained radius has one
        # selected point in each adjacent *closed* cap.  The two radius hits
        # on each side are disjoint, and one of those two hits is strict on
        # each side.  This is the finite projection of
        # `MoserSelectorShapeAt` / `IsM44.exists_nonSurplusMoserSelectorShapes`
        # and the source producer's strict-adjacent conversion lemmas.
        for radius in range(2):
            self.add(bsum(self.radius_points[radius]) == 4)
            for side, cap in enumerate(ADJACENT_CAPS):
                for point in range(N):
                    self.add(
                        self.grid_hits[radius][side][point]
                        == z3.And(self.radius_points[radius][point], b.cap[cap][point])
                    )
                self.add(bsum(self.grid_hits[radius][side]) == 1)
                self.add(
                    z3.Or(
                        *[
                            z3.And(self.grid_hits[r][side][point], self.interior(cap, point))
                            for r in range(2) for point in range(N)
                        ]
                    )
                )
                for point in range(N):
                    self.add(z3.Not(z3.And(
                        self.grid_hits[0][side][point], self.grid_hits[1][side][point]
                    )))
...
def validate(canary: BoundaryCanary, model: z3.ModelRef) -> list[str]:
    ...
    for radius in range(2):
        radius_members = [p for p in range(N) if concrete_bool(model, canary.radius_points[radius][p])]
        if len(radius_members) != 4:
            errors.append(f"radius{radius} cardinality={len(radius_members)}")
    for side, cap in enumerate(ADJACENT_CAPS):
        side_hits: list[list[int]] = []
        strict_seen = False
        for radius in range(2):
            hits = [p for p in range(N) if concrete_bool(model, canary.grid_hits[radius][side][p])]
            side_hits.append(hits)
            if len(hits) != 1:
                errors.append(f"side{side}/radius{radius} closed-cap hits={hits}")
            if hits and concrete_bool(model, canary.interior(cap, hits[0])):
                strict_seen = True
        if not strict_seen:
            errors.append(f"side{side} has no strict adjacent-interior hit")
        if set(side_hits[0]) & set(side_hits[1]):
            errors.append(f"side{side} radius hits collide")
    return errors


def grid_witness(canary: BoundaryCanary, model: z3.ModelRef) -> str:
    parts = []
    for side, cap in enumerate(ADJACENT_CAPS):
        hits = []
        for radius in range(2):
            point = next(p for p in range(N) if concrete_bool(model, canary.grid_hits[radius][side][p]))
            strict = concrete_bool(model, canary.interior(cap, point))
            hits.append(f"r{radius}:p{point}{'*' if strict else ''}")
        parts.append(f"side{side}/cap{cap}[{','.join(hits)}]")
...
def run_case(name: str, canary: BoundaryCanary, extra: z3.BoolRef | None, expected: str) -> tuple[str, str, list[str]]:
    ...
    if result == z3.sat:
        ...
        witness += grid_witness(canary, model)
    cvc5 = cvc5_status(solver)
    return actual, cvc5, [f"{name}: z3={actual} cvc5={cvc5}{witness}"]


def main() -> int:
    results: list[str] = ["FreshThird aligned boundary v2: exact-17 finite incidence/cap diagnostic only; no Euclidean realization, universal closure, or n-to-general lift."]
    cases = [
