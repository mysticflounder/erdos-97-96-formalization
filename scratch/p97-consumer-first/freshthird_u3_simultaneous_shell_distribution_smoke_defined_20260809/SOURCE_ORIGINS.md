# Source origins and shell-distribution audit (defined smoke successor)

This successor preserves the predecessor ingress and source-derived finite
schema.  Its fresh namespace records the non-vacuous smoke control.  The
active source hashes are in `manifest.json`; the
...
The smoke control is a solver/model-production control rather than a licensed
mathematical clause: `(declare-fun smoke_dummy () Bool)` and `(assert
smoke_dummy)` are checked by both engines, and readback requires the returned
