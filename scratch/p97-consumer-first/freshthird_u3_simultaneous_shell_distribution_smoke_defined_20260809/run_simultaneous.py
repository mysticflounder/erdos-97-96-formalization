...
SHELL_P_CIRCLE_LABELS = ("q", "t1", "t2", "t3")
...
def shell_pc_var(center_name: str, label_name: str) -> str:
    return f"shell_pc_{center_name}_{label_name}"


def shell_outside_var(center_name: str) -> str:
    return f"shell_outside_{center_name}"
...
def build_live_case(patterns: list[dict[str, object]]) -> str:
    """A shell-distribution-aware, one-outside-witness live abstraction.
    ...
    is represented on the dangerous circle by four membership bits and off it
    by an aggregate count.  Pair intersections involving a shell are abstract
    integer cardinalities bounded by the source two-circle cut.
    """
    lines = ["(set-logic QF_LIA)", "(set-option :produce-models true)"]
    for c in CENTER_NAMES:
        ...
        for x in LABELS:
            lines.append(f"(declare-fun {bvar(c, x)} () Bool)")
        for x in SHELL_P_CIRCLE_LABELS:
            lines.append(f"(declare-fun {shell_pc_var(c, x)} () Bool)")
        lines.append(f"(declare-fun {shell_outside_var(c)} () Int)")
        ...
        # CriticalFourShell.support has card 4.  Its intersection with
        # insert q T is represented by these four bits; all off-circle points
        # are represented by one aggregate count.
        lines.append(f"(assert (=> (= mode_{c} 2) (= {smt_bool_sum(bvar(c, x) for x in LABELS)} 0)))")
        shell_pc_sum = smt_bool_sum(shell_pc_var(c, x) for x in SHELL_P_CIRCLE_LABELS)
        lines.append(f"(assert (=> (= mode_{c} 2) {shell_pc_var(c, 'q')}))")
        lines.append(f"(assert (=> (= mode_{c} 2) (<= {shell_pc_sum} 2)))")
        lines.append(f"(assert (=> (= mode_{c} 2) (>= {shell_outside_var(c)} 2)))")
        lines.append(f"(assert (=> (= mode_{c} 2) (= (+ {shell_pc_sum} {shell_outside_var(c)}) 4)))")
        lines.append(f"(assert (=> (not (= mode_{c} 2)) (= {shell_outside_var(c)} 0)))")
        for x in SHELL_P_CIRCLE_LABELS:
            lines.append(f"(assert (=> (not (= mode_{c} 2)) (not {shell_pc_var(c, x)})))")
...
def build_smoke(sat: bool) -> str:
    if sat:
        # Known-SAT control: the constrained dummy must appear in each engine's
        # model, so this smoke test checks model production rather than only
        # checking an unconstrained Boolean formula.
        return (
            "(set-logic QF_LIA)\n"
            "(set-option :produce-models true)\n"
            "(declare-fun smoke_dummy () Bool)\n"
            "(assert smoke_dummy)\n"
            "(check-sat)\n"
            "(exit)\n"
        )
...
def validate_control(values: dict[str, object]) -> dict[str, object]:
    return {
        "valid": values.get("smoke_dummy") is True,
        "checks": {
            "control_status": True,
            "dummy_declared_and_true": values.get("smoke_dummy") is True,
        },
        "required_values": ["smoke_dummy"],
    }
...
def validate_live(values: dict[str, object]) -> dict[str, object]:
    ...
    for c in CENTER_NAMES:
        ...
        missing.extend(shell_pc_var(c, x) for x in SHELL_P_CIRCLE_LABELS if shell_pc_var(c, x) not in values)
        if shell_outside_var(c) not in values:
            missing.append(shell_outside_var(c))
    ...
    if len(modes) == 6:
        ...
        checks["critical_shell_distribution"] = all(
            modes[c] != 2 or (
                values.get(shell_pc_var(c, "q")) is True
                and sum(bool(values.get(shell_pc_var(c, x))) for x in SHELL_P_CIRCLE_LABELS) <= 2
                and int(values.get(shell_outside_var(c), -1)) >= 2
                and sum(bool(values.get(shell_pc_var(c, x))) for x in SHELL_P_CIRCLE_LABELS)
                    + int(values.get(shell_outside_var(c), -1)) == 4
            )
            for c in CENTER_NAMES
        )
    return {
        ...
        "warning": "shell support is named only on insert q T; off-circle support is aggregate; mode 1 has one abstract outside witness",
    }
...
def main() -> None:
    ...
    schema = {
        ...
        "smoke_sat_control": {
            "declared": "smoke_dummy : Bool",
            "constraint": "smoke_dummy = true",
            "model_readback_requires": "smoke_dummy = true",
        },
        "shell_support_labels": {
            "dangerous_circle": list(SHELL_P_CIRCLE_LABELS),
            "outside": "aggregate integer count",
            "cardinality": 4,
            "licensed_by": [
                "Problem97.CriticalFourShell.inter_dangerous_p_circle_card_le_two",
                "Problem97.CriticalFourShell.two_le_sdiff_dangerous_p_circle_card",
                "Problem97.U3FixedTripleAuditFrame.criticalFourShell_dangerousCircle_distribution",
            ],
        },
        ...
        for solver in ("z3", "cvc5"):
            ...
            if result["status"] == "sat":
                ...
                if case.name == "live_escape_or_shell":
                    ...
                    validation = validate_control(values) if case.name == "smoke_sat" else {
                        "valid": True,
                        "checks": {"control_status": True},
                    }
                else:
