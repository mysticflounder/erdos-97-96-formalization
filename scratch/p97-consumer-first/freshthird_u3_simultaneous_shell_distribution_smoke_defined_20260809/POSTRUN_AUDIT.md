...
- The known controls were checked first: `smoke_sat` was SAT in Z3 and cvc5,
  and both independent models defined the constrained `smoke_dummy = true`
  (`value_count = 1`); `smoke_unsat` was UNSAT in both.
- A direct sequential replay of `models/smoke_sat.z3.model.smt2` with Z3,
  followed by `models/smoke_sat.cvc5.model.smt2` with cvc5, reproduced `sat`
  and the `smoke_dummy = true` definition in both outputs.
