...
% Vampire/TPTP syntax.  The conjecture asks whether the base is inconsistent.

fof(car_center, axiom, ! [U] : (car(U) => car(cen(U)))).
fof(center_ne_source, axiom, ! [U] : (car(U) => cen(U) != U)).
fof(mem_typed, axiom,
  ! [U,V] : (mem(U,V) => (car(U) & car(V)))).
...
fof(own_deletion_blocked, axiom,
  ! [U] : (car(U) => blocked(U,cen(U)))).
...
fof(named_carriers, axiom,
  car(pa) & car(pb) & car(ra) & car(rb) & car(qa) &
  car(qb) & car(ca) & car(cb) & car(oa)).
...
fof(same_blocker_subarm, axiom,
  cen(qa) = cen(qb) & cen(ca) = cen(qa) & cen(cb) = cen(qa) &
  cen(ca) != oa & cen(cb) != oa).
...
fof(base_inconsistent, conjecture, $false).
