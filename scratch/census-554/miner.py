...
SCRATCH = os.environ.get(
    "CENSUS554_MINER_SCRATCH",
    os.path.join(tempfile.gettempdir(), "census554_miner_scratch"),
)
...
def certifier_id():
    """Stable scheduling identity for reusable exact-certification outcomes."""

    payload = {
        "schema": CERTIFIER_SCHEMA,
...
def _dead_supports_min_k(cube, log):
    ...
    for k in range(4, L.N + 1):
        ...
        try:
            import remote_mine
            if remote_mine.enabled() and len(cands) >= remote_mine.MIN_CANDS:
                scan = remote_mine.dead_scan(
                    cands, target, MINE_WORKERS, 20, pattern_dead_fast, log)
        except ImportError:
            pass  # standalone copies (worker/, flux_worker/) have no remote_mine
        if scan is not None:
            dead, checked, early = scan
            if dead:
...
def certify_pattern_with_stats(pat, timeout=240):
    ...
    if payload is None:
        ...
        stats.update({
            ...
            "outcome": (
                "wall_guard" if stats["wall_guard_tripped"]
...
def certify_pattern(pat, timeout=240):
    """Return an exact certificate within one cumulative CPU budget.

    ``timeout`` is retained as the compatibility spelling, but its unit is
    process-plus-child CPU seconds across *all* msolve and Singular stages.
    """
