...
The public ``timeout`` accepted by the certifier is a cumulative CPU budget,
not a wall-clock allowance for every individual algebra stage.  This helper
accounts for both the Python certifier process and all reaped solver children.
Each child receives an ``RLIMIT_CPU`` no larger than the remaining cumulative
budget (and, for cheap shrink probes, no larger than the per-call slice).

A separate generous wall-clock guard remains only to catch a wedged process or
filesystem.  Wall time is reported, but it is never charged against the CPU
budget.
"""
...
class CpuBudget:
    """One cumulative process-plus-child CPU budget.

    ``resource.getrusage(RUSAGE_CHILDREN)`` is monotone for reaped direct
    children.  Certification workers execute one job at a time, so its delta
    is exactly the solver CPU attributable to this budget.
    """
    ...
    def snapshot(self) -> dict:
        return {
            "cpu_budget_seconds": self.limit_seconds,
    ...
    def _limit_child_cpu(seconds: float):
        # RLIMIT_CPU is integral.  The one-second rounding slack is reflected
        # in measured CPU, never hidden from the cumulative accounting.
    ...
    def run(
        ...
        args,
        *,
        cpu_slice_seconds: float | None = None,
        ...
        **kwargs,
        ...
        try:
            completed = subprocess.run(
                ...
                preexec_fn=self._limit_child_cpu(child_limit),
                **kwargs,
            )
