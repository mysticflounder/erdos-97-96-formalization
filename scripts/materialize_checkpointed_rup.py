...
PURE_RUP_SOURCE_SCHEMA = "p97-pure-rup-source-v1"
SOURCE_MANIFEST_KEYS = frozenset({"schema", "artifacts"})
SOURCE_ARTIFACT_KEYS = frozenset({"cnf", "normalized_lrat"})
ARTIFACT_RECORD_KEYS = frozenset({"path", "sha256", "byte_count"})
PACKAGE_ARTIFACT_KEYS = frozenset(
    {
        "start_cnf",
        "shard_1_lrat",
        "checkpoint_cnf",
...
def verify_package(package_dir: Path) -> dict[str, Any]:
    ...
    if not isinstance(records, dict) or set(records) != PACKAGE_ARTIFACT_KEYS:
        raise MaterializationError(
            "package artifacts do not match the required two-shard file set"
        )
    validate_package_source(payload.get("source"))
...
def source_artifact_records(
    ...
    """Validate the one generic source-manifest contract."""

    if not isinstance(manifest, dict):
        raise MaterializationError("source manifest is not an object")
    if set(manifest) != SOURCE_MANIFEST_KEYS:
        raise MaterializationError(
            "source manifest must contain exactly schema and artifacts"
        )
    schema = manifest.get("schema")
    if schema != PURE_RUP_SOURCE_SCHEMA:
        raise MaterializationError(
            f"unexpected source manifest schema: {schema!r}"
        )
    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict) or set(artifacts) != SOURCE_ARTIFACT_KEYS:
        raise MaterializationError(
            "source manifest artifacts must contain exactly cnf and "
            "normalized_lrat"
        )
    cnf_record = artifacts["cnf"]
    lrat_record = artifacts["normalized_lrat"]
    validate_source_artifact_record(cnf_record, label="source CNF")
    validate_source_artifact_record(lrat_record, label="normalized LRAT")
    return cnf_record, lrat_record


def validate_source_artifact_record(record: object, *, label: str) -> None:
    if not isinstance(record, dict) or set(record) != ARTIFACT_RECORD_KEYS:
        raise MaterializationError(
            f"{label} record must contain exactly path, sha256, and byte_count"
        )
    raw_path = record.get("path")
    if not isinstance(raw_path, str) or not raw_path or "\\" in raw_path:
        raise MaterializationError(f"{label} path is not canonical relative POSIX")
    ...
    if (
        ...
        raise MaterializationError(f"{label} SHA-256 is malformed")
    ...
    if (
        ...
        raise MaterializationError(f"{label} byte_count must be positive")


def validate_package_source(source: object) -> None:
    required = {
        "schema",
        "manifest",
        "manifest_content",
    ...
    if not isinstance(source, dict) or set(source) != required:
        raise MaterializationError("package source object has structural drift")
    if source.get("schema") != PURE_RUP_SOURCE_SCHEMA:
        raise MaterializationError("package source schema drift")
    manifest_record = source.get("manifest")
    validate_source_artifact_record(
        manifest_record,
        label="source manifest",
    )
    cnf_record, lrat_record = source_artifact_records(
        source.get("manifest_content")
    )
    if source.get("cnf") != cnf_record:
        raise MaterializationError("package source CNF record is inconsistent")
    if source.get("normalized_lrat") != lrat_record:
        raise MaterializationError(
            "package normalized LRAT record is inconsistent"
        )
...
def materialize(
    *,
    ...
    try:
        ...
        expected_cnf, expected_lrat = source_artifact_records(source_manifest)
        if (
            source_cnf_record["sha256"] != expected_cnf["sha256"]
            or source_cnf_record["byte_count"] != expected_cnf["byte_count"]
        ):
            raise MaterializationError("source CNF does not match its manifest")
        if (
            source_lrat_record["sha256"] != expected_lrat["sha256"]
            or source_lrat_record["byte_count"] != expected_lrat["byte_count"]
        ):
            raise MaterializationError("source LRAT does not match its manifest")
        ...
        payload: dict[str, Any] = {
            ...
            "source": {
                "schema": PURE_RUP_SOURCE_SCHEMA,
                "manifest": source_manifest_record,
                "manifest_content": source_manifest,
                "cnf": expected_cnf,
                "normalized_lrat": expected_lrat,
            },
            "numbering": {
            ...
            "materializer_invariants": [
                (
                    "The strict generic source manifest and its CNF/LRAT artifact "
                    "hashes match."
                ),
...
def load_source_manifest(
    ...
    cnf_record, lrat_record = source_artifact_records(manifest)

    source_root = manifest_path.parent.resolve()

    def manifest_artifact_path(record: dict[str, Any], *, label: str) -> Path:
        path = (source_root / str(record["path"])).resolve()
        try:
            path.relative_to(source_root)
        except ValueError as exc:
            raise MaterializationError(
                f"{label} resolves outside the source-manifest directory"
            ) from exc
        return path

    cnf_path = manifest_artifact_path(cnf_record, label="source CNF")
    lrat_path = manifest_artifact_path(
        lrat_record,
        label="normalized LRAT",
    )
    if cnf_path == lrat_path:
        raise MaterializationError("source CNF and normalized LRAT paths alias")
    ...
    if os.path.samefile(cnf_path, lrat_path):
        raise MaterializationError("source CNF and normalized LRAT files alias")
    manifest_record = artifact_record(
        manifest_path,
        relative_path=manifest_path.name,
    )
    return manifest, manifest_record, cnf_path, lrat_path
...
def main() -> int:
    ...
    mode.add_argument(
        ...
        type=Path,
        help=(
            "strict p97-pure-rup-source-v1 manifest that hash-binds the "
            "CNF and normalized dense pure-RUP LRAT"
        ),
    )
    mode.add_argument(
        "--verify-package",
