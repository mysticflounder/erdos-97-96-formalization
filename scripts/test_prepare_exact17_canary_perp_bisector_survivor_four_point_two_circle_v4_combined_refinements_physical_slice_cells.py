...
"""Adversarial tests for the v5 combined-root physical-slice preparer."""
...
import prepare_exact17_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinements_physical_slice_cells as subject
import pytest
...
def test_v5_lane_schemas_and_future_paths_are_distinct_and_unfrozen() -> None:
    ...
    assert subject.BASE_HEAD == "1730c811aa50dd83c5836262c66e34a263d3d40d"
    ...
    for schema in (
        ...
        assert "v4-combined-refinements" in schema
    assert "v4-combined-refinements" in subject.category_id(0, "none")
    assert subject.PRODUCTION_PINS_FINALIZED is False
    assert subject.REGISTERED_GENERATED_ROOT == ""


def test_immediate_parent_exporter_is_the_v4_two_k_parent() -> None:
...
_UNRELATED_PARENT_CLAUSES = (
    ...
    (-308, -277, -183),
    (-308, -277, -221),
    # Other short parent clauses are unrelated to the eight successor slots.
    (-155, -172),
    (-162,),
    (-177,),
    (-149,),
...
def _assert_expected_strict_new_profile(novelty: dict[str, Any]) -> None:
    ...
    assert novelty["strict_new_per_occurrence"] == [0, 4]
...
def _fixture(
    ...
    target_preparer = tmp_path / (
        "scripts/prepare_exact17_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinements_physical_slice_cells.py"
    )
    ...
    monkeypatch.setattr(subject, "PARENT_CLAUSES", len(_UNRELATED_PARENT_CLAUSES) + 8)
    monkeypatch.setattr(subject, "CELL_CLAUSES", len(_UNRELATED_PARENT_CLAUSES) + 8 + 6)
...
def test_successor_dimensions_and_exact_six_units() -> None:
    parent = b"p cnf 308 7409760\n1 0\n"
    payload = subject.cell_cnf_bytes(parent, 16, "unique-10")
    lines = payload.splitlines()
    assert lines[0] == b"p cnf 308 7409766"
    assert lines[-6:] == [
        b"306 0",
        b"-279 0",
        b"-280 0",
...
def test_survivor_four_point_two_circle_refinement_suffix_is_exact() -> None:
    ...
    assert subject.IMMEDIATE_PARENT_CLAUSES == 7_409_752
    assert subject.PARENT_CLAUSES == 7_409_760
    assert subject.CELL_CLAUSES == 7_409_766
...
def test_initialize_creates_exact_governed_skeleton(
    ...
    assert run["owner"] == subject.RUN_OWNER == "exact17-fourpoint-v5-preparer"
...
def test_prepares_all_76_cells_and_validates_sentinels(
    ...
    assert report["parent_novelty"]["successor_multiplicity"] == [1] * 8
    ...
    assert producer["parent_novelty"]["policy"]["subsumption"].startswith(
        "the first four exact-disjoint slots have one pinned"
    )
...
@pytest.mark.parametrize("suffix_index", range(8))
...
def test_unrelated_parent_clause_with_duplicate_literal_is_accepted(
    ...
    assert novelty["exact_parent_multiplicity"] == [0] * 8
    _assert_expected_strict_new_profile(novelty)
    assert novelty["successor_multiplicity"] == [1] * 8
