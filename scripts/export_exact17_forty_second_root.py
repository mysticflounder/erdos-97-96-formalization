"""Immutable Lean exporter for the fail-closed exact-17 Child42 boundary."""
...
def export_child42(paths: ExportPaths = PRODUCTION_PATHS, *, spec: ExportSpec = PRODUCTION_SPEC) -> dict[str, Any]:
    ...
    if paths.child.exists() or paths.receipt.exists():
        raise FileExistsError("refusing to replace existing Child42 or receipt")
...
def main(argv: list[str] | None = None) -> int:
    ...
    subparsers.add_parser("export", help="export and immutably publish the Child42 root")
    args = parser.parse_args(argv)
    if args.command == "export":
        print(json.dumps(export_child42(), indent=2, sort_keys=True))
    return 0
