...
def _validate_dotted_lean_name(value: object, *, label: str) -> str:
    if not isinstance(value, str) or not value:
        raise CompactReplayPackageError(f"{label} must be a dotted Lean name")
    components = value.split(".")
    for component in components:
        if (
            not component
            or component == "_"
            or component in LEAN_KEYWORDS
            or component[0] not in "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_"
            or any(
                character
                not in "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_0123456789"
                for character in component[1:]
            )
        ):
            raise CompactReplayPackageError(
                f"invalid {label} component: {component!r}"
            )
    return value


def _module_source_root(module_prefix: str) -> str:
    return module_prefix.replace(".", "/")


def _payload_include_path(source_path: str, payload_path: str) -> str:
    relative = posixpath.relpath(
        payload_path,
        start=posixpath.dirname(source_path),
    )
    if relative.startswith("/"):
        raise CompactReplayPackageError("payload include path is absolute")
    return relative
...
def _common_module(n: int, *, lean_namespace: str) -> str:
    return f"""/-
Copyright (c) 2026 Adam McKenna. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
...
def _checkpoint_module(
    ...
    *,
    ...
    name = f"C{index:04d}"
    include_path = _payload_include_path(source_path, payload_path)
    return f"""/-
Copyright (c) 2026 Adam McKenna. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Adam McKenna
-/
import {module_prefix}.Common

namespace {lean_namespace}.Checkpoint.{name}

/-- Canonical compact text for checkpoint {index}. -/
def text : String :=
  include_str "{include_path}"

end {lean_namespace}.Checkpoint.{name}
"""
...
def _window_module(
    ...
    *,
    ...
    imports = [
        f"import {module_prefix}.Checkpoint.{start_name}",
    ]
    if role == "rebase":
        imports.append(
            f"import {module_prefix}.Checkpoint.{finish_name}"
        )
    elif role != "terminal":
        raise CompactReplayPackageError(
...
namespace {lean_namespace}.Shard{shard}.{window_name}
...
private def actionText : String :=
  include_str "{_payload_include_path(source_path, action_payload_path)}"
...
end {lean_namespace}.Shard{shard}.{window_name}
"""
...
def _shard_module(
    ...
    *,
    ...
    imports = [
        f"import {module_prefix}.Shard{shard}.W{int(r['index']):04d}"
        for r in records
    ]
    lines = [
        "/-",
        ...
        *imports,
        ...
        "open Std.Tactic.BVDecide.LRAT.Internal",
        "",
        f"namespace {lean_namespace}.Shard{shard}",
        "",
        "open Problem97.CheckpointedRup.CompactBoundary",
        "",
    ]
    ...
    lines.extend(["", f"end {lean_namespace}.Shard{shard}", ""])
    return "\n".join(lines)
...
Authors: Adam McKenna
-/
import {module_prefix}.Shard1
import {module_prefix}.Shard2
...
def _lean_layout_record(
    ...
    *,
    ...
    return {
        "index": int(record["index"]),
        "role": str(record["role"]),
        "start_checkpoint": int(record["start_checkpoint"]),
        "end_checkpoint": int(record["end_checkpoint"]),
        "action_payload_path": action_payload_path,
    }


def _build_lean_sources(
    *,
    n: int,
    lean_namespace: str,
    module_prefix: str,
    checkpoint_payloads: dict[int, str],
    shard_1: list[dict[str, Any]],
    shard_2: list[dict[str, Any]],
) -> dict[str, str]:
    lean_namespace = _validate_dotted_lean_name(
        lean_namespace,
        label="Lean namespace",
    )
    module_prefix = _validate_dotted_lean_name(
        module_prefix,
        label="module prefix",
    )
    source_root = _module_source_root(module_prefix)
    sources: dict[str, str] = {
        f"{source_root}/Common.lean": _common_module(
            n,
            lean_namespace=lean_namespace,
        ),
    }
    for checkpoint_index in sorted(checkpoint_payloads):
        relative = f"{source_root}/Checkpoint/C{checkpoint_index:04d}.lean"
    ...
    for shard, records in ((1, shard_1), (2, shard_2)):
        for record in records:
            index = int(record["index"])
            relative = f"{source_root}/Shard{shard}/W{index:04d}.lean"
            sources[relative] = _window_module(
                shard,
                record,
                action_payload_path=str(record["action_payload_path"]),
                lean_namespace=lean_namespace,
                module_prefix=module_prefix,
                source_path=relative,
            )
        sources[f"{source_root}/Shard{shard}.lean"] = _shard_module(
            shard,
            records,
            lean_namespace=lean_namespace,
            module_prefix=module_prefix,
    ...
    if int(shard_2[0]["start_checkpoint"]) != boundary:
        raise CompactReplayPackageError("shard boundary checkpoint drift")
    sources[f"{source_root}/Compose.lean"] = _compose_module(
    ...
    *,
    ...
    try:
        ...
        shard_1_layout = [
            _lean_layout_record(
                record,
                action_payload_path=action_payloads[(1, int(record["index"]))],
            )
            for record in shard_1
        ]
        shard_2_layout = [
            _lean_layout_record(
                record,
                action_payload_path=action_payloads[(2, int(record["index"]))],
            )
            for record in shard_2
        ]
        sources = _build_lean_sources(
            n=n,
            lean_namespace=lean_namespace,
            module_prefix=module_prefix,
            checkpoint_payloads=checkpoint_payloads,
            shard_1=shard_1_layout,
            shard_2=shard_2_layout,
        )
        ...
        manifest: dict[str, Any] = {
            ...
            "source": {
                ...
                "tools": base.without_provenance_paths(
                    copy.deepcopy(payload.get("tools"))
                ),
                "pure_rup_source": copy.deepcopy(
                    payload["source"]["pure_rup_source"]
                ),
            },
            "lean": {
                "namespace": lean_namespace,
            ...
            "replay_layout": {
                "checkpoints": [
                    {
                        "index": index,
                        "payload_path": checkpoint_payloads[index],
                    }
                    for index in sorted(checkpoint_payloads)
                ],
                "shards": {
                    "shard_1": shard_1_layout,
                    "shard_2": shard_2_layout,
...
def _manifest_replay_layout(
    ...
    *,
    ...
    layout = payload.get("replay_layout")
    if not isinstance(layout, dict) or set(layout) != {"checkpoints", "shards"}:
        raise CompactReplayPackageError("compact replay layout drift")
    checkpoint_records = layout.get("checkpoints")
    if not isinstance(checkpoint_records, list) or not checkpoint_records:
        raise CompactReplayPackageError("compact replay has no checkpoints")
    checkpoint_payloads: dict[int, str] = {}
    for record in checkpoint_records:
        if not isinstance(record, dict) or set(record) != {
            ...
            raise CompactReplayPackageError("checkpoint layout record drift")
        index = record.get("index")
        if isinstance(index, bool) or not isinstance(index, int) or index < 0:
            raise CompactReplayPackageError("checkpoint layout index is invalid")
        if index in checkpoint_payloads:
            raise CompactReplayPackageError("duplicate checkpoint layout index")
        relative = _safe_relative(
            record.get("payload_path"),
            label="checkpoint payload",
        ).as_posix()
        canonical = payload_by_path.get(relative)
        if canonical is None or canonical.get("kind") != "formula":
            raise CompactReplayPackageError(
                "checkpoint layout does not name a formula payload"
            )
        checkpoint_payloads[index] = relative
    if list(checkpoint_payloads) != sorted(checkpoint_payloads):
        raise CompactReplayPackageError("checkpoint layout order is not canonical")

    shards = layout.get("shards")
    if not isinstance(shards, dict) or set(shards) != {"shard_1", "shard_2"}:
        raise CompactReplayPackageError("compact replay shard layout drift")
    for shard_key in ("shard_1", "shard_2"):
        records = shards.get(shard_key)
        if not isinstance(records, list) or not records:
            raise CompactReplayPackageError(f"{shard_key} layout is empty")
        for record in records:
            if not isinstance(record, dict) or set(record) != {
                "index",
                "role",
                ...
                raise CompactReplayPackageError(f"{shard_key} layout record drift")
            relative = _safe_relative(
                record.get("action_payload_path"),
                label=f"{shard_key} action payload",
            ).as_posix()
            canonical = payload_by_path.get(relative)
            if canonical is None or canonical.get("kind") != "actions":
                raise CompactReplayPackageError(
                    f"{shard_key} layout does not name an actions payload"
                )
    shard_1 = shards["shard_1"]
    shard_2 = shards["shard_2"]
    _, _, consumed = _validate_window_layout(
        {"windows": {"shard_1": shard_1, "shard_2": shard_2}}
    )
    if set(checkpoint_payloads) != consumed:
        raise CompactReplayPackageError(
            "checkpoint layout does not match the replay chain"
        )
    return checkpoint_payloads, shard_1, shard_2
...
def verify_compact_replay_package(
    ...
    if not isinstance(source_record, dict) or set(source_record) != {
        ...
        raise CompactReplayPackageError("compact replay source object drift")
    if source_record.get("schema") != materialize_windowed_rup.SCHEMA:
        raise CompactReplayPackageError("compact replay source schema drift")
    base.validate_package_source(source_record.get("pure_rup_source"))

    lean = payload.get("lean")
    if not isinstance(lean, dict) or set(lean) != {
        "namespace",
        "module_prefix",
        "source_root",
    }:
        raise CompactReplayPackageError("compact replay Lean identity drift")
    lean_namespace = _validate_dotted_lean_name(
        lean.get("namespace"),
        label="Lean namespace",
    )
    module_prefix = _validate_dotted_lean_name(
        lean.get("module_prefix"),
        label="module prefix",
    )
    if lean.get("source_root") != _module_source_root(module_prefix):
        raise CompactReplayPackageError(
            "module prefix and Lean source root are inconsistent"
        )
    ...
    if not isinstance(counts, dict) or not isinstance(source_counts, dict):
        raise CompactReplayPackageError("compact replay counts object drift")
    variable_count_value = counts.get("variable_count")
    n_value = counts.get("n")
    if (
        isinstance(variable_count_value, bool)
        or not isinstance(variable_count_value, int)
        or variable_count_value <= 0
    ):
        raise CompactReplayPackageError("compact variable count is invalid")
    if (
        isinstance(n_value, bool)
        or not isinstance(n_value, int)
        or n_value != variable_count_value + 1
    ):
        raise CompactReplayPackageError("compact PosFin bound drift")
    if source_counts.get("variable_count") != variable_count_value:
        raise CompactReplayPackageError("compact source variable-count drift")
    ...
    for record in payload_records:
        ...
        if kind == "formula":
            ...
            if semantic[0] != variable_count:
                raise CompactReplayPackageError(
                    f"formula variable-count drift: {relative_text}"
                )
        elif kind == "actions":
    ...
    checkpoint_payloads, shard_1, shard_2 = _manifest_replay_layout(
        payload,
        payload_by_path=payload_by_path,
    )
    expected_sources = _build_lean_sources(
        n=n_value,
        lean_namespace=lean_namespace,
        module_prefix=module_prefix,
        checkpoint_payloads=checkpoint_payloads,
    ...
    for record in lean_sources:
        ...
        expected_paths.add(relative_text)
        if relative_text in seen_lean_sources:
            raise CompactReplayPackageError(
                f"duplicate Lean source registry path: {relative_text}"
            )
        seen_lean_sources.add(relative_text)
        path = output_dir / relative
        if path.is_symlink() or not path.is_file():
            raise CompactReplayPackageError(
        ...
        expected_source = expected_sources.get(relative_text)
        if expected_source is None or source != expected_source:
            raise CompactReplayPackageError(
                f"generated Lean source content is inconsistent: "
                f"{relative_text}"
            )

    if seen_lean_sources != set(expected_sources):
        raise CompactReplayPackageError(
            "generated Lean source registry does not match module prefix"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--lean-namespace",
        help="dotted namespace used by generated Lean declarations",
    )
    parser.add_argument(
        "--module-prefix",
        help="dotted import and source-path prefix for generated modules",
    )
    ...
    try:
        if args.verify_replay_package is not None:
            if (
                ...
                or args.lean_namespace is not None
                or args.module_prefix is not None
            ):
                parser.error(
            ...
            if (
                ...
                or args.lean_namespace is None
                or args.module_prefix is None
            ):
                parser.error(
                    "--package-dir requires --output-dir, "
                    "--lean-namespace, and --module-prefix"
                )
            manifest = emit(
                args.package_dir,
                args.output_dir,
