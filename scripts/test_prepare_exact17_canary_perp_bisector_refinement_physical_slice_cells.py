...
"""Adversarial tests for the canary perpendicular-bisector successor preparer."""
...
import prepare_exact17_canary_perp_bisector_refinement_physical_slice_cells as subject
import pytest
...
def test_immediate_parent_exporter_is_the_model_refinement_parent() -> None:
...
def _successor(
    *,
    ...
    clauses = subject.EXPECTED_CANARY_PERP_BISECTOR_SUFFIX if suffix is None else suffix
    body = parent.split(b"\n", 1)[1]
...
def _fixture(
    ...
    monkeypatch.setattr(subject, "PARENT_CLAUSES", 8)
    monkeypatch.setattr(subject, "CELL_CLAUSES", 14)
...
def _validate_parent_novelty_pair(
    ...
    try:
        return subject.validate_canary_perp_bisector_parent_novelty(
...
def test_successor_dimensions_and_exact_six_units() -> None:
    parent = b"p cnf 308 7409296\n1 0\n"
    payload = subject.cell_cnf_bytes(parent, 16, "unique-10")
    lines = payload.splitlines()
    assert lines[0] == b"p cnf 308 7409302"
    assert lines[-6:] == [
        b"306 0",
        b"-279 0",
        b"-280 0",
...
def test_perp_bisector_suffix_and_canary_are_exact() -> None:
    assert subject.CANARY_PERP_BISECTOR_CLAUSE == (
        -88,
        -100,
        -105,
        -117,
    ...
    assert subject.EXPECTED_CANARY_PERP_BISECTOR_SUFFIX == (
        subject.CANARY_PERP_BISECTOR_CLAUSE,
    )
    assert subject.PARENT_VARIABLES == 308
    assert subject.IMMEDIATE_PARENT_CLAUSES == 7_409_295
    assert subject.PARENT_CLAUSES == 7_409_296
    assert subject.CELL_CLAUSES == 7_409_302
...
def test_initialize_creates_exact_governed_skeleton(
    ...
    assert (
        run["owner"]
        == subject.RUN_OWNER
        == "exact17-canary-perp-bisector-refinement-preparer"
    )
...
def test_prepares_all_76_cells_and_validates_sentinels(
    ...
    assert campaign["source"]["parent_novelty"]["parent_subsumer_count"] == 0
    assert report["parent_novelty"]["successor_multiplicity"] == 1
    ...
    assert wave["encoding"]["num_clauses"] == 14
...
def test_exact_parent_duplicate_of_canary_clause_fails_before_children(
    ...
    duplicate = _root().replace(
        b"-307 -264 -263 -179 0\n",
        (" ".join(map(str, subject.CANARY_PERP_BISECTOR_CLAUSE)) + " 0\n").encode(),
        1,
    )
...
def test_unrelated_parent_clause_with_duplicate_literal_is_accepted(
    ...
    assert novelty["exact_parent_multiplicity"] == 0
    assert novelty["parent_subsumer_count"] == 0
    assert novelty["successor_multiplicity"] == 1
    assert novelty["suffix_clauses"] == [list(subject.CANARY_PERP_BISECTOR_CLAUSE)]
...
def test_repeated_literal_in_canary_suffix_fails_before_children(
    ...
    paths, calls = _fixture(tmp_path, monkeypatch)
    repeated = subject.CANARY_PERP_BISECTOR_CLAUSE + (
        subject.CANARY_PERP_BISECTOR_CLAUSE[0],
    )
    _set_parent_pair(paths, _root(), suffix=(repeated,))
    _initialize(tmp_path, paths)
...
def test_wrong_canary_suffix_fails_before_children(
    ...
    paths, calls = _fixture(tmp_path, monkeypatch)
    candidate = subject.CANARY_PERP_BISECTOR_CLAUSE
    if mutation == "value":
        ...
        wrong = (-306, *candidate)
    _set_parent_pair(paths, _root(), suffix=(wrong,))
    _initialize(tmp_path, paths)
...
def test_duplicate_canary_suffix_clause_fails_before_children(
    ...
    paths, calls = _fixture(tmp_path, monkeypatch)
    candidate = subject.CANARY_PERP_BISECTOR_CLAUSE
    _set_parent_pair(paths, _root(), suffix=(candidate, candidate))
    ...
    try:
        with pytest.raises(
            ...
            subject.validate_canary_perp_bisector_parent_novelty(
