"""Validate the Lean exact-17 three-row two-circle child export.
...
that the suffix cuts the motivating thirteenth-root SAT model.
"""
...
def validate(parent_path: Path, child_path: Path, model_path: Path) -> dict[str, object]:
    ...
    with (
        ...
        for order_index, order in enumerate(ORDERS):
            for reverse in (False, True):
                line = child.readline()
                if not line:
                    raise ValueError("truncated two-circle bisector suffix")
                context = f"two-circle-bisector/{order_index}/{reverse}"
                observed = parse_clause(line, context=context)
                expected = expected_clause(order_index, order, reverse=reverse)
                if observed != expected:
                    raise ValueError(
                        f"two-circle bisector mismatch at {context}: "
                        f"{observed} != {expected}"
                    )
    ...
    if observed_suffix != NEW_CLAUSES:
        raise ValueError("wrong two-circle bisector clause count")
    if not model_cut_clauses:
        raise ValueError("new suffix does not cut the motivating model")
    return {
        "schema": "p97-exact17-lean-four-point-two-circle-bisector-rows-export-validation/v1",
        "parent_prefix_byte_identical": True,
        "variables": VARIABLES,
        "clauses": CHILD_CLAUSES,
        "four_point_two_circle_bisector_clauses": observed_suffix,
        "four_point_two_circle_bisector_suffix_sha256": digest.hexdigest(),
