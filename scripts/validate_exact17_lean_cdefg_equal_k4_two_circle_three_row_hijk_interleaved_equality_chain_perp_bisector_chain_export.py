"""Validate the Lean exact-17 perpendicular-bisector-chain child export.

Lean is authoritative. This independent checker validates the complete
equality-chain parent recursively, requires a byte-identical parent prefix,
regenerates every new clause, and confirms that the motivating SAT model is
cut.
"""
...
PERP_BISECTOR_CHAIN_CLAUSES = 544_544
CHILD_CLAUSES = PARENT_CLAUSES + PERP_BISECTOR_CHAIN_CLAUSES
...
PERP_BISECTOR_CHAIN_PAIRS: tuple[Pair, ...] = (
    (5, 1),
    (5, 0),
    (0, 5),
    (0, 4),
...
def expected_clause(
    ...
    return (
        ...
        *(
            ...
            for center, target in PERP_BISECTOR_CHAIN_PAIRS
        ),
    )
...
def validate(
    ...
    parent_report = load_validator(
        parent_validator_path, "exact17_equality_chain_validator"
    ).validate(
        base_path,
        ancestor_path,
        grandparent_path,
    ...
    with equality_chain_path.open(encoding="ascii") as parent, child_path.open(
        encoding="ascii"
    ) as child:
        if read_header(parent, equality_chain_path) != (VARIABLES, PARENT_CLAUSES):
            ...
            for step in (1, -1):
                for cut in range(17):
                    for offsets in SIX_OFFSET_CHOICES:
                        ...
                        if not line:
                            raise ValueError(
                                "truncated perpendicular-bisector suffix at "
                        ...
                        if observed != expected:
                            raise ValueError(
                                f"perpendicular-bisector mismatch at {context}: "
                                f"{observed} != {expected}"
                            )
                        if (
    ...
    if observed_suffix != PERP_BISECTOR_CHAIN_CLAUSES:
        raise ValueError(
            "wrong perpendicular-bisector-chain count: "
            f"{observed_suffix} != {PERP_BISECTOR_CHAIN_CLAUSES}"
        )
    if first_model_cut_clause is None:
        raise ValueError("new suffix does not cut the motivating model")
    return {
        "schema": "p97-exact17-lean-perp-bisector-chain-export-validation/v1",
        "parent": parent_report,
        "parent_prefix_byte_identical": True,
        "variables": VARIABLES,
        "clauses": CHILD_CLAUSES,
        "perp_bisector_chain_clauses": observed_suffix,
        "perp_bisector_chain_suffix_sha256": digest.hexdigest(),
