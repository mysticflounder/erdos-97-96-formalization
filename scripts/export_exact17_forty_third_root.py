"""Immutable Lean exporter for the fail-closed exact-17 Child43 boundary."""
...
def export_child43(paths: ExportPaths = PRODUCTION_PATHS, *, spec: ExportSpec = PRODUCTION_SPEC) -> dict[str, Any]:
    ...
    if paths.child.exists() or paths.receipt.exists():
        raise FileExistsError("refusing to replace existing Child43 or receipt")
...
def main(argv: list[str] | None = None) -> int:
    ...
    subparsers.add_parser("export", help="export and immutably publish the Child43 root")
    args = parser.parse_args(argv)
    if args.command == "export":
        print(json.dumps(export_child43(), indent=2, sort_keys=True))
    return 0
