"""Run the authenticated exact-17 current-root-two-kalmanson root through PIQD once.
...
ROOT = SCRIPT_ROOT
RUN_ROOT = ROOT / "scratch/runs/exact17-current-root-two-kalmanson-piqd/current-root-v2"
RUN_SCHEMA = "p97-exact17-current-root-two-kalmanson-piqd-run/v1"
LAUNCH_SCHEMA = "p97-exact17-current-root-two-kalmanson-piqd-launch/v1"
PROJECT = "erdos-97-96-exact17-current-root-two-kalmanson-v2"
MAX_POLLS = 2_400
POLL_INTERVAL_S = 2.0
...
SOURCE_MANIFEST_SHA256 = (
    "6f769f9632af7d125bc0578da8289e0a6604cd82684464a78d51edf6a9748da0"
)
PRODUCER_MANIFEST_SHA256 = (
    "2913e613cd9974b36e58c7df776fd39a0b17d0e807c6dc5b7b6cd6bc4a0127d6"
)


class CurrentRootTwoKalmansonRunnerError(RuntimeError):
        ...
        if method == "POST" and target.endswith("/jobs/prepare-cnf"):
            ...
            if response.status == 200:
                ...
                _require(
                    ...
                    "PIQD prepare crossed the pinned current-root-two-kalmanson CNF",
                )
                _require(
...
class RunnerSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.timeout_s) is not int or self.timeout_s != 3_600:
            raise ValueError(
                "current-root-two-kalmanson PIQD timeout must be exactly 3600 seconds"
            )
...
def _require(condition: bool, message: str) -> None:
    if not condition:
        raise CurrentRootTwoKalmansonRunnerError(message)
...
def build_static_manifests(
    ...
    source = {
        ...
        "source_id": "exact17-current-root-two-kalmanson-cumulative-root-v2",
        ...
        "cardinality_scope": (
            "finite exact17 cumulative current-root-two-kalmanson CNF; "
    ...
    producer = {
        ...
        "producer_id": f"exact17-current-root-two-kalmanson-receipt-v2-{spec.receipt_sha256}",
    ...
    try:
        try:
            ...
            raise FileExistsError(
                "refusing duplicate current-root-two-kalmanson PIQD launch"
            ) from exc
        run_fd = os.open(
            paths.run_root.name,
            os.O_RDONLY
    ...
    *,
    ...
    with _reserve_run(paths) as run_fd:
        ...
        if result.verdict == "UNKNOWN":
            raise CurrentRootTwoKalmansonRunnerError(result.stderr)
