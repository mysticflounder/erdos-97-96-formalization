"""Publish the Lean-owned exact-17 child34 root, fail closed."""
...
PRODUCTION_PATHS = ExportPaths(
    parent=SCRATCH / "exact17-thirty-third-root-thirty-second-model-refinements.cnf",
    lean_root=ATail / "BlockerVExactSeventeenThirtyThirdModelRefinements.lean",
    lean_export=ATail / "BlockerVExactSeventeenThirtyThirdModelRefinementsExport.lean",
    kalmanson=ATail / "BlockerVExactSeventeenThirtyThirdKalmansonRefinement.lean",
    perpendicular_bisector=ATail / "BlockerVExactSeventeenThirtyThirdPerpendicularBisectorRefinement.lean",
    child=SCRATCH / "exact17-thirty-fourth-root-thirty-third-model-refinements.cnf",
    receipt=SCRATCH / "child34-export-receipt.json",
)
...
def _unchanged(before: dict[str, FileSnapshot], paths: ExportPaths) -> dict[str, FileSnapshot]:
    ...
    if any(after[name] != snapshot for name, snapshot in before.items()):
        raise RuntimeError("authenticated input changed during child34 publication")
    return after
...
def _validate_paths(paths: ExportPaths, spec: ExportSpec) -> None:
    ...
    for name, value in actual.items():
        if _absolute(value) != Path(expected[name]):
            raise ValueError(f"{name} path is not the authenticated child34 path")
    _inputs(paths)
...
def export_child34(paths: ExportPaths = PRODUCTION_PATHS, *, spec: ExportSpec = PRODUCTION_SPEC, lean_runner: LeanRunner = _run_lean, before_receipt: Callable[[], None] | None = None) -> dict[str, Any]:
    validate_spec(spec, require_source_pins=True)
    if not spec.provisioned:
        raise UnprovisionedError("child34 Lean export is UNPROVISIONED; no live artifact pins exist")
    _validate_paths(paths, spec)
    if os.path.lexists(paths.child) or os.path.lexists(paths.receipt):
        raise FileExistsError("refusing to replace existing child34 or receipt")
    ...
    try:
        ...
        if validation["status"] != "PASS":
            raise RuntimeError("unvalidated child34 candidate cannot be published")
    ...
    try:
        ...
        if child.nlink != 1 or child.sha256 != linked["child"]["sha256"] or child.size != linked["child"]["bytes"]:
            raise RuntimeError("child34 did not survive as a sole immutable file")
        receipt = {
            "schema": "p97-exact17-child34-immutable-export-receipt/v1",
            "status": "PASS",
...
def main() -> int:
    ...
    result = export_child34(ExportPaths(args.parent.resolve(), args.lean_root.resolve(), args.lean_export.resolve(), args.kalmanson.resolve(), args.perpendicular_bisector.resolve(), args.child.resolve(), args.receipt.resolve()))
