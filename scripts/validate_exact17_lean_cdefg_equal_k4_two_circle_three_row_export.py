"""Validate the Lean exact-17 complete three-row child export.
...
regenerates every guarded three-row five-point Kalmanson clause.
"""
...
CHILD_CLAUSES = PARENT_CLAUSES + THREE_ROW_CLAUSES
NAMED_ORDER_VARIABLES = (307, 308)
ORDERS = (
    (0, 6, 8, 11, 10, 9, 12, 7, 2, 15, 16, 3, 4, 5, 1, 13, 14),
    (0, 6, 8, 11, 10, 12, 9, 7, 2, 15, 16, 3, 4, 5, 1, 13, 14),
)
FOUR_OFFSET_CHOICES = tuple(reversed(tuple(combinations(range(1, 17), 4))))
...
def expected_clause(
    ...
    return (
        ...
        -hit_var(y_point, o_point),
        -hit_var(y_point, e_point),
        -hit_var(o_point, e_point),
        -hit_var(o_point, c_point),
        -hit_var(a_point, c_point),
        -hit_var(a_point, o_point),
    )
...
def validate(
    ...
    with parent_path.open(encoding="ascii") as parent, child_path.open(
        ...
        for order_index, order in enumerate(ORDERS):
            for step in (1, -1):
                for cut in range(17):
                    for offsets in FOUR_OFFSET_CHOICES:
                        line = child.readline()
                        if not line:
                            raise ValueError(
                                "truncated three-row suffix at "
                                f"{order_index}/{step}/{cut}/{offsets}"
                            )
                        observed = parse_clause(
                            line,
                            context=f"three-row/{order_index}/{step}/{cut}/{offsets}",
                        )
    ...
    if observed_suffix != THREE_ROW_CLAUSES:
        raise ValueError(
            f"wrong three-row count: {observed_suffix} != {THREE_ROW_CLAUSES}"
        )
    return {
        "schema": "p97-exact17-lean-three-row-export-validation/v1",
        "parent": parent_report,
        "parent_prefix_byte_identical": True,
        "variables": VARIABLES,
        "clauses": CHILD_CLAUSES,
        "three_row_clauses": observed_suffix,
        "three_row_suffix_sha256": digest.hexdigest(),
    }
