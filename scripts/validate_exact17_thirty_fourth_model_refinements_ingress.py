"""Validate exact-17 child35 ingress after a fresh PIQD daemon is pinned.

The immutable child35 export is provisioned independently.  Production PIQD
...
from export_exact17_thirty_fifth_root import PRODUCTION_PATHS, ExportPaths
...
SCHEMA = "p97-exact17-thirty-fourth-model-refinements-piqd-ingress/v1"
RECEIPT_SCHEMA = "p97-exact17-child35-immutable-export-receipt/v1"
...
PRODUCTION_INGRESS_PATHS = IngressPaths(
    ...
    parent_manifest=ROOT / "scratch/exact17-lean-to-sat/piqd-ingress-manifest-thirty-third-model-refinements-core1.json",
...
PRODUCTION_INGRESS_SPEC = IngressSpec(
    ...
    parent_manifest_sha256="01a53c3772486002cbb7a54fb1db24a21027c38bd9670ab2814eb26fe3ef62a2",
...
def _check_receipt(paths: IngressPaths, spec: IngressSpec) -> dict[str, Any]:
    ...
    if set(receipt) != expected or receipt.get("schema") != RECEIPT_SCHEMA or receipt.get("status") != "PASS" or receipt.get("publication_state") != "PROVISIONED":
        raise ValueError("child35 receipt schema/status drifted")
...
def validate_ingress(paths: IngressPaths = PRODUCTION_INGRESS_PATHS, *, spec: IngressSpec = PRODUCTION_INGRESS_SPEC, expected_manifest_sha256: str | None = None) -> dict[str, Any]:
    if not spec.provisioned:
        raise UnprovisionedError("child35 PIQD ingress is UNPROVISIONED")
    ...
    if set(manifest) != {"schema", "status", "dimacs", "parent_ingress", "export_receipt", "lean", "source_modules", "validation", "piqd"} or manifest.get("schema") != SCHEMA or manifest.get("status") != "PASS":
        raise ValueError("child35 ingress schema/status drifted")
