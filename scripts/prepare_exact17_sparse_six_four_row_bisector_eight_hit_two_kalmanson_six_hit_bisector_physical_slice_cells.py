...
"""Prepare the exact-17 six-hit-bisector physical-slice successor cells.
...
root identity with the six-hit-bisector successor identities.
...
true EightHit parent and proves that the four exact successor clauses are genuinely
...
ROOT = REPO_ROOT
LANE_ID = "exact17-six-hit-bisector-preparer-20260821"
RUN_ID = "physical-slice-cell-campaign-v1"
RUN_OWNER = "exact17-six-hit-bisector-preparer"
BASE_HEAD = "8b902b9281e5e4658c339261f6c6fb0407c763d1"
...
PRODUCTION_CONFIG_SCHEMA = "p97-exact17-six-hit-bisector-preparation-config/v1"
...
PARENT_CLAUSES = 7_409_265
ORIGINAL_PARENT_CLAUSES = 7_409_261
CELL_CLAUSES = 7_409_271
CELL_COUNT = 76
...
EXPECTED_SIX_HIT_SUFFIX: tuple[tuple[int, ...], ...] = (
    (-307, -155, -167, -28, -31, -223, -232),
    (-307, -60, -58, -141, -143, -111, -107),
    (-308, -206, -218, -28, -31, -223, -232),
    (-308, -60, -58, -141, -143, -111, -107),
)
CANARY_ACTIVE_CLAUSE = EXPECTED_SIX_HIT_SUFFIX[1]
ORDER_SHA256 = sha256_bytes(
    b"exact17-sparse-six-four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-physical-slice-order-v1"
)
SCHEMA = (
    "p97-exact17-sparse-six-four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-"
    "physical-slice-preparation/v1"
)
CAMPAIGN_SCHEMA = "p97-exact17-sparse-six-four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-physical-slice-campaign/v1"
PRODUCER_SCHEMA = accepted.PRODUCER_SCHEMA
SOURCE_SCHEMA = accepted.SOURCE_SCHEMA
WAVE_SCHEMA = accepted.WAVE_SCHEMA
FINITE_SCHEMA = "p97-exact17-sparse-six-four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-physical-slice/v1"
ROOT_PRODUCER_SCHEMA = "p97-exact17-sparse-six-four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-root-producer/v1"
...
def category_id(center: int, category: str) -> str:
    ...
    return f"four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-next-center-{center:02d}-physical-{category}"
...
def validate_six_hit_bisector_parent_novelty(
    ...
    original_before = original_parent.verify("true EightHit parent")
    successor_before = successor_root.verify("six-hit-bisector successor root")
    original_fd = os.dup(original_parent.descriptor)
    successor_fd = os.dup(successor_root.descriptor)
    exact_parent_multiplicity = [0] * len(EXPECTED_SIX_HIT_SUFFIX)
    parent_subsumer_count = [0] * len(EXPECTED_SIX_HIT_SUFFIX)
    successor_multiplicity = [0] * len(EXPECTED_SIX_HIT_SUFFIX)
    try:
        ...
        with (
            ...
            if _parent_header(parent_header) != (
                ...
                raise PreparationError("true EightHit parent dimensions drifted")
            if _parent_header(successor_header) != (
                PARENT_VARIABLES,
                PARENT_CLAUSES,
            ):
                raise PreparationError("six-hit-bisector successor dimensions drifted")
            suffix_sets = tuple(set(clause) for clause in EXPECTED_SIX_HIT_SUFFIX)
            for index in range(ORIGINAL_PARENT_CLAUSES):
                ...
                for suffix_index, suffix in enumerate(EXPECTED_SIX_HIT_SUFFIX):
                    if clause == suffix:
            ...
            if parent_handle.readline():
                raise PreparationError("true EightHit parent has trailing clauses")
            observed_suffix: list[tuple[int, ...]] = []
            for suffix_index, expected in enumerate(EXPECTED_SIX_HIT_SUFFIX):
                line = successor_handle.readline()
                if not line:
                    raise PreparationError(
                        "six-hit-bisector successor suffix ended early"
                    )
                clause = _clause_tuple(
                    line,
                    PARENT_VARIABLES,
                    f"six-hit-bisector suffix clause {suffix_index}",
                )
                observed_suffix.append(clause)
                for candidate_index, candidate in enumerate(EXPECTED_SIX_HIT_SUFFIX):
                    if clause == candidate:
                        successor_multiplicity[candidate_index] += 1
                if clause != expected:
                    raise PreparationError(
                        f"six-hit-bisector successor suffix order/value drifted at {suffix_index}"
                    )
            if successor_handle.readline():
                raise PreparationError(
                    "six-hit-bisector successor has trailing clauses"
                )
    finally:
        if original_fd >= 0:
    ...
    original_after = original_parent.verify("true EightHit parent")
    successor_after = successor_root.verify("six-hit-bisector successor root")
    if _anchor(original_before) != _anchor(original_after):
        raise PreparationError("true EightHit parent changed during novelty scan")
    if _anchor(successor_before) != _anchor(successor_after):
        raise PreparationError("six-hit-bisector successor changed during novelty scan")
    if any(exact_parent_multiplicity):
        raise PreparationError(
            "a six-hit-bisector suffix clause already occurs in the parent"
        )
    if any(parent_subsumer_count):
        raise PreparationError(
            "a parent clause subsumes a six-hit-bisector suffix clause"
        )
    if successor_multiplicity != [1] * len(EXPECTED_SIX_HIT_SUFFIX):
        raise PreparationError(
            "six-hit-bisector suffix multiplicity is not exactly one"
        )
    if CANARY_ACTIVE_CLAUSE not in observed_suffix:
        raise PreparationError("canary active clause is absent from the suffix")
    return {
        "schema": "p97-exact17-six-hit-bisector-parent-novelty/v1",
        "policy": {
        ...
        "suffix_clause_count": len(EXPECTED_SIX_HIT_SUFFIX),
        "suffix_clauses": [list(clause) for clause in EXPECTED_SIX_HIT_SUFFIX],
...
def _require_production_configuration(
    ...
    if len(EXPECTED_SIX_HIT_SUFFIX) != 4:
        raise PreparationError("six-hit-bisector suffix cardinality drifted")
    if any(
        not clause or len(set(clause)) != len(clause)
        for clause in EXPECTED_SIX_HIT_SUFFIX
    ):
        raise PreparationError("six-hit-bisector suffix repeats a literal")
    if PARENT_CLAUSES != ORIGINAL_PARENT_CLAUSES + len(EXPECTED_SIX_HIT_SUFFIX):
    ...
    if CANARY_ACTIVE_CLAUSE not in EXPECTED_SIX_HIT_SUFFIX:
    ...
    *,
    ...
    with _retained_support(support_files) as custody:
        ...
        original_candidate = _capture_export_result(
            ...
            "true EightHit parent export",
        )
        original_parent = _inspect_published_cnf(
            original_candidate,
            label="true EightHit parent export",
        ...
        try:
            ...
            parent_novelty = validate_six_hit_bisector_parent_novelty(
            ...
            original_candidate.unlink("true EightHit parent temporary export")
