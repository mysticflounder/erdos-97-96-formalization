"""Immutable Lean exporter for the provisioned exact-17 Child38 root."""
...
def _publish_json_ledger_last(path: Path, payload: dict[str, Any]) -> None:
    """Publish a complete receipt atomically, refusing every existing target."""
...
def export_child38(paths: ExportPaths = PRODUCTION_PATHS, *, spec: ExportSpec = PRODUCTION_SPEC) -> dict[str, Any]:
    validate_spec(spec, require_source_pins=True)
    if not spec.provisioned:
        raise ValueError("child38 export is UNPROVISIONED")
    if paths.child.exists() or paths.receipt.exists():
        raise FileExistsError("refusing to replace existing Child38 or receipt")
    ...
    try:
        ...
        provisioned = ExportSpec(child_sha256=child_sha, child_bytes=child_bytes, model_sha256=spec.model_sha256, publication_state="PROVISIONED", lean_root_sha256=spec.lean_root_sha256, lean_export_sha256=spec.lean_export_sha256)
        ...
        receipt = {"schema": "p97-exact17-child38-immutable-export-receipt/v1", "status": "PASS", "publication_state": "PROVISIONED", "parent": {"path": str(paths.parent.resolve()), "sha256": PARENT_SHA256}, "model": {"path": str(paths.model.resolve()), "sha256": spec.model_sha256}, "lean": {"root": {"path": str(paths.lean_root.resolve()), "sha256": spec.lean_root_sha256}, "export": {"path": str(paths.lean_export.resolve()), "sha256": spec.lean_export_sha256}}, "child": {"path": str(paths.child.resolve()), "sha256": child_sha, "bytes": child_bytes, "variables": provisioned.variables, "clauses": CHILD_CLAUSES}, "validation": validation, "immutability": "exclusive-hard-link-and-ledger-last-receipt/v2"}
...
if __name__ == "__main__":
    print(json.dumps(export_child38(), indent=2, sort_keys=True))
