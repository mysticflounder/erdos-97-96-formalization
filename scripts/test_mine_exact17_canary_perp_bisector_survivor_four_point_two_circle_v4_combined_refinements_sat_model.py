...
from scripts import (
    mine_exact17_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinements_sat_model as mine,
)
...
def test_successor_lane_identity_and_dimensions_are_runner_bound() -> None:
    ...
    assert mine.NUM_CLAUSES == mine.runner.NUM_CLAUSES == 7_409_766
    assert mine.MINER_RELATIVE == (
        "scripts/mine_exact17_canary_perp_bisector_survivor_four_point_two_circle_"
        "v4_combined_refinements_sat_model.py"
    )
    assert (
        mine.MINER_NAME
        == "exact17-survivor-four-point-two-circle-v4-combined-v5-source-valid-theorem-miner"
    )
    assert mine.MINER_SCHEMA == (
        "p97-exact17-survivor-four-point-two-circle-v4-combined-v5-"
    ...
    assert mine.runner.CAMPAIGN_SCHEMA == (
        ...
        "v4-combined-v5-sat-profile-campaign/v1"
    )
    assert mine.runner.WAVE_MINE_SCHEMA == (
        "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-"
        "v4-combined-v5-wave-only-mine/v1"
    )
    assert mine.runner.SAT_REPLAY_SCHEMA == (
        "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-"
        "v4-combined-v5-independent-sat-replay/v1"
    )
    assert mine.runner.WAVE_MINE_LEDGER_SCHEMA == (
        "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-"
        "v4-combined-v5-wave-only-candidate-ledger/v1"
    )
    assert mine.runner.WAVE_MINE_CANDIDATE_SCHEMA == (
        "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-"
        "v4-combined-v5-wave-only-candidate/v1"
    )
...
def test_scanner_modules_are_not_imported_before_dependency_preflight() -> None:
    code = (
        "import sys; "
        "from scripts import "
        "mine_exact17_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinements_sat_model as m; "
