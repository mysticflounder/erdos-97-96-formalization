"""Focused lifecycle tests for the weighted-kalmanson-successor one-shot PIQD runner."""
...
import pytest
import run_piqd_exact17_weighted_kalmanson_successor_root as subject
    ...
    return {
        ...
        "ingress": {
            ...
            "export_receipt": {
                ...
                "bytes": 4_904,
                "schema": "p97-exact17-weighted-kalmanson-successor-immutable-export-receipt/v1",
            },
        },
    }
...
def _receipt(
    ...
    *,
    ...
    spec = subject.PRODUCTION_RUNNER_SPEC
    job_id = "job-weighted-kalmanson-successor"
    ...
    def make_runner(**kwargs: Any):
        ...
        def run(cnf: Path, timeout_s: int, proof_path: Path | None):
            ...
            kwargs["transport"]._verification = {
                "job_id": "job-weighted-kalmanson-successor",
                "path": "/jobs/job-weighted-kalmanson-successor/blobs/fixture",
...
def test_production_run_manifest_uses_registered_fixed_policy_layout() -> None:
    ...
    assert subject.RUN_ROOT == (
        subject.ROOT
        / "scratch/runs/exact17-weighted-kalmanson-successor-piqd-20260816/successor-v1"
    )
    ...
    assert manifest["input_digests"][
        "scratch/runs/exact17-weighted-kalmanson-successor-publication-20260816/"
        "export-v1/artifacts/exact17-weighted-kalmanson-successor.cnf"
...
def test_lane_checkpoint_must_register_exact_successor_run_root(
    ...
    unsigned = {
        ...
        "generated_roots": [
            (
                "scratch/runs/exact17-weighted-kalmanson-successor-piqd-20260816/"
                "successor-v1"
            )
        ],
    }
    ...
    checkpoint_path.write_bytes(subject.canonical_json_bytes(checkpoint) + b"\n")
    with pytest.raises(
        subject.WeightedKalmansonSuccessorRunnerError,
...
def test_manifest_transport_rejects_substituted_remote_bytes_before_confirm() -> None:
    ...
    with pytest.raises(
        subject.WeightedKalmansonSuccessorRunnerError, match="bytes drifted"
    ):
        transport(
            "POST",
            "http://127.0.0.1:7272/jobs/confirm?job_id=job-bound",
...
def test_sat_launch_uses_one_attempt_policy_and_seals_terminal(
    ...
    assert captured["max_cnf_bytes"] >= 338_646_156
...
@pytest.mark.parametrize(
    ...
    (
        (
            "confirm?job_id=job-weighted-kalmanson-successor",
            "confirm exactly once",
        ),
        ("job-weighted-kalmanson-successor/model", "model endpoint count"),
    ),
)
def test_duplicate_confirm_and_model_traces_are_rejected(
    ...
    with pytest.raises(subject.WeightedKalmansonSuccessorRunnerError, match=message):
        subject._validate_terminal_receipt(
            receipt,
            result,
            paths,
...
def test_unsat_receipt_rejects_any_model_retrieval(tmp_path: Path) -> None:
    ...
    receipt["endpoint_trace"].append(
        {
            "method": "GET",
            "target": "/jobs/job-weighted-kalmanson-successor/model",
        }
    )
...
def test_mutated_embedded_seal_is_rejected() -> None:
    ...
    with pytest.raises(
        subject.WeightedKalmansonSuccessorRunnerError, match="seal hash drifted"
    ):
...
def test_custody_reads_refuse_symlinks_and_multiply_linked_files(
    ...
    try:
        with pytest.raises(subject.WeightedKalmansonSuccessorRunnerError):
            subject._read_regular_at(directory_fd, "symlink", maximum_bytes=1024)
        with pytest.raises(
            subject.WeightedKalmansonSuccessorRunnerError, match="substituted"
        ):
