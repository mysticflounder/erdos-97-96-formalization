...
"""Prepare the exact-17 canary perpendicular-bisector physical-slice successor cells.
...
root identity with the unguarded perpendicular-bisector successor identities.
...
model-refinement parent and proves that the singleton successor clause is
...
ROOT = REPO_ROOT
LANE_ID = "exact17-canary-perp-bisector-refinement-preparer-20260821"
RUN_ID = "physical-slice-cell-campaign-v1"
RUN_OWNER = "exact17-canary-perp-bisector-refinement-preparer"
BASE_HEAD = "0d826651cbdbdc5f144f175d4e56d929870a6ab6"
...
EXPORTER_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenCanaryPerpBisectorRefinementPhysicalSliceCellExport.lean"
)
IMMEDIATE_PARENT_EXPORTER_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenSparseSixPointFourRowTwoCircleBisector"
    "EightHitTwoKalmansonCancellationSixHitBisectorCanaryTwoKalmansonModelRefinementsPhysicalSliceCellExport.lean"
)
...
TEST_PATH = ROOT / (
    "scripts/test_prepare_exact17_canary_perp_bisector_refinement_physical_slice_cells.py"
)
PRODUCTION_CONFIG_RELATIVE = Path(
    "census/p97_search/waves/exact17/canary-perp-bisector-refinement-preparation-config.json"
)
PRODUCTION_CONFIG_PATH = ROOT / PRODUCTION_CONFIG_RELATIVE
PRODUCTION_CONFIG_SCHEMA = (
    "p97-exact17-canary-perp-bisector-refinement-preparation-config/v1"
)
...
PARENT_CLAUSES = 7_409_296
IMMEDIATE_PARENT_CLAUSES = 7_409_295
CELL_CLAUSES = 7_409_302
CELL_COUNT = 76
DIRECT_SENTINELS = (
    (0, "none"),
    (0, "unique-06"),
...
CANARY_PERP_BISECTOR_CLAUSE = (-88, -100, -105, -117, -142, -143, -244, -245)
EXPECTED_CANARY_PERP_BISECTOR_SUFFIX = (CANARY_PERP_BISECTOR_CLAUSE,)
ORDER_SHA256 = sha256_bytes(
    b"exact17-canary-perp-bisector-refinement-physical-slice-order-v1"
)
SCHEMA = "p97-exact17-canary-perp-bisector-refinement-physical-slice-preparation/v1"
CAMPAIGN_SCHEMA = (
    "p97-exact17-canary-perp-bisector-refinement-physical-slice-campaign/v1"
)
PRODUCER_SCHEMA = accepted.PRODUCER_SCHEMA
SOURCE_SCHEMA = accepted.SOURCE_SCHEMA
WAVE_SCHEMA = accepted.WAVE_SCHEMA
FINITE_SCHEMA = "p97-exact17-canary-perp-bisector-refinement-physical-slice/v1"
ROOT_PRODUCER_SCHEMA = "p97-exact17-canary-perp-bisector-refinement-root-producer/v1"
...
def category_id(center: int, category: str) -> str:
    ...
    return (
        f"canary-perp-bisector-refinement-next-center-{center:02d}-physical-{category}"
    )
...
def _validate_canary_perp_bisector_contract() -> None:
    if EXPECTED_CANARY_PERP_BISECTOR_SUFFIX != (CANARY_PERP_BISECTOR_CLAUSE,):
        raise PreparationError("perpendicular-bisector suffix inventory drifted")
    if not CANARY_PERP_BISECTOR_CLAUSE:
        raise PreparationError("perpendicular-bisector clause is empty")
    if len(set(CANARY_PERP_BISECTOR_CLAUSE)) != len(CANARY_PERP_BISECTOR_CLAUSE):
        raise PreparationError("perpendicular-bisector clause repeats a literal")
    if any(
        literal == 0 or abs(literal) > PARENT_VARIABLES
        for literal in CANARY_PERP_BISECTOR_CLAUSE
    ):
        raise PreparationError("perpendicular-bisector clause exceeds the variable map")
    literal_set = set(CANARY_PERP_BISECTOR_CLAUSE)
    if any(-literal in literal_set for literal in literal_set):
        raise PreparationError("perpendicular-bisector clause is tautological")
    if PARENT_CLAUSES != IMMEDIATE_PARENT_CLAUSES + 1:
        raise PreparationError("successor root is not parent plus singleton suffix")
...
def validate_canary_perp_bisector_parent_novelty(
    immediate_parent: _PublishedFile,
    successor_root: _PublishedFile,
) -> dict[str, Any]:
    """Authenticate the exact singleton suffix against the complete parent."""
    _validate_canary_perp_bisector_contract()
    parent_before = immediate_parent.verify("immediate model-refinement parent")
    successor_before = successor_root.verify(
        "canary perpendicular-bisector successor root"
    )
    parent_fd = os.dup(immediate_parent.descriptor)
    successor_fd = os.dup(successor_root.descriptor)
    exact_parent_multiplicity = 0
    parent_subsumer_count = 0
    parent_subsumer_witness: dict[str, Any] | None = None
    successor_multiplicity = 0
    candidate_set = set(CANARY_PERP_BISECTOR_CLAUSE)
    ...
    try:
        ...
        with (
            ...
            if _parent_header(parent_header) != (
                ...
                raise PreparationError(
                    "immediate model-refinement parent dimensions drifted"
                )
            if _parent_header(successor_header) != (
                ...
                raise PreparationError(
                    "canary perpendicular-bisector successor dimensions drifted"
                )
            for index in range(IMMEDIATE_PARENT_CLAUSES):
                ...
                if clause == CANARY_PERP_BISECTOR_CLAUSE:
                    exact_parent_multiplicity += 1
                    successor_multiplicity += 1
                if set(clause) <= candidate_set:
                    parent_subsumer_count += 1
                    if parent_subsumer_witness is None:
                        parent_subsumer_witness = {
                            "parent_clause_index": index,
                            "clause": list(clause),
                        }
            if parent_handle.readline():
                raise PreparationError(
                    "immediate model-refinement parent has trailing clauses"
                )
            for suffix_index, expected in enumerate(
                EXPECTED_CANARY_PERP_BISECTOR_SUFFIX
            ):
                line = successor_handle.readline()
                if not line:
                    raise PreparationError(
                        "canary perpendicular-bisector successor suffix ended early"
                    )
                clause = _clause_tuple(
                    line,
                    PARENT_VARIABLES,
            ...
            if successor_handle.readline():
                raise PreparationError(
                    "canary perpendicular-bisector successor has trailing clauses"
                )
    finally:
        if parent_fd >= 0:
            os.close(parent_fd)
    ...
    parent_after = immediate_parent.verify("immediate model-refinement parent")
    successor_after = successor_root.verify(
        "canary perpendicular-bisector successor root"
    )
    if _anchor(parent_before) != _anchor(parent_after):
        raise PreparationError(
            "immediate model-refinement parent changed during novelty scan"
        )
    if _anchor(successor_before) != _anchor(successor_after):
        raise PreparationError(
            "canary perpendicular-bisector successor changed during novelty scan"
        )
    if exact_parent_multiplicity:
        raise PreparationError(
            "perpendicular-bisector clause already occurs in the parent"
        )
    if parent_subsumer_count:
        raise PreparationError(
            "a parent clause subsumes the perpendicular-bisector suffix"
        )
    if successor_multiplicity != 1:
        raise PreparationError(
            "perpendicular-bisector suffix multiplicity is not exactly one"
        )
    return {
        "schema": "p97-exact17-canary-perp-bisector-parent-novelty/v1",
        "policy": {
            ...
            "successor_shape": "byte-exact parent body prefix plus exact singleton suffix",
            "child_multiplicity": "the exact suffix tuple occurs once in the full successor",
        },
        "immediate_parent_clause_count_scanned": IMMEDIATE_PARENT_CLAUSES,
        "suffix_clause_count": 1,
        "suffix_clauses": [list(CANARY_PERP_BISECTOR_CLAUSE)],
        ...
        "canary_perp_bisector_clause": list(CANARY_PERP_BISECTOR_CLAUSE),
    ...
    *,
    ...
    with _retained_support(support_files) as custody:
        ...
        immediate_candidate_path = (
            output / "tmp" / "immediate-model-refinement-parent.lean.cnf"
        )
        ...
        immediate_candidate = _capture_export_result(
            immediate_result,
            immediate_candidate_path,
            "immediate model-refinement parent export",
        )
        immediate_parent = _inspect_published_cnf(
            immediate_candidate,
            label="immediate model-refinement parent export",
        ...
        try:
            ...
            parent_novelty = validate_canary_perp_bisector_parent_novelty(
            ...
            immediate_candidate.unlink(
                "immediate model-refinement parent temporary export"
            )
