...
from scripts import (
    mine_exact17_canary_perp_bisector_survivor_four_point_two_circle_v5_canary_two_kalmanson_refinements_sat_model as mine,
)
...
def test_successor_lane_identity_and_dimensions_are_runner_bound() -> None:
    ...
    assert mine.NUM_CLAUSES == mine.runner.NUM_CLAUSES == 7_409_786
    ...
    assert (
        mine.MINER_NAME
        == "exact17-survivor-four-point-two-circle-v5-canary-two-kalmanson-v6-source-valid-theorem-miner"
    )
    ...
    assert mine.MINER_VERSION == "6"
    assert mine.VERIFICATION_SCHEMA == mine.runner.MINE_VERIFICATION_SCHEMA
    assert mine.runner.CANARY_PORTFOLIO_CELL_ID == (
        "canary-perp-bisector-survivor-four-point-two-circle-v5-canary-two-kalmanson-refinements-"
        "next-center-02-physical-none-canary-two-kalmanson-v6-sat-profile-v1"
    )
    assert mine.runner.CAMPAIGN_SCHEMA == (
        "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-"
        "v5-canary-two-kalmanson-v6-sat-profile-campaign/v1"
    )
    assert mine.runner.WAVE_MINE_SCHEMA == (
        "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-"
        "v5-canary-two-kalmanson-v6-wave-only-mine/v1"
    )
    assert mine.runner.SAT_REPLAY_SCHEMA == (
        "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-"
        "v5-canary-two-kalmanson-v6-independent-sat-replay/v1"
    )
    assert mine.runner.WAVE_MINE_LEDGER_SCHEMA == (
        "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-"
        "v5-canary-two-kalmanson-v6-wave-only-candidate-ledger/v1"
    )
    assert mine.runner.WAVE_MINE_CANDIDATE_SCHEMA == (
        "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-"
        "v5-canary-two-kalmanson-v6-wave-only-candidate/v1"
    )


def test_miner_contains_no_prior_lane_identity() -> None:
    source = Path(mine.__file__).read_text()
    forbidden = (
        "v4-combined-refinements",
        "combined-v5",
        "sat-profile-portfolio-v5",
        (
            "mine_exact17_canary_perp_bisector_survivor_four_point_two_circle_"
            "v4_combined_refinements_sat_model"
        ),
    )
...
def test_scanner_modules_are_not_imported_before_dependency_preflight() -> None:
    code = (
        "import sys; "
        "from scripts import "
        "mine_exact17_canary_perp_bisector_survivor_four_point_two_circle_v5_canary_two_kalmanson_refinements_sat_model as m; "
