...
"""Adversarial tests for the canary survivor-refinement successor preparer."""
...
import prepare_exact17_canary_perp_bisector_refinement_survivor_refinements_physical_slice_cells as subject
import pytest
...
def test_immediate_parent_exporter_is_the_model_refinement_parent() -> None:
    assert subject.IMMEDIATE_PARENT_EXPORTER_PATH == subject.ROOT / (
        ...
        "ModelRefinementsPhysicalSliceCellExport.lean"
    )
...
def _successor(
    *,
    ...
    clauses = (
        subject.EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_REFINEMENT_SUFFIX
        if suffix is None
        else suffix
    )
    body = parent.split(b"\n", 1)[1]
...
def _fixture(
    ...
    target_preparer = tmp_path / (
        "scripts/prepare_exact17_canary_perp_bisector_refinement_survivor_refinements_physical_slice_cells.py"
    )
    ...
    monkeypatch.setattr(subject, "PARENT_CLAUSES", 10)
    monkeypatch.setattr(subject, "CELL_CLAUSES", 16)
...
def _validate_parent_novelty_pair(
    ...
    try:
        return subject.validate_canary_perp_bisector_survivor_refinement_parent_novelty(
...
def test_successor_dimensions_and_exact_six_units() -> None:
    parent = b"p cnf 308 7409304\n1 0\n"
    payload = subject.cell_cnf_bytes(parent, 16, "unique-10")
    lines = payload.splitlines()
    assert lines[0] == b"p cnf 308 7409310"
    assert lines[-6:] == [
        b"306 0",
        b"-279 0",
        b"-280 0",
...
def test_survivor_refinement_suffix_is_exact() -> None:
    assert subject.EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_REFINEMENT_SUFFIX == (
    ...
    assert subject.IMMEDIATE_PARENT_CLAUSES == 7_409_301
    assert subject.PARENT_CLAUSES == 7_409_304
    assert subject.CELL_CLAUSES == 7_409_310
...
def test_initialize_creates_exact_governed_skeleton(
    ...
    assert (
        run["owner"]
        == subject.RUN_OWNER
        == "exact17-canary-perp-bisector-survivor-refinements-preparer"
    )
...
def test_prepares_all_76_cells_and_validates_sentinels(
    ...
    assert campaign["source"]["parent_novelty"]["parent_subsumer_count"] == [0, 0, 0]
    assert report["parent_novelty"]["successor_multiplicity"] == [1, 1, 1]
    ...
    assert wave["encoding"]["num_clauses"] == 16
...
@pytest.mark.parametrize("suffix_index", range(3))
def test_exact_parent_duplicate_of_canary_clause_fails_before_children(
    ...
    paths, calls = _fixture(tmp_path, monkeypatch)
    candidate = subject.EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_REFINEMENT_SUFFIX[
        suffix_index
    ]
...
def test_unrelated_parent_clause_with_duplicate_literal_is_accepted(
    ...
    assert novelty["exact_parent_multiplicity"] == [0, 0, 0]
    assert novelty["parent_subsumer_count"] == [0, 0, 0]
    assert novelty["successor_multiplicity"] == [1, 1, 1]
    assert novelty["suffix_clauses"] == [
        list(clause)
        for clause in subject.EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_REFINEMENT_SUFFIX
    ]
...
def test_repeated_literal_in_canary_suffix_fails_before_children(
    ...
    paths, calls = _fixture(tmp_path, monkeypatch)
    suffix = subject.EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_REFINEMENT_SUFFIX
...
def test_duplicate_canary_suffix_clause_fails_before_children(
    ...
    try:
        with pytest.raises(
            ...
            subject.validate_canary_perp_bisector_survivor_refinement_parent_novelty(
