...
"""Prepare the exact-17 v4-combined FourPointTwoCircle physical-slice cells.
...
root identity with the eight-clause v4-combined successor identities.
...
v4 two-Kalmanson parent and proves that every successor clause has the pinned
exact/subsumption classification and is the exact ordered suffix of the
combined successor root.
"""
...
RUN_OWNER = "exact17-fourpoint-v5-preparer"
BASE_HEAD = "1730c811aa50dd83c5836262c66e34a263d3d40d"
STANDARD_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")

SOURCE_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenCanaryPerpBisectorSurvivorFourPointTwoCircleV4CombinedRefinementsPhysicalSliceCoverage.lean"
)
...
EXPORTER_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenCanaryPerpBisectorSurvivorFourPointTwoCircleV4CombinedRefinementsPhysicalSliceCellExport.lean"
)
IMMEDIATE_PARENT_EXPORTER_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenCanaryPerpBisectorSurvivorFourPointTwoCircle"
    "V4TwoKalmansonRefinementsExport.lean"
)
...
TEST_PATH = ROOT / (
    "scripts/test_prepare_exact17_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinements_physical_slice_cells.py"
)
PRODUCTION_CONFIG_RELATIVE = Path(
    "census/p97_search/waves/exact17/"
    "canary-perp-bisector-survivor-four-point-two-circle-v4-combined-refinements-preparation-config.json"
)
PRODUCTION_CONFIG_PATH = ROOT / PRODUCTION_CONFIG_RELATIVE
PRODUCTION_CONFIG_SCHEMA = "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v4-combined-refinements-preparation-config/v1"
...
PARENT_CLAUSES = 7_409_760
IMMEDIATE_PARENT_CLAUSES = 7_409_752
CELL_CLAUSES = 7_409_766
CELL_COUNT = 76
DIRECT_SENTINELS = (
    (0, "none"),
    (0, "unique-06"),
...
SOURCE_THEOREM = (
    "Problem97."
    "ATailBlockerVExactSeventeenCanaryPerpBisectorSurvivorFourPointTwoCircleV4CombinedRefinementsPhysicalSliceCoverage."
    "sourceAssign_canaryPerpBisectorSurvivorFourPointTwoCircleV4CombinedRefinementPhysicalSliceCell"
)
EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_FOUR_POINT_TWO_CIRCLE_REFINEMENT_SUFFIX = (
    (-307, -12, -14, -196, -198, -201, -18, -26, -28, -222, -233),
    (-307, -244, -245, -87, -90, -92, -151, -138, -141, -117, -108),
    (-308, -12, -14, -196, -198, -201, -18, -26, -28, -222, -233),
    (-308, -244, -245, -87, -90, -92, -151, -138, -141, -117, -108),
    (-307, -7, -12, -14, -69, -75, -196, -201, -179, -186, -103, -118),
    (-307, -252, -244, -245, -185, -184, -87, -92, -70, -76, -236, -229),
    (-308, -7, -12, -14, -69, -75, -196, -201, -179, -186, -103, -118),
    (-308, -252, -244, -245, -185, -184, -87, -92, -70, -76, -236, -229),
)
EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_FOUR_POINT_TWO_CIRCLE_REFINEMENT_SUFFIX_SHA256 = "ea8b510a8a1585246bf5d555ba1df2300aa483f10848645ad9897d4620e841d6"
# The duplicate-center occurrence has one strict parent subsumer per clause;
# the schema-M occurrence is strict-new against the full two-K parent.
EXPECTED_EXACT_PARENT_MULTIPLICITY = (0, 0, 0, 0, 0, 0, 0, 0)
EXPECTED_PARENT_SUBSUMER_COUNTS = (1, 1, 1, 1, 0, 0, 0, 0)
EXPECTED_PARENT_SUBSUMER_CLAUSES = (
    (-307, -12, -14, -196, -201, -18, -26, -222, -233),
    (-307, -244, -245, -87, -92, -151, -138, -117, -108),
    (-308, -12, -14, -196, -201, -18, -26, -222, -233),
    (-308, -244, -245, -87, -92, -151, -138, -117, -108),
    (),
    (),
    (),
    (),
)
EXPECTED_STRICT_NEW_SUFFIX_INDICES = (4, 5, 6, 7)
EXPECTED_STRICT_NEW_PER_OCCURRENCE = (0, 4)
ORDER_SHA256 = sha256_bytes(
    b"exact17-canary-perp-bisector-survivor-four-point-two-circle-v4-combined-refinements-physical-slice-order-v1"
)
SCHEMA = "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v4-combined-refinements-physical-slice-preparation/v1"
CAMPAIGN_SCHEMA = "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v4-combined-refinements-physical-slice-campaign/v1"
PRODUCER_SCHEMA = accepted.PRODUCER_SCHEMA
SOURCE_SCHEMA = accepted.SOURCE_SCHEMA
WAVE_SCHEMA = accepted.WAVE_SCHEMA
FINITE_SCHEMA = "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v4-combined-refinements-physical-slice/v1"
ROOT_PRODUCER_SCHEMA = "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v4-combined-refinements-root-producer/v1"
...
def category_id(center: int, category: str) -> str:
    ...
    return f"canary-perp-bisector-survivor-four-point-two-circle-v4-combined-refinements-next-center-{center:02d}-physical-{category}"
...
def run_lean_immediate_parent_export(
    ...
    """Run the v4 two-K immediate-parent exporter with its one-path CLI."""
    lean_root = repo_root / "lean"
    relative = exporter_path.resolve().relative_to(lean_root.resolve())
...
def _validate_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinement_contract() -> (
    None
):
    suffix = (
        EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_FOUR_POINT_TWO_CIRCLE_REFINEMENT_SUFFIX
    )
    if len(suffix) != 8 or len(set(suffix)) != 8:
        raise PreparationError(
            "survivor FourPointTwoCircle refinement suffix inventory drifted"
        )
    if (
        ...
        raise PreparationError(
            "survivor FourPointTwoCircle refinement suffix digest drifted"
        )
    for clause in suffix:
        if not clause:
            raise PreparationError(
                "survivor FourPointTwoCircle refinement clause is empty"
            )
        if len(set(clause)) != len(clause):
            raise PreparationError(
                "survivor FourPointTwoCircle refinement clause repeats a literal"
            )
        if any(literal == 0 or abs(literal) > PARENT_VARIABLES for literal in clause):
            raise PreparationError(
                "survivor FourPointTwoCircle refinement clause exceeds the variable map"
            )
        literal_set = set(clause)
        if any(-literal in literal_set for literal in literal_set):
            raise PreparationError(
                "survivor FourPointTwoCircle refinement clause is tautological"
            )
    if PARENT_CLAUSES != IMMEDIATE_PARENT_CLAUSES + len(suffix):
        raise PreparationError("successor root is not parent plus eight-clause suffix")
...
def validate_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinement_parent_novelty(
    immediate_parent: _PublishedFile,
    successor_root: _PublishedFile,
) -> dict[str, Any]:
    """Authenticate the exact eight-clause suffix against the complete parent."""
    _validate_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinement_contract()
    parent_before = immediate_parent.verify("immediate survivor-refinement parent")
    successor_before = successor_root.verify(
        "survivor FourPointTwoCircle successor root"
    )
    ...
    try:
        ...
        with (
            ...
            if _parent_header(successor_header) != (
                ...
                raise PreparationError(
                    "survivor FourPointTwoCircle successor dimensions drifted"
                )
            ...
            for suffix_index, expected in enumerate(
                ...
                if not line:
                    raise PreparationError(
                        "survivor FourPointTwoCircle successor suffix ended early"
                    )
                clause = _clause_tuple(
                    line,
                    PARENT_VARIABLES,
            ...
            if successor_handle.readline():
                raise PreparationError(
                    "survivor FourPointTwoCircle successor has trailing clauses"
                )
    finally:
        if parent_fd >= 0:
            os.close(parent_fd)
    ...
    if _anchor(successor_before) != _anchor(successor_after):
        raise PreparationError(
            "survivor FourPointTwoCircle successor changed during novelty scan"
        )
    if tuple(exact_parent_multiplicity) != EXPECTED_EXACT_PARENT_MULTIPLICITY:
        raise PreparationError("v4-combined exact parent multiplicity profile drifted")
    ...
    strict_new_per_occurrence = [
        sum(index in strict_new_suffix_indices for index in range(4)),
        sum(index in strict_new_suffix_indices for index in range(4, 8)),
    ]
    if tuple(strict_new_per_occurrence) != EXPECTED_STRICT_NEW_PER_OCCURRENCE:
        raise PreparationError("v4-combined strict-new per-occurrence profile drifted")
    if successor_multiplicity != [1] * len(suffix):
        raise PreparationError(
            "survivor FourPointTwoCircle refinement suffix multiplicity is not exactly one"
        )
    return {
        "schema": "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v4-combined-refinements-parent-novelty/v1",
        "policy": {
            ...
            "subsumption": (
                "the first four exact-disjoint slots have one pinned strict "
                "parent subsumer; the final four slots are strict-new"
            ),
            "successor_shape": "byte-exact parent body prefix plus exact ordered eight-clause suffix",
    ...
    *,
    ...
    with _retained_support(support_files) as custody:
        ...
        try:
            ...
            parent_novelty = validate_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinement_parent_novelty(
