"""Fail-closed immutable PIQD ingress for exact-17 Child42."""
...
from export_exact17_forty_second_root import PRODUCTION_PATHS, ExportPaths
...
SCHEMA = "p97-exact17-forty-second-model-refinements-piqd-ingress/v1"
RECEIPT_SCHEMA = "p97-exact17-child42-immutable-export-receipt/v1"
PARENT_MANIFEST = ROOT / "scratch/exact17-lean-to-sat/piqd-ingress-manifest-forty-first-model-refinements-core1.json"
MANIFEST = ROOT / "scratch/exact17-lean-to-sat/piqd-ingress-manifest-forty-second-model-refinements-core1.json"
MANIFEST_SHA256: str | None = "8df92c533959f8717b2d897756d5c668e95f6adf9bb7bdca6eb5d8ee7ef758e4"
...
PRODUCTION_INGRESS_SPEC = IngressSpec(
    PRODUCTION_SPEC,
    "a82887fa8029dd3241eff659d8419f904dba724abcad8b45902dba9a4817ca7d",
)
...
def _check_receipt(paths: IngressPaths, spec: IngressSpec, validation: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("child42 receipt schema/status drifted")
    ...
    if not isinstance(lean, dict) or set(lean) != {"root", "export"}:
        raise ValueError("child42 Lean receipt shape drifted")
    ...
    if receipt.get("child") != {
        ...
        raise ValueError("child42 receipt child identity drifted")
    if receipt.get("validation") != validation:
        raise ValueError("child42 receipt validation drifted")
    if receipt.get("immutability") != "exclusive-hard-link-and-ledger-last-receipt/v2":
        raise ValueError("child42 receipt immutability policy drifted")
    return receipt


def _daemon_record(paths: IngressPaths, spec: IngressSpec) -> dict[str, str]:
    if spec.daemon_build_receipt_sha256 is None:
        raise UnprovisionedError("child42 daemon receipt is unpinned")
...
def validate_ingress(paths: IngressPaths = PRODUCTION_INGRESS_PATHS, *, spec: IngressSpec = PRODUCTION_INGRESS_SPEC, expected_manifest_sha256: str | None = None) -> dict[str, Any]:
    if not spec.provisioned:
        raise UnprovisionedError("child42 PIQD ingress is UNPROVISIONED")
    if (
        MANIFEST_SHA256 is None
        or not paths.export.child.is_file()
    ...
    if sha256_file(paths.parent_manifest) != spec.parent_manifest_sha256:
        raise ValueError("child41 parent ingress hash drifted")
