...
"""Prepare the exact-17 six-hit-bisector-canary-two-kalmanson-model-refinements physical-slice successor cells.
...
root identity with the model-refinement successor identities.
...
canary two-Kalmanson parent and proves that the nine retained successor clauses
are genuinely new, unsubsumed by the parent, and the exact suffix of the
successor root. The same full-parent scan authenticates the original 16-clause
theorem orbit and requires concrete parent subsumers for all seven clauses kept
outside the production suffix.
"""
...
ROOT = REPO_ROOT
LANE_ID = (
    "exact17-six-hit-bisector-canary-two-kalmanson-model-refinements-preparer-20260821"
)
RUN_ID = "physical-slice-cell-campaign-v1"
RUN_OWNER = "exact17-six-hit-bisector-canary-two-kalmanson-model-refinements-preparer"
BASE_HEAD = "fff3303e853fea4a1d262793e55693553f38272f"
...
TEST_PATH = ROOT / (
    ...
    "eight_hit_two_kalmanson_six_hit_bisector_canary_two_kalmanson_model_refinements_physical_slice_cells.py"
)
PRODUCTION_CONFIG_RELATIVE = Path(
    "census/p97_search/waves/exact17/six-hit-bisector-canary-two-kalmanson-model-refinements-preparation-config.json"
)
PRODUCTION_CONFIG_PATH = ROOT / PRODUCTION_CONFIG_RELATIVE
PRODUCTION_CONFIG_SCHEMA = "p97-exact17-six-hit-bisector-canary-two-kalmanson-model-refinements-preparation-config/v1"
...
PARENT_CLAUSES = 7_409_295
ORIGINAL_PARENT_CLAUSES = 7_409_286
CELL_CLAUSES = 7_409_301
CELL_COUNT = 76
...
SOURCE_THEOREM = (
    ...
    "EightHitTwoKalmansonSixHitBisectorCanaryTwoKalmansonModelRefinementsPhysicalSliceCell"
)
ORIGINAL_CANARY_TWO_KALMANSON_ORBIT: tuple[tuple[int, ...], ...] = (
    (-307, -263, -260, -175, -172, -169, -161, -135, -121),
    (-307, -264, -263, -135, -130, -79, -77, -67, -59),
    (-308, -263, -260, -220, -212, -175, -172, -135, -121),
    (-308, -264, -263, -135, -130, -79, -77, -67, -59),
    (-307, -269, -263, -169, -161, -135, -121, -82, -70),
    (-307, -264, -263, -179, -177, -135, -126, -67, -59),
    (-308, -269, -263, -220, -212, -135, -121, -82, -70),
    (-308, -264, -263, -179, -177, -135, -126, -67, -59),
    (-307, -284, -275, -230, -224, -102, -94, -51, -46),
    (-307, -210, -207, -200, -189, -105, -104, -47, -40),
    (-308, -284, -275, -230, -224, -102, -94, -51, -46),
    (-308, -197, -189, -159, -156, -105, -104, -44, -40),
    (-307, -277, -275, -215, -207, -180, -175, -102, -96, -64, -61, -51, -47),
    (-307, -277, -275, -215, -207, -200, -192, -170, -157, -79, -72, -51, -47),
    (-308, -277, -275, -183, -175, -164, -156, -102, -96, -64, -61, -51, -44),
    (-308, -277, -275, -221, -208, -197, -192, -164, -156, -79, -72, -51, -44),
)
RETAINED_ORIGINAL_SUFFIX_INDICES = (0, 1, 2, 3, 4, 6, 8, 10, 12)
PARENT_SUBSUMED_ORIGINAL_SUFFIX_INDICES = (5, 7, 9, 11, 13, 14, 15)
...
ORDER_SHA256 = sha256_bytes(
    b"exact17-sparse-six-four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-canary-two-kalmanson-model-refinements-physical-slice-order-v1"
)
SCHEMA = (
    "p97-exact17-sparse-six-four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-canary-two-kalmanson-model-refinements-"
    "physical-slice-preparation/v1"
)
CAMPAIGN_SCHEMA = "p97-exact17-sparse-six-four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-canary-two-kalmanson-model-refinements-physical-slice-campaign/v1"
PRODUCER_SCHEMA = accepted.PRODUCER_SCHEMA
SOURCE_SCHEMA = accepted.SOURCE_SCHEMA
WAVE_SCHEMA = accepted.WAVE_SCHEMA
FINITE_SCHEMA = "p97-exact17-sparse-six-four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-canary-two-kalmanson-model-refinements-physical-slice/v1"
ROOT_PRODUCER_SCHEMA = "p97-exact17-sparse-six-four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-canary-two-kalmanson-model-refinements-root-producer/v1"
...
def category_id(center: int, category: str) -> str:
    ...
    return f"four-row-bisector-eight-hit-two-kalmanson-six-hit-bisector-canary-two-kalmanson-model-refinements-next-center-{center:02d}-physical-{category}"
...
def _validate_canary_two_kalmanson_orbit_contract() -> None:
    if len(ORIGINAL_CANARY_TWO_KALMANSON_ORBIT) != 16:
        raise PreparationError("original two-Kalmanson orbit cardinality drifted")
    if len(set(ORIGINAL_CANARY_TWO_KALMANSON_ORBIT)) != 16:
    ...
    if (
        len(RETAINED_ORIGINAL_SUFFIX_INDICES) != 9
    ...
    if (
        len(PARENT_SUBSUMED_ORIGINAL_SUFFIX_INDICES) != 7
...
def validate_six_hit_bisector_canary_two_kalmanson_model_refinements_parent_novelty(
    ...
    successor_before = successor_root.verify(
        "six-hit-bisector-canary-two-kalmanson-model-refinements successor root"
    )
    ...
    try:
        ...
        with (
            ...
            if _parent_header(successor_header) != (
                ...
                raise PreparationError(
                    "six-hit-bisector-canary-two-kalmanson-model-refinements successor dimensions drifted"
                )
            orbit_sets = tuple(
            ...
            for suffix_index, expected in enumerate(
                ...
                if not line:
                    raise PreparationError(
                        "six-hit-bisector-canary-two-kalmanson-model-refinements successor suffix ended early"
                    )
                clause = _clause_tuple(
                    line,
                    PARENT_VARIABLES,
                    f"six-hit-bisector-canary-two-kalmanson-model-refinements suffix clause {suffix_index}",
                )
                ...
                if clause != expected:
                    raise PreparationError(
                        f"six-hit-bisector-canary-two-kalmanson-model-refinements successor suffix order/value drifted at {suffix_index}"
                    )
            if successor_handle.readline():
                raise PreparationError(
                    "six-hit-bisector-canary-two-kalmanson-model-refinements successor has trailing clauses"
                )
    finally:
        if original_fd >= 0:
    ...
    if _anchor(successor_before) != _anchor(successor_after):
        raise PreparationError(
            "six-hit-bisector-canary-two-kalmanson-model-refinements successor changed during novelty scan"
        )
    ...
    if any(retained_parent_subsumer_count):
        raise PreparationError(
            "a parent clause subsumes a retained six-hit-bisector-canary-two-kalmanson-model-refinements suffix clause"
        )
    if any(
    ...
    if successor_multiplicity != [1] * len(EXPECTED_CANARY_TWO_KALMANSON_SUFFIX):
        raise PreparationError(
            "six-hit-bisector-canary-two-kalmanson-model-refinements suffix multiplicity is not exactly one"
        )
    if CANARY_ACTIVE_CLAUSE not in observed_suffix:
        raise PreparationError("canary active clause is absent from the suffix")
    return {
        "schema": "p97-exact17-six-hit-bisector-canary-two-kalmanson-model-refinements-parent-novelty/v1",
        "policy": {
            "parent_scan": "stream-all-original-parent-clauses",
            "exact_novelty": "ordered-literal-tuple multiplicity must be zero across the full 16-clause orbit",
    ...
    *,
    ...
    with _retained_support(support_files) as custody:
        ...
        try:
            ...
            parent_novelty = validate_six_hit_bisector_canary_two_kalmanson_model_refinements_parent_novelty(
