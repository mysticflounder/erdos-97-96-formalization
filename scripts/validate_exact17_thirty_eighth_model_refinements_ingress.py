"""Fail-closed immutable PIQD ingress for exact-17 Child39."""
...
from export_exact17_thirty_ninth_root import PRODUCTION_PATHS, ExportPaths
from validate_exact17_thirty_eighth_model_refinements_export import (
    PRODUCTION_SPEC,
    ExportSpec,
    UnprovisionedError,
    sha256_file,
...
SCHEMA = "p97-exact17-thirty-eighth-model-refinements-piqd-ingress/v1"
RECEIPT_SCHEMA = "p97-exact17-child39-immutable-export-receipt/v1"
PARENT_MANIFEST = ROOT / "scratch/exact17-lean-to-sat/piqd-ingress-manifest-thirty-seventh-model-refinements-core1.json"
MANIFEST = ROOT / "scratch/exact17-lean-to-sat/piqd-ingress-manifest-thirty-eighth-model-refinements-core1.json"
MANIFEST_SHA256 = "275a61b6febdc1ca5104ebe932919c24e2226f794e68b22caf02ffcbb076c83d"
...
def _check_receipt(paths: IngressPaths, spec: IngressSpec, validation: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("child39 receipt schema/status drifted")
    ...
    if not isinstance(lean, dict) or set(lean) != {"root", "export"}:
        raise ValueError("child39 Lean receipt shape drifted")
    ...
    if receipt.get("child") != {
        ...
        raise ValueError("child39 receipt child identity drifted")
    if receipt.get("validation") != validation:
        raise ValueError("child39 receipt validation drifted")
    if receipt.get("immutability") != "exclusive-hard-link-and-ledger-last-receipt/v2":
        raise ValueError("child39 receipt immutability policy drifted")
    return receipt


def _daemon_record(paths: IngressPaths, spec: IngressSpec) -> dict[str, str]:
    if spec.daemon_build_receipt_sha256 is None:
        raise UnprovisionedError("child39 daemon receipt is unpinned")
...
def validate_ingress(paths: IngressPaths = PRODUCTION_INGRESS_PATHS, *, spec: IngressSpec = PRODUCTION_INGRESS_SPEC, expected_manifest_sha256: str | None = None) -> dict[str, Any]:
    if not spec.provisioned:
        raise UnprovisionedError("child39 PIQD ingress is UNPROVISIONED")
    if sha256_file(paths.parent_manifest) != spec.parent_manifest_sha256:
        raise ValueError("child38 parent ingress hash drifted")
