...
"""Prepare the exact-17 two-Kalmanson physical-slice successor cells.
...
root identity with the two-Kalmanson successor identities.

The constants below pin the committed Lean source and the one registered
governed run root.  Preparation still requires an exact, preinitialized empty
run-manifest skeleton before any Lean export occurs.
"""
...
ROOT = REPO_ROOT
LANE_ID = "exact17-sparse-six-four-row-bisector-two-kalmanson-preparer-20260820"
RUN_ID = "physical-slice-cell-campaign-v1"
BASE_HEAD = "01e059778a72b01f3adbe3a6733b7eebd2d212b0"

SOURCE_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenSparseSixPointFourRowTwoCircleBisector"
    "TwoKalmansonCancellationPhysicalSliceCoverage.lean"
)
ROOT_SOURCE_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenSparseSixPointFourRowTwoCircleBisector"
    "TwoKalmansonCancellation.lean"
)
EXPORTER_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenSparseSixPointFourRowTwoCircleBisector"
    "TwoKalmansonCancellationPhysicalSliceCellExport.lean"
)
...
EXPECTED_SOURCE_SHA256 = (
    "1aa90b3e922931edcf71d6f2fcd6ab6a65274885d40b01a0ee07e5f76981d225"
)
EXPECTED_ROOT_SOURCE_SHA256 = (
    "0557a41bb119d871753f7635bddf624197383ec60ebeacc159507b900e26aea7"
)
EXPECTED_EXPORTER_SHA256 = (
    "912d2e67bb9b531f30a739442e138910d94d2ef6725a69a5fffd12e554ba91be"
)
EXPECTED_VARIABLE_MAP_SHA256 = (
    "78df650209311154e9a5fb6fdb88b6e532acaa624b7789d3028434c05e38e63f"
)
EXPECTED_CHECKPOINT_SHA256 = (
    "53efeca82d1d08d0529643f7255d50239394e9c8a8dbd116f1a2f96fcf943b0e"
)
EXPECTED_CHECKPOINT_MANIFEST_SHA256 = (
    "c66c7debc2336fd309e7de2277e55fea3c1e5502497f5d7b03eabf3a3d094bbc"
)
PINNED_SOURCE_COMMIT = "722f56df2a4e4739bcada30590ec8398e3227c88"
REGISTERED_GENERATED_ROOT = (
    "scratch/runs/exact17-sparse-six-four-row-bisector-two-kalmanson-"
    "preparer-20260820/physical-slice-cell-campaign-v1"
)
...
SOURCE_THEOREM = (
    ...
    "TwoKalmansonCancellationPhysicalSliceCoverage."
    "sourceAssign_sparseSixPointFourRowTwoCircleBisector"
    "TwoKalmansonPhysicalSliceCell"
)
ORDER_SHA256 = sha256_bytes(
    b"exact17-sparse-six-four-row-bisector-two-kalmanson-physical-slice-order-v1"
)
SCHEMA = (
    "p97-exact17-sparse-six-four-row-bisector-two-kalmanson-"
...
FINITE_SCHEMA = (
    "p97-exact17-sparse-six-four-row-bisector-two-kalmanson-physical-slice/v1"
)
ROOT_PRODUCER_SCHEMA = (
    "p97-exact17-sparse-six-four-row-bisector-two-kalmanson-root-producer/v1"
)
...
def _sha256_file(path: Path) -> str:
    return accepted._sha256_file(path)


def _require_regular(path: Path, label: str) -> None:
    accepted._require_regular(path, label)


def _repo_relative(root: Path, path: Path) -> str:
    return accepted._repo_relative(root, path)


def category_id(center: int, category: str) -> str:
    accepted.hardened.category_id(center, category)
    return (
        f"four-row-bisector-two-kalmanson-next-center-{center:02d}-physical-{category}"
    )
...
def category_units(center: int, category: str) -> tuple[int, ...]:
    return (next_center_variable(center),) + accepted.hardened.category_units(
        center, category
    )
...
def cell_cnf_bytes(parent: bytes, center: int, category: str) -> bytes:
    ...
    variables, clauses = accepted.hardened._parent_header(first + separator)
        ...
        try:
            subprocess.run(
                [
                    ...
                    str(relative),
                    str(center),
                    accepted._export_category_text(category),
                    str(bound_output),
                ],
                cwd=lean_root,
                check=True,
...
def _claims() -> dict[str, bool]:
    return accepted._claims()


def _source_claims() -> dict[str, bool]:
    return accepted._source_claims()
...
def build_run_manifest(
    ...
    *,
    ...
    run = {
        ...
        "owner": "exact17-two-kalmanson-preparer",
        "base_head": BASE_HEAD,
    ...
    *,
    ...
    with _retained_support(support_files) as custody:
        ...
        commit_verifier(
            root,
            source_commit,
            (source_path, root_source_path, exporter_path),
        )
        ...
        try:
            ...
            for center in LEGAL_CENTERS:
                for category in accepted.hardened._physical_categories(center):
                    ...
                    if is_direct_sentinel:
                        ...
                        cell = _PublishedFile.capture(cnf_path, "generated cell")
                        try:
                            ...
                            if not _published_files_equal(direct, cell):
                    ...
                    cells.append(
                        {
                            ...
                            "category": category,
                            "ordinal": shard,
                            "cnf": accepted._file_ref(
                                root, cnf_path, cnf["sha256"], cnf["bytes"]
                            ),
                            "producer_manifest": accepted._ref(
                                root, producer_path, producer
                            ),
                            "wave_manifest": accepted._ref(root, wave_path, wave),
            ...
            report = {
                ...
                "campaign_manifest": accepted._ref(root, campaign_path, campaign_bytes),
