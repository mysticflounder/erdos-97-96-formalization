...
"""Prepare the exact-17 V5 canary two-Kalmanson physical-slice cells.
...
root identity with the 20-clause V5 successor identities.
...
V4-combined parent and proves that every successor clause is strict-new and is
the exact ordered suffix of the V5 successor root.
"""
...
ROOT = REPO_ROOT
LANE_ID = "exact17-v5-canary-twok-successor-preparer-20260823"
RUN_ID = "preparation-v1"
RUN_OWNER = "exact17-fourpoint-v6-preparer"
BASE_HEAD = "9fbf80623b867cb1f319d87649ed9f7fface9469"
STANDARD_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")

SOURCE_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenCanaryPerpBisectorSurvivorFourPointTwoCircleV5CanaryTwoKalmansonRefinementsPhysicalSliceCoverage.lean"
)
...
EXPORTER_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenCanaryPerpBisectorSurvivorFourPointTwoCircleV5CanaryTwoKalmansonRefinementsPhysicalSliceCellExport.lean"
)
...
PRODUCTION_CONFIG_RELATIVE = Path(
    ...
    "canary-perp-bisector-survivor-four-point-two-circle-v5-canary-two-kalmanson-preparation-config.json"
)
PRODUCTION_CONFIG_PATH = ROOT / PRODUCTION_CONFIG_RELATIVE
PRODUCTION_CONFIG_SCHEMA = "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v5-canary-two-kalmanson-refinements-preparation-config/v1"
...
PARENT_CLAUSES = 7_409_780
IMMEDIATE_PARENT_CLAUSES = 7_409_760
CELL_CLAUSES = 7_409_786
CELL_COUNT = 76
DIRECT_SENTINELS = (
    (0, "none"),
    (0, "unique-06"),
...
SOURCE_THEOREM = (
    "Problem97."
    "ATailBlockerVExactSeventeenCanaryPerpBisectorSurvivorFourPointTwoCircleV5CanaryTwoKalmansonRefinementsPhysicalSliceCoverage."
...
EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_FOUR_POINT_TWO_CIRCLE_REFINEMENT_SUFFIX_SHA256 = "596a5c71face8fde70b277f65a311b8af824530e99ab22818c2e3a48b71ec927"
EXPECTED_EXACT_PARENT_MULTIPLICITY = (0,) * 20
EXPECTED_PARENT_SUBSUMER_COUNTS = (0,) * 20
EXPECTED_PARENT_SUBSUMER_CLAUSES = ((),) * 20
EXPECTED_STRICT_NEW_SUFFIX_INDICES = tuple(range(20))
EXPECTED_SUFFIX_OCCURRENCE = (
    0, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 5, 5, 6, 6, 6, 6, 7, 7, 7,
)
EXPECTED_STRICT_NEW_PER_OCCURRENCE = (1, 2, 3, 3, 2, 2, 4, 3)
ORDER_SHA256 = sha256_bytes(
    b"exact17-canary-perp-bisector-survivor-four-point-two-circle-v5-canary-two-kalmanson-refinements-physical-slice-order-v1"
)
SCHEMA = "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v5-canary-two-kalmanson-refinements-physical-slice-preparation/v1"
CAMPAIGN_SCHEMA = "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v5-canary-two-kalmanson-refinements-physical-slice-campaign/v1"
PRODUCER_SCHEMA = accepted.PRODUCER_SCHEMA
SOURCE_SCHEMA = accepted.SOURCE_SCHEMA
WAVE_SCHEMA = accepted.WAVE_SCHEMA
FINITE_SCHEMA = "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v5-canary-two-kalmanson-refinements-physical-slice/v1"
ROOT_PRODUCER_SCHEMA = "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v5-canary-two-kalmanson-refinements-root-producer/v1"
...
def category_id(center: int, category: str) -> str:
    ...
    return f"canary-perp-bisector-survivor-four-point-two-circle-v5-canary-two-kalmanson-refinements-next-center-{center:02d}-physical-{category}"
...
def run_lean_immediate_parent_export(
    ...
    """Run the V4-combined immediate-parent exporter's root mode."""
    lean_root = repo_root / "lean"
    relative = exporter_path.resolve().relative_to(lean_root.resolve())
...
def _validate_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinement_contract() -> (
    ...
    if len(suffix) != 20 or len(set(suffix)) != 20:
        raise PreparationError(
    ...
    if PARENT_CLAUSES != IMMEDIATE_PARENT_CLAUSES + len(suffix):
        raise PreparationError("successor root is not parent plus 20-clause suffix")
...
def validate_canary_perp_bisector_survivor_four_point_two_circle_v4_combined_refinement_parent_novelty(
    ...
    """Authenticate the exact 20-clause suffix against the complete parent."""
    ...
    if tuple(exact_parent_multiplicity) != EXPECTED_EXACT_PARENT_MULTIPLICITY:
        raise PreparationError("V5 exact parent multiplicity profile drifted")
    ...
    strict_new_per_occurrence = [
        sum(
            index in strict_new_suffix_indices and occurrence == expected_occurrence
            for index, occurrence in enumerate(EXPECTED_SUFFIX_OCCURRENCE)
        )
        for expected_occurrence in range(8)
    ]
    if tuple(strict_new_per_occurrence) != EXPECTED_STRICT_NEW_PER_OCCURRENCE:
        raise PreparationError("V5 strict-new per-occurrence profile drifted")
    if successor_multiplicity != [1] * len(suffix):
        raise PreparationError(
            "survivor FourPointTwoCircle refinement suffix multiplicity is not exactly one"
        )
    return {
        "schema": "p97-exact17-canary-perp-bisector-survivor-four-point-two-circle-v5-canary-two-kalmanson-refinements-parent-novelty/v1",
        "policy": {
            ...
            "subsumption": "all 20 selected orbit clauses are strict-new",
            "successor_shape": "byte-exact parent body prefix plus exact ordered 20-clause suffix",
