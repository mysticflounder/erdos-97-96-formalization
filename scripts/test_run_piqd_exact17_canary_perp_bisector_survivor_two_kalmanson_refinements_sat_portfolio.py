...
def _cells() -> list[dict[str, Any]]:
    ...
    for ordinal, (center, category) in enumerate(roles):
        source = f"canary-perp-bisector-survivor-two-kalmanson-refinements-next-center-{center:02d}-physical-{category}"
        result.append(
            {
...
def _identity_source_fixture(
    ...
    for ordinal, (center, category) in enumerate(roles):
        source_id = (
            "canary-perp-bisector-survivor-two-kalmanson-refinements-next-center-"
...
def test_campaign_contract_is_exactly_76_cells_and_center2_none_canary() -> None:
    assert runner.ROOT_NUM_CLAUSES == 7_409_372
    assert runner.NUM_CLAUSES == 7_409_378
    assert runner.CELL_COUNT == 76
    assert runner.SOURCE_PREPARER_RELATIVE.endswith(
        "prepare_exact17_canary_perp_bisector_survivor_two_kalmanson_"
    ...
    assert "SurvivorTwoKalmansonRefinements" in runner.EXPECTED_SOURCE_PATH
    assert "SurvivorTwoKalmansonRefinements" in runner.EXPECTED_ROOT_SOURCE_PATH
    campaign, run = _campaign()
