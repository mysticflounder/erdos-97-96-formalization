"""Validate the Lean-owned exact-17 child34 DIMACS boundary."""
...
VARIABLES = 308
PARENT_CLAUSES = 5_847_248
NEW_CLAUSES = 8
CHILD_CLAUSES = PARENT_CLAUSES + NEW_CLAUSES
PARENT_SHA256 = "775eb42db7bdac52eb7e5609a1cf33111c953d26f4cee9c8d95bde3b468563b9"
SPEC_SCHEMA = "p97-exact17-child34-export-spec/v1"
LEAN_ROOT_PATH = str(ROOT / "lean/Erdos9796Proof/P97/ATail/BlockerVExactSeventeenThirtyThirdModelRefinements.lean")
LEAN_EXPORT_PATH = str(ROOT / "lean/Erdos9796Proof/P97/ATail/BlockerVExactSeventeenThirtyThirdModelRefinementsExport.lean")
KALMANSON_PATH = str(ROOT / "lean/Erdos9796Proof/P97/ATail/BlockerVExactSeventeenThirtyThirdKalmansonRefinement.lean")
PERPENDICULAR_BISECTOR_PATH = str(ROOT / "lean/Erdos9796Proof/P97/ATail/BlockerVExactSeventeenThirtyThirdPerpendicularBisectorRefinement.lean")
PARENT_PATH = str(ROOT / "scratch/exact17-lean-to-sat/exact17-thirty-third-root-thirty-second-model-refinements.cnf")
CHILD_PATH = str(ROOT / "scratch/exact17-lean-to-sat/exact17-thirty-fourth-root-thirty-third-model-refinements.cnf")
RECEIPT_PATH = str(ROOT / "scratch/exact17-lean-to-sat/child34-export-receipt.json")
SUFFIX: tuple[Clause, ...] = (
    (-307, -9, -3, -281, -284, -267, -258, -252, -240, -21, -33,
     -257, -168, -169, -202, -201, -225, -233),
    (-307, -240, -241, -206, -210, -125, -122, -7, -9, -146, -144,
     -128, -52, -59, -86, -92, -112, -108),
    (-308, -9, -3, -281, -284, -267, -258, -252, -240, -21, -33,
     -257, -219, -220, -202, -201, -225, -233),
    (-308, -240, -241, -155, -159, -125, -122, -7, -9, -149, -144,
     -128, -52, -59, -86, -92, -115, -108),
...
PRODUCTION_SPEC = ExportSpec(
    ...
    child_sha256="ea8311540af709cf991c932c38e52f9767227cf55781508f2791b1dc42c4a819",
    child_bytes=291_620_980,
    publication_state="PROVISIONED",
    lean_root_sha256="bd9a2ee92333b50f86a3cd6f3875d47e07af3651befa894ae49709cbf689f6fb",
    lean_export_sha256="c1f02d7804e5834243a33314ddad4fd946a3cd3f326cb64b9abbb6a4b4d7d149",
    kalmanson_sha256="67b30f9a4718b1749229ef223482d43f130dc85264b062b29d8d35d2ebfba03d",
    perpendicular_bisector_sha256="973efae76fcee870759f94341987785cded5edae01bcac4ddba060e8049ce8ca",
)
...
def validate_spec(spec: ExportSpec, *, require_source_pins: bool = False) -> None:
    ...
    if spec.schema != SPEC_SCHEMA:
        raise ValueError("unsupported child34 ExportSpec schema")
    ...
    if spec.variables != VARIABLES or spec.parent_clauses != PARENT_CLAUSES or spec.new_clauses != NEW_CLAUSES:
        raise ValueError("child34 clause-count contract drifted")
    _sha(spec.parent_sha256, "parent_sha256")
    if spec.parent_sha256 != PARENT_SHA256:
        raise ValueError("child34 parent SHA-256 pin drifted")
    ...
    for name, expected in expected_paths.items():
        if getattr(spec, name) != expected:
            raise ValueError(f"{name} is not the authenticated child34 path")
    if require_source_pins and not spec.fully_authenticated:
        raise UnprovisionedError("child34 source and child pins are incomplete")
...
def validate_export(parent_path: Path, child_path: Path, *, spec: ExportSpec = PRODUCTION_SPEC, allow_unprovisioned: bool = False) -> dict[str, Any]:
    ...
    if not spec.provisioned and not allow_unprovisioned:
        raise UnprovisionedError("child34 export is UNPROVISIONED; live child pins are absent")
    ...
    if sha256_file(parent_path) != spec.parent_sha256:
        raise ValueError("parent SHA-256 does not match authenticated child33 pin")
    ...
    with parent_path.open("rb") as parent, child_path.open("rb") as child:
        ...
        for index, expected in enumerate(SUFFIX):
            ...
            if not line or _clause(line, f"suffix/{index + 1}") != expected:
                raise ValueError(f"child34 suffix mismatch at clause {index + 1}")
    ...
    return {
        "schema": "p97-exact17-child34-export-validation/v1",
        ...
        "source_clause_families": ["pairedKalmansonClauses", "child33PerpBisectorClauses"],
