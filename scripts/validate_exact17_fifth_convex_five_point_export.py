"""Validate the Lean exact-17 combined sixteenth-child export.

Lean is authoritative. This checker requires the authenticated fifteenth root
as a byte-identical prefix, independently regenerates the two four-clause
orbits, and checks that each orbit cuts the motivating fifteenth-root model.
"""
...
def validate(parent_path: Path, child_path: Path, model_path: Path) -> dict[str, object]:
    ...
    observed_suffix = 0
    cut_by_orbit: dict[str, list[int]] = {"cancellation": [], "convex_five": []}
    ...
    with (
        ...
        for orbit_name, motif in (
            ("cancellation", CANCELLATION_MOTIF),
            ("convex_five", CONVEX_FIVE_MOTIF),
        ):
            for order_index, order in enumerate(ORDERS):
                for reverse in (False, True):
                    ...
                    expected = expected_clause(
                        order_index, order, motif, reverse=reverse
                    )
    ...
    return {
        "schema": "p97-exact17-lean-fifth-convex-five-point-export-validation/v1",
        "parent_prefix_byte_identical": True,
        "variables": VARIABLES,
        "clauses": CHILD_CLAUSES,
        "generic_cancellation_clauses": ORBIT_CLAUSES,
        "convex_five_point_clauses": ORBIT_CLAUSES,
        "combined_suffix_sha256": suffix_digest.hexdigest(),
