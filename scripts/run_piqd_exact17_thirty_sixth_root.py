"""Fail-closed PIQD lifecycle adapter for exact-17 Child36."""
...
from validate_exact17_thirty_fifth_model_refinements_ingress import (
...
_base.HARDENED_ARTIFACT_NAMESPACES = _base.HARDENED_ARTIFACT_NAMESPACES | {"child36"}
...
class RunnerSpec:
    ...
    variables: int
    clauses: int
    project: str = "erdos-97-96-exact17-child36"
    daemon_name: str = "piqd"
    daemon_version: str = "0.1.0"
    artifact_namespace: str = "child36"
    timeout_s: int = TIMEOUT_S
    march_timeout_s: int = MARCH_TIMEOUT_S

    def __post_init__(self) -> None:
        if self.artifact_namespace != "child36":
            raise ValueError("child36 runner requires artifact_namespace='child36'")
        if (
            type(self.timeout_s) is not int
            or self.timeout_s <= 0
            or type(self.march_timeout_s) is not int
            or self.march_timeout_s <= 0
        ):
...
PRODUCTION_RUNNER_PATHS = RunnerPaths(
    ...
    intent=ROOT / "scratch/exact17-lean-to-sat/piqd-child36-core1-custody-intent.json",
    prepared=ROOT
    / "scratch/exact17-lean-to-sat/piqd-child36-core1-custody-prepared.json",
    state=ROOT
    / "scratch/exact17-lean-to-sat/piqd-child36-core1-custody-live-state.json",
    final=ROOT / "scratch/exact17-lean-to-sat/piqd-child36-core1-custody-final.json",
    model=ROOT / "scratch/exact17-lean-to-sat/piqd-child36-core1-custody-model.json",
    solver_log=ROOT
    / "scratch/exact17-lean-to-sat/piqd-child36-core1-custody-solver.log",
    lock=ROOT / "scratch/exact17-lean-to-sat/piqd-child36-core1-custody-runner.lock",
)
PRODUCTION_RUNNER_SPEC = RunnerSpec(
    ingress=PRODUCTION_INGRESS_SPEC,
    manifest_sha256="65a4a829a6d17629afc72aeb51bc1acdec21e8d7c474f909ed9cbbf0ecc55dea",
    root_sha256="e2236eaa12b10117c0540b2ea545f177e5070b6d96a0ac96668a733fe501eae5",
    root_bytes=291_621_334,
    variables=308,
    clauses=5_847_264,
)
...
def _require_provisioned(spec: RunnerSpec) -> None:
    if spec.artifact_namespace != "child36" or not spec.provisioned:
        raise UnprovisionedError("child36 PIQD runner is UNPROVISIONED")
...
def validate_local(
    ...
    *,
    ...
    _require_provisioned(spec)
    return _base.validate_local(
        _paths(paths), _lifecycle_spec(spec), ingress_validator=ingress_validator
    )


def live_identity(
    client: PiqdClient, spec: RunnerSpec = PRODUCTION_RUNNER_SPEC
) -> dict[str, Any]:
    _require_provisioned(spec)
...
def reconcile_prepared_job(
    ...
    *,
    ...
    _require_provisioned(spec)
    return _base.reconcile_prepared_job(
        client,
        job_id,
        _paths(paths),
        _lifecycle_spec(spec),
