"""Fail-closed PIQD lifecycle adapter for the unprovisioned exact-17 Child39."""
...
from validate_exact17_thirty_eighth_model_refinements_ingress import (
    MANIFEST_SHA256,
...
_base.HARDENED_ARTIFACT_NAMESPACES = _base.HARDENED_ARTIFACT_NAMESPACES | {"child39"}
...
class RunnerSpec:
    ...
    variables: int
    clauses: int | None
    project: str = "erdos-97-96-exact17-child39"
    daemon_name: str = "piqd"
    daemon_version: str = "0.1.0"
    artifact_namespace: str = "child39"
    timeout_s: int = TIMEOUT_S
    march_timeout_s: int = MARCH_TIMEOUT_S

    def __post_init__(self) -> None:
        if self.artifact_namespace != "child39":
            raise ValueError("child39 runner requires artifact_namespace='child39'")
...
PRODUCTION_RUNNER_PATHS = RunnerPaths(
    PRODUCTION_INGRESS_PATHS,
    ROOT / "scratch/exact17-lean-to-sat/piqd-child39-core1-custody-intent.json",
...
def _require_provisioned(spec: RunnerSpec) -> None:
    if spec.artifact_namespace != "child39" or not spec.provisioned:
        raise UnprovisionedError("child39 PIQD runner is UNPROVISIONED")
