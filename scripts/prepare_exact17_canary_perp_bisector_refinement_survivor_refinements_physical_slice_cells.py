...
"""Prepare the exact-17 canary survivor-refinement physical-slice cells.
...
root identity with the three-clause survivor-refinement successor identities.
...
five-clause model-refinement parent and proves that every successor clause is
...
ROOT = REPO_ROOT
LANE_ID = "exact17-canary-perp-bisector-survivor-refinements-preparer-20260822"
RUN_ID = "preparation-v1"
RUN_OWNER = "exact17-canary-perp-bisector-survivor-refinements-preparer"
BASE_HEAD = "69fc5c4f65345cda29bab091920d3a84aa2cd90f"
STANDARD_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")

SOURCE_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenCanaryPerpBisectorRefinementSurvivorRefinementsPhysicalSliceCoverage.lean"
)
...
IMMEDIATE_PARENT_EXPORTER_PATH = ROOT / (
    "lean/Erdos9796Proof/P97/ATail/"
    "BlockerVExactSeventeenCanaryPerpBisectorRefinementModelRefinementsPhysicalSliceCellExport.lean"
)
...
TEST_PATH = ROOT / (
    "scripts/test_prepare_exact17_canary_perp_bisector_refinement_survivor_refinements_physical_slice_cells.py"
)
PRODUCTION_CONFIG_RELATIVE = Path(
    "census/p97_search/waves/exact17/"
    "canary-perp-bisector-refinement-survivor-refinements-preparation-config.json"
)
PRODUCTION_CONFIG_PATH = ROOT / PRODUCTION_CONFIG_RELATIVE
PRODUCTION_CONFIG_SCHEMA = (
    "p97-exact17-canary-perp-bisector-survivor-refinements-preparation-config/v1"
)
...
PARENT_CLAUSES = 7_409_304
IMMEDIATE_PARENT_CLAUSES = 7_409_301
CELL_CLAUSES = 7_409_310
CELL_COUNT = 76
DIRECT_SENTINELS = (
    (0, "none"),
    (0, "unique-06"),
...
SOURCE_THEOREM = (
    "Problem97."
    "ATailBlockerVExactSeventeenCanaryPerpBisectorRefinementSurvivorRefinementsPhysicalSliceCoverage."
    "sourceAssign_canaryPerpBisectorSurvivorRefinementPhysicalSliceCell"
)
EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_REFINEMENT_SUFFIX = (
    (-307, -287, -281, -255, -239, -230, -222, -153, -151),
    (-307, -203, -198, -50, -46, -29, -20, -11, -3),
    (-307, -233, -228, -150, -143, -116, -115, -13, -12),
)
ORDER_SHA256 = sha256_bytes(
    b"exact17-canary-perp-bisector-survivor-refinements-physical-slice-order-v1"
)
SCHEMA = "p97-exact17-canary-perp-bisector-survivor-refinements-physical-slice-preparation/v1"
CAMPAIGN_SCHEMA = (
...
ROOT_PRODUCER_SCHEMA = (
    "p97-exact17-canary-perp-bisector-survivor-refinements-root-producer/v1"
)
...
def category_id(center: int, category: str) -> str:
    ...
    return f"canary-perp-bisector-survivor-refinements-next-center-{center:02d}-physical-{category}"
...
def _validate_canary_perp_bisector_survivor_refinement_contract() -> None:
    suffix = EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_REFINEMENT_SUFFIX
    if len(suffix) != 3 or len(set(suffix)) != 3:
        raise PreparationError("survivor-refinement suffix inventory drifted")
    for clause in suffix:
        if not clause:
            raise PreparationError("survivor-refinement clause is empty")
        if len(set(clause)) != len(clause):
            raise PreparationError("survivor-refinement clause repeats a literal")
        if any(literal == 0 or abs(literal) > PARENT_VARIABLES for literal in clause):
            raise PreparationError(
                "survivor-refinement clause exceeds the variable map"
            )
        literal_set = set(clause)
        if any(-literal in literal_set for literal in literal_set):
            raise PreparationError("survivor-refinement clause is tautological")
    if PARENT_CLAUSES != IMMEDIATE_PARENT_CLAUSES + len(suffix):
        raise PreparationError("successor root is not parent plus three-clause suffix")
...
def validate_canary_perp_bisector_survivor_refinement_parent_novelty(
    immediate_parent: _PublishedFile,
    successor_root: _PublishedFile,
) -> dict[str, Any]:
    """Authenticate the exact three-clause suffix against the complete parent."""
    _validate_canary_perp_bisector_survivor_refinement_contract()
    ...
    if any(exact_parent_multiplicity):
        raise PreparationError(
            "a survivor-refinement clause already occurs in the parent"
        )
    if any(parent_subsumer_count):
        raise PreparationError(
            "a parent clause subsumes the survivor-refinement suffix"
        )
    if successor_multiplicity != [1] * len(suffix):
        raise PreparationError(
            "survivor-refinement suffix multiplicity is not exactly one"
        )
    return {
        "schema": "p97-exact17-canary-perp-bisector-survivor-refinements-parent-novelty/v1",
        "policy": {
            ...
            "successor_shape": "byte-exact parent body prefix plus exact ordered three-clause suffix",
    ...
    *,
    ...
    with _retained_support(support_files) as custody:
        ...
        try:
            ...
            parent_novelty = (
                validate_canary_perp_bisector_survivor_refinement_parent_novelty(
