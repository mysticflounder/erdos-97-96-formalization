"""Fail-closed ingress validation for the exact-17 three-row PIQD root."""
...
SCHEMA = "p97-exact17-lean-cdefg-equal-k4-two-circle-three-row-piqd-ingress/v1"
ROOT_SHA256 = "009b34264ddcb114a59410f3af61a46b04b831841ce6687104c1833d7441a9d4"
ROOT_BYTES = 113_229_604
PARENT_SHA256 = "2feebcbf67b377b2e198c34e4385adccf0e8a631051c6058855d72321ddf4746"
PARENT_BYTES = 108_926_214
GRANDPARENT_SHA256 = "00e0e3e50e518833b782a1afbb686896365335d4cb15c993b3a6a25e31cc0aa6"
GRANDPARENT_BYTES = 107_954_682
ANCESTOR_SHA256 = "763bb4774a4d148ca35a4d33ecce06c158a8f941c3e5fb272f534cb2b9637527"
ANCESTOR_BYTES = 104_887_967
...
VARIABLES = 308
CLAUSES = 2_430_300
PARENT_CLAUSES = 2_306_540
GRANDPARENT_CLAUSES = 2_268_460
ANCESTOR_CLAUSES = 2_189_852
BASE_CLAUSES = 209_692
CDEFG_CLAUSES = 1_980_160
EQUAL_K4_CLAUSES = 78_608
TWO_CIRCLE_CLAUSES = 38_080
THREE_ROW_CLAUSES = 123_760
EXPECTED_SOURCE_PATHS = (
    ...
    "lean/Erdos9796Proof/P97/ATail/BlockerVExactSeventeenSourceCnfCdefgEqualK4TwoCircleThreeRowExport.lean",
...
def check_dimacs(
    ...
    *,
    ...
    require(record["sha256"] == expected_sha256, "unauthorized DIMACS root")
...
def validate(repo: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    expected_counts = {
        "base_prefix_clauses": BASE_CLAUSES,
        "checked_cdefg_clauses": CDEFG_CLAUSES,
        "checked_equal_k4_clauses": EQUAL_K4_CLAUSES,
        "checked_two_circle_clauses": TWO_CIRCLE_CLAUSES,
        "checked_three_row_clauses": THREE_ROW_CLAUSES,
    }
    ...
    parent = check_dimacs(
        repo,
        manifest["parent_dimacs"],
        expected_sha256=PARENT_SHA256,
        expected_bytes=PARENT_BYTES,
        expected_clauses=PARENT_CLAUSES,
    )
    grandparent = check_dimacs(
        repo,
        manifest["grandparent_dimacs"],
        expected_sha256=GRANDPARENT_SHA256,
        expected_bytes=GRANDPARENT_BYTES,
        expected_clauses=GRANDPARENT_CLAUSES,
    )
    ancestor = check_dimacs(
        repo,
        manifest["ancestor_dimacs"],
        expected_sha256=ANCESTOR_SHA256,
        expected_bytes=ANCESTOR_BYTES,
        expected_clauses=ANCESTOR_CLAUSES,
    )
    base = check_dimacs(
        repo,
        manifest["base_dimacs"],
        expected_sha256=BASE_SHA256,
        expected_bytes=BASE_BYTES,
        expected_clauses=BASE_CLAUSES,
    )
    ...
    validator = check_hash(
        repo, {"path": validation["script"], "sha256": validation["script_sha256"]}
    )
    parent_validator = check_hash(
        repo,
        {
    ...
    grandparent_validator = check_hash(
        repo,
        {
            "path": validation["grandparent_validator"],
            "sha256": validation["grandparent_validator_sha256"],
        },
    )
    ancestor_validator = check_hash(
        repo,
        {
            "path": validation["ancestor_validator"],
            "sha256": validation["ancestor_validator_sha256"],
        },
    )
    ...
    require(
        regenerated["parent_prefix_byte_identical"] is True,
        "parent prefix is not byte-identical",
    )
    require(
        regenerated["three_row_clauses"] == THREE_ROW_CLAUSES,
        "validator three-row mismatch",
    )
    ...
    return {
        "schema": "p97-exact17-lean-three-row-piqd-ingress-validation/v1",
        "manifest_sha256": sha256(manifest_path),
        "dimacs_sha256": sha256(root),
        "dimacs_bytes": root.stat().st_size,
        "variables": VARIABLES,
        "clauses": CLAUSES,
        "three_row_clauses": THREE_ROW_CLAUSES,
