"""Focused adversarial tests for the immutable Lean-owned Child41 gate."""
...
import export_exact17_forty_first_root as exporter
import pytest
import run_piqd_exact17_forty_first_root as runner
import validate_exact17_forty_first_ingress as ingress
import validate_exact17_forty_first_wave_export as export_validation
...
def test_lean_source_pins_and_exact_boundary() -> None:
    ...
    assert PRODUCTION_SPEC.child_bytes == CHILD_BYTES == 291_654_272
    assert LEAN_ROOT_PATH.endswith("BlockerVExactSeventeenFortiethModelRefinements.lean")
    assert LEAN_EXPORT_PATH.endswith("BlockerVExactSeventeenFortiethModelRefinementsExport.lean")
    assert len(LEAN_ROOT_SHA256) == len(LEAN_EXPORT_SHA256) == 64


def test_authenticated_child40_model_rejects_suffix() -> None:
...
def test_export_cli_requires_explicit_subcommand_and_is_non_mutating(
    ...
    monkeypatch.setattr(exporter, "export_child41", unexpected_export)
...
def test_losing_concurrent_export_preserves_winner_root(
    ...
    child = tmp_path / "child41.cnf"
    receipt = tmp_path / "child41-receipt.json"
