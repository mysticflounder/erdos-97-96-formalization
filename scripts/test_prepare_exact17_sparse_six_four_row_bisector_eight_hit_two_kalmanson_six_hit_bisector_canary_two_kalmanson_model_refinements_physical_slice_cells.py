...
"""Adversarial tests for the six-hit-bisector-canary-two-kalmanson-model-refinements successor preparer."""
...
import prepare_exact17_sparse_six_four_row_bisector_eight_hit_two_kalmanson_six_hit_bisector_canary_two_kalmanson_model_refinements_physical_slice_cells as subject
import pytest
...
def test_original_parent_exporter_is_the_immediate_six_hit_parent() -> None:
...
def _root() -> bytes:
    body = b"".join(
        ...
        for clause in _PARENT_SUBSUMER_CLAUSES
    )
    return f"p cnf 308 {len(_PARENT_SUBSUMER_CLAUSES)}\n".encode() + body
...
def _successor(
    *,
    ...
    clauses = subject.EXPECTED_CANARY_TWO_KALMANSON_SUFFIX if suffix is None else suffix
    body = parent.split(b"\n", 1)[1]
...
def _fixture(
    ...
    monkeypatch.setattr(subject, "ORIGINAL_PARENT_CLAUSES", 7)
    monkeypatch.setattr(subject, "PARENT_CLAUSES", 16)
    monkeypatch.setattr(subject, "CELL_CLAUSES", 22)
...
def _validate_parent_novelty_pair(
    ...
    try:
        return subject.validate_six_hit_bisector_canary_two_kalmanson_model_refinements_parent_novelty(
...
def test_successor_dimensions_and_exact_six_units() -> None:
    parent = b"p cnf 308 7409295\n1 0\n"
    payload = subject.cell_cnf_bytes(parent, 16, "unique-10")
    lines = payload.splitlines()
    assert lines[0] == b"p cnf 308 7409301"
    assert lines[-6:] == [
        b"306 0",
        b"-279 0",
        b"-280 0",
...
def test_model_refinement_suffix_and_canary_are_exact() -> None:
    ...
    assert subject.EXPECTED_CANARY_TWO_KALMANSON_SUFFIX == tuple(
        subject.ORIGINAL_CANARY_TWO_KALMANSON_ORBIT[index]
        for index in subject.RETAINED_ORIGINAL_SUFFIX_INDICES
    )
    assert (
        subject.CANARY_ACTIVE_CLAUSE == subject.EXPECTED_CANARY_TWO_KALMANSON_SUFFIX[1]
    )
...
def test_initialize_creates_exact_governed_skeleton(
    ...
    assert (
        run["owner"]
        == subject.RUN_OWNER
        == "exact17-six-hit-bisector-canary-two-kalmanson-model-refinements-preparer"
    )
...
def test_prepares_all_76_cells_and_validates_sentinels(
    ...
    assert campaign["source"]["parent_novelty"]["parent_subsumer_count"] == [0] * len(
    ...
    assert wave["encoding"]["num_clauses"] == 22
...
def test_immediate_six_hit_parent_duplicate_suffix_clause_fails_before_children(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    paths, calls = _fixture(tmp_path, monkeypatch)
    duplicate = subject.EXPECTED_CANARY_TWO_KALMANSON_SUFFIX[0]
    original = _root().replace(
        b"-307 -264 -263 -179 0\n",
...
@pytest.mark.parametrize(
    "orbit_index", range(len(subject.ORIGINAL_CANARY_TWO_KALMANSON_ORBIT))
)
def test_each_full_orbit_clause_is_checked_for_exact_parent_duplication(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, orbit_index: int
) -> None:
    monkeypatch.setattr(subject, "ORIGINAL_PARENT_CLAUSES", 2)
    monkeypatch.setattr(subject, "PARENT_CLAUSES", 11)
    monkeypatch.setattr(subject, "CELL_CLAUSES", 17)
    clause = subject.ORIGINAL_CANARY_TWO_KALMANSON_ORBIT[orbit_index]
    parent = (
        b"p cnf 308 2\n" + (" ".join(map(str, clause)) + " 0\n").encode() + b"1 0\n"
    )
    with pytest.raises(subject.PreparationError, match="already occurs"):
        _validate_parent_novelty_pair(tmp_path, parent, f"exact-{orbit_index}")


@pytest.mark.parametrize(
    "suffix_index", range(len(subject.EXPECTED_CANARY_TWO_KALMANSON_SUFFIX))
)
def test_each_retained_clause_is_checked_for_parent_subsumption(
    ...
    parent = f"p cnf 308 2\n{clause[0]} 0\n1 0\n".encode()
    with pytest.raises(subject.PreparationError, match="subsumes"):
        _validate_parent_novelty_pair(tmp_path, parent, f"subsumed-{suffix_index}")
...
def test_historical_parent_duplicate_literals_are_accepted_semantically(
    ...
    report = _prepare(tmp_path, paths)
    assert report["cell_count"] == 76
    assert report["parent_novelty"]["exact_parent_multiplicity"] == [0] * len(
        subject.EXPECTED_CANARY_TWO_KALMANSON_SUFFIX
    )
    assert report["parent_novelty"]["parent_subsumer_count"] == [0] * len(
        subject.EXPECTED_CANARY_TWO_KALMANSON_SUFFIX
    )
    assert report["parent_novelty"]["successor_multiplicity"] == [1] * len(
        subject.EXPECTED_CANARY_TWO_KALMANSON_SUFFIX
    ...
    full_counts = report["parent_novelty"]["full_orbit_parent_subsumer_count"]
    full_witnesses = report["parent_novelty"]["full_orbit_parent_subsumer_witnesses"]
    assert report["parent_novelty"]["full_orbit_exact_parent_multiplicity"] == [0] * 16
    assert all(
        full_counts[index] == 0 for index in subject.RETAINED_ORIGINAL_SUFFIX_INDICES
    )
    assert all(
        full_counts[index] >= 1 and full_witnesses[index] is not None
        for index in subject.PARENT_SUBSUMED_ORIGINAL_SUFFIX_INDICES
    )


def test_missing_parent_subsumer_for_omitted_clause_fails_before_children(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    paths, calls = _fixture(tmp_path, monkeypatch)
    original = _root().replace(b"-307 -264 -263 -179 0\n", b"1 0\n", 1)
    _set_parent_pair(paths, original)
    _initialize(tmp_path, paths)
    with pytest.raises(subject.PreparationError, match="lacks a parent subsumer"):
        _prepare(tmp_path, paths)
    assert calls == [(-1, "root"), (-1, "root")]
    assert not (paths["output"] / "artifacts" / "cells").exists()
...
def test_new_suffix_clause_repeated_literal_is_rejected(
    ...
    paths, calls = _fixture(tmp_path, monkeypatch)
    suffix = list(subject.EXPECTED_CANARY_TWO_KALMANSON_SUFFIX)
    suffix[0] = suffix[0] + (suffix[0][0],)
    _set_parent_pair(paths, _root(), suffix=tuple(suffix))
    _initialize(tmp_path, paths)
    with pytest.raises(
        subject.PreparationError,
        match="six-hit-bisector-canary-two-kalmanson-model-refinements suffix clause 0 repeats a literal",
    ):
        _prepare(tmp_path, paths)
    assert calls == [(-1, "root"), (-1, "root")]
