"""Independently validate the exact-17 child-30 all-core suffix.

Lean owns the exported DIMACS bytes. This validator authenticates the child-30
prefix, replays every serialized two-Kalmanson certificate, independently
regenerates the 160 suffix clauses, and checks that each occurrence cuts the
authenticated child-30 model.
"""
...
PARENT_CLAUSES = 5_846_744
ORBIT_CLAUSES = 160
PARENT_SHA256 = "02aca42eb22814540816d9e4f1d19b2c71888700147e6b832d735f9efc41acd0"
CHILD_CLAUSES = PARENT_CLAUSES + ORBIT_CLAUSES
CHILD_SHA256 = "3c847c1ebcacf85256b65d54563584a8b89eb50f427fe268e2f916d4cdafa204"
LEDGER_SCHEMA = "p97-exact17-child30-all-minimal-two-kalmanson/v1"
LEDGER_SHA256 = "4ef564d2091cd629d59945ac6af74a665383f5886b23655a2ffb05e144f7c2c1"
PARENT_LEDGER_SHA256 = "272986bdedfba002fe36dcc6b73d96368eebe16278793246546d2ecb4c47b787"
ANALYSIS_SHA256 = "878d0be5674758687747ef1346061e19ef345293a507cfafbca2a432a3c23aa3"
# Child30's serialized certificates intentionally use the canonical producer
# order.  Keep this choice named so a future noncanonical ledger cannot drift
# silently back to an implicit tuple(range(17)) replay.
CERTIFICATE_ORDER_INDEX = 0
CERTIFICATE_ORDER = ORDERS[CERTIFICATE_ORDER_INDEX]
...
def validate_ledger(ledger_path: Path, analysis_path: Path) -> list[dict[str, Any]]:
    if sha256(analysis_path) != ANALYSIS_SHA256:
        raise ValueError("child30 analysis hash drifted")
    analysis = json.loads(analysis_path.read_text(encoding="utf-8"))
    if analysis.get("status") != "PASS":
        raise ValueError("child30 analysis did not pass")
    if analysis.get("cnf_sha256") != PARENT_SHA256:
        raise ValueError("child30 analysis is not authenticated to the parent")
    if analysis.get("variables") != VARIABLES or analysis.get("clauses") != PARENT_CLAUSES:
        raise ValueError("child30 analysis dimensions drifted")
    if set(analysis.get("row_sizes", {}).values()) != {4}:
        raise ValueError("unexpected child30 selected-row shape")

    if sha256(ledger_path) != LEDGER_SHA256:
        raise ValueError("child30 all-core ledger hash drifted")
    ledger = json.loads(ledger_path.read_text(encoding="utf-8"))
    expected_counts = {
        "record_count": 80,
        "distinct_support_count": 49,
        "minimal_support_count": 40,
    ...
    for index, entry in enumerate(entries):
        ...
        checked_forward = producer_bank.certify_two_kalmanson_cancellation(
            rows_from_hits(hits), 17, CERTIFICATE_ORDER, forward["core"]
        )
        checked_reverse = producer_bank.certify_two_kalmanson_cancellation(
            rows_from_hits(reflected(hits)), 17, CERTIFICATE_ORDER, reverse["core"]
        )
...
def validate(
    ...
    if sha256(parent_path) != PARENT_SHA256:
        raise ValueError("unauthenticated child30 parent")
    if sha256(child_path) != CHILD_SHA256:
        raise ValueError("unauthenticated Lean-owned child31 export")
    ...
    if set(cut_by_occurrence) != {str(index) for index in range(len(entries))}:
        raise ValueError("not every all-core occurrence cuts child30")
    return {
        "schema": "p97-exact17-thirtieth-model-refinements-export-validation/v1",
