"""Fail-closed PIQD lifecycle adapter for the unprovisioned exact-17 Child40."""
...
from validate_exact17_thirty_ninth_model_refinements_ingress import (
    MANIFEST_SHA256,
...
_base.HARDENED_ARTIFACT_NAMESPACES = _base.HARDENED_ARTIFACT_NAMESPACES | {"child40"}
...
class RunnerSpec:
    ...
    variables: int
    clauses: int | None
    project: str = "erdos-97-96-exact17-child40"
    daemon_name: str = "piqd"
    daemon_version: str = "0.1.0"
    artifact_namespace: str = "child40"
    timeout_s: int = TIMEOUT_S
    march_timeout_s: int = MARCH_TIMEOUT_S

    def __post_init__(self) -> None:
        if self.artifact_namespace != "child40":
            raise ValueError("child40 runner requires artifact_namespace='child40'")
...
PRODUCTION_RUNNER_PATHS = RunnerPaths(
    PRODUCTION_INGRESS_PATHS,
    ROOT / "scratch/exact17-lean-to-sat/piqd-child40-core1-custody-intent.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child40-core1-custody-prepared.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child40-core1-custody-live-state.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child40-core1-custody-final.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child40-core1-custody-model.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child40-core1-custody-solver.log",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child40-core1-custody-runner.lock",
)
...
def _require_provisioned(spec: RunnerSpec) -> None:
    if spec.artifact_namespace != "child40" or not spec.provisioned:
        raise UnprovisionedError("child40 PIQD runner is UNPROVISIONED")
