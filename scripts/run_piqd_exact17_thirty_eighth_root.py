"""Fail-closed PIQD lifecycle adapter for the provisioned exact-17 Child38."""
...
from validate_exact17_thirty_seventh_model_refinements_ingress import (
...
_base.HARDENED_ARTIFACT_NAMESPACES = _base.HARDENED_ARTIFACT_NAMESPACES | {"child38"}
...
class RunnerSpec:
    ...
    variables: int
    clauses: int
    project: str = "erdos-97-96-exact17-child38"
    daemon_name: str = "piqd"
    daemon_version: str = "0.1.0"
    artifact_namespace: str = "child38"
    timeout_s: int = TIMEOUT_S
    march_timeout_s: int = MARCH_TIMEOUT_S

    def __post_init__(self) -> None:
        if self.artifact_namespace != "child38":
            raise ValueError("child38 runner requires artifact_namespace='child38'")
...
PRODUCTION_RUNNER_PATHS = RunnerPaths(PRODUCTION_INGRESS_PATHS, ROOT / "scratch/exact17-lean-to-sat/piqd-child38-core1-custody-intent.json", ROOT / "scratch/exact17-lean-to-sat/piqd-child38-core1-custody-prepared.json", ROOT / "scratch/exact17-lean-to-sat/piqd-child38-core1-custody-live-state.json", ROOT / "scratch/exact17-lean-to-sat/piqd-child38-core1-custody-final.json", ROOT / "scratch/exact17-lean-to-sat/piqd-child38-core1-custody-model.json", ROOT / "scratch/exact17-lean-to-sat/piqd-child38-core1-custody-solver.log", ROOT / "scratch/exact17-lean-to-sat/piqd-child38-core1-custody-runner.lock")
...
def _require_provisioned(spec: RunnerSpec) -> None:
    if spec.artifact_namespace != "child38" or not spec.provisioned:
        raise UnprovisionedError("child38 PIQD runner is UNPROVISIONED")
