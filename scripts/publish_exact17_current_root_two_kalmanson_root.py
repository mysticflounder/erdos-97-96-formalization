"""Create-once publication of the Lean-owned current-root two-Kalmanson CNF."""
...
COVERAGE_SCHEMA = "p97-exact17-current-root-two-kalmanson-coverage-ledger/v2"
AUDIT_SCHEMA = "p97-exact17-current-root-two-kalmanson-independent-audit/v1"
...
def _publish_candidate(reservation: CandidateReservation, child: Path) -> None:
        ...
        if linked:
            try:
                ...
                if (
                    ...
                    os.unlink(child.name, dir_fd=target_parent)
                    os.fsync(target_parent)
            except FileNotFoundError:
                pass
        raise
...
def coverage_ledger(
    ...
    return {
        ...
        "source": {
            ...
            "theorem": (
                "Problem97."
                "ATailBlockerVExactSeventeenCurrentRootTwoKalmansonRefinements."
                "sourceAssign_extendedCurrentRootTwoKalmansonCnf"
            ),
            "terminal_adapter": (
                "Problem97."
                "ATailBlockerVExactSeventeenCurrentRootTwoKalmansonRefinements."
                "false_of_sourceRealization_of_extendedCurrentRootTwoKalmansonCnf_unsat"
            ),
        },
        "parent": {
...
def publish(
    ...
    try:
        ...
        _require_source_commit(paths)
        publisher_binding = _require_publisher_provenance(paths, publisher_baseline)
        _publish_candidate(reservation, paths.child)
        ...
        _write_exclusive_readonly(paths.coverage_ledger, ledger_bytes)
