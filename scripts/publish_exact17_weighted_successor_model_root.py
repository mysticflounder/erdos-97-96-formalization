"""Create-once publication of the Lean-owned FortyEighth successor CNF."""
...
COVERAGE_SCHEMA = "p97-exact17-weighted-successor-model-coverage-ledger/v1"
AUDIT_SCHEMA = "p97-exact17-weighted-successor-model-independent-audit/v1"
...
class PublicationPaths:
    ...
    source_bridge: Path = validator.SOURCE_BRIDGE_PATH
...
def _require_source_commit(paths: PublicationPaths) -> None:
    ...
    expected_hashes = {
        ...
        paths.source_bridge: validator.SOURCE_BRIDGE_SHA256,
...
def _reserve_candidate(package_directory: Path) -> CandidateReservation:
    ...
    workspace_name = f".weighted-kalmanson-{secrets.token_hex(16)}"
...
def coverage_ledger(
    ...
    return {
        ...
        "source": {
            ...
            "theorem": (
                "Problem97."
                "ATailBlockerVExactSeventeenFortyEighthModelRefinements."
                "sourceAssign_extendedFortyEighthModelRefinementsCnf"
            ),
            "terminal_adapter": (
                "Problem97."
                "ATailBlockerVExactSeventeenFortyEighthModelRefinements."
                "false_of_sourceRealization_of_extendedFortyEighthModelRefinementsCnf_unsat"
            ),
        },
        "parent": {
        ...
        "enumeration": {
            ...
            "ordered_suffix_sha256": hashlib.sha256(
                b"".join(validator.expected_suffix_lines())
            ).hexdigest(),
            "parent_subsumption_census": validation.get(
                "parent_subsumption_census",
                {
                    "complete": True,
                    "parent_subsumed_suffix_clause_indices": validator.PARENT_SUBSUMED_SUFFIX_INDICES,
                    "parent_subsumed_suffix_clause_count": len(
                        validator.PARENT_SUBSUMED_SUFFIX_INDICES
                    ),
                    "witness_counts": validator.PARENT_SUBSUMPTION_WITNESS_COUNTS,
                    "witness_total": sum(validator.PARENT_SUBSUMPTION_WITNESS_COUNTS),
                },
            ),
        },
        "tooling": {
    ...
    try:
        ...
        receipt = {
            ...
            "source_imports": {
                ...
                "weighted_kalmanson_source_bridge": _artifact(paths.source_bridge),
            },
            "coverage_ledger": {
                "schema": COVERAGE_SCHEMA,
