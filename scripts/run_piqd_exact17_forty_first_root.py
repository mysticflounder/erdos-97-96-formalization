"""Fail-closed PIQD lifecycle adapter for the unprovisioned exact-17 Child41."""
...
from validate_exact17_forty_first_ingress import (
    MANIFEST_SHA256,
...
from validate_exact17_forty_first_wave_export import (
    CHILD_BYTES,
    CHILD_CLAUSES,
    CHILD_SHA256,
    MODEL_JOB_ID,
...
_base.HARDENED_ARTIFACT_NAMESPACES = _base.HARDENED_ARTIFACT_NAMESPACES | {"child41"}
...
class RunnerSpec:
    ...
    variables: int
    clauses: int | None
    project: str = "erdos-97-96-exact17-child41"
    daemon_name: str = "piqd"
    daemon_version: str = "0.1.0"
    artifact_namespace: str = "child41"
    timeout_s: int = TIMEOUT_S
    march_timeout_s: int = MARCH_TIMEOUT_S

    def __post_init__(self) -> None:
        if self.artifact_namespace != "child41":
            raise ValueError("child41 runner requires artifact_namespace='child41'")

    @property
    def provisioned(self) -> bool:
        return self.ingress.provisioned and self.manifest_sha256 is not None and self.root_sha256 is not None and self.root_bytes is not None and self.clauses is not None and self.root_sha256 == self.ingress.export.child_sha256 and self.root_bytes == self.ingress.export.child_bytes and self.clauses == self.ingress.export.child_clauses


PRODUCTION_RUNNER_PATHS = RunnerPaths(
    PRODUCTION_INGRESS_PATHS,
    ROOT / "scratch/exact17-lean-to-sat/piqd-child41-core1-custody-intent.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child41-core1-custody-prepared.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child41-core1-custody-live-state.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child41-core1-custody-final.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child41-core1-custody-model.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child41-core1-custody-solver.log",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child41-core1-custody-runner.lock",
)
PRODUCTION_RUNNER_SPEC = RunnerSpec(PRODUCTION_INGRESS_SPEC, MANIFEST_SHA256, CHILD_SHA256, CHILD_BYTES, 308, CHILD_CLAUSES)
...
def _require_provisioned(spec: RunnerSpec) -> None:
    if spec.artifact_namespace != "child41" or not spec.provisioned:
        raise UnprovisionedError("child41 PIQD runner is UNPROVISIONED")
