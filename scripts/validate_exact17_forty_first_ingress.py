"""Fail-closed immutable PIQD ingress for exact-17 Child41."""
...
from export_exact17_forty_first_root import PRODUCTION_PATHS, ExportPaths
...
SCHEMA = "p97-exact17-forty-first-model-refinements-piqd-ingress/v1"
RECEIPT_SCHEMA = "p97-exact17-child41-immutable-export-receipt/v1"
PARENT_MANIFEST = ROOT / "scratch/exact17-lean-to-sat/piqd-ingress-manifest-thirty-ninth-model-refinements-core1.json"
MANIFEST = ROOT / "scratch/exact17-lean-to-sat/piqd-ingress-manifest-forty-first-model-refinements-core1.json"
MANIFEST_SHA256: str | None = "a82887fa8029dd3241eff659d8419f904dba724abcad8b45902dba9a4817ca7d"
...
def _check_receipt(paths: IngressPaths, spec: IngressSpec, validation: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("child41 receipt schema/status drifted")
    ...
    if not isinstance(lean, dict) or set(lean) != {"root", "export"}:
        raise ValueError("child41 Lean receipt shape drifted")
    ...
    if receipt.get("child") != {
        ...
        raise ValueError("child41 receipt child identity drifted")
    if receipt.get("validation") != validation:
        raise ValueError("child41 receipt validation drifted")
    if receipt.get("immutability") != "exclusive-hard-link-and-ledger-last-receipt/v2":
        raise ValueError("child41 receipt immutability policy drifted")
    return receipt


def _daemon_record(paths: IngressPaths, spec: IngressSpec) -> dict[str, str]:
    if spec.daemon_build_receipt_sha256 is None:
        raise UnprovisionedError("child41 daemon receipt is unpinned")
...
def validate_ingress(paths: IngressPaths = PRODUCTION_INGRESS_PATHS, *, spec: IngressSpec = PRODUCTION_INGRESS_SPEC, expected_manifest_sha256: str | None = None) -> dict[str, Any]:
    if not spec.provisioned:
        raise UnprovisionedError("child41 PIQD ingress is UNPROVISIONED")
    if (
    ...
    if sha256_file(paths.parent_manifest) != spec.parent_manifest_sha256:
        raise ValueError("child40 parent ingress hash drifted")
