    ...
    *,
    ...
    try:
        ...
        payload: dict[str, Any] = {
            ...
            "source": {
                "pure_rup_source": parent_payload["source"],
                "parent_package": {
...
def verify_windowed_package(package_dir: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or set(source) != {"pure_rup_source", "parent_package"}
    ):
        raise MaterializationError("windowed package source object drift")
    base.validate_package_source(source.get("pure_rup_source"))
    parent_source = source.get("parent_package")
    if not isinstance(parent_source, dict):
        raise MaterializationError("windowed parent-package source is malformed")
    if parent_source.get("schema") != base.SCHEMA:
        raise MaterializationError("windowed parent-package schema drift")
