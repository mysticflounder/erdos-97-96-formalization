"""Focused lifecycle tests for the current-root-two-kalmanson one-shot PIQD runner."""
...
import pytest
import run_piqd_exact17_current_root_two_kalmanson_root as subject
    ...
    return {
        ...
        "ingress": {
            ...
            "export_receipt": {
                ...
                "bytes": 4_607,
                "schema": "p97-exact17-current-root-two-kalmanson-immutable-export-receipt/v2",
            },
        },
    }
    ...
    def make_runner(**kwargs: Any):
        ...
        def run(cnf: Path, timeout_s: int, proof_path: Path | None):
            ...
            kwargs["transport"]._verification = {
                "job_id": "job-current-root-two-kalmanson",
                "path": "/jobs/job-current-root-two-kalmanson/blobs/fixture",
...
def test_live_identity_rejects_daemon_drift() -> None:
    ...
    with pytest.raises(
        subject.CurrentRootTwoKalmansonRunnerError, match="daemon identity drifted"
    ):
...
def test_manifest_transport_rejects_substituted_remote_bytes_before_confirm() -> None:
    ...
    with pytest.raises(
        subject.CurrentRootTwoKalmansonRunnerError, match="bytes drifted"
    ):
        transport(
            "POST",
            "http://127.0.0.1:7272/jobs/confirm?job_id=job-bound",
...
def test_manifest_transport_rejects_existing_job_before_confirm() -> None:
    ...
    with pytest.raises(
        subject.CurrentRootTwoKalmansonRunnerError, match="reused an existing job"
    ):
        transport(
            "POST",
            "http://127.0.0.1:7272/jobs/prepare-cnf",
...
def test_duplicate_prepare_trace_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        subject.CurrentRootTwoKalmansonRunnerError, match="prepare exactly once"
    ):
