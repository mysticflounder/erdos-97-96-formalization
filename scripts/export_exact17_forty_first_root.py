"""Immutable Lean exporter for the fail-closed exact-17 Child41 boundary."""
...
def export_child41(paths: ExportPaths = PRODUCTION_PATHS, *, spec: ExportSpec = PRODUCTION_SPEC) -> dict[str, Any]:
    validate_spec(spec, require_source_pins=True)
    if not spec.provisioned:
        raise UnprovisionedError("child41 export is UNPROVISIONED")
    if paths.child.exists() or paths.receipt.exists():
        raise FileExistsError("refusing to replace existing Child41 or receipt")
    ...
    os.close(fd)
    candidate = Path(name)
    published_child_identity: tuple[int, int] | None = None
    try:
        candidate.unlink()
        ...
        published_child_identity = (candidate_stat.st_dev, candidate_stat.st_ino)
        os.link(candidate, paths.child, follow_symlinks=False)
        _fsync_dir(paths.child.parent)
        receipt = {"schema": "p97-exact17-child41-immutable-export-receipt/v1", "status": "PASS", "publication_state": "PROVISIONED", "parent": {"path": str(paths.parent.resolve()), "sha256": PARENT_SHA256}, "model": {"path": str(paths.model.resolve()), "sha256": published.model_sha256}, "lean": {"root": {"path": str(paths.lean_root.resolve()), "sha256": published.lean_root_sha256}, "export": {"path": str(paths.lean_export.resolve()), "sha256": published.lean_export_sha256}}, "child": {"path": str(paths.child.resolve()), "sha256": child_sha256, "bytes": child_bytes, "variables": published.variables, "clauses": published.child_clauses}, "validation": validation, "immutability": "exclusive-hard-link-and-ledger-last-receipt/v2"}
        _publish_json_ledger_last(paths.receipt, receipt)
        return receipt
    except BaseException:
        if published_child_identity is not None and not paths.receipt.exists():
            try:
                child_stat = os.stat(paths.child, follow_symlinks=False)
            except FileNotFoundError:
                pass
            else:
                current_identity = (child_stat.st_dev, child_stat.st_ino)
                if current_identity == published_child_identity:
                    paths.child.unlink()
        raise
    finally:
        candidate.unlink(missing_ok=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    subparsers.add_parser("export", help="export and immutably publish the Child41 root")
    args = parser.parse_args(argv)
    if args.command == "export":
        print(json.dumps(export_child41(), indent=2, sort_keys=True))
    return 0
