"""Validate the Lean exact-17 fourth-row convex-five-point child export.

Lean is authoritative. This checker requires a byte-identical authenticated
parent prefix, independently regenerates both four-clause orbits, and checks
that the suffix cuts the motivating twelfth-root SAT model.
"""
...
def validate(parent_path: Path, child_path: Path, model_path: Path) -> dict[str, object]:
    ...
    with (
        ...
        for motif_index, hits in enumerate(MOTIFS):
            for order_index, order in enumerate(ORDERS):
                for reverse in (False, True):
                    line = child.readline()
                    if not line:
                        raise ValueError("truncated convex-five-point suffix")
                    context = f"convex-five-point/{motif_index}/{order_index}/{reverse}"
                    ...
                    if observed != expected:
                        raise ValueError(
                            f"convex-five-point mismatch at {context}: "
                            f"{observed} != {expected}"
                        )
    ...
    if observed_suffix != NEW_CLAUSES:
        raise ValueError("wrong convex-five-point clause count")
    if not model_cut_clauses:
        raise ValueError("new suffix does not cut the motivating model")
    return {
        "schema": "p97-exact17-lean-convex-five-point-fourth-rows-export-validation/v1",
        "parent_prefix_byte_identical": True,
        "variables": VARIABLES,
        "clauses": CHILD_CLAUSES,
        "convex_five_point_clauses": observed_suffix,
        "convex_five_point_suffix_sha256": digest.hexdigest(),
