"""Regression tests for the exact13 guarded witnessed-Key wave5 lane."""
...
import cardge13_exact13_witnessed_key_guarded_cegar_wave5_piqd as subject


def test_wave5_root_identity_and_guarded_clause_shape() -> None:
...
def test_wave4_final_bank_is_pinned() -> None:
    root_path = subject.WAVE4_RUN / "artifacts/exact13-witnessed-key-effective-root.cnf"
    bank_path = subject.WAVE4_RUN / "events/cut-bank.jsonl"
    assert hashlib.sha256(root_path.read_bytes()).hexdigest() == subject.WAVE4_ROOT_SHA256
    assert hashlib.sha256(bank_path.read_bytes()).hexdigest() == subject.WAVE4_BANK_SHA256
...
def test_import_parent_partition_is_honest_if_run_exists() -> None:
    ...
    assert value["last_10000_cut_parent_partition"] == {
        "StrongValid": subject.EXPECTED_WAVE4_STRONGVALID,
    }
...
def test_imported_and_live_bank_paths_and_hashes_are_distinct_if_run_exists() -> None:
    ...
    assert value["immutable_imported_bank_sha256"] == subject.WAVE4_BANK_SHA256
...
def test_root_and_cut_identity_if_run_exists() -> None:
    ...
    if not root_path.exists() or not bank_path.exists():
        return
    header = next(line for line in root_path.read_bytes().splitlines() if line.startswith(b"p cnf "))
...
def test_producer_declares_formula_only_boundary_and_no_witness_atoms() -> None:
    ...
    assert "historical_parent_partition_preserved" in source
