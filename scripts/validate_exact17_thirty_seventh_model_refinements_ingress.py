"""Validate and emit the immutable PIQD ingress for exact-17 Child38."""
...
from export_exact17_thirty_eighth_root import PRODUCTION_PATHS, ExportPaths
from validate_exact17_thirty_seventh_model_refinements_export import (
    PRODUCTION_SPEC,
    ExportSpec,
    UnprovisionedError,
    sha256_file,
...
SCHEMA = "p97-exact17-thirty-seventh-model-refinements-piqd-ingress/v1"
RECEIPT_SCHEMA = "p97-exact17-child38-immutable-export-receipt/v1"
PARENT_MANIFEST = ROOT / "scratch/exact17-lean-to-sat/piqd-ingress-manifest-thirty-sixth-model-refinements-core1.json"
MANIFEST = ROOT / "scratch/exact17-lean-to-sat/piqd-ingress-manifest-thirty-seventh-model-refinements-core1.json"
...
PRODUCTION_INGRESS_SPEC = IngressSpec(PRODUCTION_SPEC, "3d02b8e2a18ba75b08020e8275ca7e7aadb8e0556762a062dbca3ae546026f18")
...
def _check_receipt(paths: IngressPaths, spec: IngressSpec, validation: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("child38 receipt schema/status drifted")
    ...
    if not isinstance(lean, dict) or set(lean) != {"root", "export"}:
        raise ValueError("child38 Lean receipt shape drifted")
    ...
    if receipt.get("child") != {
        ...
        raise ValueError("child38 receipt child identity drifted")
    if receipt.get("validation") != validation:
        raise ValueError("child38 receipt validation drifted")
    if receipt.get("immutability") != "exclusive-hard-link-and-ledger-last-receipt/v2":
        raise ValueError("child38 receipt immutability policy drifted")
    return receipt


def _daemon_record(paths: IngressPaths, spec: IngressSpec) -> dict[str, str]:
    if spec.daemon_build_receipt_sha256 is None:
        raise UnprovisionedError("child38 daemon receipt is unpinned")
...
def validate_ingress(paths: IngressPaths = PRODUCTION_INGRESS_PATHS, *, spec: IngressSpec = PRODUCTION_INGRESS_SPEC, expected_manifest_sha256: str | None = None) -> dict[str, Any]:
    if not spec.provisioned:
        raise UnprovisionedError("child38 PIQD ingress is UNPROVISIONED")
    if sha256_file(paths.parent_manifest) != spec.parent_manifest_sha256:
        raise ValueError("child37 parent ingress hash drifted")
    ...
    if manifest != _payload(paths, spec, validation):
        raise ValueError("child38 ingress content drifted")
    digest = sha256_file(paths.manifest)
    if expected_manifest_sha256 is not None and digest != expected_manifest_sha256:
        raise ValueError("child38 manifest SHA-256 drifted")
    return {"schema": SCHEMA, "status": "PASS", "manifest_sha256": digest, "dimacs_sha256": spec.export.child_sha256, "dimacs_bytes": spec.export.child_bytes, "variables": spec.export.variables, "clauses": spec.export.child_clauses, "new_clauses": spec.export.new_clauses, "parent_dimacs_sha256": spec.export.parent_sha256}
