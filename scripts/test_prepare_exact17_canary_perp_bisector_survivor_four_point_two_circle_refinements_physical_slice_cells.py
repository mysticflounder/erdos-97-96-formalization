...
"""Adversarial tests for the canary survivor FourPointTwoCircle refinement successor preparer."""
...
import prepare_exact17_canary_perp_bisector_survivor_four_point_two_circle_refinements_physical_slice_cells as subject
import pytest
...
def test_immediate_parent_exporter_is_the_survivor_refinement_parent() -> None:
    assert subject.IMMEDIATE_PARENT_EXPORTER_PATH == subject.ROOT / (
        "lean/Erdos9796Proof/P97/ATail/"
        "BlockerVExactSeventeenCanaryPerpBisectorSurvivor"
        "TwoKalmansonRefinementsV2Export.lean"
    )
...
def _fixture(
    ...
    target_preparer = tmp_path / (
        "scripts/prepare_exact17_canary_perp_bisector_survivor_four_point_two_circle_refinements_physical_slice_cells.py"
    )
    ...
    monkeypatch.setattr(
        subject, "PARENT_CLAUSES", len(_UNRELATED_PARENT_CLAUSES) + 4
    )
    monkeypatch.setattr(
        subject, "CELL_CLAUSES", len(_UNRELATED_PARENT_CLAUSES) + 4 + 6
    )
...
def _validate_parent_novelty_pair(
    ...
    try:
        return subject.validate_canary_perp_bisector_survivor_four_point_two_circle_refinement_parent_novelty(
...
def test_successor_dimensions_and_exact_six_units() -> None:
    parent = b"p cnf 308 7409516\n1 0\n"
    payload = subject.cell_cnf_bytes(parent, 16, "unique-10")
    lines = payload.splitlines()
    assert lines[0] == b"p cnf 308 7409522"
    assert lines[-6:] == [
        b"306 0",
        b"-279 0",
        b"-280 0",
...
def test_survivor_four_point_two_circle_refinement_suffix_is_exact() -> None:
    ...
    assert subject.IMMEDIATE_PARENT_CLAUSES == 7_409_512
    assert subject.PARENT_CLAUSES == 7_409_516
    assert subject.CELL_CLAUSES == 7_409_522
...
def test_initialize_creates_exact_governed_skeleton(
    ...
    assert (
        run["owner"]
        == subject.RUN_OWNER
        == "exact17-canary-perp-bisector-survivor-four-point-two-circle-preparer"
    )
...
def test_prepares_all_76_cells_and_validates_sentinels(
    ...
    assert report["parent_novelty"]["successor_multiplicity"] == [1] * 4
...
def test_unrelated_parent_clause_with_duplicate_literal_is_accepted(
    ...
    assert novelty["exact_parent_multiplicity"] == [0] * 4
    _assert_expected_strict_new_profile(novelty)
    assert novelty["successor_multiplicity"] == [1] * 4
...
def test_duplicate_canary_suffix_clause_fails_before_children(
    ...
    try:
        with pytest.raises(
            ...
            subject.validate_canary_perp_bisector_survivor_four_point_two_circle_refinement_parent_novelty(
