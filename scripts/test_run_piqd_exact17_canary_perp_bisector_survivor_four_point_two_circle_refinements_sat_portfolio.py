...
import pytest
import run_piqd_exact17_canary_perp_bisector_survivor_four_point_two_circle_refinements_sat_portfolio as runner
...
def _cells() -> list[dict[str, Any]]:
    ...
    for ordinal, (center, category) in enumerate(roles):
        source = f"canary-perp-bisector-survivor-four-point-two-circle-refinements-next-center-{center:02d}-physical-{category}"
        result.append(
            {
...
def _identity_source_fixture(
    ...
    for ordinal, (center, category) in enumerate(roles):
        source_id = (
            "canary-perp-bisector-survivor-four-point-two-circle-refinements-next-center-"
        ...
        producer = {
            ...
            "production_config": {
                ...
                "target_code": {
                    "commit": commit,
                    "preparer": {"path": runner.EXPECTED_SOURCE_PREPARER_RELATIVE},
                },
            },
        }
...
@pytest.mark.parametrize("mutation", ["config-path", "config-schema", "preparer-path"])
def test_provisional_identity_derivation_rejects_source_custody_path_drift(
    ...
    producer_path = tmp_path / cell["producer_manifest"]["path"]
    producer = json.loads(producer_path.read_bytes())
    if mutation == "config-path":
        producer["production_config"]["path"] = "wrong-config.json"
    elif mutation == "config-schema":
        producer["production_config"]["schema"] = "wrong-schema/v1"
    else:
        producer["production_config"]["target_code"]["preparer"]["path"] = (
            "scripts/wrong_preparer.py"
        )
    producer_raw = runner.canonical_json_bytes(producer)
    producer_path.write_bytes(producer_raw)
    producer_path.chmod(0o600)
    ...
    campaign_path.chmod(0o600)
    with pytest.raises(
        runner.PortfolioRunnerError, match="config/preparer path drifted"
    ):
        runner.derive_identities(
            root=tmp_path,
            **{
...
def test_campaign_contract_is_exactly_76_cells_and_center2_none_canary() -> None:
    assert runner.ROOT_NUM_CLAUSES == 7_409_516
    assert runner.NUM_CLAUSES == 7_409_522
    assert runner.CELL_COUNT == 76
    assert runner.SOURCE_PREPARER_RELATIVE.endswith(
        "prepare_exact17_canary_perp_bisector_survivor_four_point_two_circle_"
    ...
    assert "SurvivorFourPointTwoCircleRefinements" in runner.EXPECTED_SOURCE_PATH
    assert "SurvivorFourPointTwoCircleRefinements" in runner.EXPECTED_ROOT_SOURCE_PATH
    campaign, run = _campaign()
    ...
    producer = runner.canonical_json_bytes(
        {
            ...
            "production_config": {
                ...
                "schema": runner.EXPECTED_SOURCE_PREPARATION_CONFIG_SCHEMA,
                "target_code": {
                    "commit": runner.SOURCE_PREPARER_COMMIT,
                    "preparer": {"path": runner.EXPECTED_SOURCE_PREPARER_RELATIVE},
                },
            },
        }
