...
"""Adversarial tests for the canary survivor two-Kalmanson refinement successor preparer."""
...
import prepare_exact17_canary_perp_bisector_survivor_two_kalmanson_refinements_physical_slice_cells as subject
import pytest
...
def test_immediate_parent_exporter_is_the_survivor_refinement_parent() -> None:
    assert subject.IMMEDIATE_PARENT_EXPORTER_PATH == subject.ROOT / (
        ...
        "SurvivorRefinementsPhysicalSliceCellExport.lean"
    )
...
def _successor(
    *,
    ...
    clauses = (
        subject.EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_TWO_KALMANSON_REFINEMENT_SUFFIX
        if suffix is None
        else suffix
    )
    body = parent.split(b"\n", 1)[1]
...
def _fixture(
    ...
    target_preparer = tmp_path / (
        "scripts/prepare_exact17_canary_perp_bisector_survivor_two_kalmanson_refinements_physical_slice_cells.py"
    )
...
def _validate_parent_novelty_pair(
    ...
    try:
        return subject.validate_canary_perp_bisector_survivor_two_kalmanson_refinement_parent_novelty(
...
def test_successor_dimensions_and_exact_six_units() -> None:
    parent = b"p cnf 308 7409372\n1 0\n"
    payload = subject.cell_cnf_bytes(parent, 16, "unique-10")
    lines = payload.splitlines()
    assert lines[0] == b"p cnf 308 7409378"
    assert lines[-6:] == [
        b"306 0",
        b"-279 0",
        b"-280 0",
...
def test_survivor_two_kalmanson_refinement_suffix_is_exact() -> None:
    suffix = (
        subject.EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_TWO_KALMANSON_REFINEMENT_SUFFIX
    )
    assert len(suffix) == 68
    assert len(set(suffix)) == 68
    assert subject.sha256_bytes(subject.canonical_json_bytes(suffix)) == (
        subject.EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_TWO_KALMANSON_REFINEMENT_SUFFIX_SHA256
    )
    assert suffix[0] == (
        -307,
        -143,
    ...
    assert subject.IMMEDIATE_PARENT_CLAUSES == 7_409_304
    assert subject.PARENT_CLAUSES == 7_409_372
    assert subject.CELL_CLAUSES == 7_409_378
...
def test_initialize_creates_exact_governed_skeleton(
    ...
    assert (
        run["owner"]
        == subject.RUN_OWNER
        == "exact17-canary-perp-bisector-survivor-two-kalmanson-preparer"
    )
...
def test_unrelated_parent_clause_with_duplicate_literal_is_accepted(
    ...
    assert novelty["suffix_clauses"] == [
        list(clause)
        for clause in subject.EXPECTED_CANARY_PERP_BISECTOR_SURVIVOR_TWO_KALMANSON_REFINEMENT_SUFFIX
    ]
...
def test_duplicate_canary_suffix_clause_fails_before_children(
    ...
    try:
        with pytest.raises(
            ...
            subject.validate_canary_perp_bisector_survivor_two_kalmanson_refinement_parent_novelty(
