...
"""Contracts for the gated canary perpendicular-bisector SAT portfolio."""
...
import pytest
import run_piqd_exact17_canary_perp_bisector_refinement_sat_portfolio as subject
...
def test_lane_is_bound_to_audited_canary_perp_bisector_source() -> None:
    ...
    assert preparation.SOURCE_CAMPAIGN_BYTES == 104_850
    assert preparation.SOURCE_RUN_MANIFEST_SHA256 == (
        "c2dc5d871c8f0be4e9e780b7773b04552e748f553a7295ca12a04f694d847f06"
    )
    assert preparation.SOURCE_RUN_MANIFEST_BYTES == 2_379
    assert preparation.SOURCE_PREPARER_COMMIT == (
        "2ae9bbf7490ae8bd9318159c6d91339609607faa"
    )
    assert preparation.SOURCE_PREPARER_SHA256 == (
        "953c3b2743f8722f6ee2b50225395c63fd3d345701c465d4e3a4323710d851c8"
    )
    assert preparation.SOURCE_PREPARER_BYTES == 115_892
    for value in (
        ...
        assert "canary-perp-bisector-refinement" in value
...
def test_all_transformed_identities_are_pinned_and_six_way_unique() -> None:
    ...
    for ordinal, cell in enumerate(cells):
        ...
        source_identity = subject.SOURCE_CELL_IDENTITIES[category]
        assert source_identity == {
            "cnf_sha256": cnf_ref["sha256"],
            "cnf_bytes": cnf_ref["bytes"],
            "producer_sha256": producer_ref["sha256"],
            "producer_bytes": producer_ref["bytes"],
            "wave_sha256": wave_ref["sha256"],
            "wave_bytes": wave_ref["bytes"],
        }
...
def test_public_launch_functions_forward_only_gated_modes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(subject, "_start", fake_start)
    canary = subject.start_canary()
    rest = subject.start_rest()
    assert canary["launch_mode"] == "SINGLE_CELL_CANARY"
    assert rest["launch_mode"] == "REMAINING_FIVE"
    assert [call["launch_mode"] for call in calls] == [
        "SINGLE_CELL_CANARY",
        "REMAINING_FIVE",
    ]


def test_inherited_ungated_launch_entry_point_is_disabled() -> None:
    with pytest.raises(subject.PortfolioRunnerError, match="ungated portfolio launch"):
        subject.start(base_url="http://127.0.0.1:7272")


def test_gated_selection_and_canary_finalization_contract_are_retained() -> None:
    canary_cell = "canary-perp-bisector-refinement-next-center-02-physical-none"
    canary_portfolio_cell = preparation.portfolio_cell_id("none")
    assert canary_portfolio_cell == f"{canary_cell}-sat-profile-v1"
    assert canary_cell in subject._start.__code__.co_consts
    assert "_validate_canary_custody" in subject.static_check.__code__.co_names
    canary_validator_names = subject._validate_canary_custody.__code__.co_names
    assert "validate_attempt_journal" in canary_validator_names
    assert "wave_manifest_sha256" in canary_validator_names
    assert "sha256_json" in canary_validator_names
    start_constants = subject._start.__code__.co_consts
    assert "SINGLE_CELL_CANARY" in start_constants
    assert "REMAINING_FIVE" in start_constants
    assert "static_check" in subject._start.__code__.co_names
    assert preparation.CELL_COUNT - 1 == 5


def test_run_cell_rejects_wrong_source_dimensions_before_daemon_use(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    cnf = b"p cnf 308 7409302\n"
...
def test_public_cli_exposes_only_prepare_check_and_gated_launches() -> None:
    ...
    assert "canary perpendicular-bisector refinement" in result.stdout
    assert "{prepare,static-check,start-canary,start-rest}" in result.stdout
    rejected = subprocess.run(
        [sys.executable, str(Path(subject.__file__)), "start"],
        capture_output=True,
        text=True,
        check=False,
    )
