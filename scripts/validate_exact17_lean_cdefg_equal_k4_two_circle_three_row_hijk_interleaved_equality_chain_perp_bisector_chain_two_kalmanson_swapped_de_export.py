"""Validate the Lean exact-17 swapped-D/E two-Kalmanson child export.

Lean is authoritative. This independent checker validates the complete
two-Kalmanson parent recursively, requires a byte-identical parent prefix,
...
SWAPPED_DE_CLAUSES = 544_544
CHILD_CLAUSES = PARENT_CLAUSES + SWAPPED_DE_CLAUSES
...
def expected_clause(
    ...
    # Offsets are the increasing local positions [B,C,E,D,F,G].
    local = (0, *offsets)
    ...
    return (
        ...
        *(
            ...
            for center, target in SWAPPED_DE_PAIRS
        ),
    )
...
def validate(
    ...
    parent_report = load_validator(
        parent_validator_path, "exact17_two_kalmanson_validator"
    ).validate(
        base_path,
        ancestor_path,
        grandparent_path,
    ...
    with (
        two_kalmanson_path.open(encoding="ascii") as parent,
        child_path.open(encoding="ascii") as child,
    ):
        if read_header(parent, two_kalmanson_path) != (VARIABLES, PARENT_CLAUSES):
            ...
            for step in (1, -1):
                for cut in range(17):
                    for offsets in SIX_OFFSET_CHOICES:
                        ...
                        if not line:
                            raise ValueError(
                                "truncated swapped-D/E suffix at "
                                f"{order_index}/{step}/{cut}/{offsets}"
                            )
                        context = f"swapped-de/{order_index}/{step}/{cut}/{offsets}"
                        ...
                        if observed != expected:
                            raise ValueError(
                                f"swapped-D/E mismatch at {context}: "
                                f"{observed} != {expected}"
                            )
    ...
    if observed_suffix != SWAPPED_DE_CLAUSES:
        raise ValueError(
            f"wrong swapped-D/E count: {observed_suffix} != {SWAPPED_DE_CLAUSES}"
        )
    if first_model_cut_clause is None:
        raise ValueError("new suffix does not cut the motivating model")
    return {
        "schema": "p97-exact17-lean-two-kalmanson-swapped-de-export-validation/v1",
        "parent": parent_report,
        "parent_prefix_byte_identical": True,
        "variables": VARIABLES,
        "clauses": CHILD_CLAUSES,
        "swapped_de_clauses": observed_suffix,
        "swapped_de_suffix_sha256": digest.hexdigest(),
