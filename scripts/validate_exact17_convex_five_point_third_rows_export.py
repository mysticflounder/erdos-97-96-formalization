"""Validate the Lean exact-17 three-row convex-five-point child export.

Lean is authoritative. This checker requires a byte-identical authenticated
parent prefix, independently regenerates the four new orbit clauses, and
checks that the suffix cuts the motivating eleventh-root SAT model.
"""
...
def validate(parent_path: Path, child_path: Path, model_path: Path) -> dict[str, object]:
    ...
    with (
        ...
        for order_index, order in enumerate(ORDERS):
            for reverse in (False, True):
                ...
                context = f"convex-five-point/{order_index}/{reverse}"
    ...
    return {
        "schema": "p97-exact17-lean-convex-five-point-third-rows-export-validation/v1",
