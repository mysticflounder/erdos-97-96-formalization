"""Fail-closed PIQD lifecycle adapter for the unprovisioned exact-17 Child43."""
...
from validate_exact17_forty_third_ingress import (
    MANIFEST_SHA256,
...
_base.HARDENED_ARTIFACT_NAMESPACES = _base.HARDENED_ARTIFACT_NAMESPACES | {"child43"}
...
class RunnerSpec:
    ...
    variables: int
    clauses: int | None
    project: str = "erdos-97-96-exact17-child43"
    daemon_name: str = "piqd"
    daemon_version: str = "0.1.0"
    artifact_namespace: str = "child43"
    timeout_s: int = TIMEOUT_S
    march_timeout_s: int = MARCH_TIMEOUT_S

    def __post_init__(self) -> None:
        if self.artifact_namespace != "child43":
            raise ValueError("child43 runner requires artifact_namespace='child43'")
...
PRODUCTION_RUNNER_PATHS = RunnerPaths(
    PRODUCTION_INGRESS_PATHS,
    ROOT / "scratch/exact17-lean-to-sat/piqd-child43-core1-custody-intent.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child43-core1-custody-prepared.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child43-core1-custody-live-state.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child43-core1-custody-final.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child43-core1-custody-model.json",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child43-core1-custody-solver.log",
    ROOT / "scratch/exact17-lean-to-sat/piqd-child43-core1-custody-runner.lock",
)
...
def _require_provisioned(spec: RunnerSpec) -> None:
    if spec.artifact_namespace != "child43" or not spec.provisioned:
        raise UnprovisionedError("child43 PIQD runner is UNPROVISIONED")
