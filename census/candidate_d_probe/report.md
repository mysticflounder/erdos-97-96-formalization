...
**All artifacts:** `/Users/adam/projects/math-projects/erdos-97-96-formalization/census/candidate_d_probe/` — `formulation.md` (window grounding, read first), `kill_notes.md` (proofs), `STATE.md` (status + resume), `encoder.py`, `witness_factory.py`, `witnesses.json` (13 exact witnesses), `q2_token.py`, `q2_results_{proven,plus}.json` (D1 witnesses), `verify_tuples.py`, `float_probe_sub2.py`, `float_refine_sub2.py`, `runs/` (solver files), `q1_results.json`.
...
Per-type kills alone cannot close the tail: 13/19 types are realizable at the decorated minimal window (exact witnesses — the per-type analog of what W16-sq did to Candidate A), and Q2 shows even maximal per-type kills leave admissible patterns at every probed order — Candidate D needs cross-center coupling (the Q3 two-center window is the smallest such object). What the probe adds to any census/C-split route as free, PROVEN admissibility constraints: (i) m ≤ 2 per packet; (ii) vertex-pair packets have s = 0 (the first unconditional convexity-coupled per-type kills in this workstream — partially rebutting obstruction D2's "squeeze engine absent"); (iii) s = 2 only in straddle form; conditionally (iv) s ≤ 2 outright if SUB2 lands. [2026-07-12 audit note: (iii) was mislabeled PROVEN here at probe time — its kill direction rode SUB2 (the proven content then was k = 1 realizability; kill_notes K-D and STATE.md had the dependency right). SUB2-H has since been decided PROVEN UNSAT (`scratch/atail-sub2/`, skeptic audit PASS), so (iii) and (iv) now hold at PROVEN tier.] Ranked next steps: (1) decide SUB2-H (one 7-variable NRA sentence; install QEPCAD/Redlog or hand-prove); (2) Q3 two-center sweep with the K-toolkit; (3) fold (i)–(iii) into census/pattern enumeration; (4) settle (1,2,0,1) k=1.
