...
"""Fail-closed v2 predicate coverage for the Rigid221 card-18 packet.

This ledger derives the exact v1 predicate and source universes, then records
one reviewed Lean-source upgrade: the two crossed arms now expose their exact
five-incidence Boolean profiles.  It authenticates the frozen source and
receipt inputs.  It does not serialize Lean values, establish a Python/Lean
serialization relation, admit clauses, run a solver, or authorize a canary.
"""
...
SCHEMA: Final = "p97-rigid221-card18-predicate-coverage/v2"
LANE_ID_METADATA: Final = "rigid221-card18-predicate-coverage-v2-20260827"
LANE_BASE_HEAD_METADATA: Final = "7b24c21f0b4764dae852c747b4b384b9a6a97fab"
...
FROZEN_V2_CANDIDATE_ROWS_SHA256: Final = (
    "f9d4a1e9dff4ef038217a701cf3b6b6050af0251acfe48c0d3d4d46b7d562f98"
)
FROZEN_V2_LEAN_SOURCE_ROWS_SHA256: Final = (
...
class Rigid221Card18PredicateCoverageV2Error(ValueError):
    """The frozen v2 coverage ledger or a bound artifact is invalid."""


def _fail(message: str) -> NoReturn:
    raise Rigid221Card18PredicateCoverageV2Error(message)
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise Rigid221Card18PredicateCoverageV2Error(
            "predicate-coverage-v2 value is not strict JSON"
        ) from exc
...
def _strict_json(payload: bytes) -> Any:
    ...
    except Rigid221Card18PredicateCoverageV2Error:
        raise
...
def _v1_candidate_rows() -> list[dict[str, Any]]:
    try:
        value = _strict_copy(list(coverage_v1.frozen_candidate_predicate_records()))
    except coverage_v1.Rigid221Card18PredicateCoverageError as exc:
        raise Rigid221Card18PredicateCoverageV2Error(
            "frozen v1 candidate universe failed its own guard"
        ) from exc
    if type(value) is not list or any(type(row) is not dict for row in value):
        _fail("frozen v1 candidate universe is malformed")
    return value


def _v1_source_rows() -> list[dict[str, Any]]:
    try:
        value = _strict_copy(list(coverage_v1.frozen_lean_source_universe()))
    except coverage_v1.Rigid221Card18PredicateCoverageError as exc:
        raise Rigid221Card18PredicateCoverageV2Error(
            "frozen v1 Lean-source universe failed its own guard"
        ) from exc
    if type(value) is not list or any(type(row) is not dict for row in value):
        _fail("frozen v1 Lean-source universe is malformed")
    return value
...
def _expected_lean_source_rows() -> list[dict[str, Any]]:
    rows = _v1_source_rows()
    replacements = 0
    for index, row in enumerate(rows):
        if row.get("id") == PROFILE_SOURCE_ID:
            rows[index] = _upgraded_profile_source_row()
            replacements += 1
    if replacements != 1:
        _fail("frozen v1 Lean-source universe lacks the unique profile row")
    return rows
...
def _coverage_summary() -> dict[str, Any]:
    return {
        ...
        "candidate_rows_sha256": FROZEN_V2_CANDIDATE_ROWS_SHA256,
        "delta_count": 12,
        ...
        "lean_source_rows_sha256": FROZEN_V2_LEAN_SOURCE_ROWS_SHA256,
    }
...
def _configuration_snapshot() -> dict[str, Any]:
    return {
        ...
        "frozen_hashes": {
            ...
            "candidate_rows": FROZEN_V2_CANDIDATE_ROWS_SHA256,
            ...
            "lean_source_rows": FROZEN_V2_LEAN_SOURCE_ROWS_SHA256,
        },
...
def _assert_delta_shape(
    ...
    if len(v1_candidates) != 188 or len(v2_candidates) != 188:
        _fail("frozen candidate universe has the wrong count")
    changed_candidates: list[str] = []
    for old, new in zip(v1_candidates, v2_candidates, strict=True):
        if old.get("id") != new.get("id"):
            _fail("v2 candidate IDs diverge from v1")
        ...
        if canonical_id in DELTA_CANDIDATE_IDS:
            ...
            if (
                ...
                or old.get("lean_source_ids") != [PROFILE_SOURCE_ID]
                or new != expected
            ):
                _fail("v2 candidate delta changes more than the frozen status field")
            changed_candidates.append(canonical_id)
        elif new != old:
            _fail("non-delta v2 candidate row differs from v1")
    if changed_candidates != list(DELTA_CANDIDATE_IDS):
        _fail("v2 candidate delta is omitted, reordered, or replaced")

    if len(v1_sources) != 37 or len(v2_sources) != 37:
        _fail("frozen Lean-source universe has the wrong count")
    changed_sources: list[str] = []
    for old, new in zip(v1_sources, v2_sources, strict=True):
        if old.get("id") != new.get("id"):
            _fail("v2 Lean-source IDs diverge from v1")
        if old.get("id") == PROFILE_SOURCE_ID:
            if (
                old.get("status") != PROP_AVAILABLE_NOT_EMITTED
                or new != _upgraded_profile_source_row()
            ):
                _fail("crossed-incidence Lean-source row is not the exact upgrade")
            changed_sources.append(PROFILE_SOURCE_ID)
        elif new != old:
            _fail("non-delta v2 Lean-source row differs from v1")
    if changed_sources != [PROFILE_SOURCE_ID]:
        _fail("v2 Lean-source delta is omitted or duplicated")


def _assert_frozen_configuration() -> None:
    literal = "0e4fcd139eed1723ff2a049a25ebd8ae0d0e4cd5eaa37fb73f04db3f85dbe651"
    try:
        scalar_strings = (
            SCHEMA,
            LANE_ID_METADATA,
            ...
            FROZEN_V2_CANDIDATE_ROWS_SHA256,
            FROZEN_V2_LEAN_SOURCE_ROWS_SHA256,
        ...
        if (
            ...
            _fail("frozen predicate-coverage-v2 configuration has malformed types")
        if (
        ...
        v1_candidates = _v1_candidate_rows()
        v2_candidates = _expected_candidate_records()
        v1_sources = _v1_source_rows()
        v2_sources = _expected_lean_source_rows()
        _assert_delta_shape(v1_candidates, v2_candidates, v1_sources, v2_sources)
        candidate_ids = [row["id"] for row in v2_candidates]
        source_ids = [row["id"] for row in v2_sources]
        if (
            candidate_ids != sorted(candidate_ids)
            or len(candidate_ids) != len(set(candidate_ids))
            or source_ids != sorted(source_ids)
            or len(source_ids) != len(set(source_ids))
            or Counter(row["origin"] for row in v2_candidates)
            != Counter({CALLER_ASSERTION: 42, CALLER_DATA: 39, LOCALLY_RECOMPUTED: 107})
            or Counter(row["status"] for row in v2_sources)
            != Counter(
                {
                    LABEL_DATA_AVAILABLE: 14,
                    PROP_AVAILABLE_NOT_EMITTED: 16,
            ...
            _fail("frozen v2 predicate/source universe has wrong IDs or counts")
        if (
            ...
            or _sha256_json(v2_candidates) != FROZEN_V2_CANDIDATE_ROWS_SHA256
            or _sha256_json(v2_sources) != FROZEN_V2_LEAN_SOURCE_ROWS_SHA256
        ):
            _fail("frozen v2 predicate/source/anchor hash drifted")
        current = _sha256_json(_configuration_snapshot())
        if FROZEN_CONFIGURATION_SHA256 != literal or current != literal:
            _fail("frozen predicate-coverage-v2 configuration digest drifted")
    except Rigid221Card18PredicateCoverageV2Error:
        raise
    except (AttributeError, KeyError, TypeError, ValueError) as exc:
        raise Rigid221Card18PredicateCoverageV2Error(
            "frozen predicate-coverage-v2 configuration is malformed"
        ) from exc


def frozen_v2_candidate_predicate_records() -> tuple[dict[str, Any], ...]:
    """Return independent copies of the 188 exact v2 candidate rows."""

    _assert_frozen_configuration()
    return tuple(_strict_copy(_expected_candidate_records()))


def frozen_v2_lean_source_universe() -> tuple[dict[str, Any], ...]:
    """Return independent copies of the 37 exact v2 Lean-source rows."""
...
def _validate_v1_manifest_payload(repo_root: Path, payload: bytes) -> None:
    ...
    if (
        ...
        or value.get("candidate_predicates") != _v1_candidate_rows()
        or value.get("lean_source_universe") != _v1_source_rows()
    ):
...
def _attest_live_bindings(repo_root: Path) -> dict[str, Any]:
    ...
    return _artifact_record(payloads[COVERAGE_MODULE_PATH], COVERAGE_MODULE_PATH)


def _bindings_with_coverage(coverage_record: Mapping[str, Any]) -> dict[str, Any]:
    bindings = _fixed_bindings()
    bindings["coverage_v2_module"] = dict(coverage_record)
    return bindings
...
def _validate_bindings(value: Any) -> None:
    fixed = _fixed_bindings()
    if type(value) is not dict or set(value) != {*fixed, "coverage_v2_module"}:
        _fail("predicate-coverage-v2 artifact binding fields are malformed")
    for key, expected in fixed.items():
        _validate_binding_record(value[key], expected, key)
    coverage_record = value["coverage_v2_module"]
    if (
        type(coverage_record) is not dict
        or set(coverage_record) != {"path", "bytes", "sha256"}
        or coverage_record.get("path") != COVERAGE_MODULE_PATH
        ...
        _fail("predicate-coverage-v2 module binding is malformed")


def _validate_source_rows(rows: Any) -> dict[str, str]:
    if type(rows) is not list:
        _fail("v2 Lean-source universe is not a list")
    ids: list[str] = []
    statuses: dict[str, str] = {}
    for row in rows:
        if type(row) is not dict or set(row) != _SOURCE_FIELDS:
            _fail("v2 Lean-source row fields are malformed")
        canonical_id = row["id"]
        status = row["status"]
        ...
        if (
            ...
            _fail("v2 Lean-source row values are malformed")
        for anchor in anchors:
            if (
                ...
                _fail("v2 Lean declaration anchor is malformed")
        if anchors != sorted(
            anchors,
            ...
            _fail("v2 Lean declaration anchors are reordered or duplicated")
        if status == ABSENT_FROM_LABELED_EXPORT:
            if (
                ...
                _fail("absent v2 Lean row gained an anchor or stopped blocking")
        elif not anchors:
            _fail("non-absent v2 Lean row lacks a declaration anchor")
        if status == PROP_AVAILABLE_NOT_EMITTED and (
            representation != "LEAN_PROP_ONLY"
            or row["executable"]
            or not row["blocking"]
        ):
            _fail("Prop-only v2 Lean row is malformed")
        if canonical_id == PROFILE_SOURCE_ID:
            ...
            _fail("non-delta v2 Lean row changed its v1 custody digest")
        ids.append(canonical_id)
        statuses[canonical_id] = status
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        _fail("v2 Lean-source IDs are duplicated or reordered")
    if rows != _expected_lean_source_rows():
        _fail("v2 Lean-source universe differs from the frozen exact rows")
    if (
        _sha256_json(ids) != FROZEN_LEAN_SOURCE_ID_SET_SHA256
        or _sha256_json(rows) != FROZEN_V2_LEAN_SOURCE_ROWS_SHA256
    ):
        _fail("v2 Lean-source universe hash is stale")
    return statuses


def _validate_candidate_rows(rows: Any, source_statuses: Mapping[str, str]) -> None:
    if type(rows) is not list:
        _fail("v2 candidate predicate universe is not a list")
    ...
    for row in rows:
        if type(row) is not dict or set(row) != _CANDIDATE_FIELDS:
            _fail("v2 candidate predicate row fields are malformed")
        canonical_id = row["id"]
        origin = row["origin"]
        representation = row["representation"]
        ...
        if (
            ...
            _fail("v2 candidate row is malformed or escalated")
        ...
        if len(referenced_statuses) != 1 or row["lean_coverage_status"] != next(
            ...
            _fail("v2 candidate Lean status is not derived from source rows")
        ...
        if origin == LOCALLY_RECOMPUTED:
            if (
                ...
                _fail("locally recomputed v2 row lacks an exact algorithm")
        elif algorithm is not None:
            _fail("caller-supplied v2 row gained a recomputation algorithm")
        ids.append(canonical_id)
        records_by_id[canonical_id] = row
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        _fail("v2 candidate predicate IDs are duplicated or reordered")
    id_set = set(ids)
    for row in rows:
        for dependency in row["dependencies"]:
            if dependency not in id_set:
                _fail("v2 candidate dependency is unknown")
            if not set(row["arms"]).issubset(records_by_id[dependency]["arms"]):
                _fail("v2 candidate dependency is unavailable on a dependent arm")
    visiting: set[str] = set()
    visited: set[str] = set()

    def visit(node: str) -> None:
        if node in visiting:
            _fail("v2 candidate dependency graph contains a cycle")
        if node in visited:
            return
        visiting.add(node)
    ...
    if rows != _expected_candidate_records():
        _fail("v2 candidate predicate universe differs from the frozen exact rows")
    if (
        _sha256_json(ids) != FROZEN_CANDIDATE_ID_SET_SHA256
        or _sha256_json(rows) != FROZEN_V2_CANDIDATE_ROWS_SHA256
    ):
        _fail("v2 candidate predicate universe hash is stale")


def validate_predicate_coverage_v2_manifest(
    manifest: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate the exact v2 delta, bindings, admissions, and self-hash."""

    _assert_frozen_configuration()
    if not isinstance(manifest, Mapping):
        _fail("predicate-coverage-v2 manifest is not a mapping")
    snapshot = _strict_copy(manifest)
    if type(snapshot) is not dict or set(snapshot) != _MANIFEST_FIELDS:
        _fail("predicate-coverage-v2 manifest fields are malformed")
    if type(snapshot["schema"]) is not str or snapshot["schema"] != SCHEMA:
        _fail("predicate-coverage-v2 manifest schema is wrong")
    if type(snapshot["metadata"]) is not dict or snapshot["metadata"] != dict(
        _METADATA
    ):
        _fail("predicate-coverage-v2 lane metadata is wrong")
    ...
    if type(summary) is not dict or summary != _coverage_summary():
        _fail("predicate-coverage-v2 summary is stale")
    if (
        any(
...
def build_predicate_coverage_v2_manifest(repo_root: Path) -> dict[str, Any]:
    """Freshly attest every v1 and crossed binding and build the v2 ledger."""

    coverage_record = _attest_live_bindings(repo_root)
    body = _manifest_body(coverage_record)
    return validate_predicate_coverage_v2_manifest(
        {**body, "manifest_sha256": _sha256_json(body)}
    )


def encode_predicate_coverage_v2_manifest(manifest: Mapping[str, Any]) -> bytes:
    """Return the unique canonical UTF-8 encoding of a valid v2 manifest."""

    _assert_frozen_configuration()
    return _canonical_json_bytes(validate_predicate_coverage_v2_manifest(manifest))


def parse_predicate_coverage_v2_manifest(payload: bytes) -> dict[str, Any]:
    """Parse duplicate-free strict JSON and require its canonical bytes."""

    _assert_frozen_configuration()
    if type(payload) is not bytes:
        _fail("predicate-coverage-v2 payload is not immutable bytes")
    value = _strict_json(payload)
    if _canonical_json_bytes(value) != payload:
        _fail("predicate-coverage-v2 payload is not canonical JSON")
    return validate_predicate_coverage_v2_manifest(value)


def attest_predicate_coverage_v2_manifest(
    repo_root: Path,
    payload: bytes,
) -> dict[str, Any]:
    """Bind canonical v2 bytes to fresh no-follow reads and live custody."""

    parsed = parse_predicate_coverage_v2_manifest(payload)
    live_coverage = _attest_live_bindings(repo_root)
    if parsed["bindings"]["coverage_v2_module"] != live_coverage:
        _fail("predicate-coverage-v2 module live hash is stale")
    ...
    if payload != _canonical_json_bytes(fresh):
        _fail("predicate-coverage-v2 manifest is not live-current")
    return parsed
