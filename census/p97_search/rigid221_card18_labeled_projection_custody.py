...
SCHEMA: Final = "p97-rigid221-card18-labeled-projection-lean-source-custody/v1"
LANE_ID_METADATA: Final = "rigid221-card18-labeled-projection-custody-20260827"
LANE_BASE_HEAD_METADATA: Final = "a83fd4dbbb3233f5b99496ffde9885d4db095344"
...
class Rigid221Card18LabeledProjectionCustodyError(ValueError):
    """The frozen source set, live source tree, or receipt is malformed."""


def _module_source_path(module: str) -> str:
    if type(module) is not str:
        raise Rigid221Card18LabeledProjectionCustodyError("Lean dependency module is not a string")
...
def _open_repo_root(repo_root: Path) -> int:
    ...
    try:
        ...
        if not stat.S_ISDIR(info.st_mode):
            raise Rigid221Card18LabeledProjectionCustodyError("repository root is not a directory")
        result = descriptor
        descriptor = None
        return result
...
def _validate_source_manifest(manifest: Any) -> None:
    if not isinstance(manifest, list):
        raise Rigid221Card18LabeledProjectionCustodyError("Lean source manifest is not a list")
    modules: list[str] = []
    paths: list[str] = []
...
def validate_labeled_projection_source_custody_receipt(
    ...
    if snapshot["schema"] != SCHEMA:
        raise Rigid221Card18LabeledProjectionCustodyError("source-custody receipt schema is wrong")
...
def encode_labeled_projection_source_custody_receipt(receipt: Mapping[str, Any]) -> bytes:
    """Return the only accepted canonical byte representation of a receipt."""

    return _canonical_json_bytes(validate_labeled_projection_source_custody_receipt(receipt))
