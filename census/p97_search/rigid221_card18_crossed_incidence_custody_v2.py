...
"""Strict Lean-source custody for the Rigid221 card-18 crossed-incidence projection.
...
Python serialization with Lean data, authenticate a serializer or axiom receipt, invoke a solver or canary, or make a proof claim.
"""
...
LANE_ID_METADATA: Final = "rigid221-card18-crossed-incidence-custody-v2-20260828"
...
RECEIPT_SERIALIZER_AUTHENTICATED: Final = False
RECEIPT_SOLVER_AUTHENTICATED: Final = False
RECEIPT_CANARY_AUTHENTICATED: Final = False
...
_FROZEN_LEAN_SOURCE_MANIFEST_ROWS = (
    ...
    {
        "module": "Erdos9796Proof.P97.ATail.FrontierLiveClosure.Rigid221Card18CrossedIncidenceProjection",
        "path": "lean/Erdos9796Proof/P97/ATail/FrontierLiveClosure/Rigid221Card18CrossedIncidenceProjection.lean",
        "bytes": 19558,
        "sha256": "27e356b50f38e974dc44884937d00c03e56c36c2180e3a01c619115576377439",
    },
    {
    ...
    },
    {
        ...
        "bytes": 882910,
        "sha256": "f2a5b2bf20dd6063177a2622f86751ce8a3a8549d8c9c386d660193bf4a54530",
    },
    {
...
RECEIPT_AUTHENTICATION_SCOPE: Final = MappingProxyType(
    {
        ...
        "serializer_authenticated": RECEIPT_SERIALIZER_AUTHENTICATED,
        "solver_authenticated": RECEIPT_SOLVER_AUTHENTICATED,
        "canary_authenticated": RECEIPT_CANARY_AUTHENTICATED,
    }
)
_SOURCE_RECORD_FIELDS: Final = frozenset({"module", "path", "bytes", "sha256"})


class Rigid221Card18CrossedIncidenceCustodyV2Error(ValueError):
    """The frozen source set, live source tree, or receipt is malformed."""


Rigid221Card18CrossedIncidenceCustodyError = (
    Rigid221Card18CrossedIncidenceCustodyV2Error
)


def _module_source_path(module: str) -> str:
    if type(module) is not str:
        raise Rigid221Card18CrossedIncidenceCustodyError(
...
def frozen_crossed_incidence_lean_source_manifest() -> tuple[dict[str, Any], ...]:
    ...
    if (
        ...
        or any(
            ...
            for value in (
                ...
                RECEIPT_PYTHON_SERIALIZATION_EQUIVALENCE_AUTHENTICATED,
                RECEIPT_SERIALIZER_AUTHENTICATED,
                RECEIPT_SOLVER_AUTHENTICATED,
                RECEIPT_CANARY_AUTHENTICATED,
            )
        )
        or any(
...
def _open_repo_root(repo_root: Path) -> int:
    ...
    try:
        ...
        descriptor = None
        return result
    except Rigid221Card18CrossedIncidenceCustodyError:
        raise
...
def validate_crossed_incidence_source_custody_receipt(
...
def build_crossed_incidence_source_custody_receipt(repo_root: Path) -> dict[str, Any]:
    ...
    return validate_crossed_incidence_source_custody_receipt(
        {**body, "receipt_sha256": _sha256_json(body)}
    )


def encode_crossed_incidence_source_custody_receipt(
    receipt: Mapping[str, Any],
) -> bytes:
    """Return the only accepted canonical byte representation of a receipt."""

    return _canonical_json_bytes(
        validate_crossed_incidence_source_custody_receipt(receipt)
    )
...
def parse_crossed_incidence_source_custody_receipt(payload: bytes) -> dict[str, Any]:
    ...
    return validate_crossed_incidence_source_custody_receipt(parsed)


def attest_crossed_incidence_source_custody_receipt(
    repo_root: Path,
    payload: bytes,
) -> dict[str, Any]:
    ...
    fresh = build_crossed_incidence_source_custody_receipt(repo_root)
    if payload != encode_crossed_incidence_source_custody_receipt(fresh):
        raise Rigid221Card18CrossedIncidenceCustodyError(
...
frozen_crossed_incidence_lean_source_manifest_v2 = (
    frozen_crossed_incidence_lean_source_manifest
)
validate_crossed_incidence_source_custody_receipt_v2 = (
    validate_crossed_incidence_source_custody_receipt
)
build_crossed_incidence_source_custody_receipt_v2 = (
    build_crossed_incidence_source_custody_receipt
)
encode_crossed_incidence_source_custody_receipt_v2 = (
    encode_crossed_incidence_source_custody_receipt
)
parse_crossed_incidence_source_custody_receipt_v2 = (
    parse_crossed_incidence_source_custody_receipt
)
attest_crossed_incidence_source_custody_receipt_v2 = (
