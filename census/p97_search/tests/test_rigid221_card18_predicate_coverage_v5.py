...
from census.p97_search import rigid221_card18_predicate_coverage_v5 as coverage

REPO_ROOT = Path(__file__).parents[3]
ARTIFACT_PATH = REPO_ROOT / "certificates/rigid221_card18_predicate_coverage_v5.json"
...
def _all_live_paths() -> tuple[str, ...]:
    paths = {
        coverage.V4_MODULE_PATH,
        coverage.V4_MANIFEST_PATH,
        ...
        coverage.COVERAGE_V5_MODULE_PATH,
    }
...
def live_manifest() -> dict[str, Any]:
    return coverage.build_predicate_coverage_v5_manifest(REPO_ROOT)


def test_schema_configuration_and_immutable_input_bindings() -> None:
    assert coverage.SCHEMA == "p97-rigid221-card18-predicate-coverage/v5"
    assert (
        coverage.LANE_BASE_HEAD_METADATA == "7cf1e60bde589fe27c246c0edbc7ad20e96d6db3"
    )
    assert _json_hash(coverage._configuration_snapshot()) == (
        coverage.FROZEN_CONFIGURATION_SHA256
    )
    for path, size, digest in (
        (coverage.V4_MODULE_PATH, coverage.V4_MODULE_BYTES, coverage.V4_MODULE_SHA256),
        (
            coverage.V4_MANIFEST_PATH,
            coverage.V4_MANIFEST_BYTES,
            coverage.V4_MANIFEST_FILE_SHA256,
        ),
        (
            coverage.LABELED_CUSTODY_MODULE_PATH,
    ...
    predecessor = coverage_v4.parse_predicate_coverage_v4_manifest(
        (REPO_ROOT / coverage.V4_MANIFEST_PATH).read_bytes()
    )
    assert predecessor["manifest_sha256"] == coverage.V4_MANIFEST_SHA256


def test_exact_37_row_custody_only_delta(live_manifest: dict[str, Any]) -> None:
    old = list(coverage_v4.frozen_v4_lean_source_universe())
...
def test_candidates_admission_counts_and_hashes(live_manifest: dict[str, Any]) -> None:
    ...
    assert candidates == list(coverage_v4.frozen_v4_candidate_predicate_records())
...
def test_kernel_manifest_counts_and_excludes_exact_a2_cap_metric() -> None:
    labeled = coverage._strict_copy(
        list(
            coverage.labeled_custody_v3.frozen_labeled_projection_lean_source_manifest()
        )
    )
    crossed = coverage._strict_copy(
        list(
            coverage.crossed_custody_v3.frozen_crossed_incidence_lean_source_manifest()
        )
    )
...
def test_canonical_artifact_roundtrip_and_live_attestation(
    ...
    payload = coverage.encode_predicate_coverage_v5_manifest(live_manifest)
    assert payload == _canonical(live_manifest)
    assert ARTIFACT_PATH.read_bytes() == payload
    assert coverage.parse_predicate_coverage_v5_manifest(payload) == live_manifest
    assert coverage.attest_predicate_coverage_v5_manifest(REPO_ROOT, payload) == (
        live_manifest
    )
    assert coverage.build_predicate_coverage_v5_manifest(REPO_ROOT) == live_manifest
...
def test_strict_json_tampering_and_admission_escalation_reject(
    ...
    for malformed in (payload + b"\n", b"NaN", duplicate):
        with pytest.raises(coverage.Rigid221Card18PredicateCoverageV5Error):
            coverage.parse_predicate_coverage_v5_manifest(malformed)
    changed = copy.deepcopy(live_manifest)
    changed["lean_source_universe"][0]["custody_digest"] = "0" * 64
    with pytest.raises(coverage.Rigid221Card18PredicateCoverageV5Error):
        coverage.validate_predicate_coverage_v5_manifest(_rehash(changed))
    changed = copy.deepcopy(live_manifest)
...
def test_v1_through_v5_schema_cross_rejection(live_manifest: dict[str, Any]) -> None:
    payload = coverage.encode_predicate_coverage_v5_manifest(live_manifest)
    for parser, error in (
        (
    ...
    for old_path in (
        ...
        with pytest.raises(coverage.Rigid221Card18PredicateCoverageV5Error):
            coverage.parse_predicate_coverage_v5_manifest(
...
def test_tampered_predecessor_module_rejects(tmp_path: Path) -> None:
    ...
    (root / coverage.V4_MODULE_PATH).write_bytes(b"drift")
    with pytest.raises(coverage.Rigid221Card18PredicateCoverageV5Error):
        coverage.build_predicate_coverage_v5_manifest(root)
...
def test_public_rows_are_independent_copies() -> None:
    candidates = coverage.frozen_v5_candidate_predicate_records()
    candidates[0]["id"] = "mutated"
    assert coverage.frozen_v5_candidate_predicate_records()[0]["id"] != "mutated"
    sources = coverage.frozen_v5_lean_source_universe()
    sources[0]["lean_anchors"].clear()
    assert coverage.frozen_v5_lean_source_universe()[0]["lean_anchors"]
