...
from census.p97_search.rigid221_card18_labeled_projection_custody import (
...
def test_labeled_projection_and_legacy_receipts_reject_cross_attestation() -> None:
    ...
    legacy_payload = legacy_custody.encode_source_custody_receipt(
        legacy_custody.build_source_custody_receipt(REPO_ROOT)
    )
...
def test_receipt_roundtrip_is_deterministic_and_live_attested() -> None:
    ...
    assert (
        encode_labeled_projection_source_custody_receipt(parse_labeled_projection_source_custody_receipt(first_payload))
        == first_payload
    )
    assert attest_labeled_projection_source_custody_receipt(REPO_ROOT, first_payload) == first
...
def test_receipt_rejects_integer_substitutes_for_booleans(
    ...
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError, match="scope is wrong"):
        parse_labeled_projection_source_custody_receipt(_rehash(receipt))
...
def test_receipt_rejects_duplicate_json_keys_and_noncanonical_bytes() -> None:
    payload = encode_labeled_projection_source_custody_receipt(build_labeled_projection_source_custody_receipt(REPO_ROOT))
    duplicated = payload.replace(
        b'{"authentication":',
        b'{"authentication":{},"authentication":',
        1,
    )
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError, match="duplicate JSON"):
        parse_labeled_projection_source_custody_receipt(duplicated)

    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError, match="not canonical"):
...
def test_live_custody_rejects_byte_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError, match="bytes drifted"):
...
def test_live_custody_rejects_outside_hardlink_alias(
    ...
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError, match="singly linked"):
        if operation == "build":
            build_labeled_projection_source_custody_receipt(root)
        else:
