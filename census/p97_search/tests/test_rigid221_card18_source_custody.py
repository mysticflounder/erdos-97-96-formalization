...
from census.p97_search import rigid221_card18_source_custody as custody
from census.p97_search.rigid221_card18_source_custody import (
    ...
    Rigid221Card18SourceCustodyError,
    attest_source_custody_receipt,
    build_source_custody_receipt,
    encode_source_custody_receipt,
    parse_source_custody_receipt,
)
...
def test_live_repo_source_custody_succeeds_with_narrow_claims() -> None:
    receipt = build_source_custody_receipt(REPO_ROOT)
    ...
    assert len(receipt["lean_source_manifest"]) == 30
...
def test_frozen_manifest_has_independent_30_row_module_path_and_byte_mapping() -> None:
...
def test_receipt_roundtrip_is_deterministic_and_live_attested() -> None:
    first = build_source_custody_receipt(REPO_ROOT)
    second = build_source_custody_receipt(REPO_ROOT)
    first_payload = encode_source_custody_receipt(first)

    assert first == second
    assert first_payload == encode_source_custody_receipt(second)
    assert parse_source_custody_receipt(first_payload) == first
    assert (
        encode_source_custody_receipt(parse_source_custody_receipt(first_payload))
        == first_payload
    )
    assert attest_source_custody_receipt(REPO_ROOT, first_payload) == first
...
def test_receipt_rejects_wrong_extra_reordered_and_duplicate_manifest_entries(
    ...
    with pytest.raises(Rigid221Card18SourceCustodyError):
        parse_source_custody_receipt(_rehash(receipt))
...
def test_receipt_rejects_extra_top_level_field_and_wrong_self_hash() -> None:
    ...
    with pytest.raises(Rigid221Card18SourceCustodyError):
        parse_source_custody_receipt(_canonical_json_bytes(receipt))
...
def test_receipt_rejects_integer_substitutes_for_booleans(
    ...
    with pytest.raises(Rigid221Card18SourceCustodyError, match="scope is wrong"):
        parse_source_custody_receipt(_rehash(receipt))
...
def test_every_public_custody_path_rejects_rebound_configuration(
    ...
    receipt = build_source_custody_receipt(REPO_ROOT)
    payload = encode_source_custody_receipt(receipt)
    ...
    with pytest.raises(
        ...
        if public_api == "validate":
            custody.validate_source_custody_receipt(receipt)
        elif public_api == "encode":
            custody.encode_source_custody_receipt(receipt)
        elif public_api == "parse":
            custody.parse_source_custody_receipt(payload)
        elif public_api == "build":
            custody.build_source_custody_receipt(REPO_ROOT)
        elif public_api == "attest":
            custody.attest_source_custody_receipt(REPO_ROOT, payload)
        else:
            custody.frozen_lean_source_manifest()
...
def test_receipt_rejects_duplicate_json_keys_and_noncanonical_bytes() -> None:
    payload = encode_source_custody_receipt(build_source_custody_receipt(REPO_ROOT))
    duplicated = payload.replace(
        b'{"authentication":',
        b'{"authentication":{},"authentication":',
        1,
    )
    with pytest.raises(Rigid221Card18SourceCustodyError, match="duplicate JSON"):
        parse_source_custody_receipt(duplicated)

    with pytest.raises(Rigid221Card18SourceCustodyError, match="not canonical"):
        parse_source_custody_receipt(payload + b"\n")
...
def test_live_custody_rejects_byte_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(Rigid221Card18SourceCustodyError, match="bytes drifted"):
        build_source_custody_receipt(root)
...
def test_live_custody_rejects_missing_and_nonregular_sources(
    ...
    missing.unlink()
    with pytest.raises(Rigid221Card18SourceCustodyError, match="missing"):
        build_source_custody_receipt(missing_root)

    directory_root = _copy_frozen_tree(tmp_path / "directory")
    nonregular = directory_root / FROZEN_LEAN_SOURCE_MANIFEST[0]["path"]
    nonregular.unlink()
    nonregular.mkdir()
    with pytest.raises(Rigid221Card18SourceCustodyError, match="regular"):
        build_source_custody_receipt(directory_root)
...
def test_live_custody_rejects_outside_hardlink_alias(
    ...
    with pytest.raises(Rigid221Card18SourceCustodyError, match="singly linked"):
        if operation == "build":
            build_source_custody_receipt(root)
        else:
            payload = encode_source_custody_receipt(
                build_source_custody_receipt(REPO_ROOT)
            )
            attest_source_custody_receipt(root, payload)
...
def test_live_custody_rejects_symlinked_source_ancestor(tmp_path: Path) -> None:
    ...
    with pytest.raises(Rigid221Card18SourceCustodyError, match="symlinked"):
        build_source_custody_receipt(root)
...
def test_live_custody_rejects_symlinked_leaf_and_root(tmp_path: Path) -> None:
    ...
    with pytest.raises(Rigid221Card18SourceCustodyError, match="symlink"):
        build_source_custody_receipt(linked_root)
...
def test_live_custody_wraps_symlink_loop_resolution(tmp_path: Path) -> None:
    ...
    with pytest.raises(Rigid221Card18SourceCustodyError):
        build_source_custody_receipt(first)
