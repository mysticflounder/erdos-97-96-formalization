...
from census.p97_search import rigid221_card18_crossed_incidence_custody as v1_custody
from census.p97_search import rigid221_card18_crossed_incidence_custody_v2 as custody
...
from census.p97_search.rigid221_card18_crossed_incidence_custody_v2 import (
...
from census.p97_search.rigid221_card18_crossed_incidence_custody_v2 import (
    Rigid221Card18CrossedIncidenceCustodyV2Error as Rigid221Card18CrossedIncidenceCustodyError,
)
...
def test_v1_and_v2_receipts_reject_cross_attestation() -> None:
    ...
    v2_payload = encode_crossed_incidence_source_custody_receipt(
        build_crossed_incidence_source_custody_receipt(REPO_ROOT)
    )
    with pytest.raises(
        v1_custody.Rigid221Card18CrossedIncidenceCustodyError, match="schema is wrong"
    ):
        v1_custody.attest_crossed_incidence_source_custody_receipt(
            REPO_ROOT, v2_payload
        )
    with pytest.raises(
        Rigid221Card18CrossedIncidenceCustodyError, match="schema is wrong"
    ):
        attest_crossed_incidence_source_custody_receipt(REPO_ROOT, v1_payload)
