...
import census.p97_search.freshthird_qfiber_three_carrier_source_complete_static_v4 as runner
import scripts.check_worktree_hygiene as hygiene


def test_v4_schema_and_root_policy_are_fresh() -> None:
    assert runner.SCHEMA == (
        "p97-freshthird-qfiber-three-carrier-source-complete-static/v4"
    )
    assert runner.LANE_ID == "freshthird-source-complete-static-v4"
    assert runner.SOURCE_COMPLETE_PATH.endswith(
        "freshthird_qfiber_three_carrier_source_complete_v4.py"
    )
    assert runner.RUNNER_PATH.endswith(
        "freshthird_qfiber_three_carrier_source_complete_static_v4.py"
    )
    assert runner.SOLVER_TIMEOUT_SECONDS == 30
    assert runner.PROCESS_TIMEOUT_GRACE_SECONDS == 5
...
class _FakeEncoding:
    ...
    def validate_source_complete_assignment(self, assignment) -> None:
...
class _MissingCompleteValidationEncoding(_FakeEncoding):
    def validate_source_complete_assignment(self, assignment) -> None:
        raise AttributeError("source-complete validation API missing")
...
def custody(
    ...
    snapshot: dict[str, object] = {
        ...
        "inputs": {
            ...
            "cnf_adapter_path": runner.SOURCE_COMPLETE_PATH,
...
def test_plan_authenticates_all_four_static_artifact_triples_without_solver_calls(
    ...
    assert plan["source_theorem_source"] == {
        "module_path": runner.SOURCE_THEOREM_PATH,
...
def test_launch_is_gated_without_explicit_phase_ingress_receipt(custody) -> None:
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match="explicit built"):
...
def test_launch_refuses_a_preexisting_empty_root(custody) -> None:
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match="pre-existing"):
        runner.run_wave(
            output,
...
def test_terminal_reentry_rejects_rehashed_wrong_standard_manifest_without_calls(
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match="mismatch: owner"):
        runner.run_wave(
            output,
            source_snapshot=snapshot,
            phase_ingress_receipt=receipt,
...
def test_sat_replay_rejection_is_conservative_terminal_status(
    ...
    repo, snapshot, receipt = custody
    monkeypatch.setattr(
        runner, "FreshThirdCarrierSourceCompleteCnfEncoding", _RejectingEncoding
    )
...
def test_malformed_sat_model_fails_closed_after_one_solver_call(custody) -> None:
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match="incomplete"):
        runner.run_wave(
            output,
            source_snapshot=snapshot,
            phase_ingress_receipt=receipt,
...
def test_missing_source_complete_validation_fails_closed(
    custody, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo, snapshot, receipt = custody
    output = _out(repo, "missing-complete-validation")
    monkeypatch.setattr(
        runner, "FreshThirdCarrierSourceCompleteCnfEncoding", _MissingCompleteValidationEncoding
    )
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match="SAT validation"):
        runner.run_wave(
            output,
            source_snapshot=snapshot,
            phase_ingress_receipt=receipt,
...
def test_malformed_solver_return_code_fails_closed(
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match="return code"):
        runner.run_wave(
...
def test_solver_callback_cannot_mutate_custodied_input(custody, artifact: str) -> None:
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match=f"{artifact} drifted"):
        runner.run_wave(
...
def test_terminal_reentry_regenerates_and_rejects_tampered_artifacts(
    ...
    with pytest.raises(
        runner.SourceCompleteStaticError, match="inventory|drifted|canonical"
    ):
        runner.run_wave(
            output,
            source_snapshot=snapshot,
...
def test_source_drift_between_solver_calls_and_postflight_fails_closed(
    ...
    def verify(repo_root, expected):
        if drifted:
            raise runner.SourceCompleteStaticError("deliberate source drift")
        return snapshot
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match="postflight.*drift"):
        runner.run_wave(
...
def test_receipt_requires_passed_build_and_axiom_audit(custody) -> None:
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match="axiom audit"):
        runner.run_wave(
...
def test_receipt_rejects_unbound_commands_and_unapproved_axioms(
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match="build receipt|axiom audit"):
        runner.run_wave(
...
def test_terminal_reentry_rejects_symlinked_directory_artifacts(custody) -> None:
    ...
    with pytest.raises(runner.SourceCompleteStaticError, match="unsupported output"):
        runner.run_wave(
            output,
            source_snapshot=snapshot,
            phase_ingress_receipt=receipt,
