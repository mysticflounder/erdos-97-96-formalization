...
from census.p97_search import (
    rigid221_card18_labeled_projection_custody_v3 as v3_custody,
)
from census.p97_search import rigid221_card18_labeled_projection_custody_v4 as custody
from census.p97_search import rigid221_card18_source_custody as legacy_custody
from census.p97_search.rigid221_card18_labeled_projection_custody_v4 import (
...
from census.p97_search.rigid221_card18_labeled_projection_custody_v4 import (
    Rigid221Card18LabeledProjectionCustodyV4Error as Rigid221Card18LabeledProjectionCustodyError,
)
...
def test_v3_and_v4_receipts_reject_cross_attestation() -> None:
    ...
    v4_payload = encode_labeled_projection_source_custody_receipt(
        build_labeled_projection_source_custody_receipt(REPO_ROOT)
    )
    with pytest.raises(
        v3_custody.Rigid221Card18LabeledProjectionCustodyV3Error, match="schema is wrong"
    ):
        v3_custody.attest_labeled_projection_source_custody_receipt(REPO_ROOT, v4_payload)
    with pytest.raises(
        Rigid221Card18LabeledProjectionCustodyError, match="schema is wrong"
    ):
        attest_labeled_projection_source_custody_receipt(REPO_ROOT, v3_payload)
...
def test_published_v4_receipt_canonical_roundtrip_and_live_attestation() -> None:
    ...
    assert encode_labeled_projection_source_custody_receipt(parsed) == payload
    assert attest_labeled_projection_source_custody_receipt(REPO_ROOT, payload) == parsed
