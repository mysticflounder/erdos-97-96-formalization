...
from census.p97_search import (
    rigid221_card18_labeled_projection_custody_v2 as v2_custody,
)
from census.p97_search import rigid221_card18_labeled_projection_custody_v3 as custody
from census.p97_search import rigid221_card18_source_custody as legacy_custody
from census.p97_search.rigid221_card18_labeled_projection_custody_v3 import (
...
from census.p97_search.rigid221_card18_labeled_projection_custody_v3 import (
    Rigid221Card18LabeledProjectionCustodyV3Error as Rigid221Card18LabeledProjectionCustodyError,
)
...
def test_v3_manifest_matches_v2_except_for_source_heavy_row() -> None:
    old = {row["module"]: dict(row) for row in v2_custody.FROZEN_LEAN_SOURCE_MANIFEST}
...
def test_v2_and_v3_receipts_reject_cross_attestation() -> None:
    v2_payload = (
        REPO_ROOT
        / "certificates/rigid221_card18_labeled_projection_source_custody_v2.json"
    ).read_bytes()
    v3_payload = encode_labeled_projection_source_custody_receipt(
        build_labeled_projection_source_custody_receipt(REPO_ROOT)
    )
    with pytest.raises(
        v2_custody.Rigid221Card18LabeledProjectionCustodyV2Error, match="schema is wrong"
    ):
        v2_custody.attest_labeled_projection_source_custody_receipt(REPO_ROOT, v3_payload)
    with pytest.raises(
        Rigid221Card18LabeledProjectionCustodyError, match="schema is wrong"
    ):
        attest_labeled_projection_source_custody_receipt(REPO_ROOT, v2_payload)
