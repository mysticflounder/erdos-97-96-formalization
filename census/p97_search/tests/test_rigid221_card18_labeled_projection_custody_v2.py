...
from census.p97_search import rigid221_card18_labeled_projection_custody as v1_custody
from census.p97_search import rigid221_card18_labeled_projection_custody_v2 as custody
from census.p97_search import rigid221_card18_source_custody as legacy_custody
from census.p97_search.rigid221_card18_labeled_projection_custody_v2 import (
...
from census.p97_search.rigid221_card18_labeled_projection_custody_v2 import (
    Rigid221Card18LabeledProjectionCustodyV2Error as Rigid221Card18LabeledProjectionCustodyError,
)
...
def test_live_repo_source_custody_succeeds_with_narrow_claims() -> None:
    receipt = build_labeled_projection_source_custody_receipt(REPO_ROOT)
    ...
    assert len(receipt["lean_source_manifest"]) == 26
...
def test_v2_manifest_matches_v1_except_for_source_heavy_row() -> None:
    old = {row["module"]: dict(row) for row in v1_custody.FROZEN_LEAN_SOURCE_MANIFEST}
...
def test_v1_and_v2_receipts_reject_cross_attestation() -> None:
    v1_payload = (
        REPO_ROOT
        / "certificates/rigid221_card18_labeled_projection_source_custody_v1.json"
    ).read_bytes()
    v2_payload = encode_labeled_projection_source_custody_receipt(
        build_labeled_projection_source_custody_receipt(REPO_ROOT)
    )
    with pytest.raises(
        v1_custody.Rigid221Card18LabeledProjectionCustodyError, match="schema is wrong"
    ):
        v1_custody.attest_labeled_projection_source_custody_receipt(
            REPO_ROOT, v2_payload
        )
    with pytest.raises(
        Rigid221Card18LabeledProjectionCustodyError, match="schema is wrong"
    ):
        attest_labeled_projection_source_custody_receipt(REPO_ROOT, v1_payload)


def test_labeled_projection_and_legacy_custody_contracts_are_separate() -> None:
    expected_root = (
        "Erdos9796Proof.P97.ATail.FrontierLiveClosure.Rigid221Card18LabeledProjection"
    )
    assert custody.SCHEMA != legacy_custody.SCHEMA
    assert custody.Rigid221Card18LabeledProjectionCustodyError is not (
        legacy_custody.Rigid221Card18SourceCustodyError
    )
    assert LEAN_ROOT_MODULES == (expected_root,)
    ...
    assert not hasattr(custody, "parse_source_custody_receipt")


def test_labeled_projection_and_legacy_receipts_reject_cross_attestation() -> None:
    new_payload = encode_labeled_projection_source_custody_receipt(
        build_labeled_projection_source_custody_receipt(REPO_ROOT)
    )
    legacy_payload = (
        REPO_ROOT
    ...
    with pytest.raises(
        ...
        legacy_custody.attest_source_custody_receipt(REPO_ROOT, new_payload)
    with pytest.raises(
        Rigid221Card18LabeledProjectionCustodyError,
        match="schema is wrong",
    ):
        attest_labeled_projection_source_custody_receipt(REPO_ROOT, legacy_payload)


def test_frozen_manifest_has_independent_26_row_module_path_and_byte_mapping() -> None:
    ...
    assert modules == list(LEAN_DEPENDENCY_MODULES)
...
def test_receipt_roundtrip_is_deterministic_and_live_attested() -> None:
    first = build_labeled_projection_source_custody_receipt(REPO_ROOT)
    second = build_labeled_projection_source_custody_receipt(REPO_ROOT)
    first_payload = encode_labeled_projection_source_custody_receipt(first)

    assert first == second
    assert first_payload == encode_labeled_projection_source_custody_receipt(second)
    assert parse_labeled_projection_source_custody_receipt(first_payload) == first
    assert (
        encode_labeled_projection_source_custody_receipt(
            parse_labeled_projection_source_custody_receipt(first_payload)
        )
        == first_payload
    )
    assert (
        attest_labeled_projection_source_custody_receipt(REPO_ROOT, first_payload)
        == first
    )
...
def test_receipt_rejects_wrong_extra_reordered_and_duplicate_manifest_entries(
    ...
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError):
        parse_labeled_projection_source_custody_receipt(_rehash(receipt))
...
def test_receipt_rejects_extra_top_level_field_and_wrong_self_hash() -> None:
    ...
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError):
        parse_labeled_projection_source_custody_receipt(_canonical_json_bytes(receipt))
...
def test_receipt_rejects_integer_substitutes_for_booleans(
    ...
    with pytest.raises(
        Rigid221Card18LabeledProjectionCustodyError, match="scope is wrong"
    ):
        parse_labeled_projection_source_custody_receipt(_rehash(receipt))
...
def test_receipt_rejects_nul_and_escape_source_paths() -> None:
    for malformed in ("lean/\x00escape.lean", "../escape.lean"):
        receipt = build_labeled_projection_source_custody_receipt(REPO_ROOT)
        receipt["lean_source_manifest"][0]["path"] = malformed
        with pytest.raises(
            Rigid221Card18LabeledProjectionCustodyError,
            match="escapes repository custody",
        ):
            parse_labeled_projection_source_custody_receipt(_rehash(receipt))
...
def test_every_public_custody_path_rejects_rebound_configuration(
    ...
    receipt = build_labeled_projection_source_custody_receipt(REPO_ROOT)
    payload = encode_labeled_projection_source_custody_receipt(receipt)
    ...
    with pytest.raises(
        ...
        if public_api == "validate":
            custody.validate_labeled_projection_source_custody_receipt(receipt)
        elif public_api == "encode":
            custody.encode_labeled_projection_source_custody_receipt(receipt)
        elif public_api == "parse":
            custody.parse_labeled_projection_source_custody_receipt(payload)
        elif public_api == "build":
            custody.build_labeled_projection_source_custody_receipt(REPO_ROOT)
        elif public_api == "attest":
            custody.attest_labeled_projection_source_custody_receipt(REPO_ROOT, payload)
        else:
            custody.frozen_labeled_projection_lean_source_manifest()
...
def test_receipt_rejects_duplicate_json_keys_and_noncanonical_bytes() -> None:
    payload = encode_labeled_projection_source_custody_receipt(
        build_labeled_projection_source_custody_receipt(REPO_ROOT)
    )
    ...
    with pytest.raises(
        Rigid221Card18LabeledProjectionCustodyError, match="duplicate JSON"
    ):
        parse_labeled_projection_source_custody_receipt(duplicated)

    with pytest.raises(
        Rigid221Card18LabeledProjectionCustodyError, match="not canonical"
    ):
        parse_labeled_projection_source_custody_receipt(payload + b"\n")
...
def test_receipt_rejects_nonfinite_json_constants(constant: bytes) -> None:
    ...
    with pytest.raises(
        ...
        parse_labeled_projection_source_custody_receipt(payload)
...
def test_live_custody_rejects_byte_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        Rigid221Card18LabeledProjectionCustodyError, match="bytes drifted"
    ):
        build_labeled_projection_source_custody_receipt(root)
...
def test_live_custody_rejects_missing_and_nonregular_sources(
    ...
    missing.unlink()
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError, match="missing"):
        build_labeled_projection_source_custody_receipt(missing_root)

    directory_root = _copy_frozen_tree(tmp_path / "directory")
    nonregular = directory_root / FROZEN_LEAN_SOURCE_MANIFEST[0]["path"]
    nonregular.unlink()
    nonregular.mkdir()
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError, match="regular"):
        build_labeled_projection_source_custody_receipt(directory_root)
...
def test_live_custody_rejects_outside_hardlink_alias(
    ...
    with pytest.raises(
        Rigid221Card18LabeledProjectionCustodyError, match="singly linked"
    ):
        if operation == "build":
            build_labeled_projection_source_custody_receipt(root)
        else:
            ...
            attest_labeled_projection_source_custody_receipt(root, payload)
...
def test_live_custody_rejects_symlinked_source_ancestor(tmp_path: Path) -> None:
    ...
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError, match="symlinked"):
        build_labeled_projection_source_custody_receipt(root)
...
def test_live_custody_rejects_symlinked_leaf_and_root(tmp_path: Path) -> None:
    ...
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError, match="symlink"):
        build_labeled_projection_source_custody_receipt(linked_root)
...
def test_live_custody_wraps_symlink_loop_resolution(tmp_path: Path) -> None:
    ...
    with pytest.raises(Rigid221Card18LabeledProjectionCustodyError):
        build_labeled_projection_source_custody_receipt(first)
