"""Authenticated one-shot runner for the FirstNonHit cap-endpoint v3 CNF.
...
import census.p97_search.freshthird_firstnonhit_complete_finite_v2 as predecessor_emitter
...
LANE_ID = "firstnonhit-cap-endpoint-v3"
CHECKPOINT_RELATIVE = ".codex/worktree-checkpoints/firstnonhit-cap-endpoint-v3.json"
...
def _validate_terminal_evidence(
    root: Path, receipt: Mapping[str, object], bundle: emitter.Bundle
) -> None:
    if (
    ...
    if status not in {"SAT", "UNSAT", "FAILED", "UNKNOWN", "ERROR"} or type(result) is not dict:
    ...
    for summary in summaries:
        if set(summary) != {"name", "receipt", "receipt_sha256", "status", "returncode"}:
    ...
    if not required.issubset(process_receipts) or result.get("verdict") != status:
    ...
    if production.get("status") != "COMPLETED" or production.get("returncode") != expected_returncode:
    ...
    if trim.get("status") != "COMPLETED" or trim.get("returncode") != 0 or b"s VERIFIED" not in trim_stdout:
...
def terminal_reentry(root: Path) -> dict[str, object]:
    ...
    canonical_source_paths = {
        "encoder": Path(emitter.__file__).resolve().relative_to(_repo_root()).as_posix(),
        "predecessor_encoder": Path(predecessor_emitter.__file__)
        .resolve()
    ...
    _validate_terminal_evidence(root, receipt, bundle)
    return {
        "status": receipt.get("status"),
...
def _initial_manifest(
    ...
    source_files = {
        ...
        "predecessor_encoder": Path(predecessor_emitter.__file__).resolve(),
