"""Independent, fail-closed validator for the exact-12 v24 arm-static canary.

This is deliberately not a wrapper around the frozen v23 validator.  It checks
the v12 descriptor/receipt/custody envelope, live source bytes, and either a
...
PROJECT = "p97-exact12-next-row-arm-static-cell6-v12-r1"
...
EXPECTED_PREFIX_VARS = 47136
EXPECTED_PREFIX_CLAUSES = 676147
EXPECTED_PREFIX_SHA256 = "f6109195cea858ae0e0a179a9ccd4118118bb3e786d1169e40a94e399435166b"
EXPECTED_POST_ARM_CLAUSES = 677014
EXPECTED_POST_ARM_SHA256 = "d5e2d4e0ae8ff750b0f4d1ce2d49721e095a9fb61ec7ca29ed1999f24f1c8947"
...
EXPECTED_FINAL_VARS = 47136
EXPECTED_FINAL_CLAUSES = 677095
EXPECTED_FINAL_SHA256 = "695f71b45a7157132e3eb61e0d4bf4c553c2d3655cbf421de8130a516611d8be"
...
def _fail(message: str) -> None:
    raise V24ValidationError(message)
...
def _json(raw: bytes, path: Path) -> Any:
    ...
    except (UnicodeDecodeError, json.JSONDecodeError, RecursionError, V24ValidationError) as exc:
...
def validate_v24_workdir(workdir: Path, repo_root: Path, *, piqd_root: Path | None = None, proof_verifier: Callable[[Path, Path], bool] = _verify_drat) -> dict[str, Any]:
    ...
    if job.get("schema") != JOB_SCHEMA or job.get("arm_cell_index") != EXPECTED_ARM_CELL:
        _fail("v12 job identity mismatch")
    ...
    if promotion.get("lean_terminal_ingress_ready") is not True:
        _fail("job promotion does not affirm the v12 Lean-ingress contract")
    if summary.get("lean_terminal_ingress_ready") is not True:
        _fail("summary does not affirm the v12 Lean-ingress contract")
    if summary.get("lean_ingress_theorem") != LEAN_INGRESS_THEOREM:
        _fail("summary Lean-ingress theorem is not the frozen v12 constant")
    if summary.get("terminal_promotion_status") != TERMINAL_STATUS:
        _fail("summary terminal promotion status is not the frozen v12 constant")
    ...
    if (identity.get("backend") != "cadical"
        ...
        _fail("receipt job identity is not bound to the v24 contract")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = validate_v24_workdir(args.workdir, args.repo_root, piqd_root=args.piqd_journal_root)
    except (OSError, V24ValidationError, subprocess.SubprocessError) as exc:
