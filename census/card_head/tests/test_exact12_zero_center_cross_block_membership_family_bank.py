...
def _parent():
    ...
    third_bank = install_second_cap_apex_surplus_membership_family_bank(
        REPO_ROOT, instance, layout, second_bank, cell_index=1
    )
    parent_bank = install_surplus_three_triad_membership_family_bank(
        REPO_ROOT, instance, layout, third_bank, cell_index=1
    )
...
def test_family_rebuild_matches_frozen_delta(bank_and_parent) -> None:
    ...
    rebuilt = build_zero_center_cross_block_membership_family_bank(
    ...
    assert family_roles() == tuple(
        (x, b, y) for x in (3, 4, 5) for b in (6, 7, 8, 9) for y in (10, 11)
    )
    ...
    assert compiled["initial_n_variables"] == 45_010
    assert compiled["initial_n_clauses"] == 637_414
...
def test_family_patterns_preserve_the_requested_role_and_row_order() -> None:
    assert family_patterns()[0] == (
        ...
        {"center": 0, "support": [3, 6, 10]},
    )
    assert family_patterns()[-1] == (
        {"center": 5, "support": [1, 9]},
        {"center": 11, "support": [1, 9]},
        {"center": 0, "support": [5, 9, 11]},
    )
...
def test_install_appends_the_complete_family_once() -> None:
    ...
    bank = install_zero_center_cross_block_membership_family_bank(
        REPO_ROOT, instance, layout, parent, cell_index=1
    )
...
def test_family_selection_is_cell_keyed_and_fail_closed() -> None:
    ...
    with pytest.raises(
        Exact12ZeroCenterCrossBlockMembershipFamilyBankError,
        match="no production zero-center cross-block family for cell 0",
    ):
