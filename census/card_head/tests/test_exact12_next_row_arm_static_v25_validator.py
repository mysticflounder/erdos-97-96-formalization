"""Adversarial unit tests for the independent v25 validator boundary."""
...
from census.card_head import exact12_next_row_arm_static_v25_validator as validator
from census.card_head.exact12_next_row_arm_static_v25_validator import (
    V25ValidationError,
    _assignment,
    _dimacs,
    _read,
...
def test_descriptor_claims_use_descriptor_contract_not_receipt_contract() -> None:
    ...
    with pytest.raises(V25ValidationError, match="missing or extra claim"):
...
def test_v13_summary_promotion_fields_are_frozen_constants() -> None:
    ...
    assert len(validator._BANK_KEYS) == 22
...
def test_receipt_job_id_is_the_piqd_server_identity_not_root_job_id() -> None:
    ...
    with pytest.raises(V25ValidationError, match="server job identity"):
...
def test_custody_receipt_identity_matches_producer_device_and_inode(tmp_path: Path) -> None:
    ...
    with pytest.raises(V25ValidationError, match="custody seal does not bind receipt"):
        _validate_custody_receipt_binding(custody, "receipt-self-hash", receipt_path, receipt_raw)
...
def test_tampered_assignment_is_not_complete_or_replayable() -> None:
    with pytest.raises(V25ValidationError, match="duplicate|cover"):
        _assignment([1, 1, -3], 3)
    _, _, clauses = _dimacs(b"p cnf 2 1\n1 2 0\n", Path("discovery.cnf"))
    with pytest.raises(V25ValidationError, match="satisfy"):
        _replay(clauses, set())
...
def test_tampered_proof_pointer_or_status_fails_closed() -> None:
    ...
    with pytest.raises(V25ValidationError, match="verdict/status"):
        _validate_receipt_outcome(receipt, "SAT")
    receipt["adapter_verdict"] = "SAT"
    receipt["proof_pointer"] = "elsewhere.drat"
    with pytest.raises(V25ValidationError, match="proof"):
        _validate_receipt_flags(receipt)
...
def test_tampered_live_source_manifest_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(V25ValidationError, match="live source"):
...
def test_direct_symlink_and_hardlink_controls_are_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(V25ValidationError, match="regular|symlink|Too many"):
        _read(symlink)
    hardlink = tmp_path / "hardlink.json"
    os.link(real, hardlink)
    with pytest.raises(V25ValidationError, match="hard-linked"):
        _read(hardlink)
