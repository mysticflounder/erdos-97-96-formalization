...
def _parent():
    return materialize_positive_membership_static_cell(REPO_ROOT, 1)


@pytest.fixture(scope="module")
def bank_and_parent():
    instance, _compiled, layout, three_triad_bank = _parent()
...
def test_install_appends_the_complete_family_once() -> None:
    instance, _compiled, layout, parent = _parent()
    before = len(instance.cnf.clauses)
    bank = install_surplus_apex_pair_membership_family_bank(
        REPO_ROOT, instance, layout, parent, cell_index=1
    )
...
def test_family_selection_is_cell_keyed_and_fail_closed() -> None:
    ...
    with pytest.raises(
        Exact12SurplusApexPairMembershipFamilyBankError,
        match="no production surplus/apex-pair family for cell 0",
    ):
