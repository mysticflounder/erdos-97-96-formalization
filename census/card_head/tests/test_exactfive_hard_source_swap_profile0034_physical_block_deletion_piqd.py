...
def test_embedded_exact_rational_witnesses_are_frozen(
    ...
    for branch in lane.BRANCH_IDS:
        ...
        assert witness["evidence"]["accepted"] is True
...
def test_base_query_is_physical_and_keeps_source_order(
    ...
    ledger = lane.deletion.assertion_group_ledger(prepared.system["source_system"])
    expected = tuple(
        command
        for group in ledger
        if group["group_id"] in lane.BASE_GROUP_IDS[branch]
...
def test_sat_replay_checks_exactly_the_retained_physical_groups(
    ...
    assert rejected.evidence["reason"] == "edge-index-01"
...
def test_anchor_is_the_exact_parent_journal(
    ...
    expected = tuple(
        lane.deletion.source._build_smt_commands_current(
...
def test_source_snapshots_bind_all_transitive_inputs(custody: dict[str, Any]) -> None:
    ...
    expected_sources = {
        ...
        Path(lane.group_core.__file__),
        Path(lane.adapter.__file__),
        Path(lane.deletion.source.__file__),
        Path(lane.deletion.source.predecessor.__file__),
        Path(lane.deletion.source.bo_source.__file__),
    }
...
def test_parent_custody_binds_full_source_unsat_journals(
    ...
    assert (
        custody["source_parent_terminal_sha256"] == lane.SOURCE_PARENT_TERMINAL_SHA256
    )
    assert (
    ...
    for branch in lane.BRANCH_IDS:
        commands = lane.deletion.source._build_smt_commands_current(
            custody["source_systems"][branch]
        )
        assert (
