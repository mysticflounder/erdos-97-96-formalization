...
def _parent():
    ...
    parent_bank = install_surplus_apex_pair_membership_family_bank(
        REPO_ROOT,
        instance,
        layout,
        three_triad_bank,
...
def test_family_rebuild_matches_frozen_delta(bank_and_parent) -> None:
    ...
    rebuilt = build_adjacent_apex_cross_block_membership_family_bank(
...
def test_install_appends_the_complete_family_once() -> None:
    ...
    bank = install_adjacent_apex_cross_block_membership_family_bank(
        REPO_ROOT, instance, layout, parent, cell_index=1
    )
...
def test_family_selection_is_cell_keyed_and_fail_closed() -> None:
    ...
    with pytest.raises(
        Exact12AdjacentApexCrossBlockMembershipFamilyBankError,
        match="no production adjacent-apex family for cell 0",
    ):
