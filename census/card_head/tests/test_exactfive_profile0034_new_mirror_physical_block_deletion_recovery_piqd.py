...
from census.card_head import (
    exactfive_profile0034_new_mirror_physical_block_deletion_recovery_piqd as lane,
)
...
@pytest.mark.parametrize(
    "algebraic",
    ["(root-obj (+ (^ x 2) (- 3)) 1)", "0.125?"],
)
def test_algebraic_readback_is_fail_closed_unresolved_evidence(
    algebraic: str,
    custody: dict[str, Any],
) -> None:
    prepared = lane.prepare_query(lane.anchor_spec("lt"), parent=custody)
    first = prepared.query.get_values[0]
    values = (
        "("
        + " ".join(
            f"({term} {algebraic if term == first else '0'})"
            for term in prepared.query.get_values
        )
        + ")"
    )
    replay = lane.verify_sat_model(prepared.query, lane.SOLVER, "model", values)
    assert replay.accepted is False
    assert replay.evidence["reason"] == "exact_readback_unsupported"
    assert replay.evidence["failure"] == "BoSourceQfnraError"
...
def test_verifier_failure_artifact_requires_exact_local_reproduction(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    custody: dict[str, Any],
) -> None:
    prepared = lane.prepare_query(lane.anchor_spec("lt"), parent=custody)
    output = tmp_path / prepared.key
    transport = GenericNamedPiqd({"z3": "SAT"})

    def fail_with_runtime_error(*_args: object, **_kwargs: object) -> None:
        raise RuntimeError("unexpected verifier failure")

    monkeypatch.setattr(lane, "verify_sat_model", fail_with_runtime_error)
    result = lane.run_query(
        prepared,
        output,
        transport,
    ...
    assert result["engine"]["effective_status"] == (
        "INCONCLUSIVE_SEMANTIC_VERIFIER_FAILURE"
    )
    assert result["engine"]["semantic_replay"] == {
        "accepted": False,
        "failure": "RuntimeError",
        "verifier": prepared.query.descriptor["semantic_verifier"],
    }
    assert lane.classify_result(result) == "SAT_REPLAY_UNRESOLVED"

    def fail_with_key_error(*_args: object, **_kwargs: object) -> None:
        raise KeyError("different verifier failure")

    monkeypatch.setattr(lane, "verify_sat_model", fail_with_key_error)
    with pytest.raises(
        lane.Profile0034PhysicalBlockDeletionError,
        match="verifier-failure",
    ):
        lane.verify_query_tree(prepared, output, lane.SemanticVerdictCache())
...
def test_custody_reads_reject_hard_links(tmp_path: Path) -> None:
...
def test_init_only_constructs_no_transport(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    manifest = {"manifest_sha256": "1" * 64}
    monkeypatch.setattr(lane, "ensure_run_root", lambda: (tmp_path, manifest))
    monkeypatch.setattr(lane, "authenticate_parent_runs", lambda: {"parent": True})
    monkeypatch.setattr(lane, "_inventory", lambda _root: "INITIALIZED")

    def reject_transport(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("transport constructed during init-only")

    monkeypatch.setattr(adapter, "UrllibPiqdTransport", reject_transport)
    result = lane.run_campaign(init_only=True, server="not-a-server")
    assert result["status"] == "INITIALIZED"
    assert result["run_manifest_sha256"] == manifest["manifest_sha256"]


def test_offline_verify_constructs_no_transport(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    manifest = {"manifest_sha256": "1" * 64}
    launch = {"execution_commit": "2" * 40}
    parent = {"parent": True}
    terminal = {"status": "TARGET_INCONCLUSIVE"}
    monkeypatch.setattr(lane, "ensure_run_root", lambda: (tmp_path, manifest))
    monkeypatch.setattr(lane, "authenticate_parent_runs", lambda: parent)
    monkeypatch.setattr(lane, "_inventory", lambda _root: "TERMINAL")
    monkeypatch.setattr(lane, "_read_launch", lambda _path: launch)
    monkeypatch.setattr(
        lane,
        "authenticate_recorded_execution_commit",
        lambda observed_manifest, observed_launch: "2" * 40,
    )
    monkeypatch.setattr(
        lane,
        "verify_terminal_run",
        lambda root, observed_manifest, observed_launch, observed_parent: terminal,
    )

    def reject_transport(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("transport constructed during offline verify")

    monkeypatch.setattr(adapter, "UrllibPiqdTransport", reject_transport)
    assert lane.run_campaign(verify=True, server="not-a-server") == terminal
