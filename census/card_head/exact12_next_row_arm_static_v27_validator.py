"""Independent, fail-closed validator for the exact-12 v27 arm-static canary.

This is deliberately not a wrapper around the frozen v26 validator.  It checks
the v15 descriptor/receipt/custody envelope, live source bytes, and either a
...
PROJECT = "p97-exact12-next-row-arm-static-cell6-v15-r1"
...
EXPECTED_PREFIX_VARS = 47211
EXPECTED_PREFIX_CLAUSES = 703533
EXPECTED_PREFIX_SHA256 = "415be05fdedfb008b0a1eb5a5ca5e299ef7564e9287b89333dc4705235916a93"
EXPECTED_POST_ARM_CLAUSES = 704400
EXPECTED_POST_ARM_SHA256 = "8f072d08ada262fcb3ba98012a5fc638297c681509e8355e04dadd4dd350d2d3"
...
class V27ValidationError(ValueError):
...
def _fail(message: str) -> None:
    raise V27ValidationError(message)
...
def _json(raw: bytes, path: Path) -> Any:
    ...
    except (UnicodeDecodeError, json.JSONDecodeError, RecursionError, V27ValidationError) as exc:
...
def validate_v27_workdir(workdir: Path, repo_root: Path, *, piqd_root: Path | None = None, proof_verifier: Callable[[Path, Path], bool] = _verify_drat) -> dict[str, Any]:
    ...
    if job.get("schema") != JOB_SCHEMA or job.get("arm_cell_index") != EXPECTED_ARM_CELL:
        _fail("v15 job identity mismatch")
    ...
    if promotion.get("lean_terminal_ingress_ready") is not True:
        _fail("job promotion does not affirm the v15 Lean-ingress contract")
    if summary.get("lean_terminal_ingress_ready") is not True:
        _fail("summary does not affirm the v15 Lean-ingress contract")
    if summary.get("lean_ingress_theorem") != LEAN_INGRESS_THEOREM:
        _fail("summary Lean-ingress theorem is not the frozen v15 constant")
    if summary.get("terminal_promotion_status") != TERMINAL_STATUS:
        _fail("summary terminal promotion status is not the frozen v15 constant")
    ...
    if (identity.get("backend") != "cadical"
        ...
        _fail("receipt job identity is not bound to the v27 contract")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = validate_v27_workdir(args.workdir, args.repo_root, piqd_root=args.piqd_journal_root)
    except (OSError, V27ValidationError, subprocess.SubprocessError) as exc:
