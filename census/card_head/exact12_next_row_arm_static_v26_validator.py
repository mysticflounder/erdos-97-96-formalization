"""Independent, fail-closed validator for the exact-12 v26 arm-static canary.

This is deliberately not a wrapper around the frozen v25 validator.  It checks
the v14 descriptor/receipt/custody envelope, live source bytes, and either a
...
PROJECT = "p97-exact12-next-row-arm-static-cell6-v14-r1"
...
class V26ValidationError(ValueError):
...
def _fail(message: str) -> None:
    raise V26ValidationError(message)
...
def _json(raw: bytes, path: Path) -> Any:
    ...
    except (UnicodeDecodeError, json.JSONDecodeError, RecursionError, V26ValidationError) as exc:
...
def validate_v26_workdir(workdir: Path, repo_root: Path, *, piqd_root: Path | None = None, proof_verifier: Callable[[Path, Path], bool] = _verify_drat) -> dict[str, Any]:
    ...
    if job.get("schema") != JOB_SCHEMA or job.get("arm_cell_index") != EXPECTED_ARM_CELL:
        _fail("v14 job identity mismatch")
    ...
    if promotion.get("lean_terminal_ingress_ready") is not True:
        _fail("job promotion does not affirm the v14 Lean-ingress contract")
    if summary.get("lean_terminal_ingress_ready") is not True:
        _fail("summary does not affirm the v14 Lean-ingress contract")
    if summary.get("lean_ingress_theorem") != LEAN_INGRESS_THEOREM:
        _fail("summary Lean-ingress theorem is not the frozen v14 constant")
    if summary.get("terminal_promotion_status") != TERMINAL_STATUS:
        _fail("summary terminal promotion status is not the frozen v14 constant")
    ...
    if (identity.get("backend") != "cadical"
        ...
        _fail("receipt job identity is not bound to the v26 contract")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = validate_v26_workdir(args.workdir, args.repo_root, piqd_root=args.piqd_journal_root)
    except (OSError, V26ValidationError, subprocess.SubprocessError) as exc:
