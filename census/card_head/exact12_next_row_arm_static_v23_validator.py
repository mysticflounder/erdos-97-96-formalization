"""Independent, fail-closed validator for the exact-12 v23 arm-static canary.

This is deliberately not a wrapper around the frozen v22 validator.  It checks
the v11 descriptor/receipt/custody envelope, live source bytes, and either a
...
PROJECT = "p97-exact12-next-row-arm-static-cell6-v11-r1"
...
EXPECTED_PREFIX_VARS = 45489
EXPECTED_PREFIX_CLAUSES = 646103
EXPECTED_PREFIX_SHA256 = "dc63ac918ed05c66cb9207f05219f66b2ad1df0310537bad1c3028abe980c300"
EXPECTED_POST_ARM_CLAUSES = 646970
EXPECTED_POST_ARM_SHA256 = "258dee064cdd39b33a38620c25b8bd47e8d61ab6d9b78b283cf559497d9fd944"
...
EXPECTED_FINAL_VARS = 45489
EXPECTED_FINAL_CLAUSES = 647051
EXPECTED_FINAL_SHA256 = "ccf284562911954d27db02db9589100046ac03a986b6f4d9b2a47ae9c5ce8755"
...
class V23ValidationError(ValueError):
...
def _fail(message: str) -> None:
    raise V23ValidationError(message)
...
def _json(raw: bytes, path: Path) -> Any:
    ...
    except (UnicodeDecodeError, json.JSONDecodeError, RecursionError, V23ValidationError) as exc:
...
def validate_v23_workdir(workdir: Path, repo_root: Path, *, piqd_root: Path | None = None, proof_verifier: Callable[[Path, Path], bool] = _verify_drat) -> dict[str, Any]:
    ...
    if job.get("schema") != JOB_SCHEMA or job.get("arm_cell_index") != EXPECTED_ARM_CELL:
        _fail("v11 job identity mismatch")
    ...
    if promotion.get("lean_terminal_ingress_ready") is not True:
        _fail("job promotion does not affirm the v11 Lean-ingress contract")
    if summary.get("lean_terminal_ingress_ready") is not True:
        _fail("summary does not affirm the v11 Lean-ingress contract")
    if summary.get("lean_ingress_theorem") != LEAN_INGRESS_THEOREM:
        _fail("summary Lean-ingress theorem is not the frozen v11 constant")
    if summary.get("terminal_promotion_status") != TERMINAL_STATUS:
        _fail("summary terminal promotion status is not the frozen v11 constant")
    ...
    if (identity.get("backend") != "cadical"
        ...
        _fail("receipt job identity is not bound to the v23 contract")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = validate_v23_workdir(args.workdir, args.repo_root, piqd_root=args.piqd_journal_root)
    except (OSError, V23ValidationError, subprocess.SubprocessError) as exc:
