"""Independent, fail-closed validator for the exact-12 v22 arm-static canary.

This is deliberately not a wrapper around the frozen v21 validator.  It checks
the v9 descriptor/receipt/custody envelope, live source bytes, and either a
...
EXPECTED_FINAL_CLAUSES = 645299
EXPECTED_FINAL_SHA256 = "d77aec4a937245dc6f0330969ccbf9c07bb2317a5e83c2b0602a5ca2d5be9b3b"
...
_SUMMARY_KEYS = frozenset({"schema", "scope", "job_id", "job_sha256", "arm_cell_index",
                           ...
                           "lean_terminal_ingress_ready", "replay", "timeout_seconds", "nice",
...
def _fail(message: str) -> None:
    raise V22ValidationError(message)
...
def _json(raw: bytes, path: Path) -> Any:
    ...
    except (UnicodeDecodeError, json.JSONDecodeError, RecursionError, V22ValidationError) as exc:
...
def validate_v22_workdir(workdir: Path, repo_root: Path, *, piqd_root: Path | None = None, proof_verifier: Callable[[Path, Path], bool] = _verify_drat) -> dict[str, Any]:
    ...
    if promotion.get("lean_terminal_ingress_ready") is not False:
        _fail("job promotion has ambiguous Lean-ingress claim")
    if summary.get("lean_terminal_ingress_ready") is not False:
        _fail("summary has ambiguous Lean-ingress claim")
    ...
    if (identity.get("job_id") != job["job_id"] or identity.get("backend") != "cadical"
        ...
        _fail("receipt job identity is not bound to the v22 contract")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = validate_v22_workdir(args.workdir, args.repo_root, piqd_root=args.piqd_journal_root)
    except (OSError, V22ValidationError, subprocess.SubprocessError) as exc:
