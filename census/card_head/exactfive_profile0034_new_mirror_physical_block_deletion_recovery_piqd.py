"""Fresh governed recovery of the NEW-mirror profile-0034 block deletion."""
...
from census.card_head import (
    exactfive_profile0034_new_mirror_supporting_edge_qfnra_piqd as deletion,
)
from census.card_head import (
    exactfive_profile0034_new_mirror_supporting_edge_qfnra_piqd as group_core,
)
from census.p97_search import phase3_piqd_smt_source_adapter as adapter

LANE_ID = "exactfive-profile0034-new-mirror-physical-block-deletion-recovery-piqd-20260905"
RUN_ID = "run-0001"
SOLVER = "z3"
TIMEOUT_MS = 60_000
...
MAX_WORKERS = 20
BRANCH_IDS = ("lt", "gt")
AUTHENTICATED_ORDER = ("U", "p", "a", "s", "d", "c2", "q", "O", "y", "x", "v", "t", "u")
...
TEST_PATH = REPOSITORY_ROOT / (
    ...
    "physical_block_deletion_recovery_piqd.py"
)
SPEC_PATH = REPOSITORY_ROOT / (
    "docs/specs/p97-exactfive-profile0034-new-mirror-physical-block-"
    "deletion-recovery-piqd-v1.md"
)
RUNNER_PATH = REPOSITORY_ROOT / (
    "scripts/run_exactfive_profile0034_new_mirror_physical_block_deletion_recovery_piqd.py"
)

CHECKPOINT_OWNER = "profile0034-new-mirror-physical-block-deletion-recovery-piqd"
CHECKPOINT_BASE_HEAD = "abab02eb9ffc1b0063731263c2821aa7d3c729e8"
_OWNED_PATHS = [
    f".codex/worktree-checkpoints/{LANE_ID}.json",
    "census/card_head/exactfive_profile0034_new_mirror_physical_block_deletion_recovery_piqd.py",
    "census/card_head/tests/test_exactfive_profile0034_new_mirror_physical_block_deletion_recovery_piqd.py",
    "docs/audits/2026-09-05-exactfive-profile0034-new-mirror-physical-block-deletion-run0001-failure.md",
    "docs/specs/p97-exactfive-profile0034-new-mirror-physical-block-deletion-recovery-piqd-v1.md",
    "scripts/run_exactfive_profile0034_new_mirror_physical_block_deletion_recovery_piqd.py",
]
...
GROUP_CORE_ROOT = deletion.RUN_ROOT
GROUP_CORE_MANIFEST_PATH = deletion.RUN_ROOT / "run_manifest.json"
GROUP_CORE_LAUNCH_PATH = deletion.RUN_ROOT / "events/launch.json"
GROUP_CORE_TERMINAL_PATH = deletion.RUN_ROOT / "events/terminal.json"
...
SOURCE_SHA256 = "fd8906d1b698c4779ccf8d6f373af9cd5d55c57eb7c3c89507f94862632696b3"
SOURCE_PARENT_ROOT = deletion.RUN_ROOT
SOURCE_PARENT_TERMINAL_PATH = deletion.RUN_ROOT / "events/terminal.json"
SOURCE_PARENT_TERMINAL_FILE_SHA256 = "0" * 64
SOURCE_PARENT_TERMINAL_SHA256 = "621c8f7f8bd435e63004b238b28ba108e291078157e3a164c3c2b7a931b30204"
SOURCE_PARENT_RESULT_FILES = {
    "lt": "eb3644e2176b2bf17351cde18bdf9d478a80e49007188573b8351c555bf1eba0",
    "gt": "f7b07f6ce060ffcc3950a3f97e6be64944ffa831406acff8138a14b6bd004c9f",
}
SOURCE_PARENT_JOURNAL_FILES = {
    "lt": "db29694cf075b978bb586fde6b5490fa0aa3cf318635cef59cddbf4c18c3862a",
    "gt": "9a5c4cfd3154224d46f3cb81df6c5e4a13f0ecc8783e9bfaadcdeb9040402cbf",
}
...
_ENGINE_KEYS = frozenset({
    ...
    "request_sha256", "assumption_labels", "request_replay_attempted", "request_replayed",
...
def _require_regular(path: Path, where: str, *, required: bool = True) -> bool:
    """Check a custody path without following links before opening it."""
    try:
        info = path.lstat()
    except FileNotFoundError:
        if required:
            raise Profile0034PhysicalBlockDeletionError(f"{where} is missing")
        return False
    except OSError as exc:
        raise Profile0034PhysicalBlockDeletionError(f"{where} is unreadable") from exc
    if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1:
        raise Profile0034PhysicalBlockDeletionError(f"{where} is unsafe")
    return True
...
def _witness_coordinates(
    ...
    coords = deletion._frame_coordinates(source_system["order_id"])
    for class_id in deletion.CLASS_IDS:
        if class_id in {"c000", "c001", "c003"}:
            continue
        coords[class_id] = (
...
def authenticate_parent_runs() -> dict[str, Any]:
    ...
    if _sha(_read(Path(deletion.__file__))) != SOURCE_SHA256:
    ...
    for branch in BRANCH_IDS:
        ...
        if (
            ...
            or source_journal
            != _journal(deletion._build_smt_commands_current(source_system))
        ):
        ...
        if (
            ...
            != tuple(deletion._build_smt_commands_current(source_system))
        ):
...
def _control_source(key: str) -> dict[str, Any]:
    ...
    return deletion.build_control_system(control)
...
def build_smt_commands(system: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if system.get("kind") == "control":
        return tuple(
            deletion._build_smt_commands_current(system["source_system"])
        )
...
def _manifest_sources() -> list[Path]:
    paths = [
        ...
        Path(deletion.__file__),
        Path(deletion.__file__),
        Path(deletion.bo_source.__file__),
    ]
...
def verify_sat_model(
    ...
    try:
        if system["kind"] == "control":
            parsed = deletion._parse_control_value(values)
            ...
            coords = deletion._parse_coordinates(values, query.get_values)
            evidence = _replay_target(_replay_system(system), coords)
            evidence["exact_coordinate_count"] = len(coords)
            evidence["free_coordinate_count"] = len(query.get_values)
    except Exception as exc:
        allowed = (
            deletion.Profile0034NewMirrorSupportingEdgeError,
            deletion.bo_source.BoSourceQfnraError,
            ValueError,
            KeyError,
        )
...
def classify_result(result: object) -> str:
    ...
    return {
        ...
        "INCONCLUSIVE_SEMANTIC_VERIFIER_FAILURE": "SAT_REPLAY_UNRESOLVED",
...
def _validate_engine(
    ...
    if raw == "SAT":
        ...
        if "semantic" not in payloads or _strict(
            payloads["semantic"], "semantic replay"
        ) != semantic:
            raise Profile0034PhysicalBlockDeletionError("SAT replay drifted")
        try:
            expected_semantic = _expected_semantic(prepared, solve, cache)
        except Exception as exc:
            expected_semantic = {
                "accepted": False,
                "failure": type(exc).__name__,
            ...
            expected_effective = (
                ...
                if expected_semantic["accepted"] is True
                else "INCONCLUSIVE_SEMANTIC_REPLAY_REJECTED"
            )
            if (
                semantic != expected_semantic
                or engine["effective_status"] != expected_effective
            ):
                raise Profile0034PhysicalBlockDeletionError("SAT replay drifted")
        if classify_result({"engine": dict(engine)}) == "SAT_REPLAYED" and (
            semantic.get("accepted") is not True
        ):
            raise Profile0034PhysicalBlockDeletionError("SAT replay drifted")
    elif raw == "UNSAT":
...
def _publish_once(path: Path, payload: bytes) -> None:
    try:
        ...
        if _read(path) != payload:
            raise Profile0034PhysicalBlockDeletionError("immutable artifact drifted")
    except Exception as exc:
        raise Profile0034PhysicalBlockDeletionError(
...
def ensure_run_root() -> tuple[Path, dict[str, Any]]:
    ...
    _require_regular(manifest_path, "run manifest", required=False)
...
def _read_launch(path: Path) -> dict[str, Any]:
    _require_regular(path, "launch record")
...
def _inventory(root: Path) -> str:
    ...
    _require_regular(manifest_path, "run manifest")
    ...
    for event_name in ("launch.json", "terminal.json"):
        event_path = root / "events" / event_name
        _require_regular(event_path, f"{event_name} event record", required=False)
    rounds_path = root / "events" / "rounds"
    if "rounds" in event_names and (rounds_path.is_symlink() or not rounds_path.is_dir()):
        raise Profile0034PhysicalBlockDeletionError("round directory is unsafe")
    if not event_names:
...
def _read_terminal(path: Path) -> dict[str, Any]:
    _require_regular(path, "terminal record")
...
def run_campaign(
    *,
    ...
    if verify:
        if phase != "TERMINAL":
            raise Profile0034PhysicalBlockDeletionError(
                "offline verify requires a completed terminal run"
            )
        stored_launch = _read_launch(root / "events" / "launch.json")
        authenticate_recorded_execution_commit(manifest, stored_launch)
...
def _commands_for_groups(source_system: Mapping[str, Any], retained: Sequence[str]) -> tuple[str, ...]:
    """Emit the exact source formula with whole physical groups omitted."""
    retained_ids = canonical_group_ids(retained)
    commands = ["(set-logic QF_NRA)"] + [
        f"(declare-fun {v} () Real)" for v in source_system["variables"]
    ]
    for group in retained_ids:
        if group.startswith("edge-index-"):
            index = int(group[-2:])
            commands.extend(
        ...
        elif group.startswith("source-row-equality-"):
            row = source_system["source_row_equalities"][int(group[-2:])]
            commands.append(f"(assert (= {row['left']} {row['right']}))")
        elif group == RADIUS_GROUP_ID:
            row = source_system["radius_branch"]
            commands.append(f"(assert ({row['relation']} {row['left']} {row['right']}))")
        elif group == STRICT_GROUP_ID:
            row = source_system["source_strict"]
            commands.append(f"(assert (< {row['left']} {row['right']}))")
    return tuple(commands)


def assertion_group_ledger(source_system: Mapping[str, Any]) -> tuple[dict[str, Any], ...]:
    """Return the 26 named physical blocks in authenticated assertion order."""
    ...
    for index in range(13):
        ...
        rows.append({"group_id": f"edge-index-{index:02d}", "source_commands": commands})
    for index, row in enumerate(source_system["source_row_equalities"]):
        rows.append({"group_id": f"source-row-equality-{index:02d}", "source_commands": [f"(assert (= {row['left']} {row['right']}))"]})
    radius = source_system["radius_branch"]
    strict = source_system["source_strict"]
    rows.append({"group_id": RADIUS_GROUP_ID, "source_commands": [f"(assert ({radius['relation']} {radius['left']} {radius['right']}))"]})
    rows.append({"group_id": STRICT_GROUP_ID, "source_commands": [f"(assert (< {strict['left']} {strict['right']}))"]})
    if tuple(row["group_id"] for row in rows) != GROUP_UNIVERSE or sum(len(row["source_commands"]) for row in rows) != 156:
        raise Profile0034PhysicalBlockDeletionError("physical assertion ledger drifted")
    return tuple(rows)


def reconstruct_unguarded_commands(source_system: Mapping[str, Any], retained: Sequence[str] = GROUP_UNIVERSE) -> tuple[str, ...]:
    return _commands_for_groups(source_system, retained)


def build_smt_commands(system: Mapping[str, Any]) -> tuple[str, ...]:  # noqa: F811
    if type(system) is not dict or system.get("schema") != SYSTEM_SCHEMA or system.get("claims") != FALSE_CLAIMS:
        raise Profile0034PhysicalBlockDeletionError("system is malformed")
    if system.get("kind") == "control":
        return tuple(deletion._build_smt_commands_current(system["source_system"]))
    if system.get("kind") != "target":
        raise Profile0034PhysicalBlockDeletionError("system kind is invalid")
    return _commands_for_groups(system["source_system"], system["retained_group_ids"])


def _replay_target(system: Mapping[str, Any], coords: Mapping[str, tuple[Fraction, Fraction]]) -> dict[str, Any]:
    """Replay only retained physical assertions against exact rational coordinates."""
    source = system["source_system"]
    if set(coords) != set(deletion.CLASS_IDS):
        return {"accepted": False, "reason": "coordinate_replay_is_not_total"}
    for fixed, value in deletion._frame_coordinates("mirror").items():
        if coords.get(fixed) != value:
            return {"accepted": False, "reason": "equilateral_frame"}
    retained = set(system["retained_group_ids"])
    for edge in source["supporting_edges"]:
        if f"edge-index-{edge['edge_index']:02d}" in retained and deletion._area_value(coords, *edge["edge"], edge["vertex"]) <= 0:
                return {"accepted": False, "reason": "supporting_edge"}
    for index, row in enumerate(source["source_row_equalities"]):
        if f"source-row-equality-{index:02d}" in retained:
            left = deletion._distance_value(coords, row["center_role"], row["first_support_role"])
            right = deletion._distance_value(coords, row["center_role"], row["other_support_role"])
            if left != right:
                return {"accepted": False, "reason": f"source_row_equality_{index:02d}"}
    radius = source["radius_branch"]
    if RADIUS_GROUP_ID in retained:
        left = deletion._distance_value(coords, source["role_class"]["c2"], source["role_class"]["O"])
        right = deletion._distance_value(coords, source["role_class"]["c2"], source["role_class"]["d"])
    ...
    if STRICT_GROUP_ID in retained:
        left = deletion._distance_value(coords, source["role_class"]["e"], source["role_class"]["d"])
        right = deletion._distance_value(coords, source["role_class"]["e"], source["role_class"]["a"])
        if not left < right:
...
def _manifest_sources() -> list[Path]:
    return [Path(__file__), Path(deletion.__file__), Path(deletion.bo_source.__file__), Path(adapter.__file__), SPEC_PATH]


def _manifest_inputs() -> list[Path]:
    paths = [SOURCE_PARENT_ROOT / "run_manifest.json", SOURCE_PARENT_ROOT / "events/launch.json", SOURCE_PARENT_ROOT / "events/terminal.json"]
    for branch in BRANCH_IDS:
        root = SOURCE_PARENT_ROOT / "artifacts" / f"mirror-radius-{branch}"
        paths.extend((root / "result.json", root / "original.smt2", root / "journal.smt2", root / "source-manifest.json"))
    return paths


def authenticate_parent_runs() -> dict[str, Any]:  # noqa: F811
    """Authenticate the completed NEW-mirror run and bind both source queries."""
    if _sha(_read(Path(deletion.__file__))) != "fd8906d1b698c4779ccf8d6f373af9cd5d55c57eb7c3c89507f94862632696b3":
        raise Profile0034PhysicalBlockDeletionError("source producer bytes drifted")
    manifest = _record(SOURCE_PARENT_ROOT / "run_manifest.json", file_sha="35b379483456df4e4e2ecb5e960c5697bb1ca25087b810fc95b2f38a8f03f279", self_field="manifest_sha256", self_sha="966b9cfcb90f8542e19d613d4817a733ae766c74635ace3b3e83a328c3731c05", where="mirror parent manifest")
    launch = _record(SOURCE_PARENT_ROOT / "events/launch.json", file_sha="a2c553ff19c8c57434379568ac74eaa64a2582599397c9ccdcfaf443da92d58c", self_field="launch_sha256", self_sha="95e49630f86c2e7cd7961cd45302fd672b228c999f69fcdfffd51d2f6a9e0648", where="mirror parent launch")
    terminal = _record(SOURCE_PARENT_ROOT / "events/terminal.json", file_sha="7563a658da37ca524debaba7c5286d1358478f1c8458a099cf548e0985d49f76", self_field="terminal_sha256", self_sha="621c8f7f8bd435e63004b238b28ba108e291078157e3a164c3c2b7a931b30204", where="mirror parent terminal")
    if launch.get("timeout_ms") != TIMEOUT_MS or launch.get("workers") != 2 or terminal.get("status") != "TARGET_INCONCLUSIVE" or not isinstance(terminal.get("claims"), dict) or any(terminal["claims"].values()):
        raise Profile0034PhysicalBlockDeletionError("mirror parent status drifted")
    if launch.get("run_manifest_sha256") != manifest["manifest_sha256"] or terminal.get("run_manifest_sha256") != manifest["manifest_sha256"]:
        raise Profile0034PhysicalBlockDeletionError("mirror parent manifest binding drifted")
    for field in ("source_digests", "input_digests"):
        digests = manifest.get(field)
        if not isinstance(digests, dict):
            raise Profile0034PhysicalBlockDeletionError("mirror parent digest inventory is malformed")
        for relative, digest in digests.items():
            if not isinstance(relative, str) or not isinstance(digest, str) or _sha(_read(REPOSITORY_ROOT / relative)) != digest:
                raise Profile0034PhysicalBlockDeletionError("mirror parent source bytes drifted")
    prior = deletion.authenticate_source_orders()
    systems = {branch: deletion._build_target_from_prior(prior, "mirror", branch) for branch in BRANCH_IDS}
    for branch, digest in SOURCE_PARENT_RESULT_FILES.items():
        key = f"mirror-radius-{branch}"
        artifact = SOURCE_PARENT_ROOT / "artifacts" / key
        prepared = deletion.prepare_query(systems[branch], timeout_ms=TIMEOUT_MS)
        deletion.verify_adapter_tree(prepared, artifact)
        if _sha(_read(artifact / "original.smt2")) != SOURCE_PARENT_JOURNAL_FILES[branch] or terminal.get("results", {}).get(key, {}).get("result_sha256") != digest:
            raise Profile0034PhysicalBlockDeletionError("mirror source query custody drifted")
    record = {"schema": "p97-exactfive-profile0034-new-mirror-physical-block-parent-custody/v1", "parent_manifest_sha256": manifest["manifest_sha256"], "parent_launch_sha256": launch["launch_sha256"], "parent_terminal_sha256": terminal["terminal_sha256"], "source_parent_result_file_sha256s": dict(SOURCE_PARENT_RESULT_FILES), "source_parent_journal_file_sha256s": dict(SOURCE_PARENT_JOURNAL_FILES), "order": list(prior["orders"]["mirror"]), "order_sha256": prior["order_sha256"], "source_systems": systems, "base_witnesses": {branch: verify_base_witness(branch, systems[branch]) for branch in BRANCH_IDS}, "claims": dict(FALSE_CLAIMS)}
    record["parent_custody_sha256"] = _self_hash(record, "parent_custody_sha256")
    return record
