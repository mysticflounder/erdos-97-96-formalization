...
The paper argument below was implemented and validated on task branch
`codex/native-exact15-coverage-20260905` in an isolated worktree while the
shared main source was frozen. This audit records that validation.
...
The fresh probe completed with no source changes after probing. Validation ran
on the task branch before integration into main. The generated evidence is under
