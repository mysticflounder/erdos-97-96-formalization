...
|---|---|---|
| A1 exact-13 publication / Fin-13 interval | **Repair is source-validated but not yet published.** Published HEAD still uses `Ico 8 13` / `Ico 9 13`, where `(13 : Fin 13)` wraps to zero. The owner lane changes these to `Icc 8 12` / `Icc 9 12`, adds cardinality/membership guards and image transports, and reports zero remaining forbidden endpoints. Governed receipts show `DRExactThirteenValuation` and `CardGeThirteenExact13RawIngress` exits 0 with the branch dependency rebuilt. | Owner `luna-swarm-radial-lemmas` must stage/publish its checkpoint-owned repair and provide clean-HEAD replay. A separate active whole-project build owns the lock and must not be raced. |
...
| A6 canonical negative incidence | **Exact information-loss point identified; no honest edit available.** `exactFour_fourSurvivingCenters_survivalSquare_split` consumes branch-specific canonical-support omissions and immediately projects them through `cross_deletion_survives_iff_not_mem_selected_support`; `TwoDeletionOutcome.survivalSquare` stores only the four-way survival disjunction. | The first unavailable antecedent is the named selected-row/support omission (or canonical-row witness) for the newly erased center; finite B1 closure additionally needs center localization/order. Preserve that fact upstream in any new producer. No survival-only wrapper was added. |
