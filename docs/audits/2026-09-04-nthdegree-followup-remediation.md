...
| A6 canonical negative incidence | **Recoverable at the source-rich caller; richer adapter in progress.** `exactFour_fourSurvivingCenters_survivalSquare_split` exports survival rather than the branch-specific omission, but `cross_deletion_survives_iff_not_mem_selected_support` is an equivalence, so `X ∉ Kq.support` can be recovered without changing the private finite splitter. With `pX := centerAt X`, current source also proves `pX ≠ bq`, `pX ≠ apex`, and transports positive incidence when `pX` equals `bu`, `bv`, or `br`. | A Luna lane is implementing only this source-clean sibling adapter. It is bookkeeping, not a contradiction: known-center arms still need a new geometric consumer, while the fresh arm additionally lacks reverse omission `q ∉ selectedAt X`. The decisive `X=apex, pX=bu` finite/cyclic test is a separate lane. |
