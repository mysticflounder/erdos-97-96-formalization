...
The evidence descriptions below remain scoped to that prose synthesis. The
remaining-boundary column was corrected on 2026-08-30 against the registry at
source head `eb6a12039aba1e3c4cc48baa9f1299337fc6a2ac`: 25 reachable and 6
off-spine declarations. The cached reference mine has one stale reference, so
refresh it before making a claim about newer source.
...
| B family / Rigid221 | Revision 4 gives the historical B1/B2/B3 contracts. The 2026-08-29 B1 refinement source-closes the old wrapper through a checked split. | B1 now lands on `false_of_b1PhysicalClassFiveSixNormalForm`; B2 and B3 remain open. The registry separately records 15 reachable Rigid221 declarations. |
| F1 TriApex | The 2026-08-23 refactor kernel-checks `nonempty_pairedApexClassJointDeletion_of_reverseHit`: the shared retained source lowers the union of the two shell/class intersections to at most three, so a four-point frontier class supplies a source omitted from both shells. D3--D9 are source-clean compatibility wrappers through D1. D2 is kernel-closed by the scaled-coordinate, convex-nesting, escape-synchronization, zero-cut boundary-sign, and polynomial route. The v87 audit supplies the checked finite low-span selector `finFour_exists_nearby_mutualFalse_of_card_le_two`. The new `StrictApexFourWitness`/`StrictApexFourFamily` layer preserves the one-radius/two-radii provenance, proves the aggregate support has cardinality twelve, and `triApexFiveSurviveOneFailOfApexRich` extracts an exact four-source packet outside both retained shells with five surviving centers and one failing actual blocker. | One TriApex declaration remains reachable in the 2026-08-30 registry. D2, the finite selector, and the D1 producer have core-only axiom closure; the D1 route and publish target still reach `sorryAx`. |
| F2 TwoSource | Four intrinsic prose packets reduce to Root A/Root B; checkpoint 36 supplies a cross-radius mutual-omission-cycle producer; the preferred FreshThird route is prose-complete. | Six TwoSource declarations remain reachable. Root A/Root B are prose grouping labels, not registry declaration counts. |
| F3 TwoDeletion | Local joint-locus calculus, physical traces, sparse conflict graphs, survival amplification, and bounded/grid/diagonal normal forms are prose-complete. | Three two-deletion declarations remain reachable. The five metric roots are prose subproblems, not registry declaration counts. |
