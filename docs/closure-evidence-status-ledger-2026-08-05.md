...
| F1 TriApex | The 2026-08-23 refactor kernel-checks `nonempty_pairedApexClassJointDeletion_of_reverseHit`: the shared retained source lowers the union of the two shell/class intersections to at most three, so a four-point frontier class supplies a source omitted from both shells. D3--D9 are now source-clean compatibility wrappers through D1, and the reverse-hit coordinator uses that route directly. D2 remains analytically closed in prose. | Exactly two independent live `sorry` roots remain: D1, `false_of_pairedCommonDeletion_apexClassJointDeletion_triApexAllLarge_core`, and D2, `false_of_pairedCommonDeletion_twoRadiusGrid_triApexAllLarge_core`. The selector is kernel-clean, but the D1 route and publish target still reach `sorryAx`; this is frontier reduction, not closure. |
