...
| B family / Rigid221 | Revision 4 gives source-validated B1/B2/B3 contracts. Commit `6d0fcc389` kernel-closes the exact-seventeen/cap-nine A2 terminal through `ExactA2CapMetric.false_of_exactA2_boundary_data` and its source adapter; the former SAT route is not a proof dependency. | The B1 positive terminal, B2/B3 geometric consumers, and scalable `|A|≥18` A3 transfer remain. The 57/57 check was static only. |
| F1 TriApex | The 2026-08-23 refactor kernel-checks `nonempty_pairedApexClassJointDeletion_of_reverseHit`: the shared retained source lowers the union of the two shell/class intersections to at most three, so a four-point frontier class supplies a source omitted from both shells. D3--D9 are source-clean compatibility wrappers through D1. D2 is kernel-closed by the scaled-coordinate, convex-nesting, escape-synchronization, zero-cut boundary-sign, and polynomial route. The v87 audit supplies the checked finite low-span selector `finFour_exists_nearby_mutualFalse_of_card_le_two`. The new `StrictApexFourWitness`/`StrictApexFourFamily` layer preserves the one-radius/two-radii provenance, proves the aggregate support has cardinality twelve, and `triApexFiveSurviveOneFailOfApexRich` extracts an exact four-source packet outside both retained shells with five surviving centers and one failing actual blocker. | Exactly one independent live `sorry` root remains in the D1 route, now isolated as `false_of_pairedCommonDeletion_fiveSurviveOneFail_triApexAllLarge_core`; the former D1 declaration is a source-clean wrapper constructing that packet. This is no net root-count reduction. No bridge upgrades a same-cap pair to four ordered same-radius sources. The cross-radius transverse `2 x 2` ingress, conditional same-radius `mu = 0` fixed point, and conditional disjoint outside `K2,2` plus global escapes remain open. D2, the finite selector, and the D1 producer have core-only axiom closure; the D1 route and publish target still reach `sorryAx`. The refreshed post-refactor kernel spine has 28 global obligations and one TriApex obligation. |
