...
| K-B-PIN | PARTIAL; OPEN-ADAPTER | `Problem97.isM44PinnedSurplusGeneralMResidualsExcluded` in `PinnedSurplusGeneralM.lean` | `isM44PinnedSurplusResidualsExcluded` | DONE in Lean: the open `5 < m` branch is forced to `m = 6`, `A.card = 11`, with exact `(6,4,4)` cap-selected hit counts; all six incidence fields and the native placement soundness are proved. `CapSelectedPinnedShellBridge` now turns every right aligned residual packet into `PinnedShellOK` and the native `ClosureCoreAlternative`; the terminal source invokes that right bridge. OPEN: transport and consume the four actual native alternatives (duplicate-center, exact off-circle, perpendicular-bisector, oriented convex-five) on the geometric carrier, then add the explicit left-to-right finite reflection and its source adapter. |
