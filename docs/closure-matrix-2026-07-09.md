...
| PIN-METRIC-BRIDGE | PARTIAL-PROVEN; OPEN-FINITE-BRIDGE | PIN-METRIC-TRIAGE; `CapSelectedRowCounting`; concrete `CriticalShellSystem`; convex-order API | DONE: twelve generic metric-core consumers, arbitrary-cardinality `rowPattern`/`Realizes`, and orientation-symmetric finite cyclic predicates. DONE in Lean: `5 < m` forces the card-eleven `(6,4,4)` case; `CapSelectedGeometry.exists_boundaryBlocks_of_isM44_surplus_card_eq_six` gives exact `(4,2,2)` direct-or-mirror boundary blocks; `CapSelectedClosureColor` gives a canonical closure color. PARTIAL VERIFIED CLASSIFIER: the native row search proves placement `(pin,deleted)=(6,4)` by `native_decide`; its finite-facts and soundness modules prove candidate-row coverage, the fixed pinned seed, row overlap, Q3, cyclic separation, and `rowsCompatible`. EXACT WITHIN MODEL: all 12 placements remain exhaustive with zero residuals under the four classifier families. | OPEN: prove `pairBoundOK`, finite-search/certificate coverage, checked closure paths/core extraction, the remaining placement checks, and the canonical `Fin 11` labeling from `BoundaryBlocks`. Acceptance is a kernel-checked total theorem, not the JSON verdict or one placement. |
| PIN-GENERAL | OPEN-PROOF; CLASSIFIER PARTIAL | CTRL-BANK-PREFLIGHT; `5 < S.surplusCap.card`; `SurplusCapPacket.surplus_card_eq_six_of_convexIndep_K4`; `SurplusCapPacket.card_eq_eleven_of_surplus_card_gt_five` | The formerly general-`m` branch is proved to have `m = 6`, `A.card = 11`. The exact cap counts, boundary-block packet, canonical closure coloring, and first native classifier/soundness layer are kernel-checked. | Complete finite coverage/core soundness and the boundary-block-to-canonical-label bridge, then apply the existing consumers and remove the sole declaration-level `sorry`. Shared-pair interval descent remains fallback research. Do not import downstream residual or containment conclusions. |
