...
| PIN-METRIC-BRIDGE | PARTIAL | PIN-METRIC-TRIAGE; `CapSelectedRowCounting`; concrete `CriticalShellSystem`; convex-order API | DONE: twelve generic metric-core consumers, arbitrary-cardinality `rowPattern`/`Realizes`, and orientation-symmetric finite cyclic predicates. DONE in Lean: `5 < m` forces the card-eleven `(6,4,4)` case; `CapSelectedGeometry.exists_boundaryBlocks_of_isM44_surplus_card_eq_six` gives exact `(4,2,2)` direct-or-mirror boundary blocks; `CapSelectedClosureColor` gives a canonical closure color. VERIFIED CLASSIFIER INTERFACE: candidate coverage, every pruning rule, recursive semantic tracing, and all twelve native placement equalities (`CapSelectedNativePlacements.allPlacementChecks`) are proved. `CapSelectedNogoodCertificate` checks direct row/flip paths and extracts public cores; `CapSelectedNogoodClassifier` proves the static-bank semantic DFS bridge. The aggregate native theorem's kernel closure is `propext`, `Lean.ofReduceBool`, and approved `Lean.trustCompiler`; the evaluated project modules have no `unsafe`, `@[implemented_by]`, or `@[extern]` declaration. EXACT WITHIN MODEL: the independent external four-family run is exhaustive with zero residuals in all 12 placements. | OPEN: emit and kernel-replay the compact checked nogood payload, prove its twelve static placement checks, and construct the canonical `Fin 11` labeling from `BoundaryBlocks`. Acceptance is the checked bank theorem plus the geometry/source consumer, not the JSON verdict or native placement equalities alone. |
| PIN-GENERAL | PARTIAL | CTRL-BANK-PREFLIGHT; `5 < S.surplusCap.card`; `SurplusCapPacket.surplus_card_eq_six_of_convexIndep_K4`; `SurplusCapPacket.card_eq_eleven_of_surplus_card_gt_five` | The formerly general-`m` branch is proved to have `m = 6`, `A.card = 11`. Exact cap counts, the boundary-block packet, canonical closure coloring, every native pruning rule, all twelve placement replays, checked path/core extraction, and the static-bank semantic search bridge are kernel-checked. | Generate/replay the finite nogood payload and complete the boundary-block-to-canonical-label bridge, then apply the existing consumers and remove the sole declaration-level `sorry`. Shared-pair interval descent remains fallback research. Do not import downstream residual or containment conclusions. |
