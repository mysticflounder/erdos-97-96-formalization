...
| ID | Status | Dependencies | Exact deliverable | Consumer and acceptance |
|---|---|---|---|---|
| A11-OPS-HOME | OPEN-ENG | Census process stopped | Move the now-tracked but provisional code, state, bank, schemas, and deterministic fixtures from `scratch/census-554/` to the Python package `census/census_554/`. Keep bulk certificates/run outputs in a documented ignored artifact directory; copy before deleting anything and verify the bank digest. | `git ls-files census/census_554` contains the driver, encoder, miner, verifier, generator, tests, README/STATE, schemas, bank, and small fixtures; `python -m census.census_554.cover_probe_smoke` runs from repo root; no tracked code imports the scratch path; Census554 docstrings contain no stale `{{NEEDS_*}}` marker or scratch provenance path. |
| A11-OPS-LOCK | OPEN-ENG | A11-OPS-HOME | Add a lifetime-exclusive driver lease shared by `frontier_loop`, direct/broad CEGAR entry points, and retry-bank commands. Under a separate transaction lock, allocate the next pattern ID, canonical-dedupe the motif, revalidate the certificate, write it to a temp file, fsync+rename, append+fsync one bank row, then unlock. | Two-driver test admits exactly one driver. Two-writer fault-injection test produces no duplicate ID/motif, orphan cert, partial JSON, or bank row without a verified cert. `cegar.py` now holds the bank lock for its whole run and `frontier_add.py` locks each append; the missing frontier-driver lease and atomic transaction keep this row open. |
...
| A11-OPS-TEST | OPEN-ENG | A11-OPS-HOME, A11-OPS-LOCK, A11-OPS-CNF | Commit self-contained fixtures and deterministic tests for SAT gates, `unlabeled_key` relabel invariance, non-isomorphic separation, AUTOS containment, and `embed_into_cube` validity. Do not require ignored certificate payloads or cwd-relative imports. | `uv run python -m census.census_554.cover_probe_smoke` passes from repo root and in a clean checkout. The tracked scratch-local suite passed on 2026-07-09 but still depends on local cert payloads and scratch cwd. |
| A11-OPS-TERM | OPEN-ENG | A11-OPS-HOME, A11-OPS-LOCK, A11-OPS-CNF | Write a terminal staging directory containing final CNF, checked DRAT/LRAT, lock-consistent normalized bank snapshot, and an instance manifest whose rows name source PID, canonical motif key, support injection, and emitted exclusion. Record hashes, script commit, command, Python/SymPy/CaDiCaL/checker versions, and elapsed time. Atomically rename `COVERAGE_COMPLETE.json` last, only after persisted-CNF re-solve and proof check succeed. | Synthetic UNSAT fixture passes `drat-trim`/chosen checker and manifest replay; corrupted CNF/proof or failed re-solve produces no completion marker. Current prototype incorrectly can write the marker when `drat_ok = false`, so it does not pass. |
| A11-BANK-AUDIT | OPEN-ENG | A11-OPS-HOME | Produce an immutable normalized bank audit: every line valid JSON; unique canonical PID/motif; cert exists; cert PID/pattern matches row; exact verifier passes; orphan certs reported. Preserve the known content-identical `pat_00003` duplicate only in raw provenance, not the normalized snapshot. | Report gives raw/normalized counts and digests; zero unexplained duplicate IDs/motifs, missing certs, row/cert mismatches, or failed exact identities. |
| A11-RUN | PAUSED | A11-OPS-LOCK, A11-OPS-TEST, A11-OPS-TERM | Resume the authoritative frontier driver and reach exactly one recorded terminal state: solver UNSAT, genuine ALIVE candidate, zero-certified/certify failure, or explicit bounded stop. | Terminal record includes bank rows/digest, last completed iteration, solver versions, command, and elapsed time. Current nonterminal checkpoint: 5,431 rows; log stopped at frontier iteration 1154; no process or terminal marker at 18:32 PDT. |
...
| A11-CORE | OPEN-ENG | A11-COVER-FMT | Extract the distinct bank pattern IDs actually referenced by the checked cover witness; emit sorted IDs and cert-size histogram. | Every cover leaf names one core ID; removing any non-core bank row leaves the checked witness unchanged. All later replay rows consume this core, not the full bank by default. |
...
| A11-CERT-BENCH | OPEN-ENG | A11-GEN | Build representative smallest/median/p90/largest cover-core certs serially and record source bytes, coefficient terms, wall time, and peak memory. Set the naive-replay threshold from the core measurements, targeting 10 minutes and never exceeding 30 minutes or 16,384 MB per Lean worker. | Benchmark report names the exact core IDs and classifies every core cert LIGHT or HEAVY by a deterministic rule. |
...
| ID | Status | Dependencies | Exact deliverable | Consumer and acceptance |
|---|---|---|---|---|
| A11-GEO-CARD | OPEN-PROOF | Existing cap-sum and lower-bound lemmas | `Census554.card_ge_eleven_of_twoLargeCaps`: surplus cap `>4`, distinct second cap `>=5`, third cap `>=4` imply `11 <= D.A.card`. | Used at both Front-A split sites; targeted geometry module builds. |
| A11-GEO-LABEL | OPEN-PROOF | A11-GEO-CARD; `capProfile_eq_554_of_card_eq_eleven`; `MultiCenter/CapProfile.lean` | `Card11Labeling` with `pointOf : Fin 11 → ℝ²`, injective, range exactly `D.A`, labels 0/1/2 at U/V/W, and remaining labels bijective with the three cap interiors of sizes 3/3/2. | Constructor theorem from leaf hypotheses and `D.A.card = 11`; all range/cap membership fields proved. |
| A11-GEO-CLASS | OPEN-PROOF | A11-GEO-LABEL; `exists_selectedFourClass_of_globalK4` | `Card11SelectedCube`: one `SelectedFourClass` at every `pointOf p`, cube mask as the exact preimage of its support, and `RealizesCube pointOf cube`. | Support cardinality and equidistance transport lemmas compile; no choice is trusted without its existence proof. |
| A11-GEO-C1C2 | OPEN-PROOF | A11-GEO-CLASS | C1 and C2 fields: mask card 4, center absent, pairwise class intersections at most 2 via `SelectedFourClass.inter_card_le_two`. | The corresponding `CubeOk` conjuncts are proved for the selected cube. |
| A11-GEO-C4 | OPEN-PROOF | A11-GEO-LABEL, A11-GEO-CLASS | C4: every point pair occurs in at most two selected classes, using `Dumitrescu.perpBisector_apex_bound` and injectivity of `pointOf`. | C4 `CubeOk` conjunct proved for every distinct pair. |
| A11-GEO-H1 | OPEN-PROOF | A11-GEO-LABEL, cap frame | One-hit at census labels V=1 and W=2, transported from `leftAdjacentCap_at_opposite_card_le_one_of_convexIndep` and right sibling for the selected class radii. | `OnehitOk` proved at exactly labels 1 and 2; no stronger unproved all-center claim. |
| A11-GEO-CUBE | OPEN-PROOF | A11-GEO-C1C2, A11-GEO-C4, A11-GEO-H1 | `Card11SelectedCube.cubeOk : CubeOk selected.cube`. | `lake-build Erdos9796Proof.P97.Census554.GeometryBridge` succeeds; theorem core axioms only. |
...
| A11-COVER-REL | OPEN-ENG | A11-EMBED | Definitions `PatternEmbedsUnder`, `RealizesCube`, generated cover-row schema, and proof that a covered cube realizes the relabeled pattern whenever it realizes all cube classes. | Small hand-written two-row example proves the intended direction; `CubeOk` alone is never used to derive `False`. |
...
| A11-COVER-DEAD | OPEN-PROOF | A11-CERT-INDEX, A11-MOTIF, A11-EMBED, A11-COVER-REL, A11-COVER-CHECK | `not_realizesCube_of_coverCore : CubeOk kappa -> not (RealizesCube x kappa)`. | Proof chooses a covered row, applies its `IsDead` theorem and `motif_transfer_of_supportInjOn`. Core axioms plus approved native boundary only. |
| A11-CONSUME | OPEN-PROOF | A11-GEO-CUBE, A11-COVER-DEAD | `Census554.false_of_cardEleven_twoLargeCaps`, consuming a `Card11SelectedCube` and deriving `False` from its `RealizesCube` field. | `lake-build Erdos9796Proof.P97.Census554.Card11` succeeds. The theorem has no liveData-row hypotheses. |
...
| AHEAD-SPEC | OPTIONAL | AHEAD-DEC chooses head census | Move supported fixed-n tooling to `census/card_head/`; delete CONJECTURED cuts from the proof tier; specify and prove the geometric necessity of every retained cut. Enumerate labeled profiles: n=12 `{654,564,555}`; n=13 `{754,664,655,574,565,556}`; n=14 `{854,764,674,584,755,665,575,656,566,557}`. | Clean-checkout tests reproduce candidate counts for every profile; a Lean arithmetic/profile lemma proves the lists exhaustive under first/second cap >=5 and third cap >=4. |
