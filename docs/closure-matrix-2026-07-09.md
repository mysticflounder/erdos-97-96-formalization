...
| K-A-N11-EXACT5-G3 | THREE G3 SELECTOR BRANCHES SOURCE-WIRED; `s2_o0` RETAINED-CORE SOURCE-WIRED; FOCUSED BUILD VALIDATION IN PROGRESS; NOT THEOREM-LEVEL CLOSURE | `P97/ATail/CardElevenUniqueFourCertificate/Support/ExactFiveCommonShellV7/`; `G3Checkpoint0ClauseBank`; `S2O0TextIngress`; `S2O0RetainedCoreClauseBank`; `FiniteN11Frontier.lean` | The card-11 common-center coordinator dispatches `.s2_o9`, `.s3_o0`, and `.s3_o9` through `CanonicalPacket.false_of_g3Checkpoint0_clause_bank`; the direct `.s2_o0` source branch now calls `CanonicalPacket.false_of_s2O0RetainedCore`, pending focused compilation. | The source bank declares 21 retained families over the frozen 81,253-clause core and supplies the source-clause permutation plus direct valuation bridge. Completion still requires a green focused and full build followed by the approved-native-trust axiom/code audit, with no `sorryAx` or unapproved trust. Until that validation passes, neither the card-11 exact-five branch nor its parent unique-radius obligation is theorem-level closed. |
...
| K-A-LIVE | CARD-11 EXACT-FOUR CLOSED; PARENT AND CAP-SOURCE COORDINATORS SOURCE-CLEAN; FIVE TRANSITIVE TERMINALS OPEN | `Problem97.u1_largeCap_routeB_tail_liveData_false`; `ATail/FrontierLiveClosure.lean` | `u1_largeCap_routeB_tail_false` | The live caller extracts one concrete `CriticalShellSystem` and dispatches through `CriticalPairFrontier`. The source-clean unique-radius parent closes exact four at card 11; its card-at-least-12 exact-four arm is narrowed by checked physical ingress into robust post-card-eleven and swapped protected-exact-four leaves, followed by exact-five leaves with distinct or common obstruction centers. In the all-large tri-apex subtree, `false_of_retainedOmission_triApexAllLarge_core` remains the directed-omission terminal. On the exact-collision side, `false_of_crossBlockerCoincidence` is one independent sibling and `false_of_capSourceThirdCanonicalRowSurface` is a source-clean exhaustive coordinator whose three sorry-bearing descendants are `false_of_twoCapSources_freshOutsideFirstBlockerFiber`, `false_of_twoCapSources_freshThirdBlockerFiber_normalized_residual`, and the current anchor `false_of_capSource_firstFiber_collisionFiveCenterDeletion`. The anchor consumes `FirstFiberCollisionFiveCenterExactRowsResidual` plus the stronger `FirstFiberOutsidePairExactRowsOrCrossedThreeRowsResidual`; checked producers preserve the same two-deletion/two-crossed fan-out. Crossed arms expose exact fixed-role supports, both anonymous off-cap fourth points, both complementary cross-row omissions, and two source-clean strict distance non-equalities. The corrected full-row dual-deletion CEGAR remains `UNKNOWN_TIMEOUT` after 271 candidates, 730 structural cuts, and 11,378 theorem cuts. For the source-at-common crossed arm, the omission-correct schema-v4 run returns a replay-PASS finite SAT shadow after 889 candidates and 43,556 theorem cuts; a second run reconstructing 48,978 universal cuts returns a stronger replay-PASS SAT shadow after 905 candidates and 85,003 total cuts. This is bounded incidence/order discovery at `n = 17`, profile `(6,8,6)`, not Euclidean realizability or universal Lean closure. A separate complete-radius QF_NRA check reports uncertified metric-only `UNSAT` on that shadow, but no minimized universal core has been extracted. Sound replay instead exposes eight violations of the generic cyclic `btw_sep` theorem, including the live source/source/apex/common-blocker instance. Production now states the exact alternation consequence as `firstFiber_capSources_alternate_between_firstApex_and_commonBlocker`, and schema-v5 adds its recut-invariant raw-equality cut with exhaustive four- and five-point self-checks. The long target still must be rerun; any survivor must be re-mined, while Lean closure requires an opposite-order bridge from a live crossed/deletion arm or a separately proved universal metric lemma. No terminal was closed. Compatibility-only adapters receive no closure credit. `docs/live-blueprint.md` becomes authoritative after the next successful generated refresh. |
...
| A11-WIRE-PARENT | DONE-PRODUCTION VIA NON-CENSUS INGRESS; OLD CENSUS WIRE SUPERSEDED | K-A-UNIQUE-POST11 | `false_of_firstApexUniqueRadiusExactFourResidual` splits at `D.A.card = 11`, uses `false_of_firstApexUniqueRadiusExactFourResidual_of_card_eq_eleven`, and sends the other branch to the `12 ≤ D.A.card` leaf. | The checked split is in `ATail/FrontierLiveClosure.lean`; no Census554 consumer or legacy pair/LIVE helper wiring is on the production path. |
...
|---|---|---|
| `ATailFrontierLiveClosure.false_of_exactFourPostCardElevenRobustSurface` | `FrontierLiveClosure.lean:84` | 1 |
| `ATailFrontierLiveClosure.false_of_exactFourPhysicalConsumerSwappedUniqueFourOutcome` | `FrontierLiveClosure.lean:116` | 1 |
| `ATailFrontierLiveClosure.false_of_firstApexUniqueRadiusExactFiveDistinctObstructionCentersResidual` | `FrontierLiveClosure.lean:208` | 1 |
| `ATailFrontierLiveClosure.false_of_firstApexUniqueRadiusExactFiveCommonObstructionCenterResidual` | `FrontierLiveClosure.lean:219` | 1 |
| `ATailFrontierLiveClosure.false_of_localizedCollisionMutualOmissionCycle_exactTwo_and_all_low_hits` | `FrontierLiveClosure.lean:647` | 1 |
| `ATailFrontierLiveClosure.false_of_retainedInteriorDirectedOmission_and_all_low_hits` | `FrontierLiveClosure.lean:726` | 1 |
| **Total** | | **6** |
