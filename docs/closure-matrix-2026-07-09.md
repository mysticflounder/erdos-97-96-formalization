...
| A11-GEO-CLASS | DONE | A11-GEO-LABEL; `exists_selectedFourClass_of_globalK4` | `Card11SelectedCube.ofGlobalK4` chooses one proved-nonempty `SelectedFourClass` at every `pointOf p`; `cube` is its exact support preimage; `image_cube_eq_support` and `equidist_of_mem_cube` provide the schema-neutral geometric realization facts. | Support cardinality, exact support image, and equidistance transport compile. Packaging these facts into the not-yet-defined `RealizesCube` record/relation belongs to A11-COVER-REL, not this completed geometry row. |
...
| A11-COVER-REL | OPEN-ENG | A11-EMBED, A11-GEO-CLASS | Definitions `PatternEmbedsUnder`, `RealizesCube`, generated cover-row schema, proof that a covered cube realizes the relabeled pattern whenever it realizes all cube classes, and a `Card11SelectedCube` adapter using `L.injective` plus `S.equidist_of_mem_cube`. | Small hand-written two-row example proves the intended direction; the selected-cube adapter compiles; `CubeOk` alone is never used to derive `False`. |
