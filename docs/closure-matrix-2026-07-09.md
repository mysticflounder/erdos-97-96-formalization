...
| PIN-METRIC-BRIDGE | SEARCH-COVERAGE PROVEN; OPEN-CORE-EXTRACTION | PIN-METRIC-TRIAGE; `CapSelectedRowCounting`; concrete `CriticalShellSystem`; convex-order API | DONE: twelve generic metric-core consumers, arbitrary-cardinality `rowPattern`/`Realizes`, and orientation-symmetric finite cyclic predicates. DONE in Lean: `5 < m` forces the card-eleven `(6,4,4)` case; `CapSelectedGeometry.exists_boundaryBlocks_of_isM44_surplus_card_eq_six` gives exact `(4,2,2)` direct-or-mirror boundary blocks; `CapSelectedClosureColor` gives a canonical closure color. VERIFIED CLASSIFIER INTERFACE: the soundness layer proves candidate-row coverage, the fixed seed, row overlap, Q3, cyclic separation, `pairBoundOK`, and complete `compatibleWith` soundness. `CapSelectedNativeClassifierCoverage` proves the recursive semantic trace and turns a successful placement replay into a duplicate-free semantic prefix with `hasPrefixCore = true`. Placement `(pin,deleted)=(6,4)` is closed by `native_decide`. EXACT WITHIN MODEL: all 12 placements remain exhaustive with zero residuals under the four classifier families. | OPEN: replay explicit closure paths and extract a proved core from `hasPrefixCore`, finish the remaining placement checks, and construct the canonical `Fin 11` labeling from `BoundaryBlocks`. Acceptance is a kernel-checked total theorem, not the JSON verdict or one placement. |
| PIN-GENERAL | OPEN-PROOF; SEARCH-COVERAGE PROVEN | CTRL-BANK-PREFLIGHT; `5 < S.surplusCap.card`; `SurplusCapPacket.surplus_card_eq_six_of_convexIndep_K4`; `SurplusCapPacket.card_eq_eleven_of_surplus_card_gt_five` | The formerly general-`m` branch is proved to have `m = 6`, `A.card = 11`. Exact cap counts, the boundary-block packet, canonical closure coloring, every native pruning rule, and the recursive semantic search trace are kernel-checked. | Complete finite core extraction/all-placement replay and the boundary-block-to-canonical-label bridge, then apply the existing consumers and remove the sole declaration-level `sorry`. Shared-pair interval descent remains fallback research. Do not import downstream residual or containment conclusions. |
