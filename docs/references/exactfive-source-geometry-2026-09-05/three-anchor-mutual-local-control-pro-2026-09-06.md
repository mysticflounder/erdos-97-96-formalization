# Exact-five three-anchor local control

Consult: `01M1VS1YXCHXSJCQBXN5PNC80P`. Date: September 6, 2026.

This exact 24-point configuration is NOT a P97 counterexample and does NOT satisfy global K4, total H, or the complete physical source. It realizes the local MEC/cap geometry, an exact-five first-apex class with three interior anchors, two full rich second-apex four-classes, a minimum admissible long pair, and two mutually incident actual local blocker rows. Only O, V, b, c have rich classes; the third anchor u has no actual blocker.

Copy the Python block to a file and run with Python 3.10 or later. Only the standard library is used. All assertions use exact arithmetic in Q(sqrt(3)); approximate values are output diagnostics only. The executed checker passed all 528 strict supporting-edge tests and all class, disk, deletion, cap, and non-globality checks. An additional replay checked all 2024 cyclically ordered triples with exact positive orientation.

The full mathematical response is posted as this consult's completion. Full report SHA-256: `926e2e3d6ba0b40a05177a22afd07614148e6ad8c43027a465e7a573641ef65a`.
