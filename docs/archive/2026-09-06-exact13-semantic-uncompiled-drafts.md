# ARCHIVED — superseded exact-thirteen semantic drafts

**Historical snapshot only. No active task or current proof status is recorded here.**
The text below is the complete proof note from commit `fa66927f7`, before its
Lean implementations were installed. All statements that a draft is uncompiled
or that the raw-cover helper is missing describe that historical checkpoint.

Use [the current semantic note](../erdos-97-exact13-semantic-canonicalization.md)
and [the authoritative atomic proof](../erdos-97-descent-prose-proof-atomic.md)
for current claims and remaining work. The checked source implementations replace
the code blocks below. Historical payload SHA-256:
`cf1a4549e57322252e187a9da3651d3fbbbb846e0b862b86af697a754914e9ca`.
Relative links in the preserved payload retain their original `docs/` base.
