...
private theorem rs_R003_ueqv_R003NY_rules_check :
    rulesValidForPid "s1_026" (.pair .u .v)
      rs_R003_ueqv_R003NY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R003_ueqv_R003NY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_026"] = [exactRow.pid]) :
...
theorem rs_R003_ueqv_R003NY_evaluationZeros_of_metricShadow
  ...
  change ∀ p ∈ computedProductPolys rs_R003_ueqv_R003NY_productBlocks,
    evalPoly (endpointPairAssignment pointOf .u .v) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointPairAssignment pointOf .u .v) rs_R003_ueqv_R003NY_productBlocks ?_
  simpa [rs_R003_ueqv_R003NY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_026")
      (assignment := (.pair .u .v)) (rules := rs_R003_ueqv_R003NY_rules)
      rs_R003_ueqv_R003NY_rules_check hmetric hrow hmasks hpidEq)
