...
private theorem rs_R005_ueqv_R005NYN_rules_check :
    rulesValidForPid "s1_016" (.pair .u .v)
      rs_R005_ueqv_R005NYN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R005_ueqv_R005NYN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_016"] = [exactRow.pid]) :
...
theorem rs_R005_ueqv_R005NYN_evaluationZeros_of_metricShadow
  ...
  change ∀ p ∈ computedProductPolys rs_R005_ueqv_R005NYN_productBlocks,
    evalPoly (endpointPairAssignment pointOf .u .v) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointPairAssignment pointOf .u .v) rs_R005_ueqv_R005NYN_productBlocks ?_
  simpa [rs_R005_ueqv_R005NYN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_016")
      (assignment := (.pair .u .v)) (rules := rs_R005_ueqv_R005NYN_rules)
      rs_R005_ueqv_R005NYN_rules_check hmetric hrow hmasks hpidEq)
