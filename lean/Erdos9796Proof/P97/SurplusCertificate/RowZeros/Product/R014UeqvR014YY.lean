...
/-!
# Product-sum row zeros for relaxed split surplus certificate R014:u=v:R014YY
...
private theorem rs_R014_ueqv_R014YY_rules_check :
    rulesValidForPid "s3_019" (.pair .u .v)
      rs_R014_ueqv_R014YY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R014_ueqv_R014YY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s3_019"] = [exactRow.pid]) :
    exactRow.pid = "s3_019" := by
  simpa using hpid.symm

/-- Every computed block in product-sum relaxed split surplus certificate
`R014:u=v:R014YY` evaluates to zero under the row-local normal-axis assignment. -/
theorem rs_R014_ueqv_R014YY_evaluationZeros_of_metricShadow
    ...
    CertificatePayload.evaluationZeros
      (.productSum Problem97.SurplusCertificate.RelaxedSplit.rs_R014_ueqv_R014YY_blocks)
      (endpointPairAssignment pointOf .u .v) := by
  have hpidEq := rs_R014_ueqv_R014YY_exactRow_pid_eq hpid
  change ∀ p ∈ computedProductPolys rs_R014_ueqv_R014YY_productBlocks,
    evalPoly (endpointPairAssignment pointOf .u .v) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointPairAssignment pointOf .u .v) rs_R014_ueqv_R014YY_productBlocks ?_
  simpa [rs_R014_ueqv_R014YY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s3_019")
      (assignment := (.pair .u .v)) (rules := rs_R014_ueqv_R014YY_rules)
      rs_R014_ueqv_R014YY_rules_check hmetric hrow hmasks hpidEq)
