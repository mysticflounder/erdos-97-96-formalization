...
private theorem rs_R009_ueqv_R009YNNNN_rules_check :
    rulesValidForPid "s2_046" (.pair .u .v)
      rs_R009_ueqv_R009YNNNN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R009_ueqv_R009YNNNN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s2_046"] = [exactRow.pid]) :
...
theorem rs_R009_ueqv_R009YNNNN_evaluationZeros_of_metricShadow
  ...
  change ∀ p ∈ computedProductPolys rs_R009_ueqv_R009YNNNN_productBlocks,
    evalPoly (endpointPairAssignment pointOf .u .v) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointPairAssignment pointOf .u .v) rs_R009_ueqv_R009YNNNN_productBlocks ?_
  simpa [rs_R009_ueqv_R009YNNNN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s2_046")
      (assignment := (.pair .u .v)) (rules := rs_R009_ueqv_R009YNNNN_rules)
      rs_R009_ueqv_R009YNNNN_rules_check hmetric hrow hmasks hpidEq)
