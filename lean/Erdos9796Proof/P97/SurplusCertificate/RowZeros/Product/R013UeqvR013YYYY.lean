...
private theorem rs_R013_ueqv_R013YYYY_rules_check :
    rulesValidForPid "s3_004" (.pair .u .v)
      rs_R013_ueqv_R013YYYY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R013_ueqv_R013YYYY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s3_004"] = [exactRow.pid]) :
...
theorem rs_R013_ueqv_R013YYYY_evaluationZeros_of_metricShadow
  ...
  change ∀ p ∈ computedProductPolys rs_R013_ueqv_R013YYYY_productBlocks,
    evalPoly (endpointPairAssignment pointOf .u .v) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointPairAssignment pointOf .u .v) rs_R013_ueqv_R013YYYY_productBlocks ?_
  simpa [rs_R013_ueqv_R013YYYY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s3_004")
      (assignment := (.pair .u .v)) (rules := rs_R013_ueqv_R013YYYY_rules)
      rs_R013_ueqv_R013YYYY_rules_check hmetric hrow hmasks hpidEq)
