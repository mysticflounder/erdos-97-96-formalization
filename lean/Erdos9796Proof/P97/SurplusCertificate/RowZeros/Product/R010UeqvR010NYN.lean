...
/-!
# Product-sum row zeros for relaxed split surplus certificate R010:u=v:R010NYN
...
private theorem rs_R010_ueqv_R010NYN_rules_check :
    rulesValidForPid "s2_066" (.pair .u .v)
      rs_R010_ueqv_R010NYN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R010_ueqv_R010NYN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s2_066"] = [exactRow.pid]) :
    exactRow.pid = "s2_066" := by
  simpa using hpid.symm

/-- Every computed block in product-sum relaxed split surplus certificate
`R010:u=v:R010NYN` evaluates to zero under the row-local normal-axis assignment. -/
theorem rs_R010_ueqv_R010NYN_evaluationZeros_of_metricShadow
    ...
    CertificatePayload.evaluationZeros
      (.productSum Problem97.SurplusCertificate.RelaxedSplit.rs_R010_ueqv_R010NYN_blocks)
      (endpointPairAssignment pointOf .u .v) := by
  have hpidEq := rs_R010_ueqv_R010NYN_exactRow_pid_eq hpid
  change ∀ p ∈ computedProductPolys rs_R010_ueqv_R010NYN_productBlocks,
    evalPoly (endpointPairAssignment pointOf .u .v) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointPairAssignment pointOf .u .v) rs_R010_ueqv_R010NYN_productBlocks ?_
  simpa [rs_R010_ueqv_R010NYN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s2_066")
      (assignment := (.pair .u .v)) (rules := rs_R010_ueqv_R010NYN_rules)
      rs_R010_ueqv_R010NYN_rules_check hmetric hrow hmasks hpidEq)
