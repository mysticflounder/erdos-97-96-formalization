...
/-!
# Product-sum row zeros for relaxed split surplus certificate R011:u=v:R011YYY
...
private theorem rs_R011_ueqv_R011YYY_rules_check :
    rulesValidForPid "s2_054" (.pair .u .v)
      rs_R011_ueqv_R011YYY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R011_ueqv_R011YYY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s2_054"] = [exactRow.pid]) :
    exactRow.pid = "s2_054" := by
  simpa using hpid.symm

/-- Every computed block in product-sum relaxed split surplus certificate
`R011:u=v:R011YYY` evaluates to zero under the row-local normal-axis assignment. -/
theorem rs_R011_ueqv_R011YYY_evaluationZeros_of_metricShadow
    ...
    CertificatePayload.evaluationZeros
      (.productSum Problem97.SurplusCertificate.RelaxedSplit.rs_R011_ueqv_R011YYY_blocks)
      (endpointPairAssignment pointOf .u .v) := by
  have hpidEq := rs_R011_ueqv_R011YYY_exactRow_pid_eq hpid
  change ∀ p ∈ computedProductPolys rs_R011_ueqv_R011YYY_productBlocks,
    evalPoly (endpointPairAssignment pointOf .u .v) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointPairAssignment pointOf .u .v) rs_R011_ueqv_R011YYY_productBlocks ?_
  simpa [rs_R011_ueqv_R011YYY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s2_054")
      (assignment := (.pair .u .v)) (rules := rs_R011_ueqv_R011YYY_rules)
      rs_R011_ueqv_R011YYY_rules_check hmetric hrow hmasks hpidEq)
