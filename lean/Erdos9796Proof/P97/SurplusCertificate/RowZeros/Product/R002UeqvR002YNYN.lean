...
/-!
# Product-sum row zeros for relaxed split surplus certificate R002:u=v:R002YNYN
...
private theorem rs_R002_ueqv_R002YNYN_rules_check :
    rulesValidForPid "s1_007" (.pair .u .v)
      rs_R002_ueqv_R002YNYN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R002_ueqv_R002YNYN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_007"] = [exactRow.pid]) :
    exactRow.pid = "s1_007" := by
  simpa using hpid.symm

/-- Every computed block in product-sum relaxed split surplus certificate
`R002:u=v:R002YNYN` evaluates to zero under the row-local normal-axis assignment. -/
theorem rs_R002_ueqv_R002YNYN_evaluationZeros_of_metricShadow
    ...
    CertificatePayload.evaluationZeros
      (.productSum Problem97.SurplusCertificate.RelaxedSplit.rs_R002_ueqv_R002YNYN_blocks)
      (endpointPairAssignment pointOf .u .v) := by
  have hpidEq := rs_R002_ueqv_R002YNYN_exactRow_pid_eq hpid
  change ∀ p ∈ computedProductPolys rs_R002_ueqv_R002YNYN_productBlocks,
    evalPoly (endpointPairAssignment pointOf .u .v) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointPairAssignment pointOf .u .v) rs_R002_ueqv_R002YNYN_productBlocks ?_
  simpa [rs_R002_ueqv_R002YNYN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_007")
      (assignment := (.pair .u .v)) (rules := rs_R002_ueqv_R002YNYN_rules)
      rs_R002_ueqv_R002YNYN_rules_check hmetric hrow hmasks hpidEq)
