...
/-!
# Product-sum row zeros for relaxed split surplus certificate R009:u=v:R009NNNNN
...
private theorem rs_R009_ueqv_R009NNNNN_rules_check :
    rulesValidForPid "s2_028" (.pair .u .v)
      rs_R009_ueqv_R009NNNNN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R009_ueqv_R009NNNNN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s2_028"] = [exactRow.pid]) :
    exactRow.pid = "s2_028" := by
  simpa using hpid.symm

/-- Every computed block in product-sum relaxed split surplus certificate
`R009:u=v:R009NNNNN` evaluates to zero under the row-local normal-axis assignment. -/
theorem rs_R009_ueqv_R009NNNNN_evaluationZeros_of_metricShadow
    ...
    CertificatePayload.evaluationZeros
      (.productSum Problem97.SurplusCertificate.RelaxedSplit.rs_R009_ueqv_R009NNNNN_blocks)
      (endpointPairAssignment pointOf .u .v) := by
  have hpidEq := rs_R009_ueqv_R009NNNNN_exactRow_pid_eq hpid
  change ∀ p ∈ computedProductPolys rs_R009_ueqv_R009NNNNN_productBlocks,
    evalPoly (endpointPairAssignment pointOf .u .v) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointPairAssignment pointOf .u .v) rs_R009_ueqv_R009NNNNN_productBlocks ?_
  simpa [rs_R009_ueqv_R009NNNNN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s2_028")
      (assignment := (.pair .u .v)) (rules := rs_R009_ueqv_R009NNNNN_rules)
      rs_R009_ueqv_R009NNNNN_rules_check hmetric hrow hmasks hpidEq)
