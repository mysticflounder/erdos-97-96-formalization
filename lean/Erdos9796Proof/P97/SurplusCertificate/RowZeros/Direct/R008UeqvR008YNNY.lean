...
private theorem rs_R008_ueqv_R008YNNY_rules_check :
    rulesValidForPid "s2_013" (.pair .u .v)
      rs_R008_ueqv_R008YNNY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R008_ueqv_R008YNNY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s2_013"] = [exactRow.pid]) :
...
theorem rs_R008_ueqv_R008YNNY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R008_ueqv_R008YNNY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s2_013")
      (assignment := (.pair .u .v)) (rules := rs_R008_ueqv_R008YNNY_rules)
      rs_R008_ueqv_R008YNNY_rules_check hmetric hrow hmasks hpidEq)
