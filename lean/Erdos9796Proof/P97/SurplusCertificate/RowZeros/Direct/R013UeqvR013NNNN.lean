...
private theorem rs_R013_ueqv_R013NNNN_rules_check :
    rulesValidForPid "s3_016" (.pair .u .v)
      rs_R013_ueqv_R013NNNN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R013_ueqv_R013NNNN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s3_016"] = [exactRow.pid]) :
...
theorem rs_R013_ueqv_R013NNNN_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R013_ueqv_R013NNNN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s3_016")
      (assignment := (.pair .u .v)) (rules := rs_R013_ueqv_R013NNNN_rules)
      rs_R013_ueqv_R013NNNN_rules_check hmetric hrow hmasks hpidEq)
