...
private theorem rs_R014_ueqv_R014NNN_rules_check :
    rulesValidForPid "s3_022" (.pair .u .v)
      rs_R014_ueqv_R014NNN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R014_ueqv_R014NNN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s3_022"] = [exactRow.pid]) :
...
theorem rs_R014_ueqv_R014NNN_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R014_ueqv_R014NNN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s3_022")
      (assignment := (.pair .u .v)) (rules := rs_R014_ueqv_R014NNN_rules)
      rs_R014_ueqv_R014NNN_rules_check hmetric hrow hmasks hpidEq)
