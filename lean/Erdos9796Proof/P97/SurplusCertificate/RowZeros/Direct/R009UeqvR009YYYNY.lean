...
private theorem rs_R009_ueqv_R009YYYNY_rules_check :
    rulesValidForPid "s2_031" (.pair .u .v)
      rs_R009_ueqv_R009YYYNY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R009_ueqv_R009YYYNY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s2_031"] = [exactRow.pid]) :
...
theorem rs_R009_ueqv_R009YYYNY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R009_ueqv_R009YYYNY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s2_031")
      (assignment := (.pair .u .v)) (rules := rs_R009_ueqv_R009YYYNY_rules)
      rs_R009_ueqv_R009YYYNY_rules_check hmetric hrow hmasks hpidEq)
