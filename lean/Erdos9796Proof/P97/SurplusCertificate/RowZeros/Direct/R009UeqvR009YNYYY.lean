...
private theorem rs_R009_ueqv_R009YNYYY_rules_check :
    rulesValidForPid "s2_041" (.pair .u .v)
      rs_R009_ueqv_R009YNYYY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R009_ueqv_R009YNYYY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s2_041"] = [exactRow.pid]) :
...
theorem rs_R009_ueqv_R009YNYYY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R009_ueqv_R009YNYYY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s2_041")
      (assignment := (.pair .u .v)) (rules := rs_R009_ueqv_R009YNYYY_rules)
      rs_R009_ueqv_R009YNYYY_rules_check hmetric hrow hmasks hpidEq)
