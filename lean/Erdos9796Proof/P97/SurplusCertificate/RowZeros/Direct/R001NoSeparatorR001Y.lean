...
private theorem rs_R001_no_separator_R001Y_rules_check :
    rulesValidForPid "s1_000" .s1s3
      rs_R001_no_separator_R001Y_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R001_no_separator_R001Y_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_000"] = [exactRow.pid]) :
...
theorem rs_R001_no_separator_R001Y_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R001_no_separator_R001Y_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_000")
      (assignment := .s1s3) (rules := rs_R001_no_separator_R001Y_rules)
      rs_R001_no_separator_R001Y_rules_check hmetric hrow hmasks hpidEq)
