...
private theorem rs_R004_ueqv_R004NNY_rules_check :
    rulesValidForPid "s1_033" (.pair .u .v)
      rs_R004_ueqv_R004NNY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R004_ueqv_R004NNY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_033"] = [exactRow.pid]) :
...
theorem rs_R004_ueqv_R004NNY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R004_ueqv_R004NNY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_033")
      (assignment := (.pair .u .v)) (rules := rs_R004_ueqv_R004NNY_rules)
      rs_R004_ueqv_R004NNY_rules_check hmetric hrow hmasks hpidEq)
