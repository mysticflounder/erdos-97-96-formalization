...
private theorem rs_R005_ueqv_R005YNNY_rules_check :
    rulesValidForPid "s1_018" (.pair .u .v)
      rs_R005_ueqv_R005YNNY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R005_ueqv_R005YNNY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_018"] = [exactRow.pid]) :
...
theorem rs_R005_ueqv_R005YNNY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R005_ueqv_R005YNNY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_018")
      (assignment := (.pair .u .v)) (rules := rs_R005_ueqv_R005YNNY_rules)
      rs_R005_ueqv_R005YNNY_rules_check hmetric hrow hmasks hpidEq)
