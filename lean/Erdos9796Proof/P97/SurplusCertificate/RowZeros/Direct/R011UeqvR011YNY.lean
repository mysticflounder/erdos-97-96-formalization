...
private theorem rs_R011_ueqv_R011YNY_rules_check :
    rulesValidForPid "s2_055" (.pair .u .v)
      rs_R011_ueqv_R011YNY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R011_ueqv_R011YNY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s2_055"] = [exactRow.pid]) :
...
theorem rs_R011_ueqv_R011YNY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R011_ueqv_R011YNY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s2_055")
      (assignment := (.pair .u .v)) (rules := rs_R011_ueqv_R011YNY_rules)
      rs_R011_ueqv_R011YNY_rules_check hmetric hrow hmasks hpidEq)
