...
private theorem rs_R014_ueqv_R014NY_rules_check :
    rulesValidForPid "s3_018" (.pair .u .v)
      rs_R014_ueqv_R014NY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R014_ueqv_R014NY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s3_018"] = [exactRow.pid]) :
...
theorem rs_R014_ueqv_R014NY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R014_ueqv_R014NY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s3_018")
      (assignment := (.pair .u .v)) (rules := rs_R014_ueqv_R014NY_rules)
      rs_R014_ueqv_R014NY_rules_check hmetric hrow hmasks hpidEq)
