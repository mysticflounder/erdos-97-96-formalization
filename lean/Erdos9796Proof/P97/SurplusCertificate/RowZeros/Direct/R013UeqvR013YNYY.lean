...
private theorem rs_R013_ueqv_R013YNYY_rules_check :
    rulesValidForPid "s3_001" (.pair .u .v)
      rs_R013_ueqv_R013YNYY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R013_ueqv_R013YNYY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s3_001"] = [exactRow.pid]) :
...
theorem rs_R013_ueqv_R013YNYY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R013_ueqv_R013YNYY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s3_001")
      (assignment := (.pair .u .v)) (rules := rs_R013_ueqv_R013YNYY_rules)
      rs_R013_ueqv_R013YNYY_rules_check hmetric hrow hmasks hpidEq)
