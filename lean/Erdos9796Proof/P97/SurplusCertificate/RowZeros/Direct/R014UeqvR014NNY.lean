...
private theorem rs_R014_ueqv_R014NNY_rules_check :
    rulesValidForPid "s3_020" (.pair .u .v)
      rs_R014_ueqv_R014NNY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R014_ueqv_R014NNY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s3_020"] = [exactRow.pid]) :
...
theorem rs_R014_ueqv_R014NNY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R014_ueqv_R014NNY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s3_020")
      (assignment := (.pair .u .v)) (rules := rs_R014_ueqv_R014NNY_rules)
      rs_R014_ueqv_R014NNY_rules_check hmetric hrow hmasks hpidEq)
