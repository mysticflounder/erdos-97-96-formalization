...
private theorem rs_R002_ueqv_R002NYN_rules_check :
    rulesValidForPid "s1_013" (.pair .u .v)
      rs_R002_ueqv_R002NYN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R002_ueqv_R002NYN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_013"] = [exactRow.pid]) :
...
theorem rs_R002_ueqv_R002NYN_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R002_ueqv_R002NYN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_013")
      (assignment := (.pair .u .v)) (rules := rs_R002_ueqv_R002NYN_rules)
      rs_R002_ueqv_R002NYN_rules_check hmetric hrow hmasks hpidEq)
