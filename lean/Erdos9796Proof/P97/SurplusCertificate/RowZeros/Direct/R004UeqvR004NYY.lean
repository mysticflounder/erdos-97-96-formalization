...
private theorem rs_R004_ueqv_R004NYY_rules_check :
    rulesValidForPid "s1_029" (.pair .u .v)
      rs_R004_ueqv_R004NYY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R004_ueqv_R004NYY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_029"] = [exactRow.pid]) :
...
theorem rs_R004_ueqv_R004NYY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R004_ueqv_R004NYY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_029")
      (assignment := (.pair .u .v)) (rules := rs_R004_ueqv_R004NYY_rules)
      rs_R004_ueqv_R004NYY_rules_check hmetric hrow hmasks hpidEq)
