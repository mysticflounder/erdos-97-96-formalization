...
private theorem rs_R003_ueqv_R003YN_rules_check :
    rulesValidForPid "s1_025" (.pair .u .v)
      rs_R003_ueqv_R003YN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R003_ueqv_R003YN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_025"] = [exactRow.pid]) :
...
theorem rs_R003_ueqv_R003YN_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R003_ueqv_R003YN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_025")
      (assignment := (.pair .u .v)) (rules := rs_R003_ueqv_R003YN_rules)
      rs_R003_ueqv_R003YN_rules_check hmetric hrow hmasks hpidEq)
