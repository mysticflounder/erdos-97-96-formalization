...
private theorem rs_R008_ueqv_R008NYYN_rules_check :
    rulesValidForPid "s2_002" (.pair .u .v)
      rs_R008_ueqv_R008NYYN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R008_ueqv_R008NYYN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s2_002"] = [exactRow.pid]) :
...
theorem rs_R008_ueqv_R008NYYN_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R008_ueqv_R008NYYN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s2_002")
      (assignment := (.pair .u .v)) (rules := rs_R008_ueqv_R008NYYN_rules)
      rs_R008_ueqv_R008NYYN_rules_check hmetric hrow hmasks hpidEq)
