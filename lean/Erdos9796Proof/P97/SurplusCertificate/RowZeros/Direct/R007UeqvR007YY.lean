...
private theorem rs_R007_ueqv_R007YY_rules_check :
    rulesValidForPid "s1_038" (.pair .u .v)
      rs_R007_ueqv_R007YY_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R007_ueqv_R007YY_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_038"] = [exactRow.pid]) :
...
theorem rs_R007_ueqv_R007YY_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R007_ueqv_R007YY_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_038")
      (assignment := (.pair .u .v)) (rules := rs_R007_ueqv_R007YY_rules)
      rs_R007_ueqv_R007YY_rules_check hmetric hrow hmasks hpidEq)
