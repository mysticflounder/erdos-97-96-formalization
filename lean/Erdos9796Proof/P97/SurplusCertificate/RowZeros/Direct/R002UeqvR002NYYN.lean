...
private theorem rs_R002_ueqv_R002NYYN_rules_check :
    rulesValidForPid "s1_005" (.pair .u .v)
      rs_R002_ueqv_R002NYYN_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R002_ueqv_R002NYYN_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s1_005"] = [exactRow.pid]) :
...
theorem rs_R002_ueqv_R002NYYN_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R002_ueqv_R002NYYN_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s1_005")
      (assignment := (.pair .u .v)) (rules := rs_R002_ueqv_R002NYYN_rules)
      rs_R002_ueqv_R002NYYN_rules_check hmetric hrow hmasks hpidEq)
