...
private theorem rs_R012_ueqv_R012_rules_check :
    rulesValidForPid "s3_000" (.pair .u .v)
      rs_R012_ueqv_R012_rules = true := by
  native_decide

/-- The matched exact row has the generated singleton exact pid. -/
private theorem rs_R012_ueqv_R012_exactRow_pid_eq
    {exactRow : SurplusCOMPGBank.Row}
    (hpid : ["s3_000"] = [exactRow.pid]) :
...
theorem rs_R012_ueqv_R012_evaluationZeros_of_metricShadow
  ...
  simpa [rs_R012_ueqv_R012_generators, RowAssignment.evaluation] using
    (evaluationZeros_of_rulesValidForPid
      (pointOf := pointOf) (centerClass := centerClass)
      (exactRow := exactRow) (pid := "s3_000")
      (assignment := (.pair .u .v)) (rules := rs_R012_ueqv_R012_rules)
      rs_R012_ueqv_R012_rules_check hmetric hrow hmasks hpidEq)
