...
/-!
# Lower-branch `C₁`-hit forcing for the `y = a₃` Form-`c` tree

This is the `C₂ ↔ C₃`, `v₂ ↔ v₃`, `I₂ ↔ I₃` mirror of
`N4dFormBLowerForcesV3.lean`.  Under Form `c` at `v₁` with a `4`-class,
both `a₃` and any named `C₁` candidate `p ∈ {a₁,b₁}` of the `v₃` selector
lie on `circle(v₁, r) ∩ circle(v₃, s)`; the same chord-frame computation on
`u = v₃ - v₁` forces the named candidate to be `v₂`.

No new geometric input is used here.  The proof reuses the same
apex-generic ingredients as the Form-`b` source: the two relevant Moser
apices on the MEC boundary, disk containment for `A`, the non-Moser
one-cap count, `v₃ ∉ C₃`, and pure chord-frame algebra from `0 < s`.
-/
...
/-- Local mirror of `SharedCirclePointForcesV3`: a named `C₁` candidate of the
`v₃` selector class that lies on the `v₁` witness circle is forced to be
`v₂`. -/
...
-- degree-3 monomials (same budget as `sharedCirclePointForcesV3_of_formB_lower`)
/-- Unequal-radius shared-circle forcing: under Form `c` at `v₁` with the
`y = a₃` side pin, any named `C₁` candidate of the `v₃` selector class that
lies on both witness circles must be `v₂`.  This is the coordinate-swap mirror
of `sharedCirclePointForcesV3_of_formB_lower`. -/
...
/-- In the `y = a₃` lower-side branches of `selectorShape_v3_split`, the named
`C₁` witness of the `v₃` selector is forced to be `v₂`. -/
