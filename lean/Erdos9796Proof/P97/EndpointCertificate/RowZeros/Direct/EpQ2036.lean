...
/-!
# Endpoint row-zero certificate ep_Q2_036

This generated module proves that the direct certificate payload for endpoint
row `ep_Q2_036` vanishes under the endpoint normal-axis assignment attached to any
...
/-- Finite endpoint row paired with direct certificate `ep_Q2_036`. -/
private def ep_Q2_036_row : ShadowBank.EndpointRow :=
  ShadowBank.endpointRowsQ2.get (Fin.mk 36 (by decide))

set_option linter.style.nativeDecide false in
/-- Every generator in direct endpoint certificate `ep_Q2_036` evaluates to zero
under a metric interpretation of its finite shadow. -/
theorem ep_Q2_036_evaluationZeros_of_metricShadow
    {pointOf : ShadowBank.Label → ℝ²}
    (hmetric : EndpointMetricShadow pointOf ep_Q2_036_row.toShadow) :
    Patterns.CertificatePayload.evaluationZeros (.direct Patterns.ep_Q2_036)
      (endpointS1S3Assignment pointOf) := by
  dsimp [Patterns.CertificatePayload.evaluationZeros, Patterns.ep_Q2_036]
  intro g hg
  simp only [Patterns.ep_Q2_036_generators, List.mem_cons, List.not_mem_nil,
    or_false] at hg
  ...
  · exact evalPoly_eq_zero_of_normalizePoly_eq
          ...
          (by
            exact evalPoly_endpoint_twiceVarMinusOnePoly_eq_zero_of_metricShadow
              ...
              (by decide) (by decide))
  · exact evalPoly_eq_zero_of_normalizePoly_eq
          (endpointS1S3Assignment pointOf) (q := sqNormFirstMinusSqDistPoly EndpointVar.s1x.index EndpointVar.s1y.index EndpointVar.pux.index EndpointVar.puy.index)
          (by native_decide)
          (by
