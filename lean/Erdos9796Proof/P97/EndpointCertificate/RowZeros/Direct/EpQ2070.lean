...
/-!
# Endpoint row-zero certificate ep_Q2_070

This generated module proves that the direct certificate payload for endpoint
row `ep_Q2_070` vanishes under the endpoint normal-axis assignment attached to any
...
/-- Finite endpoint row paired with direct certificate `ep_Q2_070`. -/
private def ep_Q2_070_row : ShadowBank.EndpointRow :=
  ShadowBank.endpointRowsQ2.get (Fin.mk 70 (by decide))

set_option linter.style.nativeDecide false in
/-- Every generator in direct endpoint certificate `ep_Q2_070` evaluates to zero
under a metric interpretation of its finite shadow. -/
theorem ep_Q2_070_evaluationZeros_of_metricShadow
    {pointOf : ShadowBank.Label → ℝ²}
    (hmetric : EndpointMetricShadow pointOf ep_Q2_070_row.toShadow) :
    Patterns.CertificatePayload.evaluationZeros (.direct Patterns.ep_Q2_070)
      (endpointS1S3Assignment pointOf) := by
  dsimp [Patterns.CertificatePayload.evaluationZeros, Patterns.ep_Q2_070]
  intro g hg
  simp only [Patterns.ep_Q2_070_generators, List.mem_cons, List.not_mem_nil,
    or_false] at hg
