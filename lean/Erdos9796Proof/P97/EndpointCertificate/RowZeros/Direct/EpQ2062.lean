...
/-!
# Endpoint row-zero certificate ep_Q2_062

This generated module proves that the direct certificate payload for endpoint
row `ep_Q2_062` vanishes under the endpoint normal-axis assignment attached to any
metric interpretation of the row shadow.

Source certificate: `certificates/endpoint/ep_Q2_062.json`.
-/
...
/-- Finite endpoint row paired with direct certificate `ep_Q2_062`. -/
private def ep_Q2_062_row : ShadowBank.EndpointRow :=
  ShadowBank.endpointRowsQ2.get (Fin.mk 62 (by decide))

set_option linter.style.nativeDecide false in
/-- Every generator in direct endpoint certificate `ep_Q2_062` evaluates to zero
under a metric interpretation of its finite shadow. -/
theorem ep_Q2_062_evaluationZeros_of_metricShadow
    {pointOf : ShadowBank.Label → ℝ²}
    (hmetric : EndpointMetricShadow pointOf ep_Q2_062_row.toShadow) :
    Patterns.CertificatePayload.evaluationZeros (.direct Patterns.ep_Q2_062)
      (endpointS1S3Assignment pointOf) := by
  dsimp [Patterns.CertificatePayload.evaluationZeros, Patterns.ep_Q2_062]
  intro g hg
  simp only [Patterns.ep_Q2_062_generators, List.mem_cons, List.not_mem_nil,
    or_false] at hg
