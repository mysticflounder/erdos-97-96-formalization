...
/-!
# Endpoint row-zero certificate ep_Q2_037

This generated module proves that the direct certificate payload for endpoint
row `ep_Q2_037` vanishes under the endpoint normal-axis assignment attached to any
...
/-- Finite endpoint row paired with direct certificate `ep_Q2_037`. -/
private def ep_Q2_037_row : ShadowBank.EndpointRow :=
  ShadowBank.endpointRowsQ2.get (Fin.mk 37 (by decide))

set_option linter.style.nativeDecide false in
/-- Every generator in direct endpoint certificate `ep_Q2_037` evaluates to zero
under a metric interpretation of its finite shadow. -/
theorem ep_Q2_037_evaluationZeros_of_metricShadow
    {pointOf : ShadowBank.Label → ℝ²}
    (hmetric : EndpointMetricShadow pointOf ep_Q2_037_row.toShadow) :
    Patterns.CertificatePayload.evaluationZeros (.direct Patterns.ep_Q2_037)
      (endpointS1S3Assignment pointOf) := by
  dsimp [Patterns.CertificatePayload.evaluationZeros, Patterns.ep_Q2_037]
  intro g hg
  simp only [Patterns.ep_Q2_037_generators, List.mem_cons, List.not_mem_nil,
    or_false] at hg
  ...
  · exact evalPoly_eq_zero_of_normalizePoly_eq
          ...
          (by
            exact evalPoly_endpoint_sqDistToCenterDiffPoly_eq_zero_of_metricShadow
              ...
              (by decide) (by decide))
  · exact evalPoly_eq_zero_of_normalizePoly_eq
          (endpointS1S3Assignment pointOf) (q := sqDistToCenterDiffPoly EndpointVar.s2x.index EndpointVar.s2y.index EndpointVar.s1x.index EndpointVar.s1y.index EndpointVar.pux.index EndpointVar.puy.index)
          (by native_decide)
          (by
            exact evalPoly_endpoint_sqDistToCenterDiffPoly_eq_zero_of_metricShadow
              (center := .s2) (a := .s1)
