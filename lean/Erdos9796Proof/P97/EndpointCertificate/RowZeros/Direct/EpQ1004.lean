...
/-!
# Endpoint row-zero certificate ep_Q1_004

This generated module proves that the direct certificate payload for endpoint
row `ep_Q1_004` vanishes under the endpoint normal-axis assignment attached to any
...
/-- Finite endpoint row paired with direct certificate `ep_Q1_004`. -/
private def ep_Q1_004_row : ShadowBank.EndpointRow :=
  ShadowBank.endpointRowsQ1.get (Fin.mk 4 (by decide))

set_option linter.style.nativeDecide false in
/-- Every generator in direct endpoint certificate `ep_Q1_004` evaluates to zero
under a metric interpretation of its finite shadow. -/
theorem ep_Q1_004_evaluationZeros_of_metricShadow
    {pointOf : ShadowBank.Label → ℝ²}
    (hmetric : EndpointMetricShadow pointOf ep_Q1_004_row.toShadow) :
    Patterns.CertificatePayload.evaluationZeros (.direct Patterns.ep_Q1_004)
      (endpointS1S3Assignment pointOf) := by
  dsimp [Patterns.CertificatePayload.evaluationZeros, Patterns.ep_Q1_004]
  intro g hg
  simp only [Patterns.ep_Q1_004_generators, List.mem_cons, List.not_mem_nil,
    or_false] at hg
