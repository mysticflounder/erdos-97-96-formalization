...
/-!
# Endpoint computed product row-zero certificate ep_Q2_008
...
private def ep_Q2_008_row : ShadowBank.EndpointRow :=
  ShadowBank.endpointRowsQ2.get (Fin.mk 8 (by decide))

set_option linter.style.nativeDecide false in
/-- Every computed product in endpoint certificate `ep_Q2_008` evaluates to zero
under a metric interpretation of its finite shadow. -/
theorem ep_Q2_008_evaluationZeros_of_metricShadow
    {pointOf : ShadowBank.Label → ℝ²}
    (hmetric : EndpointMetricShadow pointOf ep_Q2_008_row.toShadow) :
    Patterns.CertificatePayload.evaluationZeros
      (.productSum Patterns.ep_Q2_008_blocks)
      (endpointS1S3Assignment pointOf) := by
  change ∀ p ∈ computedProductPolys Patterns.ep_Q2_008_productBlocks,
    evalPoly (endpointS1S3Assignment pointOf) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointS1S3Assignment pointOf) Patterns.ep_Q2_008_productBlocks ?_
  intro g hg
  simp only [Patterns.EpQ2008Generators.ep_Q2_008_generators,
