...
/-!
# Endpoint computed product row-zero certificate ep_Q1_009
...
private def ep_Q1_009_row : ShadowBank.EndpointRow :=
  ShadowBank.endpointRowsQ1.get (Fin.mk 9 (by decide))

set_option linter.style.nativeDecide false in
/-- Every computed product in endpoint certificate `ep_Q1_009` evaluates to zero
under a metric interpretation of its finite shadow. -/
theorem ep_Q1_009_evaluationZeros_of_metricShadow
    {pointOf : ShadowBank.Label → ℝ²}
    (hmetric : EndpointMetricShadow pointOf ep_Q1_009_row.toShadow) :
    Patterns.CertificatePayload.evaluationZeros
      (.productSum Patterns.ep_Q1_009_blocks)
      (endpointS1S3Assignment pointOf) := by
  change ∀ p ∈ computedProductPolys Patterns.ep_Q1_009_productBlocks,
    evalPoly (endpointS1S3Assignment pointOf) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointS1S3Assignment pointOf) Patterns.ep_Q1_009_productBlocks ?_
  intro g hg
  simp only [Patterns.EpQ1009Generators.ep_Q1_009_generators,
