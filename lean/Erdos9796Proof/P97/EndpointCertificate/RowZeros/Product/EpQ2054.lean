...
/-!
# Endpoint computed product row-zero certificate ep_Q2_054
...
private def ep_Q2_054_row : ShadowBank.EndpointRow :=
  ShadowBank.endpointRowsQ2.get (Fin.mk 54 (by decide))

set_option linter.style.nativeDecide false in
/-- Every computed product in endpoint certificate `ep_Q2_054` evaluates to zero
under a metric interpretation of its finite shadow. -/
theorem ep_Q2_054_evaluationZeros_of_metricShadow
    {pointOf : ShadowBank.Label → ℝ²}
    (hmetric : EndpointMetricShadow pointOf ep_Q2_054_row.toShadow) :
    Patterns.CertificatePayload.evaluationZeros
      (.productSum Patterns.ep_Q2_054_blocks)
      (endpointS1S3Assignment pointOf) := by
  change ∀ p ∈ computedProductPolys Patterns.ep_Q2_054_productBlocks,
    evalPoly (endpointS1S3Assignment pointOf) p = 0
  refine evaluationZeros_of_computedProductBlocks
    (endpointS1S3Assignment pointOf) Patterns.ep_Q2_054_productBlocks ?_
  intro g hg
  simp only [Patterns.EpQ2054Generators.ep_Q2_054_generators,
