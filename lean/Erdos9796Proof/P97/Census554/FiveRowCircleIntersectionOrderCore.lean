...
import Erdos9796Proof.P97.Census554.FiveRowCircleIntersectionGeometry

/-!
# Equality-closure consumer for a five-row circle-intersection order pattern

This file packages the metric hypotheses of
`FiveRowCircleIntersectionGeometry.false_of_five_row_metric_data` as a
label-generic `EqualityCore.EdgeClosure` witness.  The result depends only
on five selected rows and the six orientations implied by the cyclic spine
`O,A,D,E,C`; it uses neither exactness nor a minimum enclosing circle.
-/
...
/-- The ten equality-closure relations used by the five-row geometry
consumer.  The auxiliary labels `X5`, `X6`, and `X9` name the three
circle-intersection witnesses. -/
...
theorem false_of_core
  ...
  exact FiveRowCircleIntersectionGeometry.false_of_five_row_metric_data
