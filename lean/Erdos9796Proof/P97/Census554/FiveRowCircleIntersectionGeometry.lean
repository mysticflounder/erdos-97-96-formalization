...
import Erdos9796Proof.P97.Census554.CircleIntersectionInequalityCore

/-!
# Five-row circle-intersection geometry

This file turns five-row metric equalities and the strict cyclic order
`O,A,D,E,C` into three scalar chord inequalities.  The proof uses neither a
minimum enclosing circle nor an exact-row hypothesis: the common `O`-circle,
the equilateral triangle `OAC`, and the cyclic subsequence already force the
minor-arc parametrisation.
-/
...
namespace Problem97
namespace Census554
namespace FiveRowCircleIntersectionGeometry
...
/-- Exact metric/order data extracted from the five rows.  The six
...
theorem false_of_five_row_metric_data
  ...
  exact
    CircleIntersectionInequalityCore.angle_lower_chord_margins_incompatible
...
end FiveRowCircleIntersectionGeometry
end Census554
end Problem97
