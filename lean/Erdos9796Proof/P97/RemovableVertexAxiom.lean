...
scaffold has removed the `(0,1,3,0)` row. -/
...
discharged the cheap cross-separation left-right rows, one same-side-heavy
row, and terminal seed input.
-/
...
theorem rightNonSurplusSameSideHeavyRowsExcluded_of_finiteResidualRows
  ...
  rcases hrows with ⟨h0004, h1003, h1102, h2002⟩
  exact
    ⟨h0004,
...
theorem leftNonSurplusSameSideHeavyRowsExcluded_of_finiteResidualRows
  ...
  rcases hrows with ⟨h0040, h1030, h1120, h2020⟩
  exact
    ⟨h0040,
