...
Status: `ON-SPINE` since 2026-08-05, via
`ATail.RetainedMatchingSourceReturnRadiusSplit` →
`ATail.PairedCommonDeletionNormalForm`: the paired arm of the E1
retained-omission core is built as a `RetainedMatchingTwoStepCommonDeletionWalk`
with `nextLocation := .sourceReturn`.  The endpoint-classification theorems
(`nonempty_twoStepEndpointOutcome` and the large-cap outcome adapters) remain
bank-only.
...
namespace Problem97
namespace ATailRetainedMatchingCommonDeletionCycle
...
/-- A consumed large-cap outcome already contains all data needed for the
source-tagged two-step endpoint normal form. -/
theorem nonempty_matchingCommonDeletionCycleNormalForm_of_largeCapOutcome
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : FrontierCommonDeletionParentResidual F}
...
theorem nonempty_matchingCommonDeletionCycleNormalForm
  ...
  exact nonempty_matchingCommonDeletionCycleNormalForm_of_largeCapOutcome L


end ATailRetainedMatchingCommonDeletionCycle
end Problem97
