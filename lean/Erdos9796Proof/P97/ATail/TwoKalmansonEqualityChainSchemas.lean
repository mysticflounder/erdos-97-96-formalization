...
/-!
# Two-Kalmanson equality-chain schemas

This module banks the cardinality-independent obstruction extracted from the
exact-seventeen PIQD survivor after the perpendicular-bisector-chain
refinement.  Five selected rows transport the four left-hand distances of two
strict Kalmanson inequalities onto their four right-hand distances.

For seven vertices in the conceptual cyclic order `A < B < C < D < E < F < G`,
the positive incidences are

```text
C : B,D    D : C,A    A : D,F,E    F : A,B    G : E,C.
```

The first four rows imply `BC = BF`, the row at `A` also gives `AF = AE`, and
the row at `G` gives `EG = CG`.  These equalities cancel the strict Kalmanson
inequalities on `A,B,C,F` and `A,C,E,G`.
-/
...
/-- The five-row two-Kalmanson equality chain is impossible when its seven
named vertices occur in increasing boundary order. -/
theorem false_of_five_selected_rows_two_kalmanson_chain
  ...
  have hK1 := dist_add_dist_lt_diagonal_sum_of_ccw
    hcarrier hboundaryInjective hboundaryImage hboundaryCcw
      hab hbc (hcd.trans (hde.trans hef))
  have hK2 := complementary_dist_add_dist_lt_diagonal_sum_of_ccw
    hcarrier hboundaryInjective hboundaryImage hboundaryCcw
      (hab.trans hbc) (hcd.trans hde) (hef.trans hfg)
  simp only [dist_comm] at hC hD hAD hAE hF hG hK1 hK2
  linarith

/-- Decreasing-index companion of
`false_of_five_selected_rows_two_kalmanson_chain`. -/
theorem false_of_five_selected_rows_two_kalmanson_chain_of_decreasing
    {carrier : Finset ℝ²} (hcarrier : ConvexIndep carrier)
    {boundary : Fin carrier.card → ℝ²}
    (hboundaryInjective : Function.Injective boundary)
    (hboundaryImage : Finset.univ.image boundary = carrier)
  ...
  have hK1 := dist_add_dist_lt_diagonal_sum_of_ccw
    hcarrier hboundaryInjective hboundaryImage hboundaryCcw
      (hfe.trans (hed.trans hdc)) hcb hba
  have hK2 := complementary_dist_add_dist_lt_diagonal_sum_of_ccw
    hcarrier hboundaryInjective hboundaryImage hboundaryCcw
      (hgf.trans hfe) (hed.trans hdc) (hcb.trans hba)
  simp only [dist_comm] at hC hD hAD hAE hF hG hK1 hK2
  linarith

/-- Cyclic-shift form of the increasing two-Kalmanson equality-chain
consumer. -/
theorem false_of_five_selected_rows_two_kalmanson_chain_cyclicShift
    {carrier : Finset ℝ²} (hcarrier : ConvexIndep carrier)
    {boundary : Fin carrier.card → ℝ²}
    (hboundaryInjective : Function.Injective boundary)
    (hboundaryImage : Finset.univ.image boundary = carrier)
  ...
  exact false_of_five_selected_rows_two_kalmanson_chain hcarrier
    ...
    hab hbc hcd hde hef hfg CRow DRow ARow FRow GRow
    hbC hdC hcD haD hdA hfA heA haF hbF heG hcG

/-- Cyclic-shift form of the decreasing two-Kalmanson equality-chain
consumer. -/
theorem false_of_five_selected_rows_two_kalmanson_chain_cyclicShift_of_decreasing
    {carrier : Finset ℝ²} (hcarrier : ConvexIndep carrier)
    {boundary : Fin carrier.card → ℝ²}
    (hboundaryInjective : Function.Injective boundary)
    (hboundaryImage : Finset.univ.image boundary = carrier)
    ...
    (hcG : boundary (c + cut) ∈ GRow.support) : False := by
  let shifted : Fin carrier.card → ℝ² := fun i => boundary (i + cut)
  exact false_of_five_selected_rows_two_kalmanson_chain_of_decreasing hcarrier
    (by simpa only [shifted] using injective_cyclicShift hboundaryInjective cut)
    (by simpa only [shifted] using
      (image_univ_cyclicShift boundary cut).trans hboundaryImage)
    (by simpa only [shifted] using
      isCcwConvexPolygon_cyclicShift hboundaryInjective hboundaryCcw cut)
    hba hcb hdc hed hfe hgf CRow DRow ARow FRow GRow
    hbC hdC hcD haD hdA hfA heA haF hbF heG hcG

#print axioms false_of_five_selected_rows_two_kalmanson_chain
#print axioms false_of_five_selected_rows_two_kalmanson_chain_of_decreasing
#print axioms false_of_five_selected_rows_two_kalmanson_chain_cyclicShift
#print axioms false_of_five_selected_rows_two_kalmanson_chain_cyclicShift_of_decreasing
