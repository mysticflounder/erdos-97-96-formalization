/-
Copyright (c) 2026 Adam McKenna. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE
Authors: Adam McKenna
-/

import Erdos9796Proof.P97.ATail.BlockerVExactSeventeenThirtyNinthModelRefinements
import Erdos9796Proof.P97.ListCNF

/-! # Child41 two-Kalmanson cancellation bank, shard 1

This shard contains seven source-checked subset-minimal occurrences mined from
the authenticated Child40 SAT model.
-/

open scoped EuclideanGeometry
namespace Problem97
namespace ATailBlockerVExactSeventeenFortiethModelRefinementsShard1
open ATailBlockerVExactSeventeenSourceNormalForm
open ATailBlockerVExactSeventeenSourceCnf
open ATailBlockerVExactSeventeenSourceCnfCdefg
open ATailFrontierLiveClosure.GenericRowNogoodCertificate
open ATailBlockerVExactSeventeenTwentyEighthModelRefinements
/-- P97 ATail BlockerVExactSeventeenFortiethModelRefinementsShard1 abbrev. -/
private abbrev occurrenceClauses :=
  ATailBlockerVExactSeventeenSeventeenthModelRefinements.occurrenceClauses

/-- P97 ATail BlockerVExactSeventeenFortiethModelRefinementsShard1 def. -/
def cancellationOccurrences : List CancellationOccurrence :=
[
  { hits := [(9, 8), (9, 14), (11, 8), (11, 15), (12, 14), (12, 15)]
    forwardChoices := [{ center := 9, support := {8, 14} },
        { center := 11, support := {8, 15} },
        { center := 12, support := {14, 15} }]
    reverseChoices := [{ center := 4, support := {1, 2} },
        { center := 5, support := {1, 8} },
        { center := 7, support := {2, 8} }]
    forwardData := {
      firstQuad := ⟨8, 9, 11, 14⟩
      firstForm := .adjacentSides
      secondQuad := ⟨11, 12, 14, 15⟩
      secondForm := .innerOuter
      permutation := ⟨1, 2, 3, 0⟩
      path0 := ⟨(8, 9), [.flip 8 9, .row 9 8 14], (9, 14)⟩
      path1 := ⟨(11, 14), [], (11, 14)⟩
      path2 := ⟨(12, 14), [.row 12 14 15], (12, 15)⟩
      path3 := ⟨(11, 15), [.row 11 15 8, .flip 11 8], (8, 11)⟩ }
    reverseData := {
      firstQuad := ⟨2, 5, 7, 8⟩
      firstForm := .adjacentSides
      secondQuad := ⟨1, 2, 4, 5⟩
      secondForm := .innerOuter
      permutation := ⟨3, 0, 2, 1⟩
      path0 := ⟨(2, 5), [], (2, 5)⟩
      path1 := ⟨(7, 8), [.row 7 8 2, .flip 7 2], (2, 7)⟩
      path2 := ⟨(2, 4), [.flip 2 4, .row 4 2 1, .flip 4 1], (1, 4)⟩
      path3 := ⟨(1, 5), [.flip 1 5, .row 5 1 8], (5, 8)⟩ } },
  { hits := [(10, 1), (10, 3), (14, 0), (14, 1), (16, 0), (16, 3)]
    forwardChoices := [{ center := 10, support := {1, 3} },
        { center := 14, support := {0, 1} },
        { center := 16, support := {0, 3} }]
    reverseChoices := [{ center := 0, support := {13, 16} },
        { center := 2, support := {15, 16} },
        { center := 6, support := {13, 15} }]
    forwardData := {
      firstQuad := ⟨0, 3, 14, 16⟩
      firstForm := .innerOuter
      secondQuad := ⟨1, 3, 10, 14⟩
      secondForm := .innerOuter
      permutation := ⟨3, 1, 2, 0⟩
      path0 := ⟨(3, 14), [], (3, 14)⟩
      path1 := ⟨(0, 16), [.flip 0 16, .row 16 0 3, .flip 16 3], (3, 16)⟩
      path2 := ⟨(3, 10), [.flip 3 10, .row 10 3 1, .flip 10 1], (1, 10)⟩
      path3 := ⟨(1, 14), [.flip 1 14, .row 14 1 0, .flip 14 0], (0, 14)⟩ }
    reverseData := {
      firstQuad := ⟨0, 2, 13, 16⟩
      firstForm := .innerOuter
      secondQuad := ⟨2, 6, 13, 15⟩
      secondForm := .innerOuter
      permutation := ⟨2, 0, 3, 1⟩
      path0 := ⟨(2, 13), [], (2, 13)⟩
      path1 := ⟨(0, 16), [.row 0 16 13], (0, 13)⟩
      path2 := ⟨(6, 13), [.row 6 13 15], (6, 15)⟩
      path3 := ⟨(2, 15), [.row 2 15 16], (2, 16)⟩ } },
  { hits := [(13, 9), (13, 16), (14, 1), (14, 9), (15, 1), (15, 16)]
    forwardChoices := [{ center := 13, support := {9, 16} },
        { center := 14, support := {1, 9} },
        { center := 15, support := {1, 16} }]
    reverseChoices := [{ center := 1, support := {0, 15} },
        { center := 2, support := {7, 15} },
        { center := 3, support := {0, 7} }]
    forwardData := {
      firstQuad := ⟨9, 13, 14, 16⟩
      firstForm := .adjacentSides
      secondQuad := ⟨1, 14, 15, 16⟩
      secondForm := .adjacentSides
      permutation := ⟨1, 3, 0, 2⟩
      path0 := ⟨(9, 13), [.flip 9 13, .row 13 9 16], (13, 16)⟩
      path1 := ⟨(14, 16), [], (14, 16)⟩
      path2 := ⟨(1, 14), [.flip 1 14, .row 14 1 9, .flip 14 9], (9, 14)⟩
      path3 := ⟨(15, 16), [.row 15 16 1, .flip 15 1], (1, 15)⟩ }
    reverseData := {
      firstQuad := ⟨0, 1, 3, 15⟩
      firstForm := .adjacentSides
      secondQuad := ⟨2, 3, 7, 15⟩
      secondForm := .innerOuter
      permutation := ⟨1, 3, 0, 2⟩
      path0 := ⟨(0, 1), [.flip 0 1, .row 1 0 15], (1, 15)⟩
      path1 := ⟨(3, 15), [], (3, 15)⟩
      path2 := ⟨(3, 7), [.row 3 7 0, .flip 3 0], (0, 3)⟩
      path3 := ⟨(2, 15), [.row 2 15 7], (2, 7)⟩ } },
  { hits := [(14, 0), (14, 1), (15, 1), (15, 11), (16, 0), (16, 11)]
    forwardChoices := [{ center := 14, support := {0, 1} },
        { center := 15, support := {1, 11} },
        { center := 16, support := {0, 11} }]
    reverseChoices := [{ center := 0, support := {5, 16} },
        { center := 1, support := {5, 15} },
        { center := 2, support := {15, 16} }]
    forwardData := {
      firstQuad := ⟨0, 11, 15, 16⟩
      firstForm := .innerOuter
      secondQuad := ⟨0, 1, 14, 15⟩
      secondForm := .innerOuter
      permutation := ⟨3, 1, 2, 0⟩
      path0 := ⟨(11, 15), [.flip 11 15, .row 15 11 1, .flip 15 1], (1, 15)⟩
      path1 := ⟨(0, 16), [.flip 0 16, .row 16 0 11, .flip 16 11], (11, 16)⟩
      path2 := ⟨(1, 14), [.flip 1 14, .row 14 1 0, .flip 14 0], (0, 14)⟩
      path3 := ⟨(0, 15), [], (0, 15)⟩ }
    reverseData := {
      firstQuad := ⟨0, 1, 5, 15⟩
      firstForm := .innerOuter
      secondQuad := ⟨0, 2, 15, 16⟩
      secondForm := .innerOuter
      permutation := ⟨1, 2, 3, 0⟩
      path0 := ⟨(1, 5), [.row 1 5 15], (1, 15)⟩
      path1 := ⟨(0, 15), [], (0, 15)⟩
      path2 := ⟨(2, 15), [.row 2 15 16], (2, 16)⟩
      path3 := ⟨(0, 16), [.row 0 16 5], (0, 5)⟩ } },
  { hits := [(1, 4), (1, 13), (2, 1), (2, 13), (13, 4), (13, 16), (15, 1), (15, 16)]
    forwardChoices := [{ center := 1, support := {4, 13} },
        { center := 2, support := {1, 13} },
        { center := 13, support := {4, 16} },
        { center := 15, support := {1, 16} }]
    reverseChoices := [{ center := 1, support := {0, 15} },
        { center := 3, support := {0, 12} },
        { center := 14, support := {3, 15} },
        { center := 15, support := {3, 12} }]
    forwardData := {
      firstQuad := ⟨1, 2, 4, 13⟩
      firstForm := .adjacentSides
      secondQuad := ⟨1, 13, 15, 16⟩
      secondForm := .adjacentSides
      permutation := ⟨1, 3, 0, 2⟩
      path0 := ⟨(1, 2), [.flip 1 2, .row 2 1 13], (2, 13)⟩
      path1 := ⟨(4, 13), [.flip 4 13, .row 13 4 16], (13, 16)⟩
      path2 := ⟨(1, 13), [.row 1 13 4], (1, 4)⟩
      path3 := ⟨(15, 16), [.row 15 16 1, .flip 15 1], (1, 15)⟩ }
    reverseData := {
      firstQuad := ⟨0, 1, 3, 15⟩
      firstForm := .adjacentSides
      secondQuad := ⟨3, 12, 14, 15⟩
      secondForm := .adjacentSides
      permutation := ⟨1, 3, 0, 2⟩
      path0 := ⟨(0, 1), [.flip 0 1, .row 1 0 15], (1, 15)⟩
      path1 := ⟨(3, 15), [.flip 3 15, .row 15 3 12, .flip 15 12], (12, 15)⟩
      path2 := ⟨(3, 12), [.row 3 12 0, .flip 3 0], (0, 3)⟩
      path3 := ⟨(14, 15), [.row 14 15 3, .flip 14 3], (3, 14)⟩ } },
  { hits := [(2, 6), (2, 11), (4, 6), (4, 12), (11, 12), (11, 15), (12, 11), (12, 15)]
    forwardChoices := [{ center := 2, support := {6, 11} },
        { center := 4, support := {6, 12} },
        { center := 11, support := {12, 15} },
        { center := 12, support := {11, 15} }]
    reverseChoices := [{ center := 4, support := {1, 5} },
        { center := 5, support := {1, 4} },
        { center := 12, support := {4, 10} },
        { center := 14, support := {5, 10} }]
    forwardData := {
      firstQuad := ⟨4, 11, 12, 15⟩
      firstForm := .adjacentSides
      secondQuad := ⟨2, 4, 6, 11⟩
      secondForm := .innerOuter
      permutation := ⟨3, 1, 0, 2⟩
      path0 := ⟨(4, 11), [], (4, 11)⟩
      path1 := ⟨(12, 15), [.row 12 15 11, .flip 12 11, .row 11 12 15], (11, 15)⟩
      path2 := ⟨(4, 6), [.row 4 6 12], (4, 12)⟩
      path3 := ⟨(2, 11), [.row 2 11 6], (2, 6)⟩ }
    reverseData := {
      firstQuad := ⟨1, 4, 5, 12⟩
      firstForm := .adjacentSides
      secondQuad := ⟨5, 10, 12, 14⟩
      secondForm := .innerOuter
      permutation := ⟨0, 2, 1, 3⟩
      path0 := ⟨(1, 4), [.flip 1 4, .row 4 1 5, .flip 4 5, .row 5 4 1, .flip 5 1], (1, 5)⟩
      path1 := ⟨(5, 12), [], (5, 12)⟩
      path2 := ⟨(10, 12), [.flip 10 12, .row 12 10 4, .flip 12 4], (4, 12)⟩
      path3 := ⟨(5, 14), [.flip 5 14, .row 14 5 10, .flip 14 10], (10, 14)⟩ } },
  { hits := [(3, 4), (3, 16), (10, 1), (10, 3), (15, 1), (15, 16), (16, 3), (16, 4)]
    forwardChoices := [{ center := 3, support := {4, 16} },
        { center := 10, support := {1, 3} },
        { center := 15, support := {1, 16} },
        { center := 16, support := {3, 4} }]
    reverseChoices := [{ center := 0, support := {12, 13} },
        { center := 1, support := {0, 15} },
        { center := 6, support := {13, 15} },
        { center := 13, support := {0, 12} }]
    forwardData := {
      firstQuad := ⟨1, 4, 15, 16⟩
      firstForm := .adjacentSides
      secondQuad := ⟨1, 3, 4, 10⟩
      secondForm := .innerOuter
      permutation := ⟨2, 0, 1, 3⟩
      path0 := ⟨(1, 4), [], (1, 4)⟩
      path1 := ⟨(15, 16), [.row 15 16 1, .flip 15 1], (1, 15)⟩
      path2 := ⟨(3, 4), [.row 3 4 16, .flip 3 16, .row 16 3 4, .flip 16 4], (4, 16)⟩
      path3 := ⟨(1, 10), [.flip 1 10, .row 10 1 3, .flip 10 3], (3, 10)⟩ }
    reverseData := {
      firstQuad := ⟨0, 1, 12, 15⟩
      firstForm := .adjacentSides
      secondQuad := ⟨6, 12, 13, 15⟩
      secondForm := .innerOuter
      permutation := ⟨1, 3, 0, 2⟩
      path0 := ⟨(0, 1), [.flip 0 1, .row 1 0 15], (1, 15)⟩
      path1 := ⟨(12, 15), [], (12, 15)⟩
      path2 := ⟨(12, 13), [.flip 12 13, .row 13 12 0, .flip 13 0, .row 0 13 12], (0, 12)⟩
      path3 := ⟨(6, 15), [.row 6 15 13], (6, 13)⟩ } }
]

/-- P97 ATail BlockerVExactSeventeenFortiethModelRefinementsShard1 theorem. -/
theorem cancellationOccurrences_length : cancellationOccurrences.length = 7 := by
  native_decide

/-- P97 ATail BlockerVExactSeventeenFortiethModelRefinementsShard1 theorem. -/
theorem cancellationOccurrences_all_check :
    cancellationOccurrences.all CancellationOccurrence.check = true := by
  native_decide

/-- P97 ATail BlockerVExactSeventeenFortiethModelRefinementsShard1 def. -/
def refinementClauses : ListCNF Atom :=
  cancellationOccurrences.flatMap fun occ => occurrenceClauses occ.hits

/-- P97 ATail BlockerVExactSeventeenFortiethModelRefinementsShard1 theorem. -/
theorem refinementClauses_length : refinementClauses.length = 28 := by
  native_decide

/-- P97 ATail BlockerVExactSeventeenFortiethModelRefinementsShard1 theorem. -/
theorem sourceAssign_refinementClauses {A : Finset ℝ²}
    (source : SourceRealization A) :
    ∀ clause ∈ refinementClauses,
      Std.Sat.CNF.Clause.eval (sourceAssign source.model) clause = true := by
  intro clause hclause
  simp only [refinementClauses, List.mem_flatMap] at hclause
  rcases hclause with ⟨occ, hocc, hclause⟩
  have hcheck := List.all_eq_true.mp cancellationOccurrences_all_check occ hocc
  simp only [occurrenceClauses,
    ATailBlockerVExactSeventeenSeventeenthModelRefinements.occurrenceClauses,
    List.mem_flatMap, List.mem_map] at hclause
  obtain ⟨order, _horder, direction, _hdirection, rfl⟩ := hclause
  exact sourceAssign_cancellationOccurrenceClause source occ hcheck order direction

#print axioms sourceAssign_refinementClauses
end ATailBlockerVExactSeventeenFortiethModelRefinementsShard1
end Problem97
