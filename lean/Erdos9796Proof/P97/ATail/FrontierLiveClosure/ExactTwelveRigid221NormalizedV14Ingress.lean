...
/-!
# Normalized v14 source ingress for the exact-twelve Rigid221 lane

This module transports the complete source-produced v14 packet to one of the
12 checked separated placement representatives.  It preserves the physical
row, five blocker traces, and all three v14 Boolean branch predicates, while
rederiving the safe-cube contract from the geometric source.

This is an ingress and transport theorem, not finite representative coverage
...
/-- The complete v14 source packet can be moved to one of the 12 checked
separated placement representatives without losing any finite branch data. -/
theorem exists_source_normalized_v14_physicalCycle_namedArm_distinguishedD
  ...
  obtain ⟨carrierPattern, labeling, _hsafe, hphysicalRow, hdeletedRow,
    ...
    exists_source_v14_physicalCycle_namedArm_distinguishedD
      Q hcard source hsourceOutside hsourceNamed hsurvives hcommon hmissing
    ...
    simpa [normalizedBlocker, normalized,
      ...
      FrozenRoleLabeling.relabel] using
        rowTrace_reindex labeling.e relabeling oldRow (blocker i)
          ...
          (hrowTrace i)
  have harm' : FrozenNamedDeletionSixArm newRow normalizedBlocker := by
    rw [hrowEq]
    exact harm.reindex relabeling hadmissible
  have hd' : FrozenDistinguishedDCommonMissing newRow normalizedBlocker := by
    rw [hrowEq]
    exact hd.reindex relabeling hadmissible
  have hdAway' :
      FrozenDistinguishedDCommonMissingAwayFromEight newRow
        normalizedBlocker := by
    rw [hrowEq]
    exact hdAway.reindex relabeling hadmissible
  refine ⟨carrierPattern, normalized, hrepresentative, hsafe,
    ...
    hblockerNeSource', hblockerNeSecondApex', hrowTrace', harm', hd',
    hdAway'⟩
