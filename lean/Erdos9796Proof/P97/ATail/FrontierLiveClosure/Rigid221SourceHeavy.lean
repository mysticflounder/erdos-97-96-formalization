...
/-- The deleted-blocker branch has the same four-point strict-cap order as
the off-class branch, with the blocker index replaced by the joint-deletion
index.  This is an order ingress only: no position of `v` is assumed. -/
theorem exactFourRigid221_sourceHeavy_pentagon_deleted_exists_forced_secondCap_order
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
      ...
      ((B.L.points (CGN.firstIndex B.Packet.hm) =
        ...
        (B.L.points (CGN.firstIndex B.Packet.hm) =
          ...
          B.L.points (CGN.lastIndex B.Packet.hm) =
            (S.triangleByIndex S.oppIndex2).v2)) ∧
      B.L.points iu = P.u.1 ∧
      B.L.points id = P.jointDeletion.deleted.1 ∧
      B.L.points ixv = packet.xv ∧
      B.L.points ixu = packet.xu ∧
      ((iu < id ∧ id < ixv ∧ ixv < ixu) ∨
        (ixu < ixv ∧ ixv < id ∧ id < iu)) := by
  classical
  let Hlate := lateFirstApexSystem R
  ...
  have hdeletedInterior :
      P.jointDeletion.deleted.1 ∈ S.capInteriorByIndex S.oppIndex2 := by
    simpa only [hblockerEqDeleted] using hblockerInterior
  have hD : dist P.jointDeletion.deleted.1 P.u.1 =
      dist P.jointDeletion.deleted.1 packet.xv := by
    have h := (Kxv.support_eq_radius _ huXvRow).trans
      (Kxv.support_eq_radius _ Kxv.q_mem_support).symm
    have hblockerEqDeleted' :
        Hlate.centerAt packet.xv hxvA = P.jointDeletion.deleted.1 := by
      simpa only [Hlate] using hblockerEqDeleted
    calc
      dist P.jointDeletion.deleted.1 P.u.1 =
          dist (Hlate.centerAt packet.xv hxvA) P.u.1 := by
            rw [hblockerEqDeleted']
      _ = dist (Hlate.centerAt packet.xv hxvA) packet.xv := h
      _ = dist P.jointDeletion.deleted.1 packet.xv := by
            rw [hblockerEqDeleted']
  have huNeDeleted : P.u.1 ≠ P.jointDeletion.deleted.1 := by
    intro h
    exact P.jointDeletion.deleted_ne_u (Subtype.ext h.symm)
  have hdeletedNeU : P.jointDeletion.deleted.1 ≠ P.u.1 := huNeDeleted.symm
  have hxvNotURow : packet.xv ∉ Ku.support := by
    intro h
    apply Ku.center_not_mem_support
    have hm := congrArg (fun z : ℝ² => z ∈ Ku.support) hcenterU
    exact Eq.mp hm.symm h
  have huNeXv : P.u.1 ≠ packet.xv := by
    intro h
    apply hxvNotURow
    simpa only [← h] using Ku.q_mem_support
  ...
  have hdeletedNeXu : P.jointDeletion.deleted.1 ≠ packet.xu := by
    ...
    apply P.jointDeletion.deleted_not_mem_uRow
    simpa only [h] using hxuURow
  have hxvVRow : packet.xv ∈
      ((Hlate.selectedAt P.v.1 P.v.2).toCriticalFourShell.support) := by
    have hmem : packet.xv ∈
        (Hlate.selectedAt P.v.1 P.v.2).toCriticalFourShell.support ∩
          SelectedClass D.A S.oppApex2 P.rho := by
      rw [packet.opposite_row_trace]
  ...
  have interiorCap {x : ℝ²} (hx : x ∈ S.capInteriorByIndex S.oppIndex2) :
  ...
  have hxuImage : packet.xu ∈ Finset.univ.image B.L.points := by
    ...
    exact interiorCap (by
      have hmem : packet.xu ∈ ({P.u.1, packet.xu} : Finset ℝ²) ∩
        ...
        simp
      exact (Finset.mem_inter.mp hmem).2)
  have hdeletedImage : P.jointDeletion.deleted.1 ∈
      Finset.univ.image B.L.points := by
    rw [B.cap_image]
    exact interiorCap hdeletedInterior
  have hxvImage : packet.xv ∈ Finset.univ.image B.L.points := by
    rw [B.cap_image]
    exact interiorCap (exactFourRigid221_sourceHeavy_xv_mem_secondCapInterior
      P packet)
  rcases Finset.mem_image.mp huImage with ⟨iu, _hiuMem, hiu⟩
  rcases Finset.mem_image.mp hdeletedImage with ⟨id, _hidMem, hid⟩
  rcases Finset.mem_image.mp hxvImage with ⟨ixv, _hixvMem, hixv⟩
  rcases Finset.mem_image.mp hxuImage with ⟨ixu, _hixuMem, hixu⟩
  have hiu_id : iu ≠ id := by
    intro h
    exact huNeDeleted (by simpa only [hiu, hid] using congrArg B.L.points h)
  have hiu_ixv : iu ≠ ixv := by
    intro h
    exact huNeXv (by simpa only [hiu, hixv] using congrArg B.L.points h)
  have hiu_ixu : iu ≠ ixu := by
    intro h
    exact huNeXu (by simpa only [hiu, hixu] using congrArg B.L.points h)
  have hid_ixv : id ≠ ixv := by
    intro h
    exact hdeletedNeXv (by simpa only [hid, hixv] using congrArg B.L.points h)
  have hid_ixu : id ≠ ixu := by
    intro h
    exact hdeletedNeXu (by simpa only [hid, hixu] using congrArg B.L.points h)
  have hixv_ixu : ixv ≠ ixu := by
    intro h
    exact hxuNeXv (by
      simpa only [hixv, hixu] using (congrArg B.L.points h).symm)
  have hYbetween :
  ...
  have hDbetween :
      (iu < id ∧ id < ixv) ∨ (ixv < id ∧ id < iu) := by
    rcases lt_or_gt_of_ne hiu_ixv with h | h
    · exact Or.inl (CGN.index_strictly_between_of_equidistant
        B.Packet B.Hside B.Hord h hiu_id.symm hid_ixv (by
          simpa only [hiu, hid, hixv] using hD))
    · exact Or.inr (CGN.index_strictly_between_of_equidistant
        B.Packet B.Hside B.Hord h hid_ixv hiu_id.symm (by
          simpa only [hiu, hid, hixv] using hD.symm))
  refine ⟨B, iu, id, ixv, ixu, hBorient, hiu, hid, hixv, hixu, ?_⟩
  rcases hYbetween with hYbetween | hYbetween <;>
    rcases hDbetween with hDbetween | hDbetween <;> omega

/-- If the `xu`-row center is in the physical five-class, it is the
opposite source `v`; its strict-cap localization is returned alongside the
identity.  The only remaining named alternative before the order step is
`u`, and the deleted four-point order rules that alternative out. -/
theorem exactFourRigid221_sourceHeavy_pentagon_xu_center_class_reduction
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
    ...
    (hxuNotDeletedRow :
      packet.xu ∉
        ((lateFirstApexSystem R).selectedAt
          P.jointDeletion.deleted.1 P.jointDeletion.deleted.2).toCriticalFourShell.support)
    (hblockerEqDeleted :
      (lateFirstApexSystem R).centerAt packet.xv hxvA =
        P.jointDeletion.deleted.1)
    (htraceBound : ∀ x ∈
    ...
    (lateFirstApexSystem R).centerAt packet.xu hxuA ∈
        SelectedClass D.A S.oppApex2 P.rho →
      ((lateFirstApexSystem R).centerAt packet.xu hxuA = P.v.1) ∧
      (lateFirstApexSystem R).centerAt packet.xu hxuA ∈
        S.capInteriorByIndex S.oppIndex2 := by
  classical
  let Hlate := lateFirstApexSystem R
  let Ku := (Hlate.selectedAt P.u.1 P.u.2).toCriticalFourShell
  let Kxu := (Hlate.selectedAt packet.xu hxuA).toCriticalFourShell
  let Kxv := (Hlate.selectedAt packet.xv hxvA).toCriticalFourShell
  let Kdel :=
    (Hlate.selectedAt P.jointDeletion.deleted.1
      P.jointDeletion.deleted.2).toCriticalFourShell
  ...
  have hcxEq :
      dist (Hlate.centerAt packet.xu hxuA) packet.xu =
        dist (Hlate.centerAt packet.xu hxuA) P.jointDeletion.deleted.1 :=
    (Kxu.support_eq_radius _ Kxu.q_mem_support).trans
      (Kxu.support_eq_radius _ hdeletedXuRow).symm
  ...
  have hcxInterior :
    ...
    commonPhysicalPair_center_mem_secondCapInterior
      ...
      (P.surface.secondApex_robust.centerAt_ne Hlate packet.xu hxuA)
      hxuInterior hdeletedInterior hxuNeDeleted hcxEq hphysicalXuDeleted
  have hcenterU : Hlate.centerAt P.u.1 P.u.2 = packet.xv := by
    simpa only [Hlate, P.huSource] using packet.blocker_eq_xv
  have hxvNotURow : packet.xv ∉ Ku.support := by
    intro hxvSupport
    apply Ku.center_not_mem_support
    have hm := congrArg (fun z : ℝ² => z ∈ Ku.support) hcenterU
    exact Eq.mp hm.symm hxvSupport
  have huNeXv : P.u.1 ≠ packet.xv := by
    intro h
    apply hxvNotURow
    simpa only [← h] using Ku.q_mem_support
  have hxuNeXv : packet.xu ≠ packet.xv := by
    ...
    exact hxuURow
  have hcxNeSource :
      Hlate.centerAt packet.xu hxuA ≠ packet.xu :=
    centerAt_ne_source Hlate packet.xu hxuA
  have hcxNeDeleted :
      Hlate.centerAt packet.xu hxuA ≠ P.jointDeletion.deleted.1 := by
    intro hcxDeleted
    have hblockerEqDeleted' :
        Hlate.centerAt packet.xv hxvA = P.jointDeletion.deleted.1 := by
      simpa only [Hlate] using hblockerEqDeleted
    have hcenters : Hlate.centerAt packet.xu hxuA =
        Hlate.centerAt packet.xv hxvA := by
      calc
        Hlate.centerAt packet.xu hxuA = P.jointDeletion.deleted.1 := hcxDeleted
        _ = Hlate.centerAt packet.xv hxvA := hblockerEqDeleted'.symm
    have hsupports :=
      ATailSurvivalCover.selectedSupports_eq_of_actualBlockers_eq
        Hlate hxuA hxvA hcenters
    have hxuSupportXv : packet.xu ∈ Kxv.support := by
      rw [← hsupports]
      exact Kxu.q_mem_support
    have hxuSupportXv' : packet.xu ∈
        ((lateFirstApexSystem R).selectedAt packet.xv hxvA).toCriticalFourShell.support := by
      simpa only [Hlate, Kxv] using hxuSupportXv
    rcases htraceBound packet.xu hxuSupportXv' hxuClass with h | h
    · exact hxuNeXv h
    · exact packet.xu_ne_u h
  have hcxNeXv :
      Hlate.centerAt packet.xu hxuA ≠ packet.xv := by
    intro hcxXv
    have hcenters : Hlate.centerAt P.u.1 P.u.2 =
        Hlate.centerAt packet.xu hxuA := by
      calc
        Hlate.centerAt P.u.1 P.u.2 = packet.xv := hcenterU
        _ = Hlate.centerAt packet.xu hxuA := hcxXv.symm
    have hsupports :=
      ATailSurvivalCover.selectedSupports_eq_of_actualBlockers_eq
        Hlate P.u.2 hxuA hcenters
    have huSupportXu : P.u.1 ∈ Kxu.support := by
      rw [← hsupports]
      exact Ku.q_mem_support
    apply huNotXuRow
    simpa only [Hlate, Kxu] using huSupportXu
  intro hcxClass
  have hcxUV :
      Hlate.centerAt packet.xu hxuA = P.u.1 ∨
        Hlate.centerAt packet.xu hxuA = P.v.1 := by
    rcases hclassFive _ hcxClass with h | h | h | h | h
    · exact Or.inl h
    · exact (hcxNeSource h).elim
    · exact (hcxNeDeleted h).elim
    · exact Or.inr h
    · exact (hcxNeXv h).elim
  rcases exactFourRigid221_sourceHeavy_pentagon_deleted_exists_forced_secondCap_order
      P packet hxuA hxvA huXvRow hblockerEqDeleted with
    ⟨B, iu, id, ixv, ixu, _hBorient, hiu, hid, hixv, hixu, hforced⟩
  rcases hcxUV with hcxU | hcxV
  · have hiu_id : iu ≠ id := by
      intro h
      rcases hforced with h | h <;> omega
    have hiu_ixu : iu ≠ ixu := by
      intro h
      rcases hforced with h | h <;> omega
    have hid_ixu : id ≠ ixu := by
      ...
      rcases hforced with h | h <;> omega
    have hEq :
        dist (B.L.points iu) (B.L.points ixu) =
          dist (B.L.points iu) (B.L.points id) := by
      simpa only [Hlate, hcxU, hiu, hid, hixu] using hcxEq
    rcases lt_or_gt_of_ne hid_ixu with hidixu | hixuid
    · have hbetween := CGN.index_strictly_between_of_equidistant
        B.Packet B.Hside B.Hord hidixu hiu_id hiu_ixu hEq.symm
      omega
    · have hbetween := CGN.index_strictly_between_of_equidistant
        B.Packet B.Hside B.Hord hixuid hiu_ixu hiu_id hEq
      omega
  · exact ⟨by simpa only [Hlate] using hcxV, by simpa only [Hlate] using hcxInterior⟩

/-! A direct tagged residual for the three rows that can still carry a
    blocker.  The first class arm is discharged by the order adapter: once
...
theorem exactFourRigid221_sourceHeavy_pentagon_tagged_offClass_strictCap_row_center
    ...
    (hclassFive : ∀ q ∈ SelectedClass D.A S.oppApex2 P.rho,
      q = P.u.1 ∨ q = packet.xu ∨ q = P.jointDeletion.deleted.1 ∨
        q = P.v.1 ∨ q = packet.xv)
    (_hxvInterior : packet.xv ∈ S.capInteriorByIndex S.oppIndex2)
    (_hblockerInterior :
      (lateFirstApexSystem R).centerAt packet.xv hxvA ∈
        S.capInteriorByIndex S.oppIndex2)
    (hblockerEqDeleted :
    ...
    ((lateFirstApexSystem R).centerAt P.jointDeletion.deleted.1
      ...
      (∀ x ∈
        ((lateFirstApexSystem R).selectedAt
          P.jointDeletion.deleted.1 P.jointDeletion.deleted.2).toCriticalFourShell.support,
        x ∈ SelectedClass D.A S.oppApex2 P.rho →
          x = P.jointDeletion.deleted.1 ∨ x = P.v.1)) ∨
    ((lateFirstApexSystem R).centerAt P.v.1 P.v.2 ∉
        SelectedClass D.A S.oppApex2 P.rho ∧
      (lateFirstApexSystem R).centerAt P.v.1 P.v.2 ∈
        S.capInteriorByIndex S.oppIndex2 ∧
  ...
  have hxuNeDeleted : packet.xu ≠ P.jointDeletion.deleted.1 := by
    ...
    rw [h]
    exact Kdel.q_mem_support
  have hxuInterior : packet.xu ∈ I := by
    have hmem : packet.xu ∈ ({P.u.1, packet.xu} : Finset ℝ²) ∩ I := by
      rw [packet.source_pair_interior]
      simp
    exact (Finset.mem_inter.mp hmem).2
  have hblockerInterior :=
  ...
  have hdeletedInterior : P.jointDeletion.deleted.1 ∈ I := by
    simpa only [I, hblockerEqDeleted] using hblockerInterior
  have hcxEq :
      dist (Hlate.centerAt packet.xu hxuA) packet.xu =
        dist (Hlate.centerAt packet.xu hxuA) P.jointDeletion.deleted.1 :=
    (Kxu.support_eq_radius _ Kxu.q_mem_support).trans
  ...
  have hphysicalXuDeleted :
    ...
    (mem_selectedClass.mp hxuClass).2.trans
      (mem_selectedClass.mp P.jointDeletion.deleted_mem_class).2.symm
  have hcxInterior : Hlate.centerAt packet.xu hxuA ∈ I :=
    commonPhysicalPair_center_mem_secondCapInterior
      (Finset.mem_of_mem_erase Kxu.center_mem)
      (P.surface.secondApex_robust.centerAt_ne Hlate packet.xu hxuA)
      hxuInterior hdeletedInterior hxuNeDeleted hcxEq hphysicalXuDeleted
  ...
  have huNeXv : P.u.1 ≠ packet.xv := by
    ...
    apply hxvNotURow
    simpa only [← h] using Ku.q_mem_support
  have hxvVRow : packet.xv ∈ Kv.support := by
  ...
  have hdeletedNeV : P.jointDeletion.deleted.1 ≠ P.v.1 := by
    intro h
    exact P.jointDeletion.deleted_ne_v (Subtype.ext h)
  have hdelPair : ∀ x ∈ Kdel.support, x ∈ C →
      x = P.jointDeletion.deleted.1 ∨ x = P.v.1 := by
    intro x hx hxc
    exact exactFourRigid221_sourceHeavy_row_class_pair P
      P.jointDeletion.deleted.1 P.jointDeletion.deleted.2
      Kdel.q_mem_support hvDeletedRow
      P.jointDeletion.deleted_mem_class P.hvClass hdeletedNeV hx hxc
  have hxuPair : ∀ x ∈ Kxu.support, x ∈ C →
      x = packet.xu ∨ x = P.jointDeletion.deleted.1 :=
    exactFourRigid221_sourceHeavy_pentagon_xuRow_trace_bound
      P packet hxuA hdeletedXuRow hxuNotDeletedRow
  by_cases hcxOff : Hlate.centerAt packet.xu hxuA ∉ C
  · exact Or.inl ⟨hcxOff, hcxInterior, hxuPair⟩
  have hcxClass : Hlate.centerAt packet.xu hxuA ∈ C := by
    by_contra h
    exact hcxOff h
  have hcxData :=
    exactFourRigid221_sourceHeavy_pentagon_xu_center_class_reduction
      P packet hxuA hxvA huXvRow huNotXuRow hdeletedXuRow
        hxuNotDeletedRow hblockerEqDeleted htraceBound hclassFive hcxClass
  have hcxV : Hlate.centerAt packet.xu hxuA = P.v.1 := hcxData.1
  ...
  have hcdEq :
    ...
    (Kdel.support_eq_radius _ Kdel.q_mem_support).trans
      (Kdel.support_eq_radius _ hvDeletedRow).symm
  ...
  have hcdNeDeleted :
    ...
    centerAt_ne_source Hlate P.jointDeletion.deleted.1
      P.jointDeletion.deleted.2
  have hcdNeV :
      Hlate.centerAt P.jointDeletion.deleted.1 P.jointDeletion.deleted.2 ≠ P.v.1 := by
    intro hcdV
    have hvSupportDel : P.v.1 ∈ Kdel.support := by
      simpa only [Kdel, Hlate] using hvDeletedRow
    have hm := congrArg (fun z : ℝ² => z ∈ Kdel.support) hcdV
    exact Kdel.center_not_mem_support (Eq.mp hm.symm hvSupportDel)
  have hcdNeXv :
      Hlate.centerAt P.jointDeletion.deleted.1 P.jointDeletion.deleted.2 ≠
        packet.xv := by
    intro hcdXv
    have hcenters :
        Hlate.centerAt P.jointDeletion.deleted.1 P.jointDeletion.deleted.2 =
          Hlate.centerAt P.u.1 P.u.2 := by
      calc
        Hlate.centerAt P.jointDeletion.deleted.1 P.jointDeletion.deleted.2 =
            packet.xv := hcdXv
        _ = Hlate.centerAt P.u.1 P.u.2 := hcenterU.symm
    have hsupports :=
      ATailSurvivalCover.selectedSupports_eq_of_actualBlockers_eq
        Hlate P.jointDeletion.deleted.2 P.u.2 hcenters
    have hxuSupportDel : packet.xu ∈ Kdel.support := by
      change packet.xu ∈
        (Hlate.selectedAt P.jointDeletion.deleted.1
          P.jointDeletion.deleted.2).toCriticalFourShell.support
      rw [hsupports]
      exact hxuURow
    exact hxuNotDeletedRow (by simpa only [Hlate, Kdel] using hxuSupportDel)
  by_cases hcdOff : Hlate.centerAt P.jointDeletion.deleted.1
      P.jointDeletion.deleted.2 ∉ C
  · have hcdInterior :
        Hlate.centerAt P.jointDeletion.deleted.1
            P.jointDeletion.deleted.2 ∈ I :=
      commonPhysicalPair_center_mem_secondCapInterior
        (Finset.mem_of_mem_erase Kdel.center_mem)
        (P.surface.secondApex_robust.centerAt_ne Hlate
          P.jointDeletion.deleted.1 P.jointDeletion.deleted.2)
        hdeletedInterior hvInterior hdeletedNeV hcdEq hphysicalDeletedV
    exact Or.inr (Or.inl ⟨hcdOff, hcdInterior, hdelPair⟩)
  have hcdClass :
      Hlate.centerAt P.jointDeletion.deleted.1 P.jointDeletion.deleted.2 ∈ C := by
    by_contra h
    exact hcdOff h
  have hcdUXu :
      Hlate.centerAt P.jointDeletion.deleted.1 P.jointDeletion.deleted.2 =
      ...
      Hlate.centerAt P.jointDeletion.deleted.1 P.jointDeletion.deleted.2 =
        packet.xu := by
    rcases hclassFive _ hcdClass with h | h | h | h | h
    · exact Or.inl h
    · exact Or.inr h
    · exact (hcdNeDeleted h).elim
    · exact (hcdNeV h).elim
    · exact (hcdNeXv h).elim
  rcases exactFourRigid221_sourceHeavy_pentagon_deleted_exists_forced_secondCap_order
      P packet hxuA hxvA huXvRow hblockerEqDeleted with
    ⟨B, iu, id, ixv, ixu, _hBorient, hiu, hid, hixv, hixu, hforced⟩
  have hvCap : P.v.1 ∈ S.capByIndex S.oppIndex2 :=
    S.capInteriorByIndex_subset_capByIndex S.oppIndex2 hvInterior
  rcases B.exists_index_of_mem_cap hvCap with ⟨iv, hiv⟩
  have hxuNeV : packet.xu ≠ P.v.1 := by
    intro h
    apply P.hvOmitted
    simpa only [← h] using hxuURow
  have hiv_ixu : iv ≠ ixu := by
    intro h
    exact hxuNeV (by
      simpa only [hiv, hixu] using (congrArg B.L.points h).symm)
  have hiv_id : iv ≠ id := by
    intro h
    exact hdeletedNeV (by
      simpa only [hiv, hid] using (congrArg B.L.points h).symm)
  have hixu_id : ixu ≠ id := by
    intro h
    exact hxuNeDeleted (by simpa only [hixu, hid] using congrArg B.L.points h)
  have hVeq :
      dist (B.L.points iv) (B.L.points ixu) =
        dist (B.L.points iv) (B.L.points id) := by
    simpa only [hcxV, hiv, hixu, hid] using hcxEq
  have hVbetween :
      (ixu < iv ∧ iv < id) ∨ (id < iv ∧ iv < ixu) := by
    rcases lt_or_gt_of_ne hixu_id with h | h
    · exact Or.inl (CGN.index_strictly_between_of_equidistant
        B.Packet B.Hside B.Hord h hiv_ixu hiv_id hVeq)
    · exact Or.inr (CGN.index_strictly_between_of_equidistant
        B.Packet B.Hside B.Hord h hiv_id hiv_ixu hVeq.symm)
  have hcdBetween {ic : Fin B.m}
      (hic : B.L.points ic =
        Hlate.centerAt P.jointDeletion.deleted.1 P.jointDeletion.deleted.2) :
      (id < ic ∧ ic < iv) ∨ (iv < ic ∧ ic < id) := by
    have hic_id : ic ≠ id := by
      intro h
      exact hcdNeDeleted (by
        simpa only [hic, hid] using congrArg B.L.points h)
    have hic_iv : ic ≠ iv := by
      intro h
      exact hcdNeV (by
        simpa only [hic, hiv] using congrArg B.L.points h)
    have hid_iv : id ≠ iv := by
      intro h
      exact hdeletedNeV (by simpa only [hid, hiv] using congrArg B.L.points h)
    have hEq : dist (B.L.points ic) (B.L.points id) =
        dist (B.L.points ic) (B.L.points iv) := by
      simpa only [hic, hid, hiv] using hcdEq
    rcases lt_or_gt_of_ne hid_iv with h | h
    · exact Or.inl (CGN.index_strictly_between_of_equidistant
        B.Packet B.Hside B.Hord h hic_id hic_iv hEq)
    · exact Or.inr (CGN.index_strictly_between_of_equidistant
        B.Packet B.Hside B.Hord h hic_iv hic_id hEq.symm)
  rcases hcdUXu with hcdU | hcdXu
  · have hcdIndex : B.L.points iu =
        Hlate.centerAt P.jointDeletion.deleted.1 P.jointDeletion.deleted.2 := by
      simpa only [hcdU] using hiu
    have hcdBetween' := hcdBetween hcdIndex
    rcases hVbetween with hV | hV <;>
      rcases hcdBetween' with hcd | hcd <;>
      rcases hforced with hforced | hforced <;> omega
  · have hcdIndex : B.L.points ixu =
        Hlate.centerAt P.jointDeletion.deleted.1 P.jointDeletion.deleted.2 := by
      simpa only [hcdXu] using hixu
    have hcdBetween' := hcdBetween hcdIndex
    rcases hVbetween with hV | hV <;>
      rcases hcdBetween' with hcd | hcd <;>
      rcases hforced with hforced | hforced <;> omega

/- The exact six-point cap subcase of the deleted-blocker leaf is already
...
theorem false_of_exactFourRigid221_sourceHeavy_pentagonBlockerDeleted_card_eq_six
  ...
  let C := SelectedClass D.A S.oppApex2 P.rho
  let I := S.capInteriorByIndex S.oppIndex2
  have hsecond : 6 ≤ S.oppCap2.card := by omega
  have hIcard : I.card = 4 := by
    have hcard := ATailCapApexRadiusRigidity.capInteriorByIndex_card_add_two
      S S.oppIndex2
    rw [capByIndex_oppIndex2_eq_oppCap2_sourceHeavy S] at hcard
    change (S.capInteriorByIndex S.oppIndex2).card = 4
    omega
  have huI : P.u.1 ∈ I := by
    have hmem : P.u.1 ∈ ({P.u.1, packet.xu} : Finset ℝ²) ∩ I := by
      rw [packet.source_pair_interior]
      simp [I]
    exact (Finset.mem_inter.mp hmem).2
  have hxuI : packet.xu ∈ I := by
    have hmem : packet.xu ∈ ({P.u.1, packet.xu} : Finset ℝ²) ∩ I := by
      rw [packet.source_pair_interior]
      simp [I]
    exact (Finset.mem_inter.mp hmem).2
  have hdeletedI : P.jointDeletion.deleted.1 ∈ I := by
    have h := exactFourRigid221_sourceHeavy_pentagon_blocker_mem_secondCapInterior
      P packet hxvA huXvRow
    simpa [I, hblockerEqDeleted] using h
  have hxvI : packet.xv ∈ I := by simpa [I] using hxvInterior
  have hxuInter : packet.xu ∈
      ((Hlate.selectedAt P.u.1 P.u.2).toCriticalFourShell.support ∩ C) := by
    have h : packet.xu ∈
        ((lateFirstApexSystem R).selectedAt P.u.1 P.u.2).toCriticalFourShell.support ∩
  ...
  have hxvInter : packet.xv ∈
    ...
    have h : packet.xv ∈
      ...
      simp
    simpa [Hlate, C] using h
  have huC : P.u.1 ∈ C := by simpa [C] using P.huClass
  have hxuC : packet.xu ∈ C := (Finset.mem_inter.mp hxuInter).2
  have hdeletedC : P.jointDeletion.deleted.1 ∈ C := by
    simpa [C] using P.jointDeletion.deleted_mem_class
  have hxvC : packet.xv ∈ C := (Finset.mem_inter.mp hxvInter).2
  have hxuNeU : packet.xu ≠ P.u.1 := packet.xu_ne_u
  have hdeletedNeU : P.jointDeletion.deleted.1 ≠ P.u.1 := by
    intro h
    exact P.jointDeletion.deleted_ne_u (Subtype.ext h)
  have hdeletedNeXu : P.jointDeletion.deleted.1 ≠ packet.xu := by
  ...
  have huNeXv : P.u.1 ≠ packet.xv := by
    ...
    have hcenterU : Hlate.centerAt P.u.1 P.u.2 = packet.xv := by
      simpa only [Hlate, P.huSource] using packet.blocker_eq_xv
    let Ku := (Hlate.selectedAt P.u.1 P.u.2).toCriticalFourShell
    have hxvSupport : packet.xv ∈ Ku.support := by
      simpa [h] using Ku.q_mem_support
    have hm := congrArg (fun z : ℝ² => z ∈ Ku.support) hcenterU
    exact Ku.center_not_mem_support (Eq.mp hm.symm hxvSupport)
  have hxuNeXv : packet.xu ≠ packet.xv := by
    intro h
    apply hxvNotXuRow
    have hq := (Hlate.selectedAt packet.xu hxuA).toCriticalFourShell.q_mem_support
    simpa [Hlate, h] using hq
  have false_of_new_offClass {c : ℝ²} (hcOff : c ∉ C)
      (hcI : c ∈ I) : False := by
    have hcU : c ≠ P.u.1 := by
      intro h
      apply hcOff
      simpa [h, C] using P.huClass
    have hcXu : c ≠ packet.xu := by
      intro h
      apply hcOff
      simpa [h] using hxuC
    have hcDeleted : c ≠ P.jointDeletion.deleted.1 := by
      intro h
      apply hcOff
      simpa [h] using hdeletedC
    have hcXv : c ≠ packet.xv := by
      intro h
      apply hcOff
      simpa [h] using hxvC
    have huNeXu : P.u.1 ≠ packet.xu := hxuNeU.symm
    have huNeDeleted : P.u.1 ≠ P.jointDeletion.deleted.1 := hdeletedNeU.symm
    have hxuNeDeleted' : packet.xu ≠ P.jointDeletion.deleted.1 :=
      hdeletedNeXu.symm
    have hxvNeDeleted : packet.xv ≠ P.jointDeletion.deleted.1 :=
      hdeletedNeXv.symm
    have hxvNeU : packet.xv ≠ P.u.1 := huNeXv.symm
    have hxvNeXu : packet.xv ≠ packet.xu := hxuNeXv.symm
    have hcNeU : P.u.1 ≠ c := hcU.symm
    have hcNeXu : packet.xu ≠ c := hcXu.symm
    have hcNeDeleted : P.jointDeletion.deleted.1 ≠ c := hcDeleted.symm
    have hcNeXv : packet.xv ≠ c := hcXv.symm
    have hsubset :
        ({P.u.1, packet.xu, P.jointDeletion.deleted.1, packet.xv, c} :
          Finset ℝ²) ⊆ I := by
      intro z hz
      simp only [Finset.mem_insert, Finset.mem_singleton] at hz
    ...
    have hfive :
      ({P.u.1, packet.xu, P.jointDeletion.deleted.1, packet.xv, c} :
          Finset ℝ²).card = 5 := by
      simp [huNeXu, huNeDeleted, huNeXv, hcNeU, hxuNeDeleted',
        hxuNeXv, hcNeXu, hdeletedNeXv, hcNeDeleted, hxvNeDeleted,
        hxvNeU, hxvNeXu, hcNeXv]
    rw [hfive, hIcard] at hle
    omega
  have htag := exactFourRigid221_sourceHeavy_pentagon_tagged_offClass_strictCap_row_center
    P packet hsecond hxuA hxvA huXvRow huNotXuRow hxvNotXuRow hdeletedXuRow
      hvDeletedRow huNotDeletedRow hxuNotDeletedRow hxvNotDeletedRow hclassFive
      hxvInterior hblockerInterior hblockerEqDeleted htraceBound htraceBoundXu
  rcases htag with h | h | h
  · exact false_of_new_offClass h.1 h.2.1
  · exact false_of_new_offClass h.1 h.2.1
  · exact false_of_new_offClass h.1 h.2.1

/-- The deleted-blocker pentagon cannot stop at the exact-six boundary.

This is the direct numerical corollary of the checked exact-six leaf: under
the live blocker-deleted hypotheses, the second opposite cap has cardinality
at least seven.  The open geometric leaf below is deliberately not used. -/
theorem exactFourRigid221_sourceHeavy_pentagonBlockerDeleted_card_ge_seven
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
    ...
    7 ≤ S.oppCap2.card := by
  by_contra hnot
  have hcap : S.oppCap2.card = 6 := by omega
  exact false_of_exactFourRigid221_sourceHeavy_pentagonBlockerDeleted_card_eq_six
    P packet hcap hxuA hxvA huXvRow huNotXuRow hxvNotXuRow hdeletedXuRow
    hvDeletedRow huNotDeletedRow hxuNotDeletedRow hxvNotDeletedRow hclassFive
    hxvInterior hblockerInterior hblockerEqDeleted htraceBound htraceBoundXu

/-- At cardinality seven, the deleted-blocker pentagon has a sharp interior
normal form.  The `xu` center is off the physical class, `v` is absent from
the strict interior, and the five displayed points exhaust that interior.
The row pair and the forced four-point order are returned for downstream
consumers. -/
theorem exactFourRigid221_sourceHeavy_pentagonBlockerDeleted_card_eq_seven_normal_form
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
    ...
    (lateFirstApexSystem R).centerAt packet.xu hxuA ∉
      ...
      P.v.1 ∉ S.capInteriorByIndex S.oppIndex2 ∧
      S.capInteriorByIndex S.oppIndex2 =
        ({P.u.1, P.jointDeletion.deleted.1, packet.xv, packet.xu,
          (lateFirstApexSystem R).centerAt packet.xu hxuA} : Finset ℝ²) ∧
      (∀ x ∈
        ((lateFirstApexSystem R).selectedAt packet.xu hxuA).toCriticalFourShell.support,
        x ∈ SelectedClass D.A S.oppApex2 P.rho →
          x = packet.xu ∨ x = P.jointDeletion.deleted.1) ∧
  ...
  have hIcard : I.card = 5 := by
    have hcard := ATailCapApexRadiusRigidity.capInteriorByIndex_card_add_two
      S S.oppIndex2
    rw [capByIndex_oppIndex2_eq_oppCap2_sourceHeavy S] at hcard
    change (S.capInteriorByIndex S.oppIndex2).card = 5
    omega
  have huI : P.u.1 ∈ I := by
    have hmem : P.u.1 ∈ ({P.u.1, packet.xu} : Finset ℝ²) ∩ I := by
      rw [packet.source_pair_interior]
  ...
  have hdeletedI : P.jointDeletion.deleted.1 ∈ I := by
    have h := exactFourRigid221_sourceHeavy_pentagon_blocker_mem_secondCapInterior
      P packet hxvA huXvRow
    simpa only [I, hblockerEqDeleted] using h
  have hxvI : packet.xv ∈ I := by
    simpa only [I] using hxvInterior
  have hxuInter : packet.xu ∈ Ku.support ∩ C := by
    rw [packet.source_row_trace]
    simp [Hlate, Ku, C]
  have hxuClass : packet.xu ∈ C := (Finset.mem_inter.mp hxuInter).2
  ...
  have hcxInterior : Hlate.centerAt packet.xu hxuA ∈ I :=
    commonPhysicalPair_center_mem_secondCapInterior
      (Finset.mem_of_mem_erase Kxu.center_mem)
      (P.surface.secondApex_robust.centerAt_ne Hlate packet.xu hxuA)
      hxuI hdeletedInterior hxuNeDeleted hcxEq hphysicalXuDeleted
  have huNeXu : P.u.1 ≠ packet.xu := packet.xu_ne_u.symm
  have hdeletedNeU : P.jointDeletion.deleted.1 ≠ P.u.1 := by
    intro h
    exact P.jointDeletion.deleted_ne_u (Subtype.ext h)
  ...
  have hxuNeXv : packet.xu ≠ packet.xv := by
    intro h
    apply hxvNotXuRow
    have hq := Kxu.q_mem_support
    simpa only [Hlate, Kxu, h] using hq
  have hvNeU : P.v.1 ≠ P.u.1 := by
    intro h
    exact P.huNeV (Subtype.ext h.symm)
  ...
  have hvNeDeleted : P.v.1 ≠ P.jointDeletion.deleted.1 := by
    intro h
    exact P.jointDeletion.deleted_ne_v (Subtype.ext h.symm)
  have hvNeXv : P.v.1 ≠ packet.xv := packet.xv_ne_v.symm
  have hxuPair : ∀ x ∈ Kxu.support, x ∈ C →
      x = packet.xu ∨ x = P.jointDeletion.deleted.1 := by
    exact exactFourRigid221_sourceHeavy_pentagon_xuRow_trace_bound
      P packet hxuA hdeletedXuRow hxuNotDeletedRow
  have false_of_v_and_offClass {c : ℝ²} (hcOff : c ∉ C)
      (hcI : c ∈ I) (hvI : P.v.1 ∈ I) : False := by
    have hcU : c ≠ P.u.1 := by
      intro h
      apply hcOff
      simpa [h, C] using P.huClass
    have hcXu : c ≠ packet.xu := by
      intro h
      apply hcOff
      simpa [h] using hxuClass
    have hcDeleted : c ≠ P.jointDeletion.deleted.1 := by
      intro h
      apply hcOff
      simpa [h, C] using P.jointDeletion.deleted_mem_class
    have hcXv : c ≠ packet.xv := by
      intro h
      apply hcOff
      ...
      simpa [h] using hxvClass
    have hcV : c ≠ P.v.1 := by
      intro h
      apply hcOff
      simpa [h, C] using P.hvClass
    have hsubset :
        ({P.u.1, packet.xu, P.jointDeletion.deleted.1, packet.xv,
          P.v.1, c} : Finset ℝ²) ⊆ I := by
      intro z hz
      simp only [Finset.mem_insert, Finset.mem_singleton] at hz
      ...
      · exact hvI
      · exact hcI
    have huNeDeleted : P.u.1 ≠ P.jointDeletion.deleted.1 :=
      hdeletedNeU.symm
    have hxvNeDeleted : packet.xv ≠ P.jointDeletion.deleted.1 :=
      hdeletedNeXv.symm
    have hxvNeU : packet.xv ≠ P.u.1 := huNeXv.symm
    have hxvNeXu : packet.xv ≠ packet.xu := hxuNeXv.symm
    have huNeV : P.u.1 ≠ P.v.1 := hvNeU.symm
    have hxuNeV : packet.xu ≠ P.v.1 := hvNeXu.symm
    have hdeletedNeV : P.jointDeletion.deleted.1 ≠ P.v.1 :=
      hvNeDeleted.symm
    have hxvNeV : packet.xv ≠ P.v.1 := hvNeXv.symm
    have huNeC : P.u.1 ≠ c := hcU.symm
    have hxuNeC : packet.xu ≠ c := hcXu.symm
    have hdeletedNeC : P.jointDeletion.deleted.1 ≠ c := hcDeleted.symm
    have hxvNeC : packet.xv ≠ c := hcXv.symm
    have hvNeC : P.v.1 ≠ c := hcV.symm
    have hsix :
        ({P.u.1, packet.xu, P.jointDeletion.deleted.1, packet.xv,
          P.v.1, c} : Finset ℝ²).card = 6 := by
      simp [huNeXu, huNeDeleted, hdeletedNeU, hxuNeDeleted, hdeletedNeXu,
        hdeletedNeXv, hxvNeDeleted, huNeXv, hxvNeU, hxuNeXv, hxvNeXu,
        hvNeU, huNeV, hvNeXu, hxuNeV, hvNeDeleted, hdeletedNeV, hvNeXv,
        hxvNeV, hcU, huNeC, hcXu, hxuNeC, hcDeleted, hdeletedNeC, hcXv,
        hxvNeC, hcV, hvNeC]
    have hle := Finset.card_le_card hsubset
    rw [hsix, hIcard] at hle
    omega
  have hcxOff : Hlate.centerAt packet.xu hxuA ∉ C := by
    intro hcxClass
    have hcxData := exactFourRigid221_sourceHeavy_pentagon_xu_center_class_reduction
      P packet hxuA hxvA huXvRow huNotXuRow hdeletedXuRow
        hxuNotDeletedRow hblockerEqDeleted htraceBound hclassFive hcxClass
    have hvI : P.v.1 ∈ I := by
      rw [← hcxData.1]
      exact hcxData.2
    have htag := exactFourRigid221_sourceHeavy_pentagon_tagged_offClass_strictCap_row_center
      P packet _hsecond hxuA hxvA huXvRow huNotXuRow hxvNotXuRow hdeletedXuRow
        hvDeletedRow huNotDeletedRow hxuNotDeletedRow hxvNotDeletedRow hclassFive
        hxvInterior hblockerInterior hblockerEqDeleted htraceBound htraceBoundXu
    rcases htag with h | h | h
    · exact false_of_v_and_offClass h.1 h.2.1 hvI
    · exact false_of_v_and_offClass h.1 h.2.1 hvI
    · exact false_of_v_and_offClass h.1 h.2.1 hvI
  have hvNotI : P.v.1 ∉ I := by
    intro hvI
    exact false_of_v_and_offClass hcxOff hcxInterior hvI
  have hFsubset :
      ({P.u.1, P.jointDeletion.deleted.1, packet.xv, packet.xu,
        Hlate.centerAt packet.xu hxuA} : Finset ℝ²) ⊆ I := by
    intro z hz
    simp only [Finset.mem_insert, Finset.mem_singleton] at hz
    rcases hz with rfl | rfl | rfl | rfl | rfl
    · exact huI
    ...
    · exact hxuI
    · exact hcxInterior
  have hcxNeU : Hlate.centerAt packet.xu hxuA ≠ P.u.1 := by
    intro h
    apply hcxOff
    simpa [h, C] using P.huClass
  have hcxNeXu : Hlate.centerAt packet.xu hxuA ≠ packet.xu :=
    centerAt_ne_source Hlate packet.xu hxuA
  have hcxNeDeleted :
      Hlate.centerAt packet.xu hxuA ≠ P.jointDeletion.deleted.1 := by
    intro h
    apply hcxOff
    simpa [h, C] using P.jointDeletion.deleted_mem_class
  have hcxNeXv : Hlate.centerAt packet.xu hxuA ≠ packet.xv := by
    intro h
    apply hcxOff
    have hxvClass : packet.xv ∈ C := by
      have hxvInter : packet.xv ∈
  ...
  have hFcard :
      ({P.u.1, P.jointDeletion.deleted.1, packet.xv, packet.xu,
        Hlate.centerAt packet.xu hxuA} : Finset ℝ²).card = 5 := by
    have huNeDeleted : P.u.1 ≠ P.jointDeletion.deleted.1 :=
      hdeletedNeU.symm
    have hxvNeDeleted : packet.xv ≠ P.jointDeletion.deleted.1 :=
      hdeletedNeXv.symm
    have hxvNeXu : packet.xv ≠ packet.xu := hxuNeXv.symm
    have huNeCx : P.u.1 ≠ Hlate.centerAt packet.xu hxuA := hcxNeU.symm
    have hdeletedNeCx :
        P.jointDeletion.deleted.1 ≠ Hlate.centerAt packet.xu hxuA :=
      hcxNeDeleted.symm
    have hxvNeCx : packet.xv ≠ Hlate.centerAt packet.xu hxuA :=
      hcxNeXv.symm
    have hxuNeCx : packet.xu ≠ Hlate.centerAt packet.xu hxuA :=
      hcxNeXu.symm
    simp [huNeXu, huNeDeleted, hdeletedNeU, hxuNeDeleted, hdeletedNeXu,
      hdeletedNeXv, hxvNeDeleted, huNeXv, hxuNeXv, hxvNeXu, hcxNeU,
      huNeCx, hcxNeXu, hxuNeCx, hcxNeDeleted, hdeletedNeCx, hcxNeXv,
      hxvNeCx]
  have hset :
      ({P.u.1, P.jointDeletion.deleted.1, packet.xv, packet.xu,
        Hlate.centerAt packet.xu hxuA} : Finset ℝ²) = I := by
    apply Finset.eq_of_subset_of_card_le hFsubset
    rw [hIcard, hFcard]
  have horder := exactFourRigid221_sourceHeavy_pentagon_deleted_exists_forced_secondCap_order
    P packet hxuA hxvA huXvRow hblockerEqDeleted
  refine ⟨?_, ?_, ?_, ?_, ?_, horder⟩
  · simpa only [Hlate, C] using hcxOff
  · simpa only [Hlate, I] using hcxInterior
  · simpa only [I] using hvNotI
  · simpa only [Hlate, I] using hset.symm
  · simpa only [Hlate, C, Kxu] using hxuPair

/-- The deleted-blocker branch exposes the next `v`-row center as three
surviving arms: the two named source points, or a center outside the
physical class.  The first two arms retain strict-interior membership. -/
theorem exactFourRigid221_sourceHeavy_pentagonBlockerDeleted_v_center_dichotomy
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
  ...
  by_cases hcenterVClass :
      (lateFirstApexSystem R).centerAt P.v.1 P.v.2 ∈
        SelectedClass D.A S.oppApex2 P.rho
  · have hcenterVData :=
      exactFourRigid221_sourceHeavy_pentagon_v_center_class_reduction
        P packet hxvA hblockerEqDeleted htraceBound hclassFive hcenterVClass
    rcases hcenterVData.1 with hcenterVU | hcenterVXu
    · exact Or.inl ⟨hcenterVU, hcenterVData.2⟩
    · exact Or.inr (Or.inl ⟨hcenterVXu, hcenterVData.2⟩)
  · exact Or.inr (Or.inr hcenterVClass)

/-- In the card-seven normal form, an off-class center on the `v` row cannot
   be one of the five strict-interior points.  The only possible off-class
   candidate is the `xu` center, but equal centers transfer the selected
   supports and violate the retained `xv` omission from the `xu` row. -/
theorem exactFourRigid221_sourceHeavy_pentagon_card_seven_offClass_v_center_not_interior
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
  ...
  let Hlate := lateFirstApexSystem R
  let C := SelectedClass D.A S.oppApex2 P.rho
  have huClass : P.u.1 ∈ C := by
    simpa only [C] using P.huClass
  have hdeletedClass : P.jointDeletion.deleted.1 ∈ C := by
    simpa only [C] using P.jointDeletion.deleted_mem_class
  have hxuClass : packet.xu ∈ C := by
    have hxuInter : packet.xu ∈
        ((Hlate.selectedAt P.u.1 P.u.2).toCriticalFourShell.support ∩ C) := by
      rw [packet.source_row_trace]
      simp [Hlate, C]
    exact (Finset.mem_inter.mp hxuInter).2
  have hxvClass : packet.xv ∈ C := by
    have hxvInter : packet.xv ∈
        ((Hlate.selectedAt P.v.1 P.v.2).toCriticalFourShell.support ∩ C) := by
      rw [packet.opposite_row_trace]
      simp [Hlate, C]
    exact (Finset.mem_inter.mp hxvInter).2
  have hcenterVNeXuCenter :
      Hlate.centerAt P.v.1 P.v.2 ≠ Hlate.centerAt packet.xu hxuA := by
    intro hcenterEq
    have hsupports :=
      ATailSurvivalCover.selectedSupports_eq_of_actualBlockers_eq
        Hlate P.v.2 hxuA hcenterEq
    have hxvInter : packet.xv ∈
        ((Hlate.selectedAt P.v.1 P.v.2).toCriticalFourShell.support ∩ C) := by
      rw [packet.opposite_row_trace]
      simp [Hlate, C]
    have hxvSupportXu : packet.xv ∈
        (Hlate.selectedAt packet.xu hxuA).toCriticalFourShell.support := by
      rw [← hsupports]
      exact (Finset.mem_inter.mp hxvInter).1
    exact hxvNotXuRow (by simpa [Hlate] using hxvSupportXu)
  intro hcenterVI
  rw [hinterior_eq] at hcenterVI
  simp only [Finset.mem_insert, Finset.mem_singleton] at hcenterVI
  rcases hcenterVI with hcenterVU | hcenterVDeleted | hcenterVXv |
    hcenterVXu | hcenterVCx
  · apply hcenterVOff
    rw [hcenterVU]
    exact huClass
  · apply hcenterVOff
  ...
  · apply hcenterVOff
    rw [hcenterVXu]
    exact hxuClass
  · exact hcenterVNeXuCenter hcenterVCx
...
/-- The opposite apex is outside its opposite cap, independently of which
surplus-triangle index names that cap. -/
...
/-- Direct source-authenticated contradiction for the exact-seventeen,
second-cap-nine exact-cover branch.  The exact incidence cover supplies an
outside row witness and an apex-row alternative; convex cyclic order and the
equal-radius fan then discharge every alternative through `ExactA2CapMetric`.
-/
theorem false_of_exactSeventeen_secondCapNine_exactCover
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
  ...
  have close
    ...
    have hcapPullback : pullback point cap = secondCapLabels := by
      ...
      have hlabelsCard : secondCapLabels.card = 9 := by decide
    ...
    have hKvOutside : (Kv.support \ cap).card = 2 :=
      outsideSlice_card_eq_two Kv.support cap P.v.1 packet.xv
        Kv.support_card (hrowCap 2)
        (Finset.mem_inter.mpr ⟨Kv.q_mem_support, interiorCap hvInterior⟩)
        (Finset.mem_inter.mpr ⟨hxvVRow, interiorCap hxvInterior⟩)
        hvNeXv
    have hKvOutsidePos : 0 < (Kv.support \ cap).card := by omega
    obtain ⟨q, hqOutside⟩ := Finset.card_pos.mp hKvOutsidePos
    have hqKv : q ∈ Kv.support := (Finset.mem_sdiff.mp hqOutside).1
    have hqNotCap : q ∉ cap := (Finset.mem_sdiff.mp hqOutside).2
    have hqA : q ∈ D.A := Kv.support_subset_A hqKv
    obtain ⟨qLabel, hqPoint⟩ : ∃ label, point label = q := by
      have hqImage : q ∈ Finset.univ.image point := by
        rw [hpointImage]
        exact hqA
      rcases Finset.mem_image.mp hqImage with ⟨label, -, hlabel⟩
      exact ⟨label, hlabel⟩
    have hqBoundary : boundary (labelIndex qLabel) = q := by
      simpa only [point] using hqPoint
    have hqLabelNotCap : qLabel ∉ secondCapLabels := by
      intro hqLabelCap
      apply hqNotCap
      have hqPullback : qLabel ∈ pullback point cap := by
        rw [hcapPullback]
        exact hqLabelCap
      simpa only [mem_pullback, hqPoint] using hqPullback
    have hqPosition : 9 ≤ (position order qLabel).val := by
      fin_cases order <;> fin_cases qLabel <;>
        simp [secondCapLabels, position] at hqLabelNotCap ⊢
    have happNotCap : S.oppApex2 ∉ cap := by
      simpa only [cap] using
        oppApex2_not_mem_capByIndex_oppIndex2_for_exactSeventeen S
    have happOutside : S.oppApex2 ∈ D.A \ cap :=
      Finset.mem_sdiff.mpr ⟨happA, happNotCap⟩
    have happBag : S.oppApex2 ∈ incidenceBag (D.A \ cap) rowSupport := by
      have happSupport :
          S.oppApex2 ∈
            (incidenceBag (D.A \ cap) rowSupport).toFinset := by
        rw [hexactCover'.2]
        exact happOutside
      simpa only [Multiset.mem_toFinset] using happSupport
    have happRow :
        S.oppApex2 ∈ rowSupport 0 ∨
        S.oppApex2 ∈ rowSupport 1 ∨
        S.oppApex2 ∈ rowSupport 2 ∨
        S.oppApex2 ∈ rowSupport 3 := by
      simpa only [incidenceBag, Multiset.mem_add, Finset.mem_val,
        Finset.mem_inter, happOutside, and_true, or_assoc] using happBag
    have hProw :
        dist P.v.1 S.oppApex2 = dist P.v.1 P.u.1 ∨
        dist packet.xv S.oppApex2 = dist packet.xv P.u.1 ∨
        dist P.jointDeletion.deleted.1 S.oppApex2 =
            dist P.jointDeletion.deleted.1 P.v.1 ∨
        dist c S.oppApex2 = dist c P.v.1 := by
      ...
      · right; right; left
        ...
        simpa only [Hlate, hcenterV] using hraw
      · right; right; right
        have hrow' : S.oppApex2 ∈ Kdel.support := by
          simpa only [rowSupport] using hrow
        simpa only [c, Kdel] using
          (Kdel.support_eq_radius _ hrow').trans
            (Kdel.support_eq_radius _ Q.hvDeletedRow).symm
    have hqRow :
        dist P.jointDeletion.deleted.1 q =
          dist P.jointDeletion.deleted.1 P.v.1 := by
      have hraw := (Kv.support_eq_radius _ hqKv).trans
        (Kv.support_eq_radius _ Kv.q_mem_support).symm
      simpa only [Hlate, hcenterV] using hraw
    have hu_d : OrientedLt orientation labelIndex 6 10 :=
      hposition 6 10 (by rw [horderZero]; decide)
    have hd_y : OrientedLt orientation labelIndex 10 7 :=
      hposition 10 7 (by rw [horderZero]; decide)
    have hy_q_order : position order 7 < position order qLabel := by
      have hqPositionZero := hqPosition
      rw [horderZero] at hqPositionZero ⊢
      change 7 < (position (0 : NamedOrder) qLabel).val
      omega
    have hy_q : OrientedLt orientation labelIndex 7 qLabel :=
      hposition 7 qLabel hy_q_order
    have hv_c : OrientedLt orientation labelIndex 8 11 :=
      hposition 8 11 (by rw [horderZero]; decide)
    have hc_d : OrientedLt orientation labelIndex 11 10 :=
      hposition 11 10 (by rw [horderZero]; decide)
    have hv_d : OrientedLt orientation labelIndex 8 10 :=
      hposition 8 10 (by rw [horderZero]; decide)
    have hd_P_order : position order 10 < position order apexLabel := by
      have hapexPositionZero := hapexPosition
      rw [horderZero] at hapexPositionZero ⊢
      change 4 < (position (0 : NamedOrder) apexLabel).val
      omega
    have hd_P : OrientedLt orientation labelIndex 10 apexLabel :=
      hposition 10 apexLabel hd_P_order
    have hc_sides :
      ...
      | forward =>
          left
          have hvcD : signedArea2 P.v.1 c P.jointDeletion.deleted.1 < 0 := by
            have hraw := hneg_of_ccw hboundaryInjective hboundaryCcw
              (by simpa only [OrientedLt] using hv_c)
              (by simpa only [OrientedLt] using hc_d)
            rw [hvPoint, hcPoint, hdeletedPoint] at hraw
            exact hraw
          have hvdP :
              signedArea2 P.v.1 P.jointDeletion.deleted.1 S.oppApex2 < 0 := by
            have hraw := hneg_of_ccw hboundaryInjective hboundaryCcw
              (by simpa only [OrientedLt] using hv_d)
              (by simpa only [OrientedLt] using hd_P)
            rw [hvPoint, hdeletedPoint, hapexBoundary] at hraw
            exact hraw
          constructor
          · rw [ExactA2CapMetric.signedArea2_swap]
            linarith
          · exact hvdP
      | reverse =>
          right
          have hdcV : signedArea2 P.jointDeletion.deleted.1 c P.v.1 < 0 := by
            have hraw := hneg_of_ccw hboundaryInjective hboundaryCcw
              (by simpa only [OrientedLt] using hc_d)
              (by simpa only [OrientedLt] using hv_c)
            rw [hdeletedPoint, hcPoint, hvPoint] at hraw
            exact hraw
          have hPdV :
              signedArea2 S.oppApex2 P.jointDeletion.deleted.1 P.v.1 < 0 := by
            have hraw := hneg_of_ccw hboundaryInjective hboundaryCcw
              (by simpa only [OrientedLt] using hd_P)
              (by simpa only [OrientedLt] using hv_d)
            rw [hapexBoundary, hdeletedPoint, hvPoint] at hraw
            exact hraw
          constructor
          · rw [ExactA2CapMetric.signedArea2_swap]
            linarith
          · rw [← ExactA2CapMetric.signedArea2_cyclic]
            exact hPdV
    have hqSide :
        signedArea2 P.u.1 packet.xu P.jointDeletion.deleted.1 *
            signedArea2 P.u.1 packet.xu q < 0 := by
      cases orientation with
      | forward =>
          have hudY :
              signedArea2 P.u.1 P.jointDeletion.deleted.1 packet.xu < 0 := by
            have hraw := hneg_of_ccw hboundaryInjective hboundaryCcw
              (by simpa only [OrientedLt] using hu_d)
              (by simpa only [OrientedLt] using hd_y)
            rw [huPoint, hdeletedPoint, hxuPoint] at hraw
            exact hraw
          have huyD :
              0 < signedArea2 P.u.1 packet.xu P.jointDeletion.deleted.1 := by
            rw [ExactA2CapMetric.signedArea2_swap]
            linarith
          have huyQ : signedArea2 P.u.1 packet.xu q < 0 := by
            have hraw := hneg_of_ccw hboundaryInjective hboundaryCcw
              (by simpa only [OrientedLt] using
                hposition 6 7 (by rw [horderZero]; decide))
              (by simpa only [OrientedLt] using hy_q)
            rw [huPoint, hxuPoint, hqBoundary] at hraw
            exact hraw
          exact mul_neg_of_pos_of_neg huyD huyQ
      | reverse =>
          have hydU :
              signedArea2 packet.xu P.jointDeletion.deleted.1 P.u.1 < 0 := by
            have hraw := hneg_of_ccw hboundaryInjective hboundaryCcw
              (by simpa only [OrientedLt] using hd_y)
              (by simpa only [OrientedLt] using hu_d)
            rw [hxuPoint, hdeletedPoint, huPoint] at hraw
            exact hraw
          have huyD :
              signedArea2 P.u.1 packet.xu P.jointDeletion.deleted.1 < 0 := by
            rw [ExactA2CapMetric.signedArea2_cyclic]
            exact hydU
          have hqyU : signedArea2 q packet.xu P.u.1 < 0 := by
            have hraw := hneg_of_ccw hboundaryInjective hboundaryCcw
              (by simpa only [OrientedLt] using hy_q)
              (by simpa only [OrientedLt] using
                hposition 6 7 (by rw [horderZero]; decide))
            rw [hqBoundary, hxuPoint, huPoint] at hraw
            exact hraw
          have huyQ : 0 < signedArea2 P.u.1 packet.xu q := by
            rw [ExactA2CapMetric.signedArea2_swap,
              ExactA2CapMetric.signedArea2_cyclic]
            linarith
          exact mul_neg_of_neg_of_pos huyD huyQ
    have hP_u : dist S.oppApex2 P.u.1 = P.rho :=
      (mem_selectedClass.mp P.huClass).2
    have hP_y : dist S.oppApex2 packet.xu = P.rho :=
      (mem_selectedClass.mp hxuClass).2
    ...
    have hP_d :
        dist S.oppApex2 P.jointDeletion.deleted.1 = P.rho :=
      (mem_selectedClass.mp P.jointDeletion.deleted_mem_class).2
    have hne_P {z : ℝ²} (hz : dist S.oppApex2 z = P.rho) :
        z ≠ S.oppApex2 := by
      intro hzP
      subst z
      rw [dist_self] at hz
      linarith [P.hrho]
    exact ExactA2CapMetric.false_of_exactA2_boundary_data
      (A := D.A) (P := S.oppApex2) (u := P.u.1) (y := packet.xu)
      (x := packet.xv) (v := P.v.1)
      (d := P.jointDeletion.deleted.1) (c := c) (q := q)
      (radius := P.rho)
      ...
      (hne_P hP_u) (hne_P hP_y) (hne_P hP_x) (hne_P hP_v)
      (hne_P hP_d) huNeXu huNeXv hvNeXv hdelNeV.symm
      P.hrho hP_u hP_y hP_x hP_v hP_d hE hD.symm hC hA.symm
      hc_sides hProw hqSide hqRow
  have closeForward
      (order : NamedOrder)
  ...
  · rcases hunused with ⟨unused, hunusedOutside, hunusedRows⟩
    ...
    exact
      false_of_exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagonBlockerV_vRowBlockerDeleted_deletedRowBlockerOffClass_card_eq_seventeen_secondCapNine_of_unusedOutside
        ...
        hcapCard
  · exact false_of_exactSeventeen_secondCapNine_exactCover Q hcenterV
...
/-- An `xu`-row center in the erased physical-class slice can only be the
remaining named source `v`. -/
theorem pentagonOffClassBlocker_xu_center_erase_u_class_eq_v
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
    ...
    (hcenterBO :
      (lateFirstApexSystem R).centerAt packet.xu Q.hxuA ∈
        SelectedClass (D.A.erase P.u.1) S.oppApex2 P.rho) :
    (lateFirstApexSystem R).centerAt packet.xu Q.hxuA = P.v.1 := by
  classical
  let Hlate := lateFirstApexSystem R
  let Ku :=
    (Hlate.selectedAt P.u.1 P.u.2).toCriticalFourShell
  ...
  have hcenterBOData := mem_selectedClass.mp hcenterBO
  have hcenterA : Hlate.centerAt packet.xu Q.hxuA ∈ D.A := by
    exact (Finset.mem_erase.mp hcenterBOData.1).2
  have hcenterClass :
      Hlate.centerAt packet.xu Q.hxuA ∈
        SelectedClass D.A S.oppApex2 P.rho := by
    apply mem_selectedClass.mpr
    exact ⟨hcenterA, hcenterBOData.2⟩
  have hcenterU : Hlate.centerAt P.u.1 P.u.2 = packet.xv := by
    simpa [Hlate, P.huSource] using packet.blocker_eq_xv
  rcases Q.hclassFive _ hcenterClass with hcu | hcxu | hcd | hcv | hxv
  · exact ((Finset.mem_erase.mp hcenterBOData.1).1 hcu).elim
  · exact ((centerAt_ne_source Hlate packet.xu Q.hxuA) hcxu).elim
  · exfalso
    apply Kxu.center_not_mem_support
    simpa [Kxu, hcd] using Q.hdeletedXuRow
  · exact hcv
  · have hcenterUXu :
        Hlate.centerAt P.u.1 P.u.2 = Hlate.centerAt packet.xu Q.hxuA := by
      exact hcenterU.trans hxv.symm
    have hsupports : Ku.support = Kxu.support := by
      simpa [Ku, Kxu] using
        (ATailSurvivalCover.selectedSupports_eq_of_actualBlockers_eq
          Hlate P.u.2 Q.hxuA hcenterUXu)
    exfalso
    apply Q.huNotXuRow
    rw [← hsupports]
    exact Ku.q_mem_support
...
seventeen points and the second opposite cap has eleven, ten or nine points,
or the carrier has at least eighteen points.  The seven named strict-interior
points of the second cap force that cap to have at least nine points, and the
cap sum together with the first-cap and surplus lower bounds forbids more
than eleven.  This is the positive half of the split that the
cardinality-at-least-seventeen coordinator below performs inline. -/
theorem exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagonBlockerV_vRowBlockerDeleted_deletedRowBlockerOffClass_card_ge_seventeen_capTrichotomy
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
    ...
    (_hcenterDeletedOffClass :
      (lateFirstApexSystem R).centerAt P.jointDeletion.deleted.1
        ...
        SelectedClass D.A S.oppApex2 P.rho)
    (_hcard : 17 ≤ D.A.card) :
    (D.A.card = 17 ∧ (S.capByIndex S.oppIndex2).card = 11) ∨
      (D.A.card = 17 ∧ (S.capByIndex S.oppIndex2).card = 10) ∨
      (D.A.card = 17 ∧ (S.capByIndex S.oppIndex2).card = 9) ∨
      18 ≤ D.A.card := by
  by_cases hcardEq : D.A.card = 17
  · by_cases hcapEleven : (S.capByIndex S.oppIndex2).card = 11
    · exact Or.inl ⟨hcardEq, hcapEleven⟩
    · by_cases hcapTen : (S.capByIndex S.oppIndex2).card = 10
      · exact Or.inr (Or.inl ⟨hcardEq, hcapTen⟩)
      · have hnamed :=
          exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagonBlockerV_vRowBlockerDeleted_deletedRowBlockerOffClass_namedSeven
            _Q _hcenterV _hcenterDeletedInterior _hcenterDeletedOffClass
        have hcapAdd :=
        ...
        have hcapNine : (S.capByIndex S.oppIndex2).card = 9 := by
          simpa only [capByIndex_oppIndex2_eq_oppCap2_sourceHeavy S] using
            hsecondNine
        exact Or.inr (Or.inr (Or.inl ⟨hcardEq, hcapNine⟩))
  · exact Or.inr (Or.inr (Or.inr (by omega)))

/-- Four-arm eliminator for the cardinality and cap trichotomy of the
off-class deleted-row `BlockerV` residual.  The exact-seventeen cap-eleven
and cap-ten arms go to the two certificate adapters, the exact-seventeen
cap-nine arm to the open second-cap-nine leaf, and the at-least-eighteen arm
to the open unbounded-cardinality leaf.  Once the coordinator below delegates
its whole split here, this theorem is the sole consumer of both of those
leaves. -/
theorem false_of_exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagonBlockerV_vRowBlockerDeleted_deletedRowBlockerOffClass_card_ge_seventeen_capSplit
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
    ...
    (hcapCases :
      (D.A.card = 17 ∧ (S.capByIndex S.oppIndex2).card = 11) ∨
        ...
        18 ≤ D.A.card) :
    False := by
  rcases hcapCases with ⟨hcardEq, hcapEleven⟩ | ⟨hcardEq, hcapTen⟩ |
    ⟨hcardEq, hcapNine⟩ | hcardEighteen
  · exact
      false_of_exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagonBlockerV_vRowBlockerDeleted_deletedRowBlockerOffClass_card_eq_seventeen_secondCapEleven
        _Q _hcenterV _hcenterDeletedInterior _hcenterDeletedOffClass
        hcardEq hcapEleven
  ...
  · exact
      false_of_exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagonBlockerV_vRowBlockerDeleted_deletedRowBlockerOffClass_card_ge_eighteen
        _Q _hcenterV _hcenterDeletedInterior _hcenterDeletedOffClass
        hcardEighteen _hnextRowPhysicalHits
...
theorem false_of_exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagonBlockerV_vRowBlockerDeleted_deletedRowBlockerOffClass_card_ge_seventeen
    ...
    False := by
  exact
    false_of_exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagonBlockerV_vRowBlockerDeleted_deletedRowBlockerOffClass_card_ge_seventeen_capSplit
      _Q _hcenterV _hcenterDeletedInterior _hcenterDeletedOffClass
      _hnextRowPhysicalHits
      (exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagonBlockerV_vRowBlockerDeleted_deletedRowBlockerOffClass_card_ge_seventeen_capTrichotomy
...
other three named class alternatives are impossible: `centerAt v = u` makes
`{u, v, xv}` an equilateral class triple, `centerAt v = xu` is killed by the
ordered-cap betweenness cycle, and `centerAt v = v` and `centerAt v = xv`
both contradict the row-center incidence of the `v` row.  This is the
positive half of the split that the `BlockerV` coordinator below performs
inline. -/
theorem exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagonBlockerV_vCenterDichotomy
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
  ...
  by_cases hcenterVClass :
      Hlate.centerAt P.v.1 P.v.2 ∈
        SelectedClass D.A S.oppApex2 P.rho
  · rcases Q.hclassFive _ hcenterVClass with
      hcenterU | hcenterXu | hcenterDeleted | hcenterV | hcenterXv
    · have hcenterXv : Hlate.centerAt packet.xv Q.hxvA = P.v.1 :=
        Q.hblockerEqV
      have hvxv_eq_vu :
          dist P.v.1 packet.xv = dist P.v.1 P.u.1 := by
        have h :=
      ...
      have huv_eq_uxv :
          dist P.u.1 P.v.1 = dist P.u.1 packet.xv := by
        have h :=
          (Kv.support_eq_radius _ Kv.q_mem_support).trans
            (Kv.support_eq_radius _ hxvVRow).symm
...
/-- The `nextRowOnlyHit` arm exports an exact four-point row after deleting
`u`, together with its three named support incidences. -/
theorem pentagonOffClassBlocker_nextRowOnlyHit_uDeletion_exactRow
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
    ...
    let hcA : c ∈ D.A := (Finset.mem_erase.mp Kxv.center_mem).2
    let Kc := (Hlate.selectedAt c hcA).toCriticalFourShell
    Nonempty (U5QDeletedK4Class D P.u.1 (Hlate.centerAt c hcA) Kc.support) ∧
      packet.xu ∈ Kc.support ∧
      P.u.1 ∉ Kc.support ∧
      packet.xv ∉ Kc.support := by
  classical
  let Hlate := lateFirstApexSystem R
  let Kxv :=
    (Hlate.selectedAt packet.xv Q.hxvA).toCriticalFourShell
  ...
  change
    Nonempty (U5QDeletedK4Class D P.u.1 (Hlate.centerAt c hcA) Kc.support) ∧
      packet.xu ∈ Kc.support ∧
      P.u.1 ∉ Kc.support ∧
      packet.xv ∉ Kc.support
  have hrow :
      U5QDeletedK4Class D P.u.1 (Hlate.centerAt c hcA) Kc.support :=
    criticalFourShellToQDeletedK4ClassOfNotMem Kc hnextRowOnlyHit.2.1
  exact ⟨⟨hrow⟩, hnextRowOnlyHit.1, hnextRowOnlyHit.2.1,
    hnextRowOnlyHit.2.2⟩

/-- Under the exact four-point interior normal form, an interior next-row
center in the `nextRowOnlyHit` arm is one of the two omitted named points. -/
theorem pentagonOffClassBlocker_nextRowOnlyHit_interior_center_eq_u_or_xv
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
    ...
    let hcA : c ∈ D.A := (Finset.mem_erase.mp Kxv.center_mem).2
    let Kc := (Hlate.selectedAt c hcA).toCriticalFourShell
    Hlate.centerAt c hcA ∈ S.capInteriorByIndex S.oppIndex2 →
      Hlate.centerAt c hcA = P.u.1 ∨
        Hlate.centerAt c hcA = packet.xv := by
  classical
  let Hlate := lateFirstApexSystem R
  let Kxv :=
    (Hlate.selectedAt packet.xv Q.hxvA).toCriticalFourShell
  ...
  have hcA : c ∈ D.A := (Finset.mem_erase.mp Kxv.center_mem).2
  let Kc := (Hlate.selectedAt c hcA).toCriticalFourShell
  let KcSupport : Finset ℝ² := Kc.support
  change
    packet.xu ∈ Kc.support ∧
      P.u.1 ∉ Kc.support ∧ packet.xv ∉ Kc.support at hnextRowOnlyHit
  change
    Hlate.centerAt c hcA ∈ S.capInteriorByIndex S.oppIndex2 →
      Hlate.centerAt c hcA = P.u.1 ∨
        Hlate.centerAt c hcA = packet.xv
  intro hcenterInterior
  have hcenterNamed :
      Hlate.centerAt c hcA ∈
        ({P.u.1, c, packet.xv, packet.xu} : Finset ℝ²) := by
    rw [← hinterior_eq]
    exact hcenterInterior
  simp only [Finset.mem_insert, Finset.mem_singleton] at hcenterNamed
  rcases hcenterNamed with hcenterU | hcenterC | hcenterXv | hcenterXu
  · exact Or.inl hcenterU
  · exact (centerAt_ne_source Hlate c hcA hcenterC).elim
  · exact Or.inr hcenterXv
  · exfalso
    apply Kc.center_not_mem_support
    have hxuSupport : packet.xu ∈ KcSupport := by
      simpa only [KcSupport] using hnextRowOnlyHit.1
    have hcenterSupport : Hlate.centerAt c hcA ∈ KcSupport := by
      exact hcenterXu.symm ▸ hxuSupport
    simpa only [KcSupport] using hcenterSupport

/- The exact next-row arm has one named physical-class hit, namely `xu`.
...
theorem pentagonOffClassBlocker_nextRowOnlyHit_namedTriple_inter_eq_xu
    ...
    let hcA : c ∈ D.A := (Finset.mem_erase.mp Kxv.center_mem).2
    let Kc := (Hlate.selectedAt c hcA).toCriticalFourShell
    Kc.support ∩ {P.u.1, packet.xu, packet.xv} = {packet.xu} := by
  classical
  let Hlate := lateFirstApexSystem R
  let Kxv :=
    (Hlate.selectedAt packet.xv Q.hxvA).toCriticalFourShell
  ...
  let Kc := (Hlate.selectedAt c hcA).toCriticalFourShell
  change
    Kc.support ∩ {P.u.1, packet.xu, packet.xv} = {packet.xu} at ⊢
  change
    packet.xu ∈ Kc.support ∧
      P.u.1 ∉ Kc.support ∧ packet.xv ∉ Kc.support at hnextRowOnlyHit
  ext z
  constructor
  · intro hz
    have hzSupport : z ∈ Kc.support := (Finset.mem_inter.mp hz).1
    have hzTriple : z ∈ ({P.u.1, packet.xu, packet.xv} : Finset ℝ²) :=
      (Finset.mem_inter.mp hz).2
    simp only [Finset.mem_insert, Finset.mem_singleton] at hzTriple
    rcases hzTriple with hzU | hzXu | hzXv
    · exact (hnextRowOnlyHit.2.1 (hzU ▸ hzSupport)).elim
    · simpa only [Finset.mem_singleton] using hzXu
    · exact (hnextRowOnlyHit.2.2 (hzXv ▸ hzSupport)).elim
  · intro hz
    have hzXu : z = packet.xu := by
      simpa only [Finset.mem_singleton] using hz
    subst z
    exact Finset.mem_inter.mpr ⟨hnextRowOnlyHit.1, by simp⟩
...
/-- Blocker-identity trichotomy of the pentagon: the `xv`-row blocker is the
joint-deletion point, or the opposite source `v`, or it lies outside the
physical second-apex class.  The other two named class alternatives are
impossible: `centerAt xv = u` contradicts the row-center incidence of the
`xv` row, and `centerAt xv = xu` makes `{u, xu, xv}` equilateral, which the
convex-hull kill forbids.  This is the positive half of the pentagon split
that the coordinator below performs inline. -/
theorem exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagon_blockerCenterTrichotomy
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
    ...
    · have hxuInter :
      ...
      exact
        (exactFourRigid221_sourceHeavy_pentagon_equilateralXu_false
          P _packet hxuA hxvA huxv_eq_xuxv huxu_eq_xuxv).elim
    · exact Or.inl hcdeleted
    · exact Or.inr (Or.inl hcv)
    · exact (centerAt_ne_source Hlate _packet.xv hxvA hcxv).elim
  · exact Or.inr (Or.inr hblockerClass)

/-- Three-arm eliminator for the pentagon blocker-identity trichotomy.  The
joint-deletion arm goes to the open pentagon blocker-deleted leaf, the
opposite-source arm to the `BlockerV` continuation, and the off-class arm to
the off-class blocker continuation.  Once the coordinator below delegates its
whole split here, this theorem is the sole consumer of the pentagon
blocker-deleted leaf. -/
theorem false_of_exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagon_blockerCenterSplit
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : ATailUniqueArmRouteAuditScratch.OriginalUniqueFourResidual F}
  ...
  have hcxvInterior :=
    exactFourRigid221_sourceHeavy_pentagon_blocker_mem_secondCapInterior
      P _packet hxvA _huXvRow
  rcases hblockerCenter with hcdeleted | hcv | hblockerClass
  · exact
...
theorem false_of_exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagon
    ...
    False := by
  exact
    false_of_exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagon_blockerCenterSplit
      P _packet _hsecond hxuA hxvA _huXvRow _huNotXuRow _hxvNotXuRow
      _hdeletedXuRow _hvDeletedRow _huNotDeletedRow _hxuNotDeletedRow
      _hxvNotDeletedRow _hclassFive
      (exactFourRigid221_sourceHeavy_secondOppositeLarge_pentagon_blockerCenterTrichotomy
