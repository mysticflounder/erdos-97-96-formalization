...
/-!
# Exact-twelve next-row static-convex cell-0 third positive cut

After replaying the first two static-convex cell-0 positive cuts, the next
authenticated survivor contains three sufficient convex-five-point cores.
Their common orientations cover all 48 source-entitled frozen boundary
orders. This file packages that finite obstruction as a proof-carrying
...
/-- The five selected rows used by the third static-convex cell-0 survivor. -/
def nextRowStaticConvexCell0ThirdPositiveChoices : List (RowChoice Label) :=
  [ { center := 3, support := {0, 4, 5, 6} }
  , { center := 6, support := {4, 9, 10, 11} }
...
theorem FrozenBoundaryOrder.sourceOrderCoverage_nextRowStaticConvexCell0Third
    ...
    CommonFiveOrientationAt order.position 4 3 6 11 7 ∨
      ...
      CommonFiveOrientationAt order.position 3 10 9 6 11 := by
  have h3ne4 : order.position 3 ≠ order.position 4 :=
    order.position_ne_of_ne_nextRowStaticConvexCell0Third (by decide)
...
/-- The third survivor's positive selected rows force a convex five-point core
in every frozen source order. -/
def nextRowStaticConvexCell0ThirdPositiveNogood : SourceOrderPositiveNogood where
  choices := nextRowStaticConvexCell0ThirdPositiveChoices
  refutes := by
    ...
    have hrow3 : ({0, 4, 5, 6} : Finset Label) ⊆ row 3 :=
      hpositive ({ center := 3, support := {0, 4, 5, 6} } : RowChoice Label)
        (by simp [nextRowStaticConvexCell0ThirdPositiveChoices])
    have hrow6 : ({4, 9, 10, 11} : Finset Label) ⊆ row 6 :=
      hpositive ({ center := 6, support := {4, 9, 10, 11} } : RowChoice Label)
        (by simp [nextRowStaticConvexCell0ThirdPositiveChoices])
    ...
    let coreC : Census554.ConvexFivePointCore.Core row :=
      { a := 3
        ...
        hab := by decide
        hxy := by decide
        xa_xb := EdgeClosure.row 10 3 9 (hrow10 (by decide)) (hrow10 (by decide))
        ya_yb := EdgeClosure.row 11 3 9 (hrow11 (by decide)) (hrow11 (by decide))
        cb_cx := EdgeClosure.row 6 9 10 (hrow6 (by decide)) (hrow6 (by decide))
        cb_cy := EdgeClosure.row 6 9 11 (hrow6 (by decide)) (hrow6 (by decide)) }
    rcases order.sourceOrderCoverage_nextRowStaticConvexCell0Third hforced with
      hA | hB | hC
...
/-- The complete positive-membership footprint used by the third survivor's
three alternative convex-five-point cores.  Unlike
`nextRowStaticConvexCell0ThirdPositiveChoices`, these supports retain only the
incidences consumed by the proof. -/
def nextRowStaticConvexCell0ThirdMembershipChoices : List (RowChoice Label) :=
...
/-- The third cell-0 obstruction is stable under arbitrary changes to the
unused points of its five selected rows.  All three core alternatives are
retained: the first two alone cover only 36 of the 48 source orders. -/
def nextRowStaticConvexCell0ThirdMembershipNogood : SourceOrderPositiveNogood where
  choices := nextRowStaticConvexCell0ThirdMembershipChoices
  refutes := by
    intro row pointOf hreal order hforced _hconv hpositive
    have hrow3 : ({4, 6} : Finset Label) ⊆ row 3 :=
      hpositive ({ center := 3, support := {4, 6} } : RowChoice Label)
        (by simp [nextRowStaticConvexCell0ThirdMembershipChoices])
    have hrow6 : ({9, 10, 11} : Finset Label) ⊆ row 6 :=
      hpositive ({ center := 6, support := {9, 10, 11} } : RowChoice Label)
        (by simp [nextRowStaticConvexCell0ThirdMembershipChoices])
    have hrow7 : ({4, 6} : Finset Label) ⊆ row 7 :=
      hpositive ({ center := 7, support := {4, 6} } : RowChoice Label)
        (by simp [nextRowStaticConvexCell0ThirdMembershipChoices])
    have hrow10 : ({3, 9} : Finset Label) ⊆ row 10 :=
      hpositive ({ center := 10, support := {3, 9} } : RowChoice Label)
        (by simp [nextRowStaticConvexCell0ThirdMembershipChoices])
    have hrow11 : ({3, 6, 7, 9} : Finset Label) ⊆ row 11 :=
      hpositive ({ center := 11, support := {3, 6, 7, 9} } : RowChoice Label)
        (by simp [nextRowStaticConvexCell0ThirdMembershipChoices])
