...
/-!
# Cycle-free fresh-canonical override

This module exposes the normalized-only FreshThird terminal and the historical
fresh-canonical row override without importing the downstream first-fiber or
generic closure coordinators.
-/
...
/-- Cycle-free fresh-third terminal built only from the normalized obstruction.
The two alignment arms and the distinct-cross-row arm close immediately; the
remaining constructor is passed to the upstream normalized residual terminal. -/
theorem false_of_capSource_freshThirdBlockerFiber_normalizedOnly
    ...
    · obtain ⟨firstInteraction⟩ :=
      ...
      let obstruction :=
        FreshThirdTwoCapSourceObstruction.normalized
          (P := P) (Pρ := Pρ) C Q firstInteraction secondInteraction
...
theorem false_of_freshCanonicalRowOverride
  ...
  exact
    false_of_capSource_freshThirdBlockerFiber_normalizedOnly
      (P := P') (Pρ := Pρ')
      (hρne := hρne)
