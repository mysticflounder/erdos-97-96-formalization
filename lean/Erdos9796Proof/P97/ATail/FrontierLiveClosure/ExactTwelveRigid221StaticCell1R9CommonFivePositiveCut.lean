...
/-!
# Exact-twelve static cell-1 r9 common-five cut

The first live r9 survivor after eager installation of the 70-entry source-order
bank contains three positive selected rows that force one convex-five-point core
...
/-- The three selected rows used by the first live r9 cell-1 survivor cut. -/
def staticCell1R9CommonFivePositiveChoices : List (RowChoice Label) :=
  [ { center := 5, support := {0, 7, 9, 10} }
  , { center := 7, support := {4, 6, 8, 10} }
  , { center := 9, support := {2, 4, 10, 11} } ]

/-- The frozen boundary blocks and forced second-cap direction give the r9
cell-1 core one common orientation. -/
theorem FrozenBoundaryOrder.commonOrientation_core_4_7_10_5_9
...
/-- The three positive rows from the first live r9 survivor force a convex
five-point equality core in every frozen source order. -/
def staticCell1R9CommonFivePositiveNogood : SourceOrderPositiveNogood where
  choices := staticCell1R9CommonFivePositiveChoices
  refutes := by
    ...
    have hrow5 : ({0, 7, 9, 10} : Finset Label) ⊆ row 5 :=
      hpositive ({ center := 5, support := {0, 7, 9, 10} } : RowChoice Label)
        (by simp [staticCell1R9CommonFivePositiveChoices])
    ...
    exact order.false_of_convexFivePointCore hreal core
      (order.commonOrientation_core_4_7_10_5_9 hforced)
