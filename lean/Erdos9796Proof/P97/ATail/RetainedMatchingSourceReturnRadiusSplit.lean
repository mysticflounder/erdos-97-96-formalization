...
namespace Problem97
namespace ATailRetainedMatchingSourceReturnRadiusSplit
...
structure RetainedSourceReturnJointU5Ingress
  ...
  firstBlockerRow :
    U5QDeletedK4Class D J.source
      (H.centerAt W.first W.first_mem_A) J.firstPacket.B₂
  secondBlockerRow :
    U5QDeletedK4Class D J.source
      (H.centerAt W.second W.second_mem_A) J.secondPacket.B₂
  actualCriticalRow :
...
theorem RetainedSourceReturnJointDeletion.nonempty_u5Ingress
  ...
  have hsourceNeApex : J.source ≠ S.oppApex1 := by
    ...
    rw [h, dist_self] at hzero
    linarith
  have hEraseThree : 3 ≤ (C.erase J.source).card := by
    have hCfour : 4 ≤ C.card := by
      simpa [C] using R.frontierRadius_class_card_ge_four
    rw [Finset.card_erase_of_mem (by simpa [C] using J.source_mem_radius)]
    omega
  rcases Finset.exists_subset_card_eq
      (s := C.erase J.source) hEraseThree with ⟨T, hTsub, hTcard⟩
  have hTsubA : T ⊆ D.A := by
    intro z hz
    have hzC : z ∈ C := (Finset.mem_erase.mp (hTsub hz)).2
  ...
  exact ⟨{
    T := T
    dangerous := dangerous
    firstBlockerRow := J.firstPacket.row₂
    secondBlockerRow := J.secondPacket.row₂
...
/-- Parent-facing source-return surface after constructing the concrete U5
rows in both exact radius-class arms. -/
inductive RetainedSourceReturnU5Outcome
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : FrontierCommonDeletionParentResidual F}
    (W : RetainedMatchingTwoStepCommonDeletionWalk R) : Type
  | jointDeletion
      (data : RetainedSourceReturnJointDeletion W)
      (ingress : RetainedSourceReturnJointU5Ingress data)
  | exactFourPartition
      (data : RetainedSourceReturnExactFourPartition W)
      (ingress : RetainedSourceReturnExactFourU5Ingress data)

/-- The source-return arm reaches one of the two concrete U5 row surfaces
without discarding the retained-radius or common-deletion provenance. -/
theorem nonempty_sourceReturnU5Outcome
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A}
    {F : CriticalPairFrontier D S radius H}
    {R : FrontierCommonDeletionParentResidual F}
    (W : RetainedMatchingTwoStepCommonDeletionWalk R)
    (hreturn : W.next = W.first) :
    Nonempty (RetainedSourceReturnU5Outcome W) := by
  rcases nonempty_sourceReturnRadiusOutcome W hreturn with ⟨outcome⟩
  cases outcome with
  | jointDeletion data =>
      rcases data.nonempty_u5Ingress with ⟨ingress⟩
      exact ⟨RetainedSourceReturnU5Outcome.jointDeletion data ingress⟩
  | exactFourPartition data =>
      rcases data.nonempty_u5Ingress with ⟨ingress⟩
      exact ⟨RetainedSourceReturnU5Outcome.exactFourPartition data ingress⟩
...
#print axioms nonempty_sourceReturnU5Outcome

end

end ATailRetainedMatchingSourceReturnRadiusSplit
end Problem97
