...
/-!
# Cap-strengthened bi-apex parent surface

The generic `FrontierBiApexRobustResidual` records the second-apex robustness
proof. The live large-opposite-caps branch additionally retains lower bounds
of six on both opposite caps. This module packages that complete branch
surface and the rebase ladder needed to choose a critical-shell system after
the physical frontier has been selected.

The public mathematical target on this surface is direct `False`. The rebase
definitions are optional plumbing for an internal proof using a favorable
late critical-shell system; they do not replace the parent contradiction.
-/

open scoped EuclideanGeometry

namespace Problem97
namespace ATailLargeOppositeCapsBiApexSurface
...
/-- The robust parent branch after both small-cap arms have been dispatched. -/
...
/-- Rebase the cap-strengthened wrapper by copying its geometric cap bounds. -/
...
end ATailLargeOppositeCapsBiApexSurface
end Problem97
