...
This module packages the residual in which the physical second apex has a
unique positive K4 radius whose complete ambient radius class has cardinality
five.  When the closed opposite cap has at least six points, it produces an
unused cap point outside that complete class.  Erasing the unused point
preserves the class and its uniqueness.  A retained critical-shell system then
supplies an exact critical row at the unused point whose blocker differs from
the physical apex.
-/
...
namespace Problem97
namespace ATailLargeCapUniqueFive

attribute [local instance] Classical.propDecidable

/-- A positive radius at the physical second apex whose complete ambient
radius class has cardinality exactly five and is the unique K4 radius there. -/
...
/-- An unused point in the closed opposite cap together with persistence of
the complete exact-five physical-apex filter after erasing it. -/
...
/-- The retained critical map at an unused cap point.  Its actual blocker
differs from the physical second apex because K4 at that apex survives the
source deletion. -/
...
/-- The unused sixth cap point installs a retained exact critical row whose
blocker differs from the physical apex. -/
theorem nonempty_largeCapUniqueFiveUnusedCriticalRow
  ...
  have hphysical :
    ...
    rw [unused.radiusClass_erase_eq, profile.class_card_eq_five]
    omega
  have hblockerNe :
