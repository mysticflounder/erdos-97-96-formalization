...
`J`'s actual critical row contain the other collision source `K`.

The boundary order `O < J < C < A < X < K` is compatible with the mandatory
alternation of `C,K` across the first apex `O` and common blocker `A`. In that
order the three equalities centered at `O,A,X` form a strict two-edge cycle.
-/

open scoped EuclideanGeometry

namespace Problem97
namespace ATailRetainedCollisionDifferentPairConsumer
...
/-- The prefix supplied automatically by `RetainedRadiusCollision.nonempty_rowHit`,
strengthened only by choosing another row point away from both collision sources. -/
...
pair `OJ = OC`. It does not supply an equality centered away from the first
...
The displayed order is one of the two live ordinal-cyclic order types, not the
incompatible `O < A < X < J < C < K` order of the older terminal. -/
...
/-- The second live ordinal-cyclic order type. It uses the same source-valid
cross incidence, but its boundary order is `O < C < J < X < A < K`. -/
...
/-- The alternate live cyclic order is contradictory by `JX < CX < JX`. -/
theorem AlternateOrderedDifferentPairCrossHit.false
    ...
    {R : FrontierCommonDeletionParentResidual F}
    {Q : RetainedRadiusCollision (R := R)}
    (P : AlternateOrderedDifferentPairCrossHit Q) : False := by
  have hOJ_eq_OC :
      dist (P.boundary P.iO) (P.boundary P.iJ) =
        dist (P.boundary P.iO) (P.boundary P.iC) := by
    have hEq :=
  ...
  have hJX_lt_CX :
      dist (P.boundary P.iJ) (P.boundary P.iX) <
        dist (P.boundary P.iC) (P.boundary P.iX) := by
    have hkal :=
      complementary_dist_add_dist_lt_diagonal_sum_of_ccw D.convex
        P.boundary_injective P.boundary_image P.boundary_ccw
        P.hOC P.hCJ P.hJX
    have hkal' :
        dist (P.boundary P.iO) (P.boundary P.iC) +
            dist (P.boundary P.iJ) (P.boundary P.iX) <
          dist (P.boundary P.iO) (P.boundary P.iJ) +
            dist (P.boundary P.iC) (P.boundary P.iX) := hkal
    rw [hOJ_eq_OC] at hkal'
    linarith
  have hXJ_eq_XK :
      dist (P.boundary P.iX) (P.boundary P.iJ) =
        dist (P.boundary P.iX) (P.boundary P.iK) := by
  ...
  have hAC_eq_AK :
    ...
    simpa only [P.boundary_A, P.boundary_C, P.boundary_K] using
      P.fresh.fiber_orientation.commonBlocker_equidistant
  have hCX_lt_JX :
      dist (P.boundary P.iC) (P.boundary P.iX) <
        dist (P.boundary P.iJ) (P.boundary P.iX) := by
    have hkal :=
      complementary_dist_add_dist_lt_diagonal_sum_of_ccw D.convex
        P.boundary_injective P.boundary_image P.boundary_ccw
        (P.hCJ.trans P.hJX) P.hXA P.hAK
    have hkal' :
        dist (P.boundary P.iC) (P.boundary P.iX) +
            dist (P.boundary P.iA) (P.boundary P.iK) <
          dist (P.boundary P.iC) (P.boundary P.iA) +
            dist (P.boundary P.iX) (P.boundary P.iK) := hkal
    rw [dist_comm (P.boundary P.iC) (P.boundary P.iA),
      ← hXJ_eq_XK,
      dist_comm (P.boundary P.iX) (P.boundary P.iJ),
      ← hAC_eq_AK] at hkal'
    linarith
  exact (not_lt_of_ge hJX_lt_CX.le) hCX_lt_JX

#print axioms nonempty_freshFirstApexRowPoint
#print axioms OrderedDifferentPairCrossHit.false
#print axioms AlternateOrderedDifferentPairCrossHit.false

end ATailRetainedCollisionDifferentPairConsumer
end Problem97
