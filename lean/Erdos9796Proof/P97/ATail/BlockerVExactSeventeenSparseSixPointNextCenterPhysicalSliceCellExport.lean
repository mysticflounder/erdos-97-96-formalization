...
/-! Direct exporter for one exact-17 next-center/physical-slice cell. -/

namespace Problem97
namespace ATailBlockerVExactSeventeenSparseSixPointNextCenterPhysicalSliceCellExport
...
def physicalSliceCellDimacsString
  ...
  let dimacs :=
    (sparseSixPointNextCenterPhysicalSliceCellCnf center category).map
...
def run (args : List String) : IO UInt32 := do
  ...
  | _ =>
      IO.eprintln
        "usage: lake env lean --run <exporter> <center> <none|physical-label> <output.cnf>"
      pure 2
