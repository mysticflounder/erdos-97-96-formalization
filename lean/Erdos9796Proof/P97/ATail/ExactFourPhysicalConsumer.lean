...
This production module is a source-faithful producer.  It does not claim that the
resulting physical-second-apex robust/swapped split is itself contradictory.
-/

open scoped EuclideanGeometry

namespace Problem97
namespace ATailExactFourPhysicalConsumer
...
/-- A specified good outside source whose actual blocker is not the physical
second apex can be promoted without changing the source to the full
common-deletion ingress packet. -/
theorem nonempty_exactFourPhysicalCommonDeletionIngress_of_source
    {D : CounterexampleData} {S : SurplusCapPacket D.A} {radius : ℝ}
    {H : CriticalShellSystem D.A} {F : CriticalPairFrontier D S radius H}
    (R : OriginalUniqueFourResidual F)
    {source : CarrierVertex D.A}
    (hsourceGood : source ∈ goodOutsideSources R)
    (hcenterNe :
      (lateFirstApexSystem R).centerAt source.1 source.2 ≠ S.oppApex2) :
...
theorem nonempty_exactFourPhysicalCommonDeletionIngress
  ...
  rcases nonempty_exactFourPhysicalCommonDeletionIngress_of_source
      R hsourceGood hcenterNe with ⟨ingress, _⟩
  exact ⟨ingress⟩
...
end ATailExactFourPhysicalConsumer
end Problem97
