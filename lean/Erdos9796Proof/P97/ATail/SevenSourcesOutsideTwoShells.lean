...
/- The card-thirteen threshold used by the large-cardinality two-row
residual leaves five points outside the two selected supports.  This is kept
separate from the sharper card-fifteen statement above so callers cannot
silently strengthen a `13 ≤ card` hypothesis to seven sources. -/

/-- The complement of two selected four-point supports has cardinality at
least five in a carrier of cardinality at least thirteen. -/
theorem five_le_outside_two_selected_supports_card
    {A : Finset ℝ²} (hA : 13 ≤ A.card) {c₁ c₂ : ℝ²}
    (K₁ : SelectedFourClass A c₁) (K₂ : SelectedFourClass A c₂) :
    5 ≤ (A \ (K₁.support ∪ K₂.support)).card := by
  classical
  have hsub : K₁.support ∪ K₂.support ⊆ A :=
    Finset.union_subset K₁.support_subset_A K₂.support_subset_A
  have hUnion : (K₁.support ∪ K₂.support).card ≤ 8 := by
...
/-- Choose exactly five carrier vertices whose underlying points lie outside
both selected four-point supports. -/
theorem exists_five_carrierVertices_outside_two_selected_supports
    {A : Finset ℝ²} (hA : 13 ≤ A.card) {c₁ c₂ : ℝ²}
    (K₁ : SelectedFourClass A c₁) (K₂ : SelectedFourClass A c₂) :
    ∃ T : Finset (CriticalShellSystem.CarrierVertex A),
      T.card = 5 ∧
  ...
  classical
  let outsidePoints := A \ (K₁.support ∪ K₂.support)
  have hOutside : 5 ≤ outsidePoints.card := by
    simpa [outsidePoints] using
      five_le_outside_two_selected_supports_card hA K₁ K₂
  rcases Finset.exists_subset_card_eq (s := outsidePoints) hOutside with
    ⟨B, hBsub, hBcard⟩
  have hBA : B ⊆ A := by
    intro x hx
  ...
  let T : Finset (CriticalShellSystem.CarrierVertex A) := Finset.univ.map f
  refine ⟨T, ?_, ?_⟩
  · change (Finset.univ.map f).card = 5
    rw [Finset.card_map]
    simpa using hBcard
  · intro q hq
    change q ∈ Finset.univ.map f at hq
