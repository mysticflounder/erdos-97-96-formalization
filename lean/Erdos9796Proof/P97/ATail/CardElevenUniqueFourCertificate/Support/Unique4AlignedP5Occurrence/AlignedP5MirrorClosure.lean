...
import Erdos9796Proof.P97.ATail.CardElevenUniqueFourCertificate.Support.Unique4AlignedP5Occurrence.AlignedP5KalmansonDecode
import Erdos9796Proof.P97.ATail.CardElevenUniqueFourCertificate.Support.Unique4AlignedP5Occurrence.AlignedP5MirrorSourceTrace

/-!
# Mirror aligned-p5 boundary closure

The semantic source trace for a mirror aligned-p5 boundary packet enters the
closed mirror native coverage theorem.  The resulting executable Kalmanson
occurrence is then impossible by the orientation-aware geometric decoder.
-/
...
/-- A mirror aligned-p5 boundary packet contradicts the geometric Kalmanson
terminals after tracing its selected rows through the closed native search. -/
theorem false_of_mirrorBoundaryPacket
    (Q : AlignedP5MirrorBoundaryPacket R profile P) : False := by
