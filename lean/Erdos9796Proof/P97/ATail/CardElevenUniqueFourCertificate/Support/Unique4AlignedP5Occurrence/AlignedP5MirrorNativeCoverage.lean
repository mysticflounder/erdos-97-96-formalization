...
import Erdos9796Proof.P97.ATail.CardElevenUniqueFourCertificate.Support.Unique4AlignedP5Occurrence.AlignedP5MirrorNativeTheorem
import Erdos9796Proof.P97.Census554.CapSelectedNativeClassifierSound

/-!
# Semantic coverage for the mirror aligned-p5 native classifier
...
namespace Problem97
namespace ATailAlignedP5MirrorNativeCoverageScratch
  ...
  firstRow :
    ∃ extra,
      extra ∈ ATailAlignedP5MirrorNativeClassifierScratch.extraPoints ∧
        rowOfPattern P 0 = ATailAlignedP5MirrorNativeClassifierScratch.fixedFirstRow extra
  candidate :
    ∀ center : Label,
      rowOfPattern P center ∈ ATailAlignedP5MirrorNativeClassifierScratch.candidateRows center.val
  compatible :
...
/-- Every source-valid semantic trace through the mirror aligned classifier contains
...
theorem hasKalmansonOccurrence_of_semanticTrace
  ...
  have hcheck :
      ATailAlignedP5MirrorNativeClassifierScratch.placementCheckForExtra
        extra = true := by
    have hall :
        ATailAlignedP5MirrorNativeClassifierScratch.extraPoints.all
          ATailAlignedP5MirrorNativeClassifierScratch.placementCheckForExtra =
            true := by
      simpa [ATailAlignedP5MirrorNativeClassifierScratch.placementCheck] using
        ATailAlignedP5MirrorNativeClassifierScratch.placementCheck_eq_true
    exact List.all_eq_true.mp hall extra hextra
  let fixed : Row := ATailAlignedP5MirrorNativeClassifierScratch.fixedFirstRow extra
  have hfixedLocal :
      ATailAlignedP5MirrorNativeClassifierScratch.localCandidateOK
        fixed.center fixed.support = true := by
    have hcandidate := trace.candidate 0
    have hmembership :
        fixed ∈ ATailAlignedP5MirrorNativeClassifierScratch.candidateRows 0 := by
      simpa [fixed, hfirstRow] using hcandidate
    rcases List.mem_filterMap.mp hmembership with
      ⟨support, _hsupportRange, hsupport⟩
    by_cases hlocal : ATailAlignedP5MirrorNativeClassifierScratch.localCandidateOK 0 support
    · simp [hlocal] at hsupport
      simpa [fixed, ← hsupport] using hlocal
    · simp [hlocal] at hsupport
  let domains : List Domain :=
    ATailAlignedP5MirrorNativeClassifierScratch.variableCenters.map fun center =>
      restrictDomain [fixed]
        { center
          rows :=
            ATailAlignedP5MirrorNativeClassifierScratch.candidateRows center }
  have hdomainCenters :
      (domains.map
        Census554.CapSelectedNativeClassifier.Domain.center).Nodup := by
  ...
  have hcarry :
    ...
    have hcandidate :
        rowOfPattern P center ∈
          ATailAlignedP5MirrorNativeClassifierScratch.candidateRows
            center.val :=
  ...
  have hsearch :
      allKilled ATailAlignedP5MirrorNativeClassifierScratch.variableCenters.length
          (([0] : List Label).map (rowOfPattern P)) domains = false :=
    allKilled_eq_false_of_semanticTrace trace hnoCore
      ATailAlignedP5MirrorNativeClassifierScratch.variableCenters.length
      [0] domains (by simp) hdomainCenters hcarry
  have hsearchFixed :
      allKilled
          ATailAlignedP5MirrorNativeClassifierScratch.variableCenters.length
          [fixed] domains = false := by
    simpa [fixed, hfirstRow] using hsearch
  have hplacement :
      ATailAlignedP5MirrorNativeClassifierScratch.placementCheckForExtra
        extra = false := by
    simp only [ATailAlignedP5MirrorNativeClassifierScratch.placementCheckForExtra]
    rw [show ATailAlignedP5MirrorNativeClassifierScratch.extraPoints.contains extra = true by
...
end ATailAlignedP5MirrorNativeCoverageScratch
end Problem97
