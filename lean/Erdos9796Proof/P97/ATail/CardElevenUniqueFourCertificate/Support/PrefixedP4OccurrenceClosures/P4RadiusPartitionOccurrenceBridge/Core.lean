...
/-!
# P4 radius-partition occurrence bridge: native atom interface

The P4 compact formula has its own dense numbering.  This module deliberately
defines only its equality prefix.  The radius-partition family has no class
literals, so it does not reuse the P5 valuation or P5's post-605 class
numbering.
-/
...
/-- Agreement of an arbitrary total valuation with P4's native row/equality prefix. -/
...
theorem CoreValAgreement.sortedRadius (P : P4DirectBoundaryPacket R profile distribution)
  ...
  unfold Problem97.P4RadiusPartitionOccurrenceBridgeScratch.sortedRadius
  split
