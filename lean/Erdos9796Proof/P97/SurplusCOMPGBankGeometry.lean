...
set_option maxHeartbeats 2000000 in
-- Finite case split over the generated unordered center-pair list.
/-- Every generated unordered label pair is present in the ordered search-pair
list. -/
theorem labelPairs_mem_orderedLabelPairs {c cp : Label}
    (hpair : (c, cp) ∈ labelPairs) :
    (c, cp) ∈ orderedLabelPairs := by
  cases c <;> cases cp <;> simp [labelPairs, orderedLabelPairs] at hpair ⊢
...
/-- Right-oriented erased-pin producer for generated ordered search separation.
This is the search-side companion to
`rightPinned_sepOKFor_of_exactCaps_selectedClasses`. -/
theorem rightPinned_crossSeparationOKForMasks_of_exactCaps_selectedClasses
    {A : Finset ℝ²} (S : SurplusCapPacket A)
    (hK4 : HasNEquidistantProperty 4 A)
    (hM44 : S.IsM44) (hcontain : S.NonSurplusMoserCapContainment)
    {p₁ p₂ q₁ q₂ s1 s2 s3 : ℝ²} {radius : ℝ}
  ...
  have hsame : ∀ center a b : Label,
    ...
    · exact sameRadius_of_centerClass_eq_selectedClass
        (hselectedOther .Q2 (by decide) (by decide) (by decide)
          (by decide)) a b ha hb
  intro c cp hcenterPair
  have hccp : c ≠ cp := orderedLabelPairs_ne hcenterPair
  exact
    crossSeparationOKForMasks_of_sameRadius_ccwHull
      (pointOf := pointOf) hccw hinj hccp
      (hsame c) (hsame cp)

/-- Reflected-hull right-oriented erased-pin producer for generated ordered
search separation.  This is used when the boundary order supplies the mirror
orientation of the right finite-bank template. -/
theorem rightPinned_crossSeparationOKForMasks_of_reflectedExactCaps_selectedClasses
    {A : Finset ℝ²} (S : SurplusCapPacket A)
    (hK4 : HasNEquidistantProperty 4 A)
    (hM44 : S.IsM44) (hcontain : S.NonSurplusMoserCapContainment)
    {p₁ p₂ q₁ q₂ s1 s2 s3 : ℝ²} {radius : ℝ}
  ...
  have hccp : c ≠ cp := orderedLabelPairs_ne hcenterPair
  exact
    crossSeparationOKForMasks_of_sameRadius_reflectedCcwHull
      (pointOf := pointOf) hccw hinj hccp
      (hsame c) (hsame cp)
...
theorem leftPinned_crossSeparationOKForMasks_of_exactCaps_selectedClasses
    {A : Finset ℝ²} (S : SurplusCapPacket A)
    (hK4 : HasNEquidistantProperty 4 A)
    (hM44 : S.IsM44) (hcontain : S.NonSurplusMoserCapContainment)
    {p₁ p₂ q₁ q₂ s1 s2 s3 : ℝ²} {radius : ℝ}
...
/-- Reflected-hull left-oriented erased-pin producer for generated ordered
search separation.  This is used when the boundary order supplies the mirror
orientation of the left finite-bank template. -/
theorem leftPinned_crossSeparationOKForMasks_of_reflectedExactCaps_selectedClasses
    {A : Finset ℝ²} (S : SurplusCapPacket A)
    (hK4 : HasNEquidistantProperty 4 A)
    (hM44 : S.IsM44) (hcontain : S.NonSurplusMoserCapContainment)
    {p₁ p₂ q₁ q₂ s1 s2 s3 : ℝ²} {radius : ℝ}
...
/-- Generated-domain variant of
`isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks`.
The separation hypotheses match the finite DFS checkers exactly: unordered
`labelPairs` for pointwise separation and `orderedLabelPairs` for search
cross-separation. -/
theorem
    isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    (hs : isSurplusStar seed.sstar = true)
    (hv : pointMask pointOf (centerClass .v) = firstOppExactCapMask)
...
theorem
    false_of_oneSidedSeedShadow_pointClasses_of_exact_vw_private_candidates_labelPairs
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    (hseed : seed ∈ oneSidedSeeds)
    (hv : pointMask pointOf (centerClass .v) = firstOppExactCapMask)
...
/-- A one-`.Q`, two-surplus, one-Moser row class has the generated mask. -/
theorem pointMask_eq_QSurplusPairMoserMask
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    (hinj : Function.Injective pointOf)
    {qlabel sstar other mlabel : Label}
    (hq : qlabel = .Q1 ∨ qlabel = .Q2)
    (hs : isSurplusStar sstar = true)
    (hother : isSurplusStar other = true)
    (hne : other ≠ sstar)
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    pointMask pointOf
        ({pointOf qlabel, pointOf mlabel, pointOf sstar,
            pointOf other} : Finset α) =
      maskOfLabels [qlabel, sstar, other, mlabel] := by
  have hpoint_eq : ∀ a b : Label, pointOf a = pointOf b ↔ a = b := by
    intro a b
    exact ⟨fun h => hinj h, fun h => by simp [h]⟩
  rcases hq with rfl | rfl <;>
...
/-- Swapped surplus set-order form of `pointMask_eq_QSurplusPairMoserMask`. -/
theorem pointMask_eq_QSurplusPairMoserMask_swap
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    (hinj : Function.Injective pointOf)
    {qlabel sstar other mlabel : Label}
    (hq : qlabel = .Q1 ∨ qlabel = .Q2)
    ...
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    pointMask pointOf
        ({pointOf qlabel, pointOf mlabel, pointOf other,
            pointOf sstar} : Finset α) =
      maskOfLabels [qlabel, sstar, other, mlabel] := by
  have hpoint_eq : ∀ a b : Label, pointOf a = pointOf b ↔ a = b := by
    intro a b
...
/-- A two-surplus, two-Moser row class has the generated mask. -/
theorem pointMask_eq_surplusPairMoserPairMask
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    (hinj : Function.Injective pointOf)
    {sstar other mlabel₁ mlabel₂ : Label}
    (hs : isSurplusStar sstar = true)
    (hother : isSurplusStar other = true)
    (hne : other ≠ sstar)
    (hm₁ : mlabel₁ = .u ∨ mlabel₁ = .v ∨ mlabel₁ = .w)
    ...
    (hmne : mlabel₁ ≠ mlabel₂) :
    pointMask pointOf
        ({pointOf mlabel₁, pointOf mlabel₂, pointOf sstar,
            pointOf other} : Finset α) =
      maskOfLabels [sstar, other, mlabel₁, mlabel₂] := by
  have hpoint_eq : ∀ a b : Label, pointOf a = pointOf b ↔ a = b := by
    intro a b
    exact ⟨fun h => hinj h, fun h => by simp [h]⟩
  cases sstar <;> simp [isSurplusStar] at hs
...
/-- Swapped surplus set-order form of
`pointMask_eq_surplusPairMoserPairMask`. -/
theorem pointMask_eq_surplusPairMoserPairMask_swap
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    (hinj : Function.Injective pointOf)
    {sstar other mlabel₁ mlabel₂ : Label}
    (hs : isSurplusStar sstar = true)
    ...
    (hmne : mlabel₁ ≠ mlabel₂) :
    pointMask pointOf
        ({pointOf mlabel₁, pointOf mlabel₂, pointOf other,
            pointOf sstar} : Finset α) =
      maskOfLabels [sstar, other, mlabel₁, mlabel₂] := by
  have hpoint_eq : ∀ a b : Label, pointOf a = pointOf b ↔ a = b := by
    intro a b
...
/-- A one-surplus, one-private, two-Moser row class has the generated mask. -/
theorem pointMask_eq_SurplusPrivateMoserPairMask
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    (hinj : Function.Injective pointOf)
    {sstar otherPrivate mlabel₁ mlabel₂ : Label}
    (hs : isSurplusStar sstar = true)
    (hpriv : otherPrivate = .Pw ∨ otherPrivate = .Pu)
    (hm₁ : mlabel₁ = .u ∨ mlabel₁ = .v ∨ mlabel₁ = .w)
    (hm₂ : mlabel₂ = .u ∨ mlabel₂ = .v ∨ mlabel₂ = .w)
    (hmne : mlabel₁ ≠ mlabel₂) :
    pointMask pointOf
        ({pointOf mlabel₁, pointOf mlabel₂, pointOf otherPrivate,
            pointOf sstar} : Finset α) =
      maskOfLabels [sstar, otherPrivate, mlabel₁, mlabel₂] := by
  have hpoint_eq : ∀ a b : Label, pointOf a = pointOf b ↔ a = b := by
    intro a b
    exact ⟨fun h => hinj h, fun h => by simp [h]⟩
  cases sstar <;> simp [isSurplusStar] at hs
...
theorem pointMask_eq_moserSurplusTripleMask
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    (hinj : Function.Injective pointOf) {mlabel : Label}
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    pointMask pointOf
        ({pointOf mlabel, pointOf .s1, pointOf .s2, pointOf .s3} :
          Finset α) =
      maskOfLabels [mlabel, .s1, .s2, .s3] := by
  have hpoint_eq : ∀ a b : Label, pointOf a = pointOf b ↔ a = b := by
    intro a b
    exact ⟨fun h => hinj h, fun h => by simp [h]⟩
  rcases hmoser with rfl | rfl | rfl <;>
    ...
    (hsearchSep : ∀ c cp : Label,
      (c, cp) ∈ orderedLabelPairs →
        crossSeparationOKForMasks c (pointMask pointOf (centerClass c)) cp
          (pointMask pointOf (centerClass cp)) = true) :
    False := by
  have hprivatePair : (privateCenter, .u) ∈ orderedLabelPairs := by
    rcases hpriv with rfl | rfl <;> simp [orderedLabelPairs]
  have htrue := hsearchSep privateCenter .u hprivatePair
...
/-- In row `ep_right_m1_s0_l0_r3`, the private selected class consists of
one Moser label and the three named surplus labels. -/
theorem erasedPinRow_ep_right_m1_s0_l0_r3_seed_mem_candidates_of_moser_surplusTriple
    {sstar privateCenter mlabel : Label}
    (hs : isSurplusStar sstar = true)
    (hpriv : privateCenter = .Pw ∨ privateCenter = .Pu)
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [mlabel, .s1, .s2, .s3] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_right_m1_s0_l0_r3_seeds_candidates := by
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    rcases hpriv with rfl | rfl <;>
    rcases hmoser with rfl | rfl | rfl <;>
      simp [erasedPinRow_ep_right_m1_s0_l0_r3_seeds_candidates,
        maskOfLabels, Label.bit, Label.index]

/-- In row `ep_left_m1_s0_l3_r0`, the private selected class consists of
one Moser label and the three named surplus labels. -/
theorem erasedPinRow_ep_left_m1_s0_l3_r0_seed_mem_candidates_of_moser_surplusTriple
    {sstar privateCenter mlabel : Label}
    (hs : isSurplusStar sstar = true)
    (hpriv : privateCenter = .Pw ∨ privateCenter = .Pu)
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [mlabel, .s1, .s2, .s3] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_left_m1_s0_l3_r0_seeds_candidates := by
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    rcases hpriv with rfl | rfl <;>
    rcases hmoser with rfl | rfl | rfl <;>
      simp [erasedPinRow_ep_left_m1_s0_l3_r0_seeds_candidates,
        maskOfLabels, Label.bit, Label.index]
...
/-- In row `ep_right_m1_s0_l1_r2`, the private selected class consists of one
`.Q` label, the erased surplus star, another surplus label, and one Moser
label. -/
theorem erasedPinRow_ep_right_m1_s0_l1_r2_seed_mem_candidates_of_q_surplus_pair_moser
    {qlabel sstar other privateCenter mlabel : Label}
    (hq : qlabel = .Q1 ∨ qlabel = .Q2)
    (hs : isSurplusStar sstar = true)
    (hother : isSurplusStar other = true)
    (hne : other ≠ sstar)
    ...
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [qlabel, sstar, other, mlabel] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_right_m1_s0_l1_r2_seeds_candidates := by
  rcases hq with rfl | rfl <;>
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    cases other <;> simp [isSurplusStar] at hother hne
    all_goals
      rcases hpriv with rfl | rfl <;>
      rcases hmoser with rfl | rfl | rfl <;>
        simp [erasedPinRow_ep_right_m1_s0_l1_r2_seeds_candidates,
          maskOfLabels, Label.bit, Label.index]

/-- In row `ep_left_m1_s0_l2_r1`, the private selected class consists of one
`.Q` label, the erased surplus star, another surplus label, and one Moser
label. -/
theorem erasedPinRow_ep_left_m1_s0_l2_r1_seed_mem_candidates_of_q_surplus_pair_moser
    {qlabel sstar other privateCenter mlabel : Label}
    (hq : qlabel = .Q1 ∨ qlabel = .Q2)
    (hs : isSurplusStar sstar = true)
    (hother : isSurplusStar other = true)
    ...
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [qlabel, sstar, other, mlabel] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_left_m1_s0_l2_r1_seeds_candidates := by
  rcases hq with rfl | rfl <;>
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    cases other <;> simp [isSurplusStar] at hother hne
    all_goals
      rcases hpriv with rfl | rfl <;>
      rcases hmoser with rfl | rfl | rfl <;>
        simp [erasedPinRow_ep_left_m1_s0_l2_r1_seeds_candidates,
          maskOfLabels, Label.bit, Label.index]

/-- In row `ep_right_m1_s1_l1_r1`, the private selected class consists of one
`.Q` label, the erased surplus star, the other same-cap private label, and one
Moser label. -/
theorem erasedPinRow_ep_right_m1_s1_l1_r1_seed_mem_candidates_of_q_surplus_private_moser
    {qlabel sstar privateCenter otherPrivate mlabel : Label}
    (hq : qlabel = .Q1 ∨ qlabel = .Q2)
    (hs : isSurplusStar sstar = true)
    (hpriv :
      (privateCenter = .Pw ∧ otherPrivate = .Pu) ∨
    ...
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [qlabel, sstar, otherPrivate, mlabel] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_right_m1_s1_l1_r1_seeds_candidates := by
  rcases hq with rfl | rfl <;>
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    rcases hpriv with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩ <;>
    rcases hmoser with rfl | rfl | rfl <;>
      simp [erasedPinRow_ep_right_m1_s1_l1_r1_seeds_candidates,
        maskOfLabels, Label.bit, Label.index]

/-- In row `ep_left_m1_s1_l1_r1`, the private selected class consists of one
`.Q` label, the erased surplus star, the other same-cap private label, and one
Moser label. -/
theorem erasedPinRow_ep_left_m1_s1_l1_r1_seed_mem_candidates_of_q_surplus_private_moser
    {qlabel sstar privateCenter otherPrivate mlabel : Label}
    (hq : qlabel = .Q1 ∨ qlabel = .Q2)
    (hs : isSurplusStar sstar = true)
    (hpriv :
    ...
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [qlabel, sstar, otherPrivate, mlabel] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_left_m1_s1_l1_r1_seeds_candidates := by
  rcases hq with rfl | rfl <;>
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    rcases hpriv with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩ <;>
    rcases hmoser with rfl | rfl | rfl <;>
      simp [erasedPinRow_ep_left_m1_s1_l1_r1_seeds_candidates,
        maskOfLabels, Label.bit, Label.index]

/-- In row `ep_right_m1_s1_l0_r2`, the private selected class consists of
the erased surplus star, another surplus label, the other same-cap private
label, and one Moser label. -/
theorem erasedPinRow_ep_right_m1_s1_l0_r2_seed_mem_candidates_of_surplus_pair_private_moser
    {sstar other privateCenter otherPrivate mlabel : Label}
    (hs : isSurplusStar sstar = true)
    (hother : isSurplusStar other = true)
    (hne : other ≠ sstar)
    (hpriv :
    ...
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [sstar, other, otherPrivate, mlabel] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_right_m1_s1_l0_r2_seeds_candidates := by
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    cases other <;> simp [isSurplusStar] at hother hne
    all_goals
      rcases hpriv with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩ <;>
      rcases hmoser with rfl | rfl | rfl <;>
        simp [erasedPinRow_ep_right_m1_s1_l0_r2_seeds_candidates,
          maskOfLabels, Label.bit, Label.index]

/-- In row `ep_left_m1_s1_l2_r0`, the private selected class consists of the
erased surplus star, another surplus label, the other same-cap private label,
and one Moser label. -/
theorem erasedPinRow_ep_left_m1_s1_l2_r0_seed_mem_candidates_of_surplus_pair_private_moser
    {sstar other privateCenter otherPrivate mlabel : Label}
    (hs : isSurplusStar sstar = true)
    (hother : isSurplusStar other = true)
    (hne : other ≠ sstar)
    ...
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [sstar, other, otherPrivate, mlabel] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_left_m1_s1_l2_r0_seeds_candidates := by
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    cases other <;> simp [isSurplusStar] at hother hne
    all_goals
      rcases hpriv with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩ <;>
      rcases hmoser with rfl | rfl | rfl <;>
        simp [erasedPinRow_ep_left_m1_s1_l2_r0_seeds_candidates,
          maskOfLabels, Label.bit, Label.index]
...
/-- In row `ep_right_m2_s0_l0_r2`, the private selected class consists of two
surplus labels and two distinct Moser labels. -/
theorem erasedPinRow_ep_right_m2_s0_l0_r2_seed_mem_candidates_of_surplus_moser_pair
    {sstar other privateCenter mlabel₁ mlabel₂ : Label}
    (hs : isSurplusStar sstar = true)
    (hother : isSurplusStar other = true)
    (hne : other ≠ sstar)
    (hpriv : privateCenter = .Pw ∨ privateCenter = .Pu)
    ...
    (hmne : mlabel₁ ≠ mlabel₂) :
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [sstar, other, mlabel₁, mlabel₂] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_right_m2_s0_l0_r2_seeds_candidates := by
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    cases other <;> simp [isSurplusStar] at hother hne
    all_goals
      ...
      rcases hm₂ with rfl | rfl | rfl <;>
        first
        | contradiction
        | simp [erasedPinRow_ep_right_m2_s0_l0_r2_seeds_candidates,
            maskOfLabels, Label.bit, Label.index]

/-- In row `ep_left_m2_s0_l2_r0`, the private selected class consists of two
surplus labels and two distinct Moser labels. -/
theorem erasedPinRow_ep_left_m2_s0_l2_r0_seed_mem_candidates_of_surplus_moser_pair
    {sstar other privateCenter mlabel₁ mlabel₂ : Label}
    (hs : isSurplusStar sstar = true)
    (hother : isSurplusStar other = true)
    (hne : other ≠ sstar)
    ...
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [sstar, other, mlabel₁, mlabel₂] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_left_m2_s0_l2_r0_seeds_candidates := by
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    cases other <;> simp [isSurplusStar] at hother hne
    all_goals
      ...
      rcases hm₂ with rfl | rfl | rfl <;>
        first
        | contradiction
        | simp [erasedPinRow_ep_left_m2_s0_l2_r0_seeds_candidates,
            maskOfLabels, Label.bit, Label.index]

/-- In row `ep_right_m2_s1_l0_r1`, the private selected class consists of
the erased surplus star, the other same-cap private label, and two distinct
Moser labels. -/
theorem erasedPinRow_ep_right_m2_s1_l0_r1_seed_mem_candidates_of_surplus_private_moser_pair
    {sstar privateCenter otherPrivate mlabel₁ mlabel₂ : Label}
    (hs : isSurplusStar sstar = true)
    (hpriv :
      (privateCenter = .Pw ∧ otherPrivate = .Pu) ∨
        (privateCenter = .Pu ∧ otherPrivate = .Pw))
    ...
    (hmne : mlabel₁ ≠ mlabel₂) :
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [sstar, otherPrivate, mlabel₁, mlabel₂] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_right_m2_s1_l0_r1_seeds_candidates := by
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    rcases hpriv with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩ <;>
    rcases hm₁ with rfl | rfl | rfl <;>
    rcases hm₂ with rfl | rfl | rfl <;>
      first
      | contradiction
      | simp [erasedPinRow_ep_right_m2_s1_l0_r1_seeds_candidates,
          maskOfLabels, Label.bit, Label.index]

/-- In row `ep_left_m2_s1_l1_r0`, the private selected class consists of the
erased surplus star, the other same-cap private label, and two distinct Moser
labels. -/
theorem erasedPinRow_ep_left_m2_s1_l1_r0_seed_mem_candidates_of_surplus_private_moser_pair
    {sstar privateCenter otherPrivate mlabel₁ mlabel₂ : Label}
    (hs : isSurplusStar sstar = true)
    (hpriv :
      (privateCenter = .Pw ∧ otherPrivate = .Pu) ∨
    ...
    ({ sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [sstar, otherPrivate, mlabel₁, mlabel₂] } :
      OneSidedSeed) ∈
        erasedPinRow_ep_left_m2_s1_l1_r0_seeds_candidates := by
  cases sstar <;> simp [isSurplusStar] at hs
  all_goals
    rcases hpriv with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩ <;>
    rcases hm₁ with rfl | rfl | rfl <;>
    rcases hm₂ with rfl | rfl | rfl <;>
      first
      | contradiction
      | simp [erasedPinRow_ep_left_m2_s1_l1_r0_seeds_candidates,
          maskOfLabels, Label.bit, Label.index]
...
theorem false_of_erasedPinRow_ep_right_m0_s0_l2_r2_seed_private_w_crossSeparation
    ...
    (hprivate : pointMask pointOf (centerClass seed.privateCenter) =
      seed.privateMask)
    (hprivatePair : (seed.privateCenter, .w) ∈ orderedLabelPairs)
    (hsearchSep : ∀ c cp : Label,
      (c, cp) ∈ orderedLabelPairs →
        crossSeparationOKForMasks c (pointMask pointOf (centerClass c)) cp
          (pointMask pointOf (centerClass cp)) = true) :
    False := by
  have htrue := hsearchSep seed.privateCenter .w hprivatePair
...
/-- Exact right row `(1,0,0,3)` produces one of the generated row seeds once
the three surplus-side selected points are embedded in the named surplus
triple.  The selected class consists of one Moser label and the three named
surplus labels. -/
theorem right_row1003_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A) (hM44 : S.IsM44)
    {center x : ℝ²} {radius : ℝ}
    (hradius_pos : 0 < radius)
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_right_m1_s0_l0_r3_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
          (centerClass seed.privateCenter) = seed.privateMask := by
  classical
  rcases hM44.right_row1003_selectedClass_eq_moser_surplusTriple
      hradius_pos hm hs hl hr hRtriple hs12 hs13 hs23 with
    ⟨m, _hMeq, hmVerts, hseleq⟩
  have hxTriple : x ∈ ({s1, s2, s3} : Finset ℝ²) := hRtriple hxR
  rcases exists_surplusStar_rightPinnedLabelPoint_eq_of_mem_triple
      S p₁ p₂ q₁ q₂ s1 s2 s3 hxTriple with
    ⟨sstar, hsstar, hsstar_eq⟩
  rcases exists_moserLabel_rightPinnedLabelPoint_eq_of_mem_triangle
    ...
    ⟨mlabel, hmoser, hmlabel_eq⟩
  let seed : OneSidedSeed :=
    { sstar := sstar, privateCenter := privateCenter, kind := .own,
      privateMask := maskOfLabels [mlabel, .s1, .s2, .s3] }
  have hseedMem :
      seed ∈ erasedPinRow_ep_right_m1_s0_l0_r3_seeds_candidates := by
    exact
      erasedPinRow_ep_right_m1_s0_l0_r3_seed_mem_candidates_of_moser_surplusTriple
        hsstar hpriv hmoser
  refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
  let pointOf := rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3
  change pointMask pointOf (centerClass privateCenter) =
    maskOfLabels [mlabel, .s1, .s2, .s3]
  rw [hprivateClass, hseleq, ← hmlabel_eq]
  change pointMask pointOf
      ({pointOf mlabel, pointOf .s1, pointOf .s2, pointOf .s3} :
        Finset ℝ²) =
    maskOfLabels [mlabel, .s1, .s2, .s3]
  exact pointMask_eq_moserSurplusTripleMask
    (pointOf := pointOf) hinj hmoser

/-- Exact left row `(1,0,3,0)` produces one of the generated row seeds once
the three surplus-side selected points are embedded in the named surplus
triple.  The selected class consists of one Moser label and the three named
surplus labels. -/
theorem left_row1030_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A) (hM44 : S.IsM44)
    {center x : ℝ²} {radius : ℝ}
    (hradius_pos : 0 < radius)
    (hm : S.moserCount center radius = 1)
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_left_m1_s0_l3_r0_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
          (centerClass seed.privateCenter) = seed.privateMask := by
  classical
  rcases hM44.left_row1030_selectedClass_eq_moser_surplusTriple
      hradius_pos hm hs hl hr hLtriple hs12 hs13 hs23 with
    ⟨m, _hMeq, hmVerts, hseleq⟩
  have hxTriple : x ∈ ({s1, s2, s3} : Finset ℝ²) := hLtriple hxL
  rcases exists_surplusStar_leftPinnedLabelPoint_eq_of_mem_triple
  ...
  let seed : OneSidedSeed :=
    { sstar := sstar, privateCenter := privateCenter, kind := .own,
      privateMask := maskOfLabels [mlabel, .s1, .s2, .s3] }
  have hseedMem :
      seed ∈ erasedPinRow_ep_left_m1_s0_l3_r0_seeds_candidates := by
    exact
      erasedPinRow_ep_left_m1_s0_l3_r0_seed_mem_candidates_of_moser_surplusTriple
        hsstar hpriv hmoser
  refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
  let pointOf := leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3
  change pointMask pointOf (centerClass privateCenter) =
...
theorem false_of_right_row0022_private_w_crossSeparation
  ...
  rcases right_row0022_exists_erasedPinRowSeed_privateMask S hM44
      ...
      hprivateClass with
    ⟨seed, hseed, _hsstar, _hsstar_eq, hprivateCenter, hprivate⟩
  have hprivatePair : (seed.privateCenter, .w) ∈ orderedLabelPairs := by
    rcases hpriv with rfl | rfl <;>
      simp [orderedLabelPairs, hprivateCenter]
  exact false_of_erasedPinRow_ep_right_m0_s0_l2_r2_seed_private_w_crossSeparation
    hseed hwMask hprivate hprivatePair hsearchSep
...
theorem false_of_right_row0121_private_w_crossSeparation
  ...
  rcases right_row0121_exists_erasedPinRowSeed_privateMask S hM44
    ...
    ⟨seed, hseed, _hsstar, _hsstar_eq, hprivateCenter, hprivate⟩
  have hprivatePair : (seed.privateCenter, .w) ∈ orderedLabelPairs := by
    rcases hcenter with ⟨_, hpriv, _⟩ | ⟨_, hpriv, _⟩ <;>
      simp [orderedLabelPairs, hprivateCenter, hpriv]
...
/-- Exact right row `(1,0,1,2)` produces one of the generated row seeds once
the surplus-side pair is embedded in the named surplus triple.  The private
mask records the selected `.Q` point, the Moser point, and both surplus-side
selected points. -/
theorem right_row1012_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A) (hM44 : S.IsM44)
    {center x : ℝ²} {radius : ℝ}
    (hradius_pos : 0 < radius)
    (hm : S.moserCount center radius = 1)
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_right_m1_s0_l1_r2_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
  ...
  rcases hM44.right_row1012_selectedClass_eq_moser_oppInterior2_singleton_surplusPair
      hradius_pos hm hs hl hr with
    ⟨q, m, r₁, r₂, hr12, _hMeq, hmVerts, _hLeq, hqOpp, hReq,
      _hRsub, hseleq⟩
  rcases exists_QLabel_rightPinnedLabelPoint_eq_of_mem_oppInterior2
      S p₁ p₂ q₁ q₂ s1 s2 s3 hqpair hqOpp with
    ⟨qlabel, hqLabel, hq_eq⟩
  ...
  · have hr₂Triple : r₂ ∈ ({s1, s2, s3} : Finset ℝ²) := by
    ...
    let seed : OneSidedSeed :=
      { sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [qlabel, sstar, other, mlabel] }
    have hseedMem :
        seed ∈ erasedPinRow_ep_right_m1_s0_l1_r2_seeds_candidates := by
      exact
        erasedPinRow_ep_right_m1_s0_l1_r2_seed_mem_candidates_of_q_surplus_pair_moser
          hqLabel hsstar hother hne hpriv hmlabel
    refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
    change pointMask pointOf (centerClass privateCenter) =
      maskOfLabels [qlabel, sstar, other, mlabel]
    rw [hprivateClass, hseleq, ← hq_eq, ← hmlabel_eq, ← hx_eq_r₁,
      ← hsstar_eq, ← hother_eq]
    change pointMask pointOf
        ({pointOf qlabel, pointOf mlabel, pointOf sstar,
            pointOf other} : Finset ℝ²) =
      maskOfLabels [qlabel, sstar, other, mlabel]
    exact pointMask_eq_QSurplusPairMoserMask
      (pointOf := pointOf) hinj hqLabel hsstar hother hne hmlabel
  · have hr₁Triple : r₁ ∈ ({s1, s2, s3} : Finset ℝ²) := by
      exact hRtriple (by
        rw [hReq]
        simp)
    ...
    change pointMask pointOf (centerClass privateCenter) =
      maskOfLabels [qlabel, sstar, other, mlabel]
    rw [hprivateClass, hseleq, ← hq_eq, ← hmlabel_eq, ← hother_eq,
      ← hx_eq_r₂, ← hsstar_eq]
    change pointMask pointOf
        ({pointOf qlabel, pointOf mlabel, pointOf other,
            pointOf sstar} : Finset ℝ²) =
      maskOfLabels [qlabel, sstar, other, mlabel]
    exact pointMask_eq_QSurplusPairMoserMask_swap
      (pointOf := pointOf) hinj hqLabel hsstar hother hne hmlabel

/-- A one-`.Q`, one-surplus, one-private, one-Moser row class has the
generated mask. -/
theorem pointMask_eq_QSurplusPrivateMoserMask
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    (hinj : Function.Injective pointOf)
    {qlabel sstar otherPrivate mlabel : Label}
    (hq : qlabel = .Q1 ∨ qlabel = .Q2)
    (hs : isSurplusStar sstar = true)
    (hpriv : otherPrivate = .Pw ∨ otherPrivate = .Pu)
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    pointMask pointOf
        ({pointOf qlabel, pointOf otherPrivate, pointOf mlabel,
            pointOf sstar} : Finset α) =
      maskOfLabels [qlabel, sstar, otherPrivate, mlabel] := by
  have hpoint_eq : ∀ a b : Label, pointOf a = pointOf b ↔ a = b := by
    intro a b
    exact ⟨fun h => hinj h, fun h => by simp [h]⟩
  rcases hq with rfl | rfl <;>
...
/-- A two-surplus, one-private, one-Moser row class has the generated mask. -/
theorem pointMask_eq_SurplusPairPrivateMoserMask
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    (hinj : Function.Injective pointOf)
    {sstar other otherPrivate mlabel : Label}
    (hs : isSurplusStar sstar = true)
    (hother : isSurplusStar other = true)
    (hne : other ≠ sstar)
    (hpriv : otherPrivate = .Pw ∨ otherPrivate = .Pu)
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    pointMask pointOf
        ({pointOf otherPrivate, pointOf mlabel, pointOf sstar,
            pointOf other} : Finset α) =
      maskOfLabels [sstar, other, otherPrivate, mlabel] := by
  have hpoint_eq : ∀ a b : Label, pointOf a = pointOf b ↔ a = b := by
    intro a b
    exact ⟨fun h => hinj h, fun h => by simp [h]⟩
  cases sstar <;> simp [isSurplusStar] at hs
...
/-- Swapped surplus set-order form of
`pointMask_eq_SurplusPairPrivateMoserMask`. -/
theorem pointMask_eq_SurplusPairPrivateMoserMask_swap
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    (hinj : Function.Injective pointOf)
    {sstar other otherPrivate mlabel : Label}
    (hs : isSurplusStar sstar = true)
    ...
    (hmoser : mlabel = .u ∨ mlabel = .v ∨ mlabel = .w) :
    pointMask pointOf
        ({pointOf otherPrivate, pointOf mlabel, pointOf other,
            pointOf sstar} : Finset α) =
      maskOfLabels [sstar, other, otherPrivate, mlabel] := by
  have hpoint_eq : ∀ a b : Label, pointOf a = pointOf b ↔ a = b := by
    intro a b
...
/-- Exact right row `(1,1,1,1)` produces one of the generated row seeds once
the surplus-side singleton is embedded in the named surplus triple.  The
private mask records the same-cap singleton, the selected `.Q` point, the
Moser singleton, and the erased surplus point. -/
theorem right_row1111_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A) (hM44 : S.IsM44)
    {center x : ℝ²} {radius : ℝ}
    (hradius_pos : 0 < radius)
    (hm : S.moserCount center radius = 1)
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_right_m1_s1_l1_r1_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
  ...
  rcases hM44.right_row1111_selectedClass_eq_moser_sameCap_oppInterior2_singleton_surplus
      hradius_pos hm hs hl hr with
    ⟨q, c, m, r, hCeq, _hMeq, hmVerts, _hLeq, hqOpp, hReq,
      _hrSurplus, hseleq⟩
  rcases exists_QLabel_rightPinnedLabelPoint_eq_of_mem_oppInterior2
      S p₁ p₂ q₁ q₂ s1 s2 s3 hqpair hqOpp with
    ⟨qlabel, hqLabel, hq_eq⟩
  ...
  let seed : OneSidedSeed :=
    { sstar := sstar, privateCenter := privateCenter, kind := .own,
      privateMask := maskOfLabels [qlabel, sstar, otherPrivate, mlabel] }
  have hseedMem :
      seed ∈ erasedPinRow_ep_right_m1_s1_l1_r1_seeds_candidates := by
    exact
      erasedPinRow_ep_right_m1_s1_l1_r1_seed_mem_candidates_of_q_surplus_private_moser
        hqLabel hsstar hprivLabels hmlabel
  refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
  change pointMask pointOf (centerClass privateCenter) =
    maskOfLabels [qlabel, sstar, otherPrivate, mlabel]
  rw [hprivateClass, hseleq, ← hq_eq, hc_eq_otherPrivate, ← hmlabel_eq,
    ← hx_eq_r, ← hsstar_eq]
  change pointMask pointOf
      ({pointOf qlabel, pointOf otherPrivate, pointOf mlabel,
          pointOf sstar} : Finset ℝ²) =
    maskOfLabels [qlabel, sstar, otherPrivate, mlabel]
  exact pointMask_eq_QSurplusPrivateMoserMask
    (pointOf := pointOf) hinj hqLabel hsstar hotherPrivate hmlabel

/-- Exact right row `(1,1,0,2)` produces one of the generated row seeds once
the surplus-side pair is embedded in the named surplus triple.  The private
mask records the same-cap singleton, one Moser point, and both surplus-side
selected points. -/
theorem right_row1102_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A) (hM44 : S.IsM44)
    {center x : ℝ²} {radius : ℝ}
    (hradius_pos : 0 < radius)
    (hm : S.moserCount center radius = 1)
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_right_m1_s1_l0_r2_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
  ...
  rcases hM44.right_row1102_selectedClass_eq_moser_sameCap_surplusPair
      hradius_pos hm hs hl hr with
    ⟨c, m, r₁, r₂, hr12, hCeq, _hMeq, hmVerts, hReq, _hRsub,
      hseleq⟩
  rcases exists_moserLabel_rightPinnedLabelPoint_eq_of_mem_triangle
      S p₁ p₂ q₁ q₂ s1 s2 s3 hmVerts with
    ⟨mlabel, hmlabel, hmlabel_eq⟩
  ...
  · have hr₂Triple : r₂ ∈ ({s1, s2, s3} : Finset ℝ²) := by
    ...
    let seed : OneSidedSeed :=
      { sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [sstar, other, otherPrivate, mlabel] }
    have hseedMem :
        seed ∈ erasedPinRow_ep_right_m1_s1_l0_r2_seeds_candidates := by
      exact
        erasedPinRow_ep_right_m1_s1_l0_r2_seed_mem_candidates_of_surplus_pair_private_moser
          hsstar hother hne hprivLabels hmlabel
    refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
    change pointMask pointOf (centerClass privateCenter) =
      maskOfLabels [sstar, other, otherPrivate, mlabel]
    rw [hprivateClass, hseleq, hc_eq_otherPrivate, ← hmlabel_eq,
      ← hx_eq_r₁, ← hsstar_eq, ← hother_eq]
    change pointMask pointOf
        ({pointOf otherPrivate, pointOf mlabel, pointOf sstar,
            pointOf other} : Finset ℝ²) =
      maskOfLabels [sstar, other, otherPrivate, mlabel]
    exact pointMask_eq_SurplusPairPrivateMoserMask
      (pointOf := pointOf) hinj hsstar hother hne hotherPrivate hmlabel
  · have hr₁Triple : r₁ ∈ ({s1, s2, s3} : Finset ℝ²) := by
      exact hRtriple (by
        rw [hReq]
        simp)
    ...
    change pointMask pointOf (centerClass privateCenter) =
      maskOfLabels [sstar, other, otherPrivate, mlabel]
    rw [hprivateClass, hseleq, hc_eq_otherPrivate, ← hmlabel_eq,
      ← hother_eq, ← hx_eq_r₂, ← hsstar_eq]
    change pointMask pointOf
        ({pointOf otherPrivate, pointOf mlabel, pointOf other,
            pointOf sstar} : Finset ℝ²) =
      maskOfLabels [sstar, other, otherPrivate, mlabel]
    exact pointMask_eq_SurplusPairPrivateMoserMask_swap
      (pointOf := pointOf) hinj hsstar hother hne hotherPrivate hmlabel
...
/-- Exact left row `(1,0,2,1)` produces one of the generated row seeds once
the surplus-side pair is embedded in the named surplus triple.  The private
mask records the selected `.Q` point, the Moser point, and both surplus-side
selected points. -/
theorem left_row1021_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A) (hM44 : S.IsM44)
    {center x : ℝ²} {radius : ℝ}
    (hradius_pos : 0 < radius)
    (hm : S.moserCount center radius = 1)
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_left_m1_s0_l2_r1_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
  ...
  rcases hM44.left_row1021_selectedClass_eq_moser_oppInterior1_singleton_surplusPair
      hradius_pos hm hs hl hr with
    ⟨q, m, l₁, l₂, hl12, _hMeq, hmVerts, _hReq, hqOpp, hLeq,
      _hLsub, hseleq⟩
  rcases exists_QLabel_leftPinnedLabelPoint_eq_of_mem_oppInterior1
      S p₁ p₂ q₁ q₂ s1 s2 s3 hqpair hqOpp with
    ⟨qlabel, hqLabel, hq_eq⟩
  ...
  · have hl₂Triple : l₂ ∈ ({s1, s2, s3} : Finset ℝ²) := by
    ...
    let seed : OneSidedSeed :=
      { sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [qlabel, sstar, other, mlabel] }
    have hseedMem :
        seed ∈ erasedPinRow_ep_left_m1_s0_l2_r1_seeds_candidates := by
      exact
        erasedPinRow_ep_left_m1_s0_l2_r1_seed_mem_candidates_of_q_surplus_pair_moser
          hqLabel hsstar hother hne hpriv hmlabel
    refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
    change pointMask pointOf (centerClass privateCenter) =
      maskOfLabels [qlabel, sstar, other, mlabel]
    rw [hprivateClass, hseleq, ← hq_eq, ← hmlabel_eq, ← hx_eq_l₁,
      ← hsstar_eq, ← hother_eq]
    change pointMask pointOf
        ({pointOf qlabel, pointOf mlabel, pointOf sstar,
            pointOf other} : Finset ℝ²) =
...
/-- Exact left row `(1,1,1,1)` produces one of the generated row seeds once
the surplus-side singleton is embedded in the named surplus triple.  The
private mask records the same-cap singleton, the selected `.Q` point, the
Moser singleton, and the erased surplus point. -/
theorem left_row1111_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A) (hM44 : S.IsM44)
    {center x : ℝ²} {radius : ℝ}
    (hradius_pos : 0 < radius)
    (hm : S.moserCount center radius = 1)
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_left_m1_s1_l1_r1_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
  ...
  rcases hM44.left_row1111_selectedClass_eq_moser_sameCap_oppInterior1_singleton_surplus
      hradius_pos hm hs hl hr with
    ⟨q, c, m, l, hCeq, _hMeq, hmVerts, _hReq, hqOpp, hLeq,
      _hlSurplus, hseleq⟩
  rcases exists_QLabel_leftPinnedLabelPoint_eq_of_mem_oppInterior1
      S p₁ p₂ q₁ q₂ s1 s2 s3 hqpair hqOpp with
    ⟨qlabel, hqLabel, hq_eq⟩
  ...
  let seed : OneSidedSeed :=
    { sstar := sstar, privateCenter := privateCenter, kind := .own,
      privateMask := maskOfLabels [qlabel, sstar, otherPrivate, mlabel] }
  have hseedMem :
      seed ∈ erasedPinRow_ep_left_m1_s1_l1_r1_seeds_candidates := by
    exact
      erasedPinRow_ep_left_m1_s1_l1_r1_seed_mem_candidates_of_q_surplus_private_moser
        hqLabel hsstar hprivLabels hmlabel
  refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
  change pointMask pointOf (centerClass privateCenter) =
    maskOfLabels [qlabel, sstar, otherPrivate, mlabel]
...
/-- Exact left row `(1,1,2,0)` produces one of the generated row seeds once
the surplus-side pair is embedded in the named surplus triple.  The private
mask records the same-cap singleton, one Moser point, and both surplus-side
selected points. -/
theorem left_row1120_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A) (hM44 : S.IsM44)
    {center x : ℝ²} {radius : ℝ}
    (hradius_pos : 0 < radius)
    (hm : S.moserCount center radius = 1)
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_left_m1_s1_l2_r0_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
  ...
  rcases hM44.left_row1120_selectedClass_eq_moser_sameCap_surplusPair
      hradius_pos hm hs hl hr with
    ⟨c, m, l₁, l₂, hl12, hCeq, _hMeq, hmVerts, hLeq, _hLsub,
      hseleq⟩
  rcases exists_moserLabel_leftPinnedLabelPoint_eq_of_mem_triangle
      S p₁ p₂ q₁ q₂ s1 s2 s3 hmVerts with
    ⟨mlabel, hmlabel, hmlabel_eq⟩
  ...
  · have hl₂Triple : l₂ ∈ ({s1, s2, s3} : Finset ℝ²) := by
    ...
    let seed : OneSidedSeed :=
      { sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [sstar, other, otherPrivate, mlabel] }
    have hseedMem :
        seed ∈ erasedPinRow_ep_left_m1_s1_l2_r0_seeds_candidates := by
      exact
        erasedPinRow_ep_left_m1_s1_l2_r0_seed_mem_candidates_of_surplus_pair_private_moser
          hsstar hother hne hprivLabels hmlabel
    refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
    change pointMask pointOf (centerClass privateCenter) =
      maskOfLabels [sstar, other, otherPrivate, mlabel]
    rw [hprivateClass, hseleq, hc_eq_otherPrivate, ← hmlabel_eq,
      ← hx_eq_l₁, ← hsstar_eq, ← hother_eq]
    change pointMask pointOf
        ({pointOf otherPrivate, pointOf mlabel, pointOf sstar,
            pointOf other} : Finset ℝ²) =
      maskOfLabels [sstar, other, otherPrivate, mlabel]
...
/-- Exact right row `(2,0,0,2)` produces one of the generated row seeds once
the two surplus-side points are embedded in the named surplus triple and the
two Moser points are matched to distinct Moser labels. -/
theorem right_row2002_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A) (hM44 : S.IsM44)
    {center x : ℝ²} {radius : ℝ}
    (hradius_pos : 0 < radius)
    (hm : S.moserCount center radius = 2)
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_right_m2_s0_l0_r2_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
  ...
  rcases hM44.right_row2002_selectedClass_eq_moserPair_surplusPair
      hradius_pos hm hs hl hr with
    ⟨m₁, m₂, r₁, r₂, hm₁₂, hr₁₂, _hMeq, hm₁Verts, hm₂Verts,
      hReq, _hRsubSurplus, hseleq⟩
  rcases exists_moserLabel_rightPinnedLabelPoint_eq_of_mem_triangle
      S p₁ p₂ q₁ q₂ s1 s2 s3 hm₁Verts with
    ⟨mlabel₁, hmlabel₁, hmlabel₁_eq⟩
  rcases exists_moserLabel_rightPinnedLabelPoint_eq_of_mem_triangle
  ...
  have hmlabel_ne : mlabel₁ ≠ mlabel₂ := by
    ...
    exact hm₁₂ (by
      rw [← hmlabel₁_eq, hsame, hmlabel₂_eq])
  let pointOf := rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3
  have hpriv : privateCenter = .Pw ∨ privateCenter = .Pu := by
    rcases hcenter with ⟨_, hpriv⟩ | ⟨_, hpriv⟩
    · exact Or.inl hpriv
    · exact Or.inr hpriv
  have hxPair : x ∈ ({r₁, r₂} : Finset ℝ²) := by
  ...
  · subst x
    have hr₂R :
        r₂ ∈ SelectedClass A center radius ∩
          S.rightAdjacentInteriorByIndex S.oppIndex1 := by
      rw [hReq]
      simp
    rcases exists_surplusStar_rightPinnedLabelPoint_eq_of_mem_triple
        S p₁ p₂ q₁ q₂ s1 s2 s3 (hRtriple hr₂R) with
      ⟨other, hother, hother_eq⟩
    rcases exists_surplusStar_rightPinnedLabelPoint_eq_of_mem_triple
        S p₁ p₂ q₁ q₂ s1 s2 s3 (hRtriple hxR) with
      ⟨sstar, hsstar, hsstar_eq⟩
    have hother_ne : other ≠ sstar := by
      intro hsame
      exact hr₁₂ (by
        rw [← hsstar_eq, ← hsame, hother_eq])
    let seed : OneSidedSeed :=
      { sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [sstar, other, mlabel₁, mlabel₂] }
    have hseedMem :
        seed ∈ erasedPinRow_ep_right_m2_s0_l0_r2_seeds_candidates := by
      exact
        erasedPinRow_ep_right_m2_s0_l0_r2_seed_mem_candidates_of_surplus_moser_pair
          hsstar hother hother_ne hpriv hmlabel₁ hmlabel₂ hmlabel_ne
    refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
    change pointMask pointOf (centerClass privateCenter) =
      maskOfLabels [sstar, other, mlabel₁, mlabel₂]
    rw [hprivateClass, hseleq, ← hmlabel₁_eq, ← hmlabel₂_eq,
      ← hsstar_eq, ← hother_eq]
    change pointMask pointOf
        ({pointOf mlabel₁, pointOf mlabel₂, pointOf sstar,
            pointOf other} : Finset ℝ²) =
      maskOfLabels [sstar, other, mlabel₁, mlabel₂]
    exact pointMask_eq_surplusPairMoserPairMask
      (pointOf := pointOf) hinj hsstar hother hother_ne hmlabel₁
      hmlabel₂ hmlabel_ne
  · subst x
    have hr₁R :
        r₁ ∈ SelectedClass A center radius ∩
    ...
    change pointMask pointOf
        ({pointOf mlabel₁, pointOf mlabel₂, pointOf other,
            pointOf sstar} : Finset ℝ²) =
      maskOfLabels [sstar, other, mlabel₁, mlabel₂]
    exact pointMask_eq_surplusPairMoserPairMask_swap
      (pointOf := pointOf) hinj hsstar hother hother_ne hmlabel₁
      hmlabel₂ hmlabel_ne

/-- Exact right terminal row `(2,1,0,1)` produces one of the generated
terminal row seeds.  The private mask records the erased surplus point, the
same-cap private singleton, and the two Moser points. -/
theorem right_row2101_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A)
    {center x : ℝ²} {radius : ℝ}
    (hpayload :
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_right_m2_s1_l0_r1_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
  ...
  rcases hpayload with
    ⟨m₁, m₂, c, hm₁₂, hMeq, hCeq, _hLeq, _hReq, hseleq, _hcard⟩
  rcases exists_surplusStar_rightPinnedLabelPoint_eq_of_mem_triple
      S p₁ p₂ q₁ q₂ s1 s2 s3 hxTriple with
    ⟨sstar, hsstar, hsstar_eq⟩
  have hm₁Verts : m₁ ∈ S.triangle.verts := by
    have hm₁M :
        m₁ ∈ SelectedClass A center radius ∩ S.triangle.verts := by
      rw [hMeq]
      simp
  ...
  let seed : OneSidedSeed :=
    { sstar := sstar, privateCenter := privateCenter, kind := .own,
      privateMask := maskOfLabels [sstar, otherPrivate, mlabel₁, mlabel₂] }
  have hseedMem :
      seed ∈ erasedPinRow_ep_right_m2_s1_l0_r1_seeds_candidates := by
    exact
      erasedPinRow_ep_right_m2_s1_l0_r1_seed_mem_candidates_of_surplus_private_moser_pair
        hsstar hprivLabels hmlabel₁ hmlabel₂ hmlabel_ne
  refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
  change pointMask pointOf (centerClass privateCenter) =
    maskOfLabels [sstar, otherPrivate, mlabel₁, mlabel₂]
  rw [hprivateClass, hseleq, ← hmlabel₁_eq, ← hmlabel₂_eq,
    hc_eq_otherPrivate, ← hsstar_eq]
  change pointMask pointOf
      ({pointOf mlabel₁, pointOf mlabel₂, pointOf otherPrivate,
          pointOf sstar} : Finset ℝ²) =
    maskOfLabels [sstar, otherPrivate, mlabel₁, mlabel₂]
  exact pointMask_eq_SurplusPrivateMoserPairMask
    (pointOf := pointOf) hinj hsstar hotherPrivate hmlabel₁ hmlabel₂
    hmlabel_ne

/-- Exact left row `(2,0,2,0)` produces one of the generated row seeds once
the two surplus-side points are embedded in the named surplus triple and the
two Moser points are matched to distinct Moser labels. -/
theorem left_row2020_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A) (hM44 : S.IsM44)
    {center x : ℝ²} {radius : ℝ}
    (hradius_pos : 0 < radius)
    (hm : S.moserCount center radius = 2)
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_left_m2_s0_l2_r0_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
  ...
  rcases hM44.left_row2020_selectedClass_eq_moserPair_surplusPair
      hradius_pos hm hs hl hr with
    ⟨m₁, m₂, l₁, l₂, hm₁₂, hl₁₂, _hMeq, hm₁Verts, hm₂Verts,
      hLeq, _hLsubSurplus, hseleq⟩
  rcases exists_moserLabel_leftPinnedLabelPoint_eq_of_mem_triangle
      S p₁ p₂ q₁ q₂ s1 s2 s3 hm₁Verts with
    ⟨mlabel₁, hmlabel₁, hmlabel₁_eq⟩
  rcases exists_moserLabel_leftPinnedLabelPoint_eq_of_mem_triangle
  ...
  · subst x
    have hl₂L :
        l₂ ∈ SelectedClass A center radius ∩
          S.leftAdjacentInteriorByIndex S.oppIndex2 := by
      rw [hLeq]
      simp
    rcases exists_surplusStar_leftPinnedLabelPoint_eq_of_mem_triple
        S p₁ p₂ q₁ q₂ s1 s2 s3 (hLtriple hl₂L) with
      ⟨other, hother, hother_eq⟩
    rcases exists_surplusStar_leftPinnedLabelPoint_eq_of_mem_triple
        S p₁ p₂ q₁ q₂ s1 s2 s3 (hLtriple hxL) with
      ⟨sstar, hsstar, hsstar_eq⟩
    have hother_ne : other ≠ sstar := by
      intro hsame
      exact hl₁₂ (by
    ...
    let seed : OneSidedSeed :=
      { sstar := sstar, privateCenter := privateCenter, kind := .own,
        privateMask := maskOfLabels [sstar, other, mlabel₁, mlabel₂] }
    have hseedMem :
        seed ∈ erasedPinRow_ep_left_m2_s0_l2_r0_seeds_candidates := by
      exact
        erasedPinRow_ep_left_m2_s0_l2_r0_seed_mem_candidates_of_surplus_moser_pair
          hsstar hother hother_ne hpriv hmlabel₁ hmlabel₂ hmlabel_ne
    refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
    change pointMask pointOf (centerClass privateCenter) =
      maskOfLabels [sstar, other, mlabel₁, mlabel₂]
...
/-- Exact left terminal row `(2,1,1,0)` produces one of the generated
terminal row seeds.  The private mask records the erased surplus point, the
same-cap private singleton, and the two Moser points. -/
theorem left_row2110_exists_erasedPinRowSeed_privateMask
    {A : Finset ℝ²} (S : SurplusCapPacket A)
    {center x : ℝ²} {radius : ℝ}
    (hpayload :
    ...
    (hprivateClass : centerClass privateCenter =
      SelectedClass A center radius) :
    ∃ seed : OneSidedSeed,
      seed ∈ erasedPinRow_ep_left_m2_s1_l1_r0_seeds_candidates ∧
      isSurplusStar seed.sstar = true ∧
      leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3 seed.sstar = x ∧
      seed.privateCenter = privateCenter ∧
      pointMask (leftPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
  ...
  let seed : OneSidedSeed :=
    { sstar := sstar, privateCenter := privateCenter, kind := .own,
      privateMask := maskOfLabels [sstar, otherPrivate, mlabel₁, mlabel₂] }
  have hseedMem :
      seed ∈ erasedPinRow_ep_left_m2_s1_l1_r0_seeds_candidates := by
    exact
      erasedPinRow_ep_left_m2_s1_l1_r0_seed_mem_candidates_of_surplus_private_moser_pair
        hsstar hprivLabels hmlabel₁ hmlabel₂ hmlabel_ne
  refine ⟨seed, hseedMem, hsstar, hsstar_eq, rfl, ?_⟩
  change pointMask pointOf (centerClass privateCenter) =
    maskOfLabels [sstar, otherPrivate, mlabel₁, mlabel₂]
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_right_m1_s0_l0_r3`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_right_m1_s0_l0_r3_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    ...
    False := by
  have hfixed : seed ∈ erasedPinFixedSeeds :=
    erasedPinRow_ep_right_m1_s0_l0_r3_seeds_candidates_subset_fixed hseed
  have hcandidateFull : ∀ center : Label,
      pointMask pointOf (centerClass center) ∈ seed.candidateMasks center := by
    intro center
    by_cases hvCenter : center = .v
    ...
    · by_cases hwCenter : center = .w
      ...
      · by_cases hprivateCenter : center = seed.privateCenter
        ...
        · exact hcandidate center hvCenter hwCenter hprivateCenter
  exact
    erasedPinRow_ep_right_m1_s0_l0_r3_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate hcandidateFull hno3 hcounts hsep hsearchSep)

/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_left_m1_s0_l3_r0`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_left_m1_s0_l3_r0_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    ...
    False := by
  have hfixed : seed ∈ erasedPinFixedSeeds :=
    erasedPinRow_ep_left_m1_s0_l3_r0_seeds_candidates_subset_fixed hseed
  have hcandidateFull : ∀ center : Label,
      pointMask pointOf (centerClass center) ∈ seed.candidateMasks center := by
    intro center
    by_cases hvCenter : center = .v
    ...
    · by_cases hwCenter : center = .w
      ...
      · by_cases hprivateCenter : center = seed.privateCenter
        ...
        · exact hcandidate center hvCenter hwCenter hprivateCenter
  exact
    erasedPinRow_ep_left_m1_s0_l3_r0_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate hcandidateFull hno3 hcounts hsep hsearchSep)
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_right_m1_s0_l1_r2`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_right_m1_s0_l1_r2_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    ...
    False := by
  have hfixed : seed ∈ erasedPinFixedSeeds :=
    erasedPinRow_ep_right_m1_s0_l1_r2_seeds_candidates_subset_fixed hseed
  exact
    erasedPinRow_ep_right_m1_s0_l1_r2_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate (by
          intro center
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_left_m1_s0_l2_r1`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_left_m1_s0_l2_r1_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    ...
    False := by
  have hfixed : seed ∈ erasedPinFixedSeeds :=
    erasedPinRow_ep_left_m1_s0_l2_r1_seeds_candidates_subset_fixed hseed
  exact
    erasedPinRow_ep_left_m1_s0_l2_r1_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate (by
          intro center
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_right_m1_s1_l1_r1`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_right_m1_s1_l1_r1_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    ...
    False := by
  have hfixed : seed ∈ erasedPinFixedSeeds :=
    erasedPinRow_ep_right_m1_s1_l1_r1_seeds_candidates_subset_fixed hseed
  exact
    erasedPinRow_ep_right_m1_s1_l1_r1_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate (by
          intro center
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_left_m1_s1_l1_r1`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_left_m1_s1_l1_r1_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    ...
    False := by
  have hfixed : seed ∈ erasedPinFixedSeeds :=
    erasedPinRow_ep_left_m1_s1_l1_r1_seeds_candidates_subset_fixed hseed
  exact
    erasedPinRow_ep_left_m1_s1_l1_r1_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate (by
          intro center
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_right_m1_s1_l0_r2`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_right_m1_s1_l0_r2_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    ...
    False := by
  have hfixed : seed ∈ erasedPinFixedSeeds :=
    erasedPinRow_ep_right_m1_s1_l0_r2_seeds_candidates_subset_fixed hseed
  exact
    erasedPinRow_ep_right_m1_s1_l0_r2_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate (by
          intro center
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_left_m1_s1_l2_r0`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_left_m1_s1_l2_r0_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    ...
    False := by
  have hfixed : seed ∈ erasedPinFixedSeeds :=
    erasedPinRow_ep_left_m1_s1_l2_r0_seeds_candidates_subset_fixed hseed
  exact
    erasedPinRow_ep_left_m1_s1_l2_r0_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate (by
          intro center
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_right_m2_s0_l0_r2`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_right_m2_s0_l0_r2_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    ...
    False := by
  have hfixed : seed ∈ erasedPinFixedSeeds :=
    erasedPinRow_ep_right_m2_s0_l0_r2_seeds_candidates_subset_fixed hseed
  exact
    erasedPinRow_ep_right_m2_s0_l0_r2_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate (by
          intro center
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_left_m2_s0_l2_r0`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_left_m2_s0_l2_r0_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
    ...
    False := by
  have hfixed : seed ∈ erasedPinFixedSeeds :=
    erasedPinRow_ep_left_m2_s0_l2_r0_seeds_candidates_subset_fixed hseed
  exact
    erasedPinRow_ep_left_m2_s0_l2_r0_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate (by
          intro center
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_right_m2_s1_l0_r1`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_right_m2_s1_l0_r1_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
  ...
  exact
    erasedPinRow_ep_right_m2_s1_l0_r1_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate (by
          intro center
...
/-- Row-specific point-class contradiction for the erased-pin finite row
`ep_left_m2_s1_l1_r0`.  Once geometry identifies one of the row's generated
candidate seeds, the generated row no-survivor theorem closes the seeded
shadow. -/
theorem false_of_erasedPinRow_ep_left_m2_s1_l1_r0_seedShadow_pointClasses
    {α : Type _} [DecidableEq α] {pointOf : Label → α}
    {centerClass : Label → Finset α} {seed : OneSidedSeed}
  ...
  exact
    erasedPinRow_ep_left_m2_s1_l1_r0_seeds_candidates_false_of_isValidOneSidedSeedShadow
      hseed
      (isValidOneSidedSeedShadow_shadowOfPointClasses_of_seed_candidates_with_exact_masks_labelPairs
        hs hv hwMask hprivate (by
          intro center
...
theorem false_of_rightOwnPwErasedPayload_pointClasses_of_seed_candidates
  ...
  let seed : OneSidedSeed :=
    { sstar := sstar, privateCenter := .Pw, kind := .own,
      privateMask := maskOfLabels [.u, .w, sstar, .Pu] }
  have hseed : seed ∈ oneSidedSeeds := ownPwSeed_mem_oneSidedSeeds hsstar
  exact false_of_oneSidedSeedShadow_pointClasses_of_exact_vw_private_candidates_labelPairs
    (pointOf := rightPinnedLabelPoint S p₁ p₂ q₁ q₂ s1 s2 s3)
    (centerClass := centerClass)
    (seed := seed)
    hseed
...
theorem false_of_rightOneSidedErasedPayload_pointClasses_of_named_pair_seed_mask_interfaces
  ...
  refine
    false_of_rightOneSidedErasedPayload_pointClasses_of_named_pair_seed_candidates
      ...
      hno3 hcounts (fun c cp a b _ _ => hsep c cp a b)
