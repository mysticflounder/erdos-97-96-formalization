...
/-!
# Same-distance disk containment

If an apex and two chord endpoints lie on a circle centered at `O`, and a point
`p` lies in the closed disk while sharing the endpoints' distance from the apex,
then `p` lies on the closed cap arc opposite the apex.
-/
