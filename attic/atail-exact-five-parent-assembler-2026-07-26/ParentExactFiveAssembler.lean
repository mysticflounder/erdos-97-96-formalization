...
This module packages the smallest source-faithful assembly from the
...
mutual-omission surface.
-/

open scoped EuclideanGeometry

namespace Problem97
namespace ATailParentExactFiveAssembler
...
/-- A sufficient source-indexed occurrence: one complete reverse outside pair
is contained in either of the two actual first-apex selected rows extracted
from the retained original frontier.  This is intentionally not the primary
producer contract: complete-radius-class containment is choice-free, whereas a
co-radial pair need not lie in either preselected four-point subrow. -/
...
/-- The sufficient named-row producer closes the all-reverse packet through
the existing first-apex co-radial terminal. -/
...
choice-free first-apex co-radial occurrence on the all-reverse packet. -/
...
end ATailParentExactFiveAssembler
end Problem97
